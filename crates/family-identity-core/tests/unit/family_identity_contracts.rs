...
trait FamilyIdentityTestResultExt<T, E> {
    fn value_or_unreachable(self, context: &str) -> T;
    fn error_or_unreachable(self, context: &str) -> E;
}

impl<T, E> FamilyIdentityTestResultExt<T, E> for Result<T, E> {
    fn value_or_unreachable(self, context: &str) -> T {
        match self {
            Ok(value) => value,
...
fn household_profile_round_trips_with_member_and_child_ids() {
    ...
    .value_or_unreachable("household profile");

    let json = serde_json::to_value(&profile).value_or_unreachable("serialize household profile");
    let round_trip: HouseholdProfile =
        serde_json::from_value(json).value_or_unreachable("deserialize household profile");
...
fn parent_member_rejects_non_parent_roles() {
    ...
    .error_or_unreachable("child device agent is not a parent member");
...
fn parent_member_round_trips_with_observer_membership_state() {
    ...
    .value_or_unreachable("parent member");

    let json = serde_json::to_value(&member).value_or_unreachable("serialize parent member");
    let round_trip: ParentMember =
        serde_json::from_value(json).value_or_unreachable("deserialize parent member");
...
fn device_registration_rejects_wrong_household_child_binding() {
    ...
    .value_or_unreachable("child profile");
    ...
    .value_or_unreachable("device registration");

    let error = registration
        .validate_child_profile(&child_profile)
        .error_or_unreachable("wrong household binding is rejected");
...
fn child_profile_round_trips_with_device_ids_and_custody_label() {
    ...
    let json = serde_json::to_value(&child_profile).value_or_unreachable("serialize child profile");
    let round_trip: ChildProfile =
        serde_json::from_value(json).value_or_unreachable("deserialize child profile");
...
fn observer_permission_round_trips_with_read_only_scope() {
    ...
    .value_or_unreachable("observer permission");

    let json =
        serde_json::to_value(&permission).value_or_unreachable("serialize observer permission");
    let round_trip: ObserverPermission =
        serde_json::from_value(json).value_or_unreachable("deserialize observer permission");
...
fn observer_permission_rejects_write_scope() {
    ...
    .error_or_unreachable("write scope is blocked for observers");
...
fn active_controller_lease_round_trips_and_stays_reusable() {
    ...
    .value_or_unreachable("active lease record");

    let json = serde_json::to_value(&lease).value_or_unreachable("serialize controller lease");
    let round_trip: ParentControllerLease =
        serde_json::from_value(json).value_or_unreachable("deserialize controller lease");

    assert_eq!(round_trip, lease);
    lease
        .ensure_reusable()
        .value_or_unreachable("active lease remains reusable");
}
...
fn revoked_controller_lease_cannot_be_reused() {
    ...
    .value_or_unreachable("revoked lease record");

    let error = lease
        .ensure_reusable()
        .error_or_unreachable("revoked lease reuse is rejected");
...
fn setup_invite_and_audit_event_round_trip_through_serde_json() {
    ...
    .value_or_unreachable("setup invite");
    let invite_json = serde_json::to_value(&invite).value_or_unreachable("serialize setup invite");
    let invite_round_trip: SetupInvite =
        serde_json::from_value(invite_json).value_or_unreachable("deserialize setup invite");
    ...
    .value_or_unreachable("audit event");
    let audit_json =
        serde_json::to_value(&audit_event).value_or_unreachable("serialize setup audit event");
    let audit_round_trip: SetupAuditEvent =
        serde_json::from_value(audit_json).value_or_unreachable("deserialize setup audit event");
...
fn recovery_record_stays_distinct_from_recovery_state_enum() {
    ...
    .value_or_unreachable("recovery record");

    assert_eq!(record.reason, RecoveryKind::HouseholdTransfer);
    assert!(record.parent_action_required);
    let json =
        serde_json::to_value(&record).value_or_unreachable("serialize recovery contract state");
    let round_trip: RecoveryContractState =
        serde_json::from_value(json).value_or_unreachable("deserialize recovery contract state");
...
fn recovery_contract_state_rejects_missing_parent_action_for_transfer() {
    ...
    .error_or_unreachable("household transfer must keep parent action required");
