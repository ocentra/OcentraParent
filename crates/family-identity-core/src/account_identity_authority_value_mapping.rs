...
pub(crate) fn map_role(role: AccountIdentityRole) -> HouseholdRole {
    role::map_role(role)
}

pub(crate) fn map_account_state(state: AccountIdentityAccountState) -> ActorAccountState {
    state::map_account_state(state)
}

pub(crate) fn map_membership_state(
    state: AccountIdentityMembershipState,
) -> HouseholdMembershipState {
    state::map_membership_state(state)
}

pub(crate) fn map_binding_state(
    binding: &AccountIdentityHouseholdChildDeviceBinding,
) -> ChildProfileBindingState {
    device::map_binding_state(binding)
}

pub(crate) fn map_device_scope(role: AccountIdentityRole) -> DeviceOwnershipScope {
    role::map_device_scope(role)
}

pub(crate) fn map_device_trust(state: AccountIdentityDeviceTrustState) -> DeviceTrustState {
    device::map_device_trust(state)
}

pub(crate) fn map_session_freshness(
