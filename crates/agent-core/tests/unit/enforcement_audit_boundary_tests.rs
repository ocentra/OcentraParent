...
fn audit_event_mirrors_unavailable_result_capability_and_status_boundary() -> TestResult {
    ...
    assert_eq!(
        outcome.audit_event.unavailable_status,
        outcome.result.unavailable_status
    );
    assert_eq!(
        outcome
            .audit_event
...
fn failed_adapter_result_records_failed_audit_without_unavailable_status() -> TestResult {
    ...
    input.adapter_outcome = Some(EnforcementAdapterOutcome {
        ...
        adapter_result_code: EnforcementAdapterResultCode::AdapterFailed,
        completed_at: Some(policy::TEST_EVALUATED_AT.to_string()),
        unavailable_reason: None,
        failed_reason: Some(enforcement::ADAPTER_FAILED.to_string()),
