...
fn durable_memory_graph_index_retrieves_relationships_after_raw_events_are_removed() {
    ...
    cleanup_store_path(&store_path);
    {
        ...
        assert!(activity_store_query(store.activity_memory_graph_indexed_citation_count()) >= 2);
        activity_store_query(store.delete_activity_events_for_memory_graph_reindex());
    }
