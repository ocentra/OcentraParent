...
pub fn registry_export_with_two_apps() -> String {
...
pub fn hidden_system_component_export() -> String {
    let mut export = registry_header();
    export.push(constants::delimiter::NEWLINE);
    export.push_str(&registry_key(constants::activity_store::TEST_STORE_SUFFIX));
    export.push(constants::delimiter::NEWLINE);
...
pub fn registry_export_path(root: &Path) -> PathBuf {
...
pub fn write_registry_export(path: &Path, export: String) {
...
pub fn temp_registry_root(suffix: &str) -> PathBuf {
    let mut path = std::env::temp_dir();
    path.push(temp_name(suffix));
    path
}

pub fn cleanup_registry_root(path: &Path) {
