...
use super::{
    activity_store_policy_preview_test_fixture::{browser_event, network_event},
    ActivityStore,
};
...
#[test]
fn policy_preview_read_model_evaluates_stored_network_flow_without_exact_url_or_enforcement() {
    let store = ActivityStore::open_in_memory().expect(constants::error::ACTIVITY_STORE_OPENS);
    ...
    assert!(!row
        .target
        .target_value
        .contains(constants::delimiter::SLASH));
    assert_eq!(row.evidence_references.len(), 1);
    assert_eq!(
        row.evidence_references[0].kind,
        ParentEvidenceReferenceKind::ActivityEvent
