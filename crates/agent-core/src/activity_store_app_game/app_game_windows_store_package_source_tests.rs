...
fn store_package_manifest_source_builds_inventory_rows_without_use_claims() {
    ...
    cleanup_store_package_root(&root);
...
fn store_package_manifest_source_respects_limit_before_journal_projection() {
    ...
    cleanup_store_package_root(&root);
    write_manifest(
        &package_dir(&root, constants::activity_store::TEST_STORE_SUFFIX),
        store_app_manifest(),
    );
    write_manifest(
        &package_dir(
...
fn store_app_manifest() -> &'static str {
    APP_GAME_TEST_STORE_PACKAGE_MANIFEST_XML
}

fn package_dir(root: &Path, suffix: &str) -> PathBuf {
...
fn write_manifest(root: &Path, manifest: &str) {
    ...
    fs::write(path, manifest).expect(constants::error::ACTIVITY_CAPTURE_RECORDS);
}
...
fn temp_store_package_root(suffix: &str) -> PathBuf {
...
fn cleanup_store_package_root(path: &Path) {
