use ocentra_parent_agent_protocol::{
    constants, APP_GAME_CAPABILITY_STATUS_AVAILABLE, APP_GAME_CAPABILITY_STATUS_PERMISSION_LIMITED,
    APP_GAME_CATALOG_PERMISSION_LIMITED, APP_GAME_CLASSIFICATION_KNOWN_APP,
    ...
    APP_GAME_TEST_CATALOG_REF, APP_GAME_TEST_EXECUTABLE_PATH_REF, APP_GAME_TEST_FILE_HASH_REF,
    ...
    APP_GAME_TEST_PARENT_PROCESS_ID, APP_GAME_TEST_PERMISSION_PROCESS_ID,
    APP_GAME_TEST_PERMISSION_PROCESS_IDENTITY, APP_GAME_TEST_PERMISSION_PROCESS_NAME,
    ...
    APP_GAME_TEST_RUNTIME_EVIDENCE_ID, APP_GAME_TEST_RUNTIME_EXIT_EVIDENCE_ID,
    APP_GAME_TEST_RUNTIME_LAUNCHER_EVIDENCE_ID, APP_GAME_TEST_RUNTIME_PERMISSION_EVIDENCE_ID,
};
...
fn process_appears_and_creates_runtime_evidence() {
    ...
    assert_eq!(rows[0].runtime_evidence_id, APP_GAME_TEST_RUNTIME_EVIDENCE_ID);
    assert_eq!(rows[0].process_identity, APP_GAME_TEST_PROCESS_IDENTITY);
    assert_eq!(rows[0].process_id, APP_GAME_TEST_PROCESS_ID);
    assert_eq!(rows[0].parent_process_id, Some(APP_GAME_TEST_PARENT_PROCESS_ID));
    assert_eq!(
...
fn same_process_persists_and_session_can_continue() {
    ...
    assert_eq!(summaries[0].primary_process_identity, APP_GAME_TEST_PROCESS_IDENTITY);
...
fn launcher_process_stays_launcher_runtime_only() {
    ...
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].process_identity, APP_GAME_TEST_LAUNCHER_PROCESS_IDENTITY);
    assert_eq!(
        rows[0].classification_state,
        APP_GAME_CLASSIFICATION_KNOWN_LAUNCHER
    );
    assert_eq!(rows[0].launcher_ref, Some(APP_GAME_TEST_LAUNCHER_REF.to_string()));
    assert_eq!(rows[0].runtime_state, APP_GAME_RUNTIME_RUNNING);
    assert_eq!(rows[0].foreground_state, APP_GAME_FOREGROUND_NOT_CLAIMED);
}
...
fn permission_limited_metadata_state_remains_explicit() {
    ...
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].process_identity, APP_GAME_TEST_PERMISSION_PROCESS_IDENTITY);
    assert_eq!(
        rows[0].classification_state,
        APP_GAME_CLASSIFICATION_PERMISSION_LIMITED
    );
    assert_eq!(rows[0].runtime_state, APP_GAME_RUNTIME_PERMISSION_LIMITED);
    assert_eq!(rows[0].catalog_ready_state, APP_GAME_CATALOG_PERMISSION_LIMITED);
    assert_eq!(rows[0].capability_status, APP_GAME_CAPABILITY_STATUS_PERMISSION_LIMITED);
