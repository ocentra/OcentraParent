...
pub(super) fn classification_state_for_record(record: &WindowsProcessRuntimeRecord) -> String {
...
pub(super) fn catalog_ready_state_for_record(record: &WindowsProcessRuntimeRecord) -> String {
    if record.capability_status == APP_GAME_CAPABILITY_STATUS_PERMISSION_LIMITED {
...
pub(super) fn capability_status_for_record(record: &WindowsProcessRuntimeRecord) -> String {
    if record.capability_status == APP_GAME_CAPABILITY_STATUS_PERMISSION_LIMITED {
...
pub(super) fn confidence_for_record(record: &WindowsProcessRuntimeRecord) -> f64 {
    if has_deterministic_runtime_ref(record) {
        record.confidence
    } else {
