...
use super::{
    activity_store_policy_preview_test_fixture::{
        active_window_event, browser_event, network_event, parent_rule_context,
        parent_rule_context_for_event, parent_rule_context_for_network_event,
    },
    ActivityStore,
};
...
#[test]
fn policy_preview_read_model_resolves_network_domain_rule_from_stored_flow() {
    let store = ActivityStore::open_in_memory().expect(constants::error::ACTIVITY_STORE_OPENS);
    ...
    store
        ...
        .expect(constants::error::ACTIVITY_STORE_INGESTS);
    store
        .replace_parent_rule_contexts(&[parent_rule_context_for_network_event(&event)])
        .expect(constants::error::ACTIVITY_STORE_INGESTS);
    ...
    assert!(row.decision.dry_run);
    assert_eq!(
        row.decision.enforcement_handoff_state.as_protocol_str(),
