...
pub(crate) fn ok<T, E: Debug>(result: Result<T, E>, context: &str) -> Result<T, String> {
    result.map_err(|error| format!("{context}: {error:?}"))
}

pub(crate) fn err<T, E: Debug>(result: Result<T, E>, context: &str) -> Result<E, String> {
    match result {
...
pub(crate) fn try_serve_devtools(
    version_body: &'static str,
    list_body: &'static str,
) -> Result<SocketAddr, String> {
    let listener = ok(
...
pub(crate) fn try_serve_devtools_owned(
    version_body: String,
    list_body: String,
) -> Result<SocketAddr, String> {
    let listener = ok(
...
pub(crate) fn try_serve_unresponsive_devtools() -> Result<SocketAddr, String> {
    let listener = ok(
        TcpListener::bind(constants::test_network::LOOPBACK_ANY_PORT),
        constants::error::LOCALHOST_BIND_SUCCEEDS,
    )?;
