...
- App/game protocol-row storage, live process journal replay, live
  foreground-window source proof, and live shortcut inventory source proof are
  staged core proof only; bounded runtime rows now feed service capture, while
  inventory/foreground service capture, portal authority/classifier/source
  rows, policy consumption, and adapter execution remain separate gaps.
