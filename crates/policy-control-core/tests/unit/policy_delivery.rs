...
#[test]
fn execution_receipt_validation_rejects_identity_and_staleness_mismatches() -> TestResult {
    let queued = sample_queued_delivery()?;
    ...
    let mut mismatched_delivery_id = acknowledged_receipt.clone();
    mismatched_delivery_id.delivery_id = test_ok!(
        PolicyDeliveryId::parse("delivery-policy-mismatch"),
        "mismatched delivery id"
    );
    let mismatched_delivery_error = test_err!(
        validate_policy_delivery_execution_receipt(
            &delivered_record,
            &acknowledged_transition,
            Some(&mismatched_delivery_id),
        ),
        "mismatched delivery id must fail"
    );
    assert_eq!(
        mismatched_delivery_error,
        EventingError::InvalidValue {
    ...
    let mut mismatched_attempt_id = acknowledged_receipt.clone();
    mismatched_attempt_id.attempt_id = test_ok!(
        PolicyDeliveryAttemptId::parse("attempt-mismatch"),
        "mismatched attempt id"
    );
    let mismatched_attempt_error = test_err!(
        validate_policy_delivery_execution_receipt(
            &delivered_record,
            &acknowledged_transition,
            Some(&mismatched_attempt_id),
        ),
        "mismatched attempt id must fail"
    );
    assert_eq!(
        mismatched_attempt_error,
        EventingError::InvalidValue {
    ...
    let mut stale_receipt = acknowledged_receipt.clone();
    stale_receipt.sequence = test_ok!(PolicyDeliverySequence::new(1), "stale sequence");
    let stale_error = test_err!(
        validate_policy_delivery_execution_receipt(
            &delivered_record,
            &acknowledged_transition,
            Some(&stale_receipt),
...
fn rolled_back_execution_receipt_requires_rollback_reference_state() -> TestResult {
    ...
    let mut rollback_mismatch_receipt = rollback_execution.receipt.clone();
    rollback_mismatch_receipt.rollback_reference_state = Some(PolicyDeliveryState::Delivered);
    let rollback_mismatch_error = test_err!(
        validate_policy_delivery_execution_receipt(
            &applied_record,
            &rollback_transition,
            Some(&rollback_mismatch_receipt),
        ),
    ...
    let missing_rollback_receipt_error = test_err!(
        validate_policy_delivery_execution_receipt(
            &applied_record,
            &rollback_transition,
    ...
    assert_eq!(
        missing_rollback_receipt_error,
        EventingError::InvalidValue {
            field: "policy_delivery.rollback_reference_state",
