...
use crate::policy_source::names::{
    policy_status_name, restored_policy_version_must_be_older_value,
};
...
pub(crate) fn assert_status_lifecycle_refs(
    ...
    match document.status {
        super::PolicySourceStatus::Superseded => assert_superseded_status_lifecycle_refs(document),
        super::PolicySourceStatus::RolledBack => assert_rolled_back_status_lifecycle_refs(document),
...
fn assert_superseded_status_lifecycle_refs(
    ...
    if replacement_policy_version.value() <= document.policy_version.value() {
        return Err(EventingError::InvalidValue {
            field: policy_control::source::FIELD_SUPERSEDED_BY_POLICY_VERSION,
            value: super::super::names::replacement_policy_version_must_be_newer_value(
