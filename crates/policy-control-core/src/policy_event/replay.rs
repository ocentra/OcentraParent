...
pub(crate) fn policy_event_contract_registry() -> Result<EventContractRegistry, EventingError> {
    ...
    for kind in super::POLICY_EVENT_KINDS.iter().copied() {
...
pub(crate) fn sample_policy_event(kind: PolicyEventKind) -> Result<PolicyEvent, EventingError> {
    ...
    Ok(PolicyEvent {
        ...
        kind,
        sequence: super::PolicyEventSequence::new(1)?,
        scope,
        audit_reference_ids,
        reason_code,
        dead_letter_reason,
...
fn policy_event_idempotency_key_value(event: &PolicyEvent) -> Result<String, EventingError> {
