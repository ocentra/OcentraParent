...
fn child_domain_runtime_events_generated_typescript_matches_checked_in_file() {
    ...
    let checked_in = fs::read_to_string(
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../packages/schema-domain/src/child-domain-runtime-events.ts"),
    )
    .expect("child domain runtime source should be readable");

    assert_eq!(generated, checked_in);
    assert!(generated.starts_with("/* generated from crates/schema/src/child_domain_runtime_events_ts.rs */"));
}
