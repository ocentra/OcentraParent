...
pub(super) fn test_event(label: &str) -> TestEvent {
    test_event_for_type_with_idempotency(label, TEST_EVENT_TYPE, TEST_IDEMPOTENCY)
}

pub(super) fn test_event_for_type(label: &str, event_type: &str) -> TestEvent {
    test_event_for_type_with_idempotency(label, event_type, TEST_IDEMPOTENCY)
}

pub(super) fn test_event_with_idempotency(label: &str, idempotency_key: &str) -> TestEvent {
    test_event_for_type_with_idempotency(label, TEST_EVENT_TYPE, idempotency_key)
}
