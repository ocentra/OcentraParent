...
use crate::journal::{policy::{JournalPolicy, JournalSelector}, EventJournal, ndjson::{NdjsonEventJournal, NdjsonJournalOptions}};
use crate::queue::policy::EventQueuePolicy;
use crate::replay::{ReplayCursor, ReplayFilter, ReplayMode};
...
async fn replay_rejects_tampered_hash_chain_payload(
    ...
    journal.append(&stored_event(test_event(TEST_LABEL))?).await?;
    journal
    ...
    match result {
        ...
        other => {
            return Err(std::io::Error::other(format!(
                "expected corrupt hash-chain replay, received {other:?}"
            ))
            .into())
        }
    }
