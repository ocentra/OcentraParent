...
fn compatibility_matrix_covers_games_lineage_semantics() {
    ...
    assert_eq!(matrix.compatible_entries().len(), 10);
    assert_eq!(matrix.intentional_deviations().len(), 2);
...
fn compatibility_matrix_marks_deviations_and_manual_required_scope() {
    ...
    assert_eq!(
        matrix.entry(BROKER_DELIVERY).expect("broker entry").status,
...
fn compatibility_matrix_renders_deterministic_markdown() {
    ...
    assert!(markdown.contains("| class-backed-contracts | Class-backed contracts expose canonical static event types | DomainEvent::contract plus EventContractRegistry descriptors | compatible |"));
