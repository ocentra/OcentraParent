...
const MIGRATION_DURABLE_OWNER_REQUIRED_NOTE: &str =
    "Migration is manual-required until a durable migration store and executor own apply, rollback, and idempotency receipts.";
...
pub(super) fn migration_execution_readiness(
    ...
    let (state, required_dependency, manual_required_note) = match preflight.migration_state {
        ...
        contracts::ExportImportMigrationState::RequiredUnsupported => (
            contracts::ExportImportMigrationExecutionState::ManualRequired,
            Some(contracts::ExportImportMigrationExecutionDependency::SupportedMigrationPath),
