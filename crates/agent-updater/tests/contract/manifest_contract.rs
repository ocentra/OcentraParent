...
fn manifest_policy_rejects_unsupported_schema_target_and_installer() {
    ...
    let mut unsupported_schema =
        sample_payload(Version::parse("0.2.0").expect("version parse failed"));
...
fn signed_manifest_round_trip_verifies_with_trusted_key() {
    let keys = generate_key_pair();
    let payload = sample_payload(Version::parse("0.2.0").expect("version parse failed"));
    let signed =
        sign_payload(payload.clone(), &keys.private_key_base64).expect("manifest signs failed");
    let verified =
        verify_manifest(signed, &keys.public_key_base64).expect("manifest verifies failed");
...
fn signed_manifest_rejects_unsigned_payload_changes() {
    ...
    let mut signed = sign_payload(
        sample_payload(Version::parse("0.2.0").expect("version parse failed")),
        &keys.private_key_base64,
    )
    .expect("manifest signs failed");
...
fn payload_policy_rejects_non_github_artifacts() {
    let keys = generate_key_pair();
    let mut payload = sample_payload(Version::parse("0.2.0").expect("version parse failed"));
...
fn payload_parser_accepts_windows_utf8_bom() {
    let json = serde_json::to_string(&sample_payload(
        Version::parse("0.2.0").expect("version parse failed"),
    ))
    .expect("json serialization failed");
    let text = format!("\u{feff}{json}");

    let parsed = parse_payload(&text).expect("payload parses failed");

    assert_eq!(parsed.version, "0.2.0");
}

fn sample_payload(version: Version) -> UpdateManifestPayload {
