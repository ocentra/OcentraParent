...
fn social_alert_report_snapshot_serializes_no_claim_boundaries() {
    let snapshot = SocialAlertReportReadModelSnapshot {
        ...
        intents: Vec::new(),
        provider_status_rows: vec![ocentra_parent_agent_protocol::SocialAlertReportProviderStatusRow {
    ...
    let json =
        serde_json::to_string(&snapshot).expect(ocentra_parent_agent_protocol::constants::error::AGENT_EVENT_SERIALIZES);
