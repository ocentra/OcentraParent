...
pub const CLAIM_BOUNDARY_DOMAIN_NETWORK_TARGET: &str =
    "Domain and network targets require a host network control adapter before blocking can upgrade.";
...
pub const FALLBACK_APP_TARGET: &str =
    "Keep app target enforcement manual-required until inventory, process, executable, package, signature, and join evidence are present.";
...
pub const FALLBACK_MANAGED_BROWSER_TARGET: &str =
    "Return manual-required for service-command browser targets and do not claim exact URL control.";
pub const FALLBACK_UNMANAGED_BROWSER_TARGET: &str =
    "Allow only process pid/name guardrails and warn/no-op behavior; exact unmanaged URL evidence remains not claimed.";
pub const FALLBACK_UNSUPPORTED_OS_TARGET: &str =
    "Return unavailable on non-Windows or unsupported host targets instead of using a Windows adapter claim.";
pub const FALLBACK_ROLLBACK_AUDIT_TARGET: &str =
    "Keep rollback and audit readiness manual-required until the same package or executable identity has apply, rollback, and custody evidence.";
