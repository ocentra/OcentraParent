...
pub const POWERSHELL_LAN_NEIGHBOR_COMMAND: &str = "[Console]::OutputEncoding=[Text.UTF8Encoding]::UTF8; Get-NetNeighbor -AddressFamily IPv4 | Where-Object { $_.IPAddress -and $_.LinkLayerAddress } | Select-Object IPAddress,LinkLayerAddress,State,InterfaceAlias | ConvertTo-Json -Compress";
pub const POWERSHELL_REVERSE_DNS_COMMAND_PREFIX: &str =
    "[Console]::OutputEncoding=[Text.UTF8Encoding]::UTF8; Resolve-DnsName -Name ";
pub const POWERSHELL_REVERSE_DNS_COMMAND_SUFFIX: &str =
    " -Type PTR -DnsOnly -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty NameHost | ConvertTo-Json -Compress";
