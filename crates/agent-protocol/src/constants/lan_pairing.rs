...
pub const POWERSHELL_LOCAL_NETWORK_IDENTITY_COMMAND: &str = "[Console]::OutputEncoding=[Text.UTF8Encoding]::UTF8; $configs = Get-CimInstance Win32_NetworkAdapterConfiguration | Where-Object { $_.IPEnabled }; Get-NetIPConfiguration | Where-Object { $_.IPv4Address -and $_.NetAdapter.Status -eq 'Up' } | ForEach-Object { $ipv4 = $_.IPv4Address | Select-Object -First 1; $gateway = $_.IPv4DefaultGateway | Select-Object -First 1; $ipv6Prefixes = @($_.IPv6Address | Where-Object { $_.IPAddress -and $_.PrefixLength -and $_.IPAddress -notlike 'fe80*' } | ForEach-Object { \"$($_.IPAddress)/$($_.PrefixLength)\" }); $config = $configs | Where-Object { $_.IPAddress -contains $ipv4.IPAddress } | Select-Object -First 1; [pscustomobject]@{ IPAddress = $ipv4.IPAddress; PrefixLength = $ipv4.PrefixLength; InterfaceAlias = $_.InterfaceAlias; MacAddress = $_.NetAdapter.MacAddress; DefaultGateway = $gateway.NextHop; DnsServers = @($config.DNSServerSearchOrder | Where-Object { $_ }); DhcpServer = $config.DHCPServer; Ipv6Prefixes = $ipv6Prefixes } } | ConvertTo-Json -Compress";
