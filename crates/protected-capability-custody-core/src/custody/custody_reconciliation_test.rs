...
fn record_with(state: SealedState, sequence: u64) -> Result<crate::storage::Record, CustodyError> {
    let binding = binding().map_err(|_| CustodyError::Tampered)?;
    let database_identity = identity().map_err(|_| CustodyError::Tampered)?;
...
fn reconciliation_currentness_rejects_cross_binding_and_accepts_exact_binding(
    ...
    let binding = binding().map_err(|_| CustodyError::Tampered)?;
    let current = record()?;
    assert!(support::validate_current(&current, &binding).is_ok());

    let other = Binding::try_new(
        BindingLocator::try_new(
            OperationId::try_new(vec![8]).map_err(|_| CustodyError::Tampered)?,
            Action::Seal,
...
fn reconciliation_attestation_rejects_rotation_regression_and_identity_drift(
    ...
    assert!(support::validate_attestation(&current, matching).is_ok());
