...
fn record(
    ...
    let database_identity = identity().map_err(|_error| StorageError::Tampered)?;
...
fn transition_accepts_only_sequential_ambiguous_and_terminal_edges() -> Result<(), StorageError> {
    let binding = binding(1).map_err(|_error| StorageError::Tampered)?;
    ...
    assert!(matches!(
        record_module::validate_transition(&prepared, &commit_intent),
        Ok(())
    ));
    assert!(matches!(
        record_module::validate_transition(&commit_intent, &committed),
        Ok(())
    ));
    Ok(())
}
...
fn transition_rejects_binding_and_epoch_drift_after_each_record_is_authenticated(
    ...
    let first_binding = binding(1).map_err(|_error| StorageError::Tampered)?;
    let other_binding = binding(2).map_err(|_error| StorageError::Tampered)?;
    ...
    assert!(matches!(
        record_module::validate_transition(&prepared, &writer_drift),
        Ok(())
    ));
    Ok(())
}
