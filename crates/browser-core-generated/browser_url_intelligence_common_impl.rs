...
pub(super) fn parse_url(value: &str) -> Option<ParsedBrowserUrl> {
...
pub(super) fn path_segments(parsed: &ParsedBrowserUrl) -> Vec<String> {
    parsed
        .path
        .split('/')
        .map(str::trim)
...
pub(super) fn first_path_segment(parsed: &ParsedBrowserUrl) -> Option<String> {
    path_segments(parsed).into_iter().next()
}

pub(super) fn query_param(parsed: &ParsedBrowserUrl, key: &str) -> Option<String> {
...
pub(super) fn domain_matches_any(domain: &str, bases: &[&str]) -> bool {
    bases
        .iter()
        .any(|base| domain == *base || domain.ends_with(&format!(".{base}")))
}

pub(super) fn non_exact_evidence_reason(value: &str) -> &'static str {
    match value {
