...
#[derive(Clone)]
pub struct TrackingConfigUpdateCausalTarget {
    bus: EventBus,
    state: TrackingConfigUpdateEventState,
    parent_subscription_report: SubscriptionReport,
    child_subscription_report: SubscriptionReport,
...
impl TrackingConfigUpdateEventFlow {
    ...
    pub async fn publish_parent_config_updated(
        ...
        flow_report(
            &self.state,
            &self.parent_subscription_report,
            &self.child_subscription_report,
            &self.applied_subscription_report,
    ...
    pub fn into_causal_target(self) -> TrackingConfigUpdateCausalTarget {
        TrackingConfigUpdateCausalTarget {
            bus: self.bus.event_bus().clone(),
            state: self.state,
            parent_subscription_report: self.parent_subscription_report,
            child_subscription_report: self.child_subscription_report,
            applied_subscription_report: self.applied_subscription_report,
        }
    }
}

impl TrackingConfigUpdateCausalTarget {
    pub async fn publish_parent_config_updated(
        &self,
        publisher: &EventPublisher,
        parent_event: &ParentTrackingConfigUpdatedEvent,
    ) -> Result<TrackingConfigUpdateEventFlowReport, EventingError> {
        let parent_request_report = publisher
            .publish_request_on(
                &self.bus,
                parent_event.clone(),
                parent_tracking_config_updated_metadata(parent_event)?,
...
fn flow_report(
    ...
    child_subscription_report: &SubscriptionReport,
    applied_subscription_report: &SubscriptionReport,
    parent_request_report: RequestReport<TrackingConfigUpdateResponse>,
) -> Result<TrackingConfigUpdateEventFlowReport, EventingError> {
    Ok(TrackingConfigUpdateEventFlowReport {
        parent_subscription_report: parent_subscription_report.clone(),
        child_subscription_report: child_subscription_report.clone(),
        applied_subscription_report: applied_subscription_report.clone(),
        parent_request_report,
        child_event: state.child_event()?,
        applied_event: state.applied_event()?,
        applied_report: state.applied_report()?,
    })
}
