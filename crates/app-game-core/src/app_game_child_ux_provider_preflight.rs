...
const INVALID_SOURCE_FIELD: &str = "app_game.child_ux_provider_preflight.source";
const INVALID_REQUIREMENTS_FIELD: &str = "app_game.child_ux_provider_preflight.requirements";

pub fn build_app_game_child_ux_provider_preflight(
    input: AppGameChildUxProviderPreflightInput,
) -> Result<AppGameChildUxProviderPreflightRow, EventingError> {
    validate_requirements(&input)?;
    validate_sources(&input)?;
    Ok(build_row(input))
}

fn build_row(input: AppGameChildUxProviderPreflightInput) -> AppGameChildUxProviderPreflightRow {
    let status = status_for(&input.scheduler_record.scheduler_state);
    let provider_ready = status == AppGameChildUxProviderPreflightStatus::ProviderAdapterRequired;
    let requirements = vec![
        input.adapter_requirement_ref,
        input.credential_requirement_ref,
        input.smoke_proof_requirement_ref,
    ];
    AppGameChildUxProviderPreflightRow {
        ...
        status,
        source_local_outbox_record_ref: provider_ready
            .then_some(input.source_outbox_record.entry_id),
        source_outbox_file_ref: provider_ready
            .then_some(input.source_outbox_record.outbox_file_ref),
        local_data_path_ref: provider_ready
        ...
        provider_channel: provider_ready.then_some(input.scheduler_record.provider_channel),
        reason_code: provider_ready.then_some(input.scheduler_record.reason_code),
        severity: provider_ready.then_some(input.scheduler_record.severity),
        ...
        adapter_requirement_refs: requirements.clone(),
        manual_proof_requirements: requirements,
...
fn validate_requirements(
    ...
    let refs = [
        ...
        input.adapter_requirement_ref.as_str(),
        input.credential_requirement_ref.as_str(),
        input.smoke_proof_requirement_ref.as_str(),
    ];
...
fn validate_sources(input: &AppGameChildUxProviderPreflightInput) -> Result<(), EventingError> {
