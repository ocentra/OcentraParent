...
impl<T, E> AppGameNotificationParentSurfaceIntentTestResultExt<T, E> for Result<T, E> {
    fn value_or_unreachable(self, context: &str) -> T {
        match self {
            Ok(value) => value,
...
fn app_game_notification_parent_surface_intent_keeps_parent_surface_redacted_and_manual() {
    ...
    .value_or_unreachable("parent surface intent read model");
...
fn generated_app_game_notification_parent_surface_intent_helper_stays_checked_in() {
    let checked_in =
        include_str!("../../src/generated/app-game-notification-parent-surface-intent.ts");
    let generated = app_game_notification_parent_surface_intent_typescript();

    assert!(checked_in.contains("buildGeneratedAppGameNotificationParentSurfaceIntentReadModel"));
    assert!(checked_in.contains("GeneratedRequiredAppGameNotificationParentSurfaceIntentNonClaims"));
    assert_eq!(
        generated
...
fn family(family_id: &str) -> AppGameNotificationFamilyReference {
