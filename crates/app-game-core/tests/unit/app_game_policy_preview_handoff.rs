...
fn generated_app_game_policy_preview_handoff_helper_stays_checked_in() {
    let checked_in = include_str!("../../src/generated/app-game-policy-preview-handoff.ts");
    let generated = app_game_policy_preview_handoff_typescript();

    assert!(checked_in.contains("buildGeneratedAppGamePolicyPreviewHandoffRow"));
    assert!(checked_in.contains("buildGeneratedAppGamePolicyPreviewHandoffReadModel"));
    assert!(checked_in.contains("generatedAppGamePolicyPreviewReadModelCountsMatchRows"));
    assert_eq!(
        generated
