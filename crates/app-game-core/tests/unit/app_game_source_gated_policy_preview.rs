...
fn generated_app_game_source_gated_policy_preview_helper_stays_checked_in() {
    let checked_in = include_str!("../../src/generated/app-game-source-gated-policy-preview.ts");
    let generated = app_game_source_gated_policy_preview_typescript();

    assert!(checked_in.contains("buildGeneratedAppGameSourceGatedPolicyPreviewReadModel"));
    assert!(checked_in.contains("buildGeneratedAppGameSourceGatedPolicyPreviewTimerStatus"));
    assert_eq!(
        generated
