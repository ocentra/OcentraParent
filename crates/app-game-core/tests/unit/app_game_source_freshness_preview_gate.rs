...
fn generated_app_game_source_freshness_preview_gate_helper_stays_checked_in() {
    let checked_in = include_str!("../../src/generated/app-game-source-freshness-preview-gate.ts");
    let generated = app_game_source_freshness_preview_gate_typescript();

    assert!(checked_in.contains("buildGeneratedAppGameSourceFreshnessPreviewGateRow"));
    assert!(checked_in.contains("buildGeneratedAppGameSourceFreshnessPreviewGateReadModel"));
    assert_eq!(
        generated
