...
fn derivation_input() -> EntitlementSnapshotDerivationInput {
    EntitlementSnapshotDerivationInput {
        snapshot_id: value_or_unreachable(
            EntitlementSnapshotId::parse("snapshot-household-default"),
        ...
        entitlement_ledger_state: EntitlementLedgerProjectionState {
            account_ref: value_or_unreachable(
                EntitlementAccountRef::parse("acct-default"),
                "account ref is non-empty",
            ),
            household_ref: value_or_unreachable(
                EntitlementHouseholdRef::parse("household-default"),
                "household ref is non-empty",
            ),
            trusted_device_ref: value_or_unreachable(
                EntitlementTrustedDeviceRef::parse("device-default"),
                "device ref is non-empty",
            ),
            feature_flags: vec![
            ...
            revocation_cursor: value_or_unreachable(
                EntitlementRevocationCursor::parse("revoke-cursor-2026-06-28"),
                "revocation cursor is non-empty",
            ),
            device_trust_required: true,
            package_build_ref: value_or_unreachable(
                EntitlementPackageBuildRef::parse("windows-msi-stable"),
                "package build ref is non-empty",
            ),
        ...
        signature_key_id: value_or_unreachable(
            EntitlementSignatureKeyId::parse("entitlement-key-2026-06"),
            "signature key id is non-empty",
        ),
...
fn value_or_unreachable<T, E>(result: Result<T, E>, context: &str) -> T
where
    E: Debug,
{
    match result {
