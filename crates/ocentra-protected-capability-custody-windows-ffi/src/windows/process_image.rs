...
pub(super) fn open_image(process: HANDLE) -> Result<ImageInner> {
    ...
    let sha256 = digest::read_image_digest(handle.raw(), size)?;
...
pub(super) fn reobserve_image(image: &ImageInner) -> Result<ImageObservation> {
    ...
    let sha256 = digest::read_image_digest(image.handle.raw(), size)?;
    let (final_identity, final_size) = query_image_file(image.handle.raw())?;
    if final_identity != identity || final_size != size || sha256 != image.observation.sha256 {
        return Err(Error::InvalidInput(InputFault::ImageChangedDuringRead));
    }
