...
use crate::{AceObservation, Error, Result};
...
pub(super) fn copy_ace(
    ...
    let index = u32::try_from(index).map_err(|_| Error::BufferTooLarge)?;
    ...
    if ace_start < acl_header_end || ace_header_end > acl_end {
        return Err(Error::InvalidInput("ACE header is outside its ACL"));
    }
    ...
    if ace_size < 8 || ace_size > usize::from(acl_size) || ace_end > acl_end {
        return Err(Error::InvalidInput("ACL contains an undersized ACE"));
    }
    ...
    if sid_offset >= ace_bytes.len() {
        return Err(Error::InvalidInput("ACL ACE has no SID payload"));
    }
