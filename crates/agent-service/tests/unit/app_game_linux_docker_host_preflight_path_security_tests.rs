...
fn linux_rejects_parent_traversal_in_candidate_root() {
    assert!(trusted_docker_candidate(
...
fn linux_rejects_world_writable_probe_root() {
    assert!(
        trusted_docker_candidate(TrustedDockerRoot("/tmp"), ExecutableName("docker")).is_none()
    );
}
...
fn unsupported_unix_platform_rejects_docker_candidate() {
    assert!(
        trusted_docker_candidate(TrustedDockerRoot("/usr/bin"), ExecutableName("docker")).is_none()
    );
}
