...
async fn app_game_adapter_dispatch_result_command_reads_latest_store_audit_evidence() {
    let paths = temp_paths(APP_GAME_ADAPTER_DISPATCH_RESULT_TEST_COMMAND_ID);
    cleanup_paths(&paths);
    let enforcement_event = build_activity_app_game_adapter_dispatch_execute_report_with_paths(
...
#[tokio::test]
async fn app_game_adapter_dispatch_execute_command_rejects_without_a_trusted_delivery() {
