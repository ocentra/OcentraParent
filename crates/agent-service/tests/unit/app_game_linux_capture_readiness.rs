...
use crate::app_game_adapter_host_capabilities::HostCapabilitySignals;
...
use ocentra_parent_screen_capture_adapter::linux_foreground_source::{
    LinuxActiveWindowObservation, LinuxForegroundSourcePreflight, LinuxSourceReadiness,
};
...
fn service_runtime_has_no_capture_readiness_authority_without_preflight_owner() {
    ...
    let generated = PlatformProofGeneratedAtText("test-generated-at".to_owned());
    ...
    assert!(row
        .open_gaps
        .iter()
        .any(|gap| gap == APP_GAME_PLATFORM_GAP_LINUX_FOREGROUND_CAPTURE));
    assert!(
        !row.adapter_dispatch_claimed
            && !row.platform_enforcement_claimed
            && !row.provider_delivery_claimed
            && !row.child_device_delivery_claimed
            && !row.private_diagnostics_claimed
    );
    assert!(
        !model.adapter_dispatch_claimed
            && !model.platform_enforcement_claimed
            && !model.provider_delivery_claimed
            && !model.child_device_delivery_claimed
            && !model.private_diagnostics_claimed
    );
}
