...
use ocentra_family_identity_core::{
    ...
    household_authority::{
        ...
        ParentStepUpAssertionSnapshot, ParentStepUpValidationInput,
    },
};
...
async fn authorized_parent_delete_persists_before_journal_and_terminal_replay_does_not_resurrect(
    ...
    let outcome = match publish_authorized_custody_delete(&flow, command.clone()).await {
        Ok(outcome) => outcome,
    ...
    let repeated = match publish_authorized_custody_delete(&flow, command).await {
        Ok(outcome) => outcome,
        Err(error) => return Err(format!("repeated delete failed: {error:?}").into()),
    };
    assert!(matches!(
        repeated,
        ChildRuntimeTombstonePublicationOutcome::Journaled(_)
    ));
...
async fn journal_failure_keeps_authorized_action_pending_until_restart_replay(
    ...
    let outcome =
        match publish_authorized_custody_delete(&flow, command("pending-retry", expired_input())?)
            .await
        {
            Ok(outcome) => outcome,
...
async fn untrusted_parent_or_non_delete_input_is_rejected_before_an_outbox_intent_exists(
    ...
    let rejection = publish_authorized_custody_delete(&flow, unauthorized).await;
    assert_eq!(
        rejection,
    ...
    malformed.parent_step_up.expected_nonce = Some("different-nonce".to_owned());
    let malformed_rejection = publish_authorized_custody_delete(&flow, malformed).await;
    assert_eq!(
        malformed_rejection,
        Err(AuthorizedCustodyDeleteError::ParentStepUpRejected(
            ocentra_family_identity_core::household_authority::ParentStepUpValidationFailureReason::ReplayRejected
        ))
    );
    assert!(RetentionDeleteTombstoneStore::open(&directory)?
        .records()?
    ...
    let non_delete =
        publish_authorized_custody_delete(&flow, command("active", active_input())?).await;
    assert_eq!(
        non_delete,
...
fn command(
    ...
    Ok(AuthorizedCustodyDeleteCommand {
        ...
        parent_step_up: ParentStepUpValidationInput {
            ...
            expected_nonce: Some(format!("nonce-{id}")),
        },
