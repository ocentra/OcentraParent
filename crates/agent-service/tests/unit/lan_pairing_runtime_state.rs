...
use crate::lan_pairing_runtime_state::passive_discovery::start_lan_passive_discovery_service_runtime;
use crate::lan_pairing_test_commands::{health_command, health_command_for_target, status_command};
...
#[test]
fn from_env_without_owner_registry_path_fails_closed_even_with_child_identity() {
...
async fn lan_pairing_runtime_state_helpers_are_used_by_real_flow() {
    ...
    assert_eq!(pairing_state, constants::value::LAN_PAIRING_UNPAIRED);

    let decoded: serde_json::Value =
        require_json_decode(br#"{"status":"unpaired"}"#, "status payload");
    assert_eq!(decoded["status"], constants::value::LAN_PAIRING_UNPAIRED);
    ...
    let _passive_runtime = start_lan_passive_discovery_service_runtime(&runtime);
}
