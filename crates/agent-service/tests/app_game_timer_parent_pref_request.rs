...
#[path = "unit/app_game_timer_parent_preference_setup_request_tests.rs"]
...
mod clippy_linkage {
    ...
    async fn public_wrapper_and_outbox_helper_are_linked() {
        ...
        let store_path = temp_store_path("app-game-pref-request-clippy");
        ...
        if let Some(snapshot) =
            activity_surface_store::local_store_snapshot_from_path(store_path.clone()).await
        {
            let _ = (
                snapshot.device_id.as_str(),
                snapshot.last_event_id.as_deref(),
        ...
        let _ = activity_surface_store::load_browser_model_from_path(store_path.clone()).await;
        let _ = activity_surface_store::load_network_model().await;
        let _ = activity_surface_store::load_network_model_from_path(store_path.clone()).await;
        let _ = activity_surface_store::load_app_game_model().await;
        let _ = activity_surface_store::load_app_game_model_from_path(store_path.clone()).await;
        let _ = activity_surface_store::load_screen_summary().await;
        let _ = activity_surface_store::load_screen_summary_from_path(store_path.clone()).await;
        let _ =
