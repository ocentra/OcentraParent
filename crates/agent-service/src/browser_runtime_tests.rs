...
fn managed_profile_ready_status_is_headless_until_explicit_launch() {
    ...
    assert_eq!(
        payload[constants::field::PROFILE_ROOT_REF],
        LogFieldValue::String(constants::browser::PROFILE_ROOT_REF_MANAGED.to_string())
    );
    assert_eq!(
        payload[constants::field::PROFILE_LIFECYCLE_STATE],
        LogFieldValue::String(constants::browser::PROFILE_STORE_LIFECYCLE_READY.to_string())
    );
}

#[test]
fn launch_pending_status_tracks_redacted_profile_and_reserved_bridge() {
    let status = launch_pending_status(
        constants::activity_store::TEST_FIRST_OBSERVED_AT.to_string(),
        launch_plan(),
        profile_store_entry(),
    );
    ...
    assert_eq!(status.managed_state, BrowserManagedState::LaunchPending);
    assert_eq!(
        status.capability_status,
        BrowserCapabilityStatus::BridgeMissing
    );
    ...
    assert_eq!(
        ...
        Some(constants::value::MANAGED_BROWSER_LAUNCH_PENDING.to_string())
    );
    assert_eq!(
        payload[constants::field::BRIDGE_ENDPOINT_REF],
        LogFieldValue::String(
...
fn launch_plan() -> BrowserManagedLaunchPlan {
    BrowserManagedLaunchPlan {
        executable_path: PathBuf::from(constants::browser::EXECUTABLE_CHROME_WINDOWS),
        args: vec![constants::browser::CHROMIUM_DEFAULT_URL.to_string()],
