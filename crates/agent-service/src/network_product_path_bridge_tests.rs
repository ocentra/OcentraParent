...
use super::network_product_path_bridge::prove_network_product_path_for_read_model;

#[test]
fn stored_network_flow_rows_drive_product_path_refs_without_enforcement() {
    let report = prove_network_product_path_for_read_model(&read_model(vec![full_metadata_row()]));
...
fn tombstoned_network_flow_rows_do_not_drive_product_path_decisions() {
    let report = prove_network_product_path_for_read_model(&ActivityNetworkFlowReadModel {
        returned: 0,
        active_rows: 0,
        tombstone_rows: 1,
        exportable_rows: 0,
...
fn rows_without_domain_target_are_skipped_without_inventing_policy_refs() {
    let report =
        prove_network_product_path_for_read_model(&read_model(vec![row(None, None, None)]));
...
fn row_ref(prefix: &str) -> String {
    ...
    value.push_str(constants::activity_store::TEST_NETWORK_EVENT_ID);
    value
}
