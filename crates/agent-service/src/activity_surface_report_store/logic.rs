...
pub(crate) fn save_report_document_to_dir(
    ...
    let file_name = report_file_name(&report).0;
    ...
    match write_report_to_dir(directory.clone(), file_name.clone(), &report) {
        Ok(()) => report,
        Err(_) => {
            report.saved_metadata = Some(saved_metadata(SavedMetadataArgs {
                report_id: report.report_id.clone(),
...
fn history_summary(report: &ActivityReportDocument) -> ReportText {
    ReportText(
        report
            .sections
            .iter()
