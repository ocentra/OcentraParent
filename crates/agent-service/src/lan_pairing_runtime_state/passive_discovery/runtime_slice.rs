...
use super::{
    ...
    LanPassiveDiscoveryRuntimeObservedState, LanPassiveDiscoveryTextSource,
    PASSIVE_DISCOVERY_MAX_DATAGRAMS_PER_SOURCE, PASSIVE_DISCOVERY_READ_TIMEOUT,
};
...
impl LanPairingRuntime {
    pub(crate) fn record_passive_rescan_trigger(
        ...
        summary: impl LanPassiveDiscoveryTextSource,
    ) {
        let observed_at: String = timestamp_now();
        let summary = summary.as_passive_discovery_text_ref();
        if let Ok(mut state) = self.passive_discovery_listener_state.lock() {
            let _ = state.record_rescan_trigger(trigger_reason, &observed_at, summary.0);
        }
    }
    ...
    pub(crate) fn record_local_network_change_triggers_if_needed(
        ...
        observed_state: &mut LanPassiveDiscoveryRuntimeObservedState,
        observed_at: impl LanPassiveDiscoveryTextSource,
        current_identity: &LanPassiveRuntimeLocalNetworkIdentity,
    ) {
        let observed_at = observed_at.as_passive_discovery_text_ref();
