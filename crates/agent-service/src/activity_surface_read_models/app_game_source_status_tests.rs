use ocentra_parent_agent_protocol::{
    ...
    APP_GAME_INVENTORY_SOURCE_STORE_PACKAGE, APP_GAME_INVENTORY_STATE_DETECTABLE,
    ...
    APP_GAME_OBSERVATION_MODE_LAUNCHER_MANIFEST, APP_GAME_OBSERVATION_MODE_PROCESS_SNAPSHOT,
    APP_GAME_PRODUCT_NATIVE_APP, APP_GAME_PRODUCT_NATIVE_GAME, APP_GAME_RUNTIME_NOT_CLAIMED,
    APP_GAME_RUNTIME_RUNNING, APP_GAME_SCHEMA_VERSION, APP_GAME_TEST_CATALOG_REF,
    APP_GAME_TEST_DISPLAY_LABEL, APP_GAME_TEST_EXECUTABLE_PATH_REF,
    APP_GAME_TEST_FOREGROUND_EVIDENCE_ID, APP_GAME_TEST_GAME_DISPLAY_LABEL,
    ...
    APP_GAME_TEST_LAUNCHER_REF, APP_GAME_TEST_PROCESS_ID, APP_GAME_TEST_PROCESS_IDENTITY,
    APP_GAME_TEST_PROCESS_NAME, APP_GAME_TEST_REGISTRY_SOURCE_REF,
...
#[test]
fn app_use_read_model_preserves_non_ready_source_status_state() {
    let mut model = service_model();
    model.inventory_rows[0].capability_status =
        APP_GAME_CAPABILITY_STATUS_MANUAL_REQUIRED.to_string();
    model.running_now_rows[0].capability_status = APP_GAME_CAPABILITY_STATUS_DEGRADED.to_string();
    model.foreground_now_rows[0].capability_status =
        APP_GAME_CAPABILITY_STATUS_NOT_CLAIMED.to_string();

    let read_model = app_use_read_model(surface_request(), Some(model));
    let source_rows = &read_model.rows[0].source_status_rows;
    let inventory_status = source_rows
        .iter()
        .find(|row| row.source_kind == APP_GAME_INVENTORY_SOURCE_OS_INSTALLED_RECORD)
        .expect(constants::error::ACTIVITY_STORE_QUERIES);
    let runtime_status = source_rows
        .iter()
        .find(|row| row.source_kind == APP_GAME_OBSERVATION_MODE_PROCESS_SNAPSHOT)
        .expect(constants::error::ACTIVITY_STORE_QUERIES);
    let foreground_status = source_rows
        .iter()
        .find(|row| row.source_kind == APP_GAME_OBSERVATION_MODE_FOREGROUND_WINDOW)
        .expect(constants::error::ACTIVITY_STORE_QUERIES);

    assert_eq!(
        inventory_status.capability_status,
    ...
    assert_eq!(runtime_status.state, ActivityReadModelState::Degraded);
    assert_eq!(
        foreground_status.capability_status,
