use ocentra_parent_agent_core::{
    prove_network_runtime_remote_delivery_durable_envelope, NetworkRuntimeRemoteDeliveryState,
};
...
pub(crate) async fn build_network_remote_delivery_status_report(
    ...
    build_event(
        ...
        LogLevel::Info,
        network_remote_delivery_status_payload().await,
        None,
    )
}

pub(crate) async fn network_remote_delivery_status_payload() -> LogFields {
...
async fn network_remote_delivery_status() -> NetworkRemoteDeliveryStatus {
...
fn count(value: usize) -> u64 {
    u64::try_from(value).expect(constants::error::AGENT_EVENT_SERIALIZES)
}
