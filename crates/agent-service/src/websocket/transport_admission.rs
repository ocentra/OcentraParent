use ocentra_parent_agent_protocol::{
    ...
    transport::{AgentCommandEnvelope, AgentCommandName, AgentEventEnvelope, AgentEventName},
};
...
pub(super) fn transport_route_rejection(
    ...
    _provenance: WebsocketPeerProvenance,
    ...
    // commands retain their existing route/pairing owners. The peer address
    // is transport provenance only: loopback does not authenticate a parent
    // session, and this boundary has no owner-controlled session capability
    // to bind to the envelope. Keep the route unavailable until such an
    // authenticated adapter is supplied; do not let source/route or peer IP
    // mint that authority.
