...
use ocentra_parent_agent_protocol::{
    ...
    AppGameEvidenceClaim, AppGamePolicyEvaluationReadModel, LogFieldValue, LogFields,
    ...
    APP_GAME_POLICY_EVALUATION_STATUS_MANUAL_REQUIRED, APP_GAME_RUNTIME_NOT_CLAIMED,
...
#[tokio::test]
async fn app_game_policy_evaluation_command_reports_service_backed_dry_run_rows() {
    ...
    let read_model = policy_evaluation_payload(
        &event.payload[constants::field::APP_GAME_POLICY_EVALUATION_READ_MODEL],
    );
    ...
    assert_eq!(
        event.event,
        AgentEventName::AgentActivityAppGamePolicyEvaluationReadModelReported
    );
    assert_eq!(
    ...
    assert_eq!(read_model.rows.len(), 4);
    assert_eq!(
        read_model.rows[0].evaluation_kind,
        APP_GAME_POLICY_EVALUATION_KIND_TIME_LIMIT
    );
    assert_eq!(
        read_model.rows[0].decision_state,
        APP_GAME_POLICY_EVALUATION_DECISION_MANUAL_REQUIRED
    );
    assert_eq!(
        read_model.rows[0].rejection_reason,
        APP_GAME_POLICY_EVALUATION_REJECTION_MISSING_POLICY_EVIDENCE
    );
    assert_eq!(
...
fn command_envelope() -> AgentCommandEnvelope {
    AgentCommandEnvelope {
        ...
        message_id: constants::event_id::ACTIVITY_APP_GAME_POLICY_EVALUATION_READ_MODEL_REPORTED
            .to_string(),
        ...
        command: AgentCommandName::AgentActivityAppGamePolicyEvaluationReadModelGet,
...
fn policy_evaluation_payload(value: &LogFieldValue) -> AppGamePolicyEvaluationReadModel {
    match value {
