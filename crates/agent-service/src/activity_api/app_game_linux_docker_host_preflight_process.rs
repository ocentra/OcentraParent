...
use super::{
    app_game_linux_docker_host_preflight_cleanup_owner::{
        CleanupWorkerRegistry, ReservedCleanupOwner,
    },
...
pub(super) fn run_docker_probe(
    ...
    let Ok(worker) = thread::Builder::new().spawn(move || {
        run_docker_probe_on_runtime(executable, arguments, deadline, cleanup_workers)
    }) else {
        return DockerProbeOutput::unavailable();
    };
    worker
...
fn run_docker_probe_on_runtime(
    ...
    runtime.block_on(run_docker_probe_async(
        executable,
        arguments,
        deadline,
        cleanup_workers,
...
async fn run_docker_probe_async(
    ...
    let mut supervisor = match spawn_process_group(&mut command, deadline, cleanup_workers) {
