...
use ocentra_parent_agent_protocol::{
    ...
    policy_constants, AgentCommandEnvelope, AgentCommandName, AgentEventEnvelope,
    AgentEventName, AgentMessageTarget, AgentPeer, AgentPeerRole, AgentRoute, LogFieldValue,
    LogFields, ParentPlatform, V08EnforcementIntegrityRuntimeAuditEntry,
    V08EnforcementIntegrityRuntimeAuditIntegrityState, V08EnforcementIntegrityRuntimeAuditReadModel,
    V08EnforcementIntegrityRuntimeAuditResult, AGENT_PROTOCOL_SCHEMA_VERSION,
};
...
fn enforcement_integrity_runtime_audit_read_model_covers_required_states() {
    ...
    assert_eq!(result_count(&result_counts, proof::RESULT_MANUAL_REQUIRED), 2);
    ...
    assert_eq!(integrity_count(&integrity_counts, proof::INTEGRITY_RUNNING), 8);
    assert_eq!(
    ...
    assert_eq!(
        integrity_count(&integrity_counts, proof::INTEGRITY_TAMPER_SIGNAL_MANUAL_REQUIRED),
        1
    );
}
...
fn count_integrity_states(
    ...
    entries.iter().fold(BTreeMap::new(), |mut counts, entry| {
        *counts.entry(integrity_name(entry.integrity_state)).or_default() += 1;
        counts
    })
}
...
fn integrity_name(state: V08EnforcementIntegrityRuntimeAuditIntegrityState) -> &'static str {
    match state {
        ...
        V08EnforcementIntegrityRuntimeAuditIntegrityState::ServiceStopped => "service-stopped",
