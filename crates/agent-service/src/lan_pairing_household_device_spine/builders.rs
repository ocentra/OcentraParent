...
use ocentra_parent_agent_protocol::{
    constants, DeviceRuntimeRouteState, LanBrowserAddDeviceDiscoveryDevice,
    ...
    LanCanonicalHouseholdDeviceSource, LanPairingDeviceReachability, LanPairingNetworkMode,
...
pub(super) fn device_from_discovery(
    ...
    let route_state = route_state_for(is_child_agent, discovered.discovery_status.clone());
    LanCanonicalHouseholdDevice {
        ...
        network_identity: network_identity_for(
            device,
...
pub(super) fn device_from_registry(
    ...
    let route_state = DeviceRuntimeRouteState::LocalNetwork;
    LanCanonicalHouseholdDevice {
        schema_version: constants::lan_pairing::SCHEMA_VERSION,
        canonical_device_id: canonical_device_id(device),
        display_name: display_name_for(device),
