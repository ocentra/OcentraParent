use ocentra_parent_agent_protocol::{
    constants, DeviceRuntimeRoleState, DeviceRuntimeRouteState,
    ...
    LanChildAgentInventoryPacket, LanPairingDeviceReachability, LanPairingDeviceRef,
    LanPairingDiscoveryRuntimeStatus, LanPairingProductionDiscoveryState, LanPairingTrustState,
};
...
pub(super) fn route_state_for(
    ...
    if !is_child_agent {
        return DeviceRuntimeRouteState::Unavailable;
    }
    match status {
        LanPairingDiscoveryRuntimeStatus::WebsocketDirect => DeviceRuntimeRouteState::LocalNetwork,
        LanPairingDiscoveryRuntimeStatus::NetworkNeighbor => {
            DeviceRuntimeRouteState::ManualRequired
        }
        LanPairingDiscoveryRuntimeStatus::PlannedUnsupported => {
            DeviceRuntimeRouteState::Unavailable
        }
    }
}
...
fn known_hostname(device: &LanPairingDeviceRef) -> Option<String> {
    device.hostname.as_ref().and_then(|hostname| {
        if hostname.is_empty()
            || hostname.eq_ignore_ascii_case(constants::lan_pairing::NETWORK_NEIGHBOR_UNKNOWN_HOSTNAME)
        {
            None
        } else {
            Some(hostname.clone())
