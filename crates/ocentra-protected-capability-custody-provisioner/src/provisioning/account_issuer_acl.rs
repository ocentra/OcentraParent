...
pub(super) fn revalidate() -> Result<(), ProvisioningError> {
    observe(map_revalidation_error)
}

fn observe(error_mapper: fn(FfiError) -> ProvisioningError) -> Result<(), ProvisioningError> {
    ...
    let mut key = AccountIssuerP256Key::open_machine().map_err(error_mapper)?;
