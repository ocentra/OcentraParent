...
fn parent_route_snapshot_serializes_with_host_bridge_snapshot_fields() {
    let value = require_ok(
        serde_json::to_value(load_parent_route_snapshot(ParentRouteId::Activity, None)),
        "parent route snapshot serializes",
    );
...
fn start_route_snapshot_attaches_setup_first_run_panel() {
    let value = require_ok(
        serde_json::to_value(load_parent_route_snapshot(ParentRouteId::Start, None)),
        "start route snapshot serializes",
    );
    ...
    assert_eq!(
        value["setupFirstRunPanel"]["cards"][2]["details"][1]["value"],
        "presentation only"
    );
}
...
fn proof_panels_route_snapshot_attaches_browser_route_panels() {
    let value = require_ok(
        ...
        "proof-panels route snapshot serializes",
    );
...
fn parent_route_snapshot_keeps_overlay_diagnostics_on_dev_routes_only() {
    ...
    let browser_value = require_ok(
        serde_json::to_value(load_parent_route_snapshot(ParentRouteId::Browser, None)),
        "browser route snapshot serializes",
    );
    let diagnostics_value = require_ok(
        serde_json::to_value(load_parent_route_snapshot(ParentRouteId::Diagnostics, None)),
        "diagnostics route snapshot serializes",
    );
    let frame_tuner_value = require_ok(
        serde_json::to_value(load_parent_route_snapshot(ParentRouteId::FrameTuner, None)),
        "frame tuner route snapshot serializes",
    );
...
fn policy_network_route_load_keeps_host_bridge_surface_and_attaches_lan_read_model() {
    ...
    let value = with_agent_addr(&address, || {
        require_ok(
            ...
            "policy-network route snapshot serializes",
        )
    });
...
fn parent_subscription_event_serializes_for_host_bridge() {
    ...
    let value = with_agent_addr(&address, || {
        require_ok(
            ...
            "parent subscription event serializes",
        )
    });
    ...
    assert_eq!(value["events"].as_array().map(|events| events.len()), Some(2));
    assert_eq!(value["events"][0]["eventId"], json!("agent.connection.ready-1"));
    assert_eq!(
...
fn parent_subscription_event_dedupes_duplicate_event_ids_while_preserving_latest_lan_event() {
    ...
    let value = with_agent_addr(&address, || {
        require_ok(
            serde_json::to_value(load_parent_subscription_event(ParentRouteId::Devices, None)),
            "parent subscription event serializes with duplicate event ids",
        )
    });

    assert_eq!(value["events"].as_array().map(|events| events.len()), Some(1));
    assert_eq!(value["events"][0]["eventId"], json!("agent.connection.ready-1"));
    assert_eq!(
...
fn parent_subscription_event_keeps_lan_diagnostics_and_history_surface_intact() {
    ...
    let snapshot_value = with_agent_addr(&snapshot_address, || {
        require_ok(
            serde_json::to_value(load_parent_route_snapshot(ParentRouteId::Devices, None)),
            "devices route snapshot serializes with explicit history",
        )
    });
    let subscription_value = with_agent_addr(&subscription_address, || {
        require_ok(
            serde_json::to_value(load_parent_subscription_event(ParentRouteId::Devices, None)),
            "devices subscription event serializes with explicit history",
        )
    });
...
fn parent_subscription_event_preserves_explicit_lan_history_state_labels() {
    for (state, expected) in [
        ...
        let value = with_agent_addr(&address, || {
            require_ok(
                serde_json::to_value(load_parent_subscription_event(ParentRouteId::Devices, None)),
                "devices subscription event serializes with explicit history state",
            )
        });
...
fn parent_subscription_event_projects_stale_lan_history_from_rust_metadata() {
    ...
    let value = with_agent_addr(&address, || {
        require_ok(
            serde_json::to_value(load_parent_subscription_event(ParentRouteId::Devices, None)),
            "devices subscription event serializes with stale LAN metadata",
        )
    });
...
fn devices_route_degrades_honestly_when_agent_service_response_times_out() {
    ...
    let value = with_agent_addr(&address, || {
        require_ok(
            serde_json::to_value(load_parent_route_snapshot(ParentRouteId::Devices, None)),
            "parent route snapshot serializes after agent timeout",
        )
    });
...
fn parent_ui_action_serializes_and_returns_snapshot() {
    ...
    let live_activity = result
        ...
        .as_ref()
        .and_then(|snapshot| snapshot.live_activity.as_ref())
        .unwrap_or_else(|| unreachable!("network flow command returns live activity snapshot"));
...
fn lan_agent_command_requested_for_devices_route_forwards_signed_child_observe_payload_and_replay_fields(
    ...
    assert_eq!(
        result
            ...
            .map(|read_model| read_model.discovery_source.as_str()),
        Some("physical-household-lan")
    );
    assert_eq!(result.events.len(), 2);
    assert_eq!(
        result
            ...
            .last()
            .and_then(|value| value.event.as_deref()),
        Some("agent.lan-pairing.signed-child-agent.reported")
    );
    assert_eq!(
        result
            .events
            ...
            .and_then(|payload| payload.get(constants::field::LAN_SIGNED_CHILD_AGENT_VERIFICATION)),
        Some(&json!(
...
#[test]
fn lan_scan_action_returns_bounded_error_when_response_times_out() {
    let (address, capture) = start_local_server_with_ready_only();
    let result = with_agent_addr(&address, || {
        dispatch_parent_ui_action(&ParentUiAction {
            action: ParentUiActionKind::LanPairingBrowserDiscoveryScanRequested,
    ...
    assert_eq!(
        result.message,
        "agent-service WebSocket command-response timed out after 15000ms"
    );
