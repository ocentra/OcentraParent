...
pub(super) fn validate_history_state(payload: &LogFields, entry_count: u64) -> Result<(), String> {
    ...
    let state_matches_entries = match history_state {
        LanDiscoveryEventHistoryState::AgentOffline | LanDiscoveryEventHistoryState::Degraded => {
            true
        }
        LanDiscoveryEventHistoryState::Ready => entry_count > 0,
