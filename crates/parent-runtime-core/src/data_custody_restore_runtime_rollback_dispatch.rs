...
> {
    ...
    let rollback_provider_operation_ref = provider_receipt
        .provider_operation_ref()
        .as_str()
        .to_owned();
    let restore = complete_restore_receipt(
        plan,
        ...
        original_restore_provider_operation_ref,
        Some(rollback_provider_operation_ref.clone()),
...
async fn rollback_migration_if_required(
    ...
    let original_provider_operation_ref = existing_migration
        .provider_operation_ref
        .as_ref()
        .map(|reference| reference.as_str().to_owned());
