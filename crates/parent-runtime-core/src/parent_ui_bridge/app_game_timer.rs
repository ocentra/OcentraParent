...
fn app_game_timer_parent_surface_empty_snapshot() -> ParentAppGameTimerParentSurfacePanelSnapshot {
    ...
    ParentAppGameTimerParentSurfacePanelSnapshot {
        ...
        local_handoff_artifact_rows: Vec::new(),
        rows: Vec::new(),
...
fn app_game_timer_parent_surface_read_model_snapshot(
    ...
    ParentAppGameTimerParentSurfacePanelSnapshot {
        ...
        local_handoff_artifact_rows: read_model.child_ux_local_handoff_artifact_records.iter().map(|record| app_game_panel_row(record.source_result_id.clone(), vec![
            app_game_detail("Target", app_game_timer_target_label(record.target_domain.as_str())),
            app_game_detail("Child reason refs", app_game_join_strings(&record.child_reason_reference_ids)),
            app_game_detail("Child status refs", app_game_join_strings(&record.child_status_reference_ids)),
            app_game_detail("Delivery", app_game_claimed_value(record.child_delivery_claimed)),
            app_game_detail("Notification delivery", app_game_claimed_value(record.notification_delivery_claimed)),
            app_game_detail("Adapter dispatch", app_game_claimed_value(record.adapter_dispatch_claimed)),
            app_game_detail("Platform state", app_game_claimed_value(record.platform_enforcement_claimed)),
            app_game_detail("Raw private source rows", app_game_claimed_value(record.raw_private_source_rows_included)),
        ])).collect(),
