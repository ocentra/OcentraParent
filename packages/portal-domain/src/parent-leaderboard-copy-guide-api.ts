...
export const PARENT_LEADERBOARD_COPY_API_GUIDES: readonly ParentLeaderboardCopyGuideTopic[] = [
  {
    id: 'api-providers',
    navLabel: 'AI',
    rank: 10,
    ...
    pages: [
      {
        ...
        body: 'Ocentra should treat AI as a helper layer, not as hidden household authority. The normal safety path is local and evidence-cited. External API AI can be useful for richer explanations, report writing, and parent chat only after a parent chooses provider, data scope, and device policy.',
        steps: [
          'Pick a default family AI profile: local only, local with API explainers, API assistant only, or disabled.',
      ...
      },
      {
        ...
        steps: [
          ...
          'Parent-device local model: runs on the parent device for reports or chat when it has enough local data.',
          'Local AI hub: a stronger home machine accepts queued typed requests from paired devices.',
          'API provider: an external AI service used only for parent-authorized summaries, explanations, or chat.',
        ],
      },
      {
