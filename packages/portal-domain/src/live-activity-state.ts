...
import { parseActivityMemoryGraphReadModel, type PortalActivityMemoryGraphReadModel } from './activity-memory-graph';
import {
...
const moduleUrl = (import.meta as ImportMeta & { readonly url?: string }).url ?? 'packages/portal-domain/src/live-activity-state.ts';
...
function parseLanAddDeviceReadModel(payload: AgentProtocolLogFields): PortalLanAddDeviceReadModel | null {
  ...
  logWarn(
    'portal-domain.live-activity-state.lan-add-device-read-model.unparsed',
    getStackTrace(),
    {
      issueCount: parsed.error.issues.length,
      fallback: 'none',
...
function normalizePortalLanAddDeviceReadModel(value: unknown): PortalLanAddDeviceReadModel | null {
  ...
  const scanSummary = normalizePortalLanScanSummary(raw.scanSummary);
  const selectedDeviceReadiness = normalizePortalLanSelectedDeviceReadiness(raw.selectedDeviceReadiness);
  const canonicalHouseholdDevices = normalizePortalLanCanonicalHouseholdDevices(raw.canonicalHouseholdDevices);
  const discoveredDevices = normalizeUnknownArray(raw.discoveredDevices);
  const pairingRequests = normalizeUnknownArray(raw.pairingRequests);
  const trustedDeviceRegistry = normalizeUnknownArray(raw.trustedDeviceRegistry);
  const householdDeviceDecisions = normalizeUnknownArray(raw.householdDeviceDecisions);
  const signedDiscoveryRelaySpine = normalizePortalLanSignedDiscoveryRelaySpine(raw.signedDiscoveryRelaySpine);
  const trustedDeviceIds = normalizeStringArray(raw.trustedDeviceIds);
  const revokedDeviceIds = normalizeStringArray(raw.revokedDeviceIds);
  const routeRequirementLabels = normalizeStringArray(raw.routeRequirementLabels);
  const auditCheckLabels = normalizeStringArray(raw.auditCheckLabels);
  const honestNonClaims = normalizeStringArray(raw.honestNonClaims);

  if (
    typeof raw.addDeviceState !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.discoverySource !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.localServiceDiscoveryState !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.physicalHouseholdLanState !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.cloudRelayState !== AgentProtocolDefaults.Primitive.String ||
    scanSummary === null ||
    selectedDeviceReadiness === null ||
    canonicalHouseholdDevices === null ||
    discoveredDevices === null ||
    pairingRequests === null ||
    trustedDeviceRegistry === null ||
    householdDeviceDecisions === null ||
    trustedDeviceIds === null ||
    revokedDeviceIds === null ||
    routeRequirementLabels === null ||
    auditCheckLabels === null ||
    honestNonClaims === null
  ) {
    return null;
  }

  const addDeviceState = raw.addDeviceState as string;
  const discoverySource = raw.discoverySource as string;
  const localServiceDiscoveryState = raw.localServiceDiscoveryState as string;
  const physicalHouseholdLanState = raw.physicalHouseholdLanState as string;
  const cloudRelayState = raw.cloudRelayState as string;
  const controllerAuthority =
    (typeof raw.controllerAuthority === AgentProtocolDefaults.Primitive.String ? raw.controllerAuthority : 'unknown') as string;
  const observerAuthority =
    (typeof raw.observerAuthority === AgentProtocolDefaults.Primitive.String ? raw.observerAuthority : 'unknown') as string;
...
function normalizePortalLanScanSummary(value: unknown): PortalLanScanSummary | null {
  ...
  if (
    typeof raw.schemaVersion !== AgentProtocolDefaults.Primitive.Number ||
    typeof raw.scannedDeviceCount !== AgentProtocolDefaults.Primitive.Number ||
    typeof raw.agentDeviceCount !== AgentProtocolDefaults.Primitive.Number ||
    typeof raw.passiveDeviceCount !== AgentProtocolDefaults.Primitive.Number ||
    typeof raw.infrastructureDeviceCount !== AgentProtocolDefaults.Primitive.Number ||
    typeof raw.unsupportedDeviceCount !== AgentProtocolDefaults.Primitive.Number ||
    sourceLabels === null
  ) {
    return null;
  }
  ...
  const scannedDeviceCount = raw.scannedDeviceCount as number;
  const agentDeviceCount = raw.agentDeviceCount as number;
  const passiveDeviceCount = raw.passiveDeviceCount as number;
  const infrastructureDeviceCount = raw.infrastructureDeviceCount as number;
  const unsupportedDeviceCount = raw.unsupportedDeviceCount as number;
...
function normalizePortalLanSelectedDeviceReadiness(value: unknown): PortalLanSelectedDeviceReadiness | null {
  ...
  if (
    typeof raw.schemaVersion !== AgentProtocolDefaults.Primitive.Number ||
    !isNullableString(raw.selectedChildDeviceId) ||
    !isNullableString(raw.routeId) ||
    !isNullableString(raw.pairingId) ||
    typeof raw.trustState !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.reachability !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.readyForControl !== 'boolean' ||
    !isNullableString(raw.staleAt) ||
    !isNullableString(raw.offlineAt)
  ) {
    return null;
  }

  const schemaVersion = raw.schemaVersion as number;
  const selectedChildDeviceId = raw.selectedChildDeviceId;
  const routeId = raw.routeId;
  const pairingId = raw.pairingId;
  const trustState = raw.trustState as string;
  const reachability = raw.reachability as string;
  const readyForControl = raw.readyForControl;
  const staleAt = raw.staleAt;
  const offlineAt = raw.offlineAt;
...
function normalizePortalLanCanonicalHouseholdDevice(value: unknown): PortalLanCanonicalHouseholdDevice | null {
  ...
  if (
    typeof raw.displayName !== AgentProtocolDefaults.Primitive.String ||
    roleBadges === null ||
    sourceLabels === null ||
...
function normalizePortalLanCanonicalNetworkIdentity(value: unknown): PortalLanCanonicalNetworkIdentity | null {
  ...
  const reachability = raw.reachability as string;
  const normalizedEvidenceRecords = evidenceRecords as readonly unknown[] | undefined;
  ...
  return {
    ...
    ipAddresses,
    reachability,
    ...(normalizedEvidenceRecords === undefined ? {} : { evidenceRecords: normalizedEvidenceRecords }),
  };
}
...
function normalizePortalLanSignedProofRow(value: unknown): PortalLanSignedProofRow | null {
  ...
  if (
    typeof raw.check !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.proofState !== AgentProtocolDefaults.Primitive.String
  ) {
    return null;
  }
  const check = raw.check as string;
  const proofState = raw.proofState as string;
  return {
    ...raw,
    check,
    proofState,
...
function normalizePortalLanRouteSafetyRow(value: unknown): PortalLanRouteSafetyRow | null {
  ...
  if (
    typeof raw.check !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.custodyLabel !== AgentProtocolDefaults.Primitive.String
  ) {
    return null;
  }
  const check = raw.check as string;
  const custodyLabel = raw.custodyLabel as string;
  return {
    ...raw,
    check,
    custodyLabel,
...
function normalizePortalLanRelayCacheRow(value: unknown): PortalLanRelayCacheRow | null {
  ...
  if (
    typeof raw.check !== AgentProtocolDefaults.Primitive.String ||
    typeof raw.decisionState !== AgentProtocolDefaults.Primitive.String
  ) {
    return null;
  }
  const check = raw.check as string;
  const decisionState = raw.decisionState as string;
  return {
    ...raw,
    check,
    decisionState,
...
function normalizeStringArray(value: unknown): readonly string[] | null {
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== AgentProtocolDefaults.Primitive.String)) {
    return null;
  }
...
function isNullableString(value: unknown): value is string | null {
  return value === null || typeof value === AgentProtocolDefaults.Primitive.String;
}
