...
function __GeneratedPortalAgentActivitySurfaceSchema<T>(decoder: (value: unknown) => T): GeneratedPortalAgentActivitySurfaceSchemaParser<T> { return { parse: decoder, safeParse(value: unknown): { readonly success: true; readonly data: T } | { readonly success: false } { try { return { success: true, data: decoder(value) }; } catch { return { success: false }; } } } as const; }
