...
export const PARENT_LEADERBOARD_COPY_OPERATION_GUIDES: readonly ParentLeaderboardCopyGuideTopic[] = [
  {
    ...
    pages: [
      {
        ...
        body: 'Billing can gate paid product value such as device limits, cloud relay, remote access, advanced reports, parent assistant, long-window summaries, and exports. It must not silently break local evidence capture, journal integrity, audit history, or critical local safety behavior.',
        steps: [
    ...
    actions: [
      {
        ...
        tone: 'gold',
        targetRoutePath: '#/settings-rules',
  ...
  },
  {
    ...
    subtitle: 'Copy status, export safely, explain failures',
    detail: 'Help without leaks',
    tone: 'purple',
    category: 'Support',
    ...
    pages: [
      {
        ...
        title: 'Diagnostics should be useful without leaking private data',
        body: 'Support views should include agent URL, connection state, event ids, timestamps, source ids, health state, concise read-model rows, version, platform, and command results. They should redact secrets, private file paths, encrypted payloads, and raw private content.',
        steps: [
          'Show service health, journal status, query-store status, capture status, and connection state.',
          'Separate logs, evidence status, storage status, protocol events, and parent-visible activity.',
          'Copy only concise diagnostics by default.',
          'Make support bundle contents obvious before export.',
        ],
      },
      {
        eyebrow: 'FAILURE STATES',
        title: 'Errors should become parent-readable guidance',
        body: 'The parent should not have to read developer logs to understand stale device, missing bridge, unsupported browser, drive revoked, billing unavailable, notification failed, or platform unavailable. Each state needs a plain-language cause and next action.',
        steps: [
          'Use visible typed states for loading, empty, stale, degraded, unavailable, failed, rejected, and unsupported.',
          'Prefer one focused result panel or timeline over endless boxes.',
          'Keep developer diagnostics behind support affordances.',
          'Tie each failure to a next action when the parent can fix it.',
        ],
      },
    ],
    tips: [
      {
        ...
        body: 'Make included data classes visible before export.',
        tone: 'gold',
      },
      {
        label: 'Parent language',
        body: 'A failure state should say what happened and what the parent can do next.',
        tone: 'purple',
      },
    ],
    actions: [
      {
        ...
        body: 'Use Support to copy safe status for troubleshooting.',
        tone: 'purple',
        targetRoutePath: '#/diagnostics',
        targetNavLabel: 'SUPPORT',
      },
      {
        label: 'Export bundle',
        body: 'Export only after reviewing included data classes.',
        tone: 'gold',
        targetRoutePath: '#/diagnostics',
        targetNavLabel: 'SUPPORT',
      },
