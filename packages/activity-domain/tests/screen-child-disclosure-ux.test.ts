...
describe('screen child disclosure UX contracts', () => {
  ...
  it('rejects hidden capture, invisible active status, remote upload, raw path exposure, and missing audit refs', () => {
    ...
    expect(ScreenChildDisclosureStatusSchema.safeParse({ ...active, rawScreenshotRemoteUploadEnabled: true }).success).toBe(
      false
    );
    expect(ScreenChildDisclosureStatusSchema.safeParse({ ...active, rawScreenshotPathVisible: true }).success).toBe(false);
