...
} from '../src/browser-social-risk-benefit-signals';
...
function buildsCandidateSignals() {
  ...
  const signalSet = buildBrowserSocialRiskBenefitSignalSet({
    signalSetId: 'social-risk-benefit-signal-set-video',
...
function acceptsManualUnknownSignals() {
  const manualRisk = BrowserSocialRiskSignalSchema.safeParse(
    riskSignal({
      signalId: 'social-risk-signal-manual',
      kind: 'unknown-risk',
      severity: 'unknown',
...
function riskSignal(overrides = {}) {
  return {
    signalId: 'social-risk-signal-addictive-feed',
    kind: 'addictive-feed',
    severity: 'medium',
    state: 'candidate',
    confidence: 'medium',
...
function socialAiAnalysisResult() {
  ...
  return buildBrowserSocialAiAnalysisResult({
    ...
    riskSignalRefs: ['social-risk-signal-ref-video'],
