...
describe('agent protocol signed discovery relay spine', () => {
  ...
  it('carries the optional spine in the browser add-device read model', () => {
    ...
    expect(parsed.signedDiscoveryRelaySpine?.routeSafetyRows.map((row) => row.check)).toContain(
...
function adapterRows() {
  return [
    ...
    adapterRow('router-dhcp-name', 'manual-required', 'manual-required', 'manual-required', 'router-infrastructure-observation', 'DHCP proof'),
    ...
    adapterRow('signed-child-agent-hello', 'manual-required', 'manual-required', 'manual-required', 'signed-child-agent-artifact', 'hello proof'),
    adapterRow('signed-child-agent-heartbeat', 'manual-required', 'manual-required', 'manual-required', 'signed-child-agent-artifact', 'heartbeat proof'),
  ] as const;
}
