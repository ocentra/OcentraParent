...
describe('agent network remote delivery status contract', () => {
  it('parses row10f status from a typed agent event', () => {
    const parsed = parseAgentNetworkRemoteDeliveryStatusEvent(
      eventWithPayload({ networkRemoteDeliveryStatus: JSON.stringify(RemoteDeliveryStatus) })
    );
  ...
  it('rejects missing fields, malformed JSON, and stale row refs', () => {
    ...
    expect(parseAgentNetworkRemoteDeliveryStatusEvent(eventWithPayload({ networkRemoteDeliveryStatus: '{' }))).toEqual({
      ok: false,
...
function expectInvalid(value: unknown) {
  expect(
    parseAgentNetworkRemoteDeliveryStatusEvent(eventWithPayload({ networkRemoteDeliveryStatus: JSON.stringify(value) }))
