...
import { AgentLanBrowserAddDeviceReadModelSchema, AgentLanDiscoverySourceMatrixSchema, AgentProtocolDefaults } from '../src/contracts';
...
describe('agent protocol LAN discovery source matrix', () => {
  ...
  it('carries the matrix in the add-device read model', () => {
    ...
    expect(parsed.lanDiscoverySourceMatrix?.sourceRows.find((row) => row.source === 'mdns-dns-sd-query')?.canConfirmChildAgent).toBe(false);
  });
});
