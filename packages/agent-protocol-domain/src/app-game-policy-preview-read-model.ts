...
const PolicyPreviewServiceReadModelSchema = withParser(
  Schema.Struct({
    ...
    Schema.filter(
      (readModel) =>
        readModel.rows.every((row) => row.decision.dryRun && row.decision.enforcementHandoffState === 'disabled') ||
...
export type AgentAppGamePolicyPreviewTargetDomain = Infer<typeof AgentAppGamePolicyPreviewRowSchema>['targetDomain'];
...
function buildAppGamePolicyPreviewRow(row: Infer<typeof PolicyPreviewServiceRowSchema>): AgentAppGamePolicyPreviewRow {
