...
export const V08SupportedAdapterRuntimeBoundarySchema = withParser(Schema.Literal('windows-app-game-owned-process-time-limit', 'windows-network-flow-observe-policy-handoff', 'windows-broad-installed-app-blocking-manual-gate', 'windows-host-network-domain-blocking-manual-gate', 'windows-broad-installed-app-artifact-status', 'windows-host-network-domain-artifact-status', 'windows-managed-browser-artifact-status', 'windows-managed-exact-active-tab-not-claimed', 'windows-adapter-permission-dependency-degraded', 'linux-host-adapter-unavailable', 'macos-host-adapter-unsupported', 'android-mobile-control-manual-gate', 'ios-mobile-control-manual-gate'));
export const V08SupportedAdapterCapabilitySchema = withParser(Schema.Literal('app-game-owned-process-time-limit', 'network-flow-observe-policy-handoff', 'broad-installed-app-blocking', 'host-network-domain-blocking', 'broad-installed-app-artifact-status', 'host-network-domain-artifact-status', 'managed-browser-artifact-status', 'managed-exact-active-tab-enforcement', 'adapter-permission-dependency', 'desktop-host-platform-adapter', 'mobile-child-control-adapter'));
export const V08SupportedAdapterRuntimeStateSchema = withParser(Schema.Literal('implemented-boundary', 'manual-required', 'unavailable', 'not-claimed', 'unsupported', 'degraded'));
export const V08SupportedAdapterResultSchema = withParser(Schema.Literal('supported-boundary-proved', 'manual-proof-required', 'target-unavailable', 'not-claimed', 'unsupported-platform', 'degraded-permission-or-dependency'));
export const V08SupportedAdapterPlatformSupportStateSchema = withParser(Schema.Literal('supported-on-windows', 'manual-required', 'unavailable-on-target', 'unsupported-platform', 'degraded'));
export const V08SupportedAdapterTargetIdentityStateSchema = withParser(Schema.Literal('process-session-evidence-backed', 'network-flow-evidence-backed', 'insufficient-for-broad-target', 'not-applicable', 'unsupported-platform-target'));
export const V08SupportedAdapterRollbackReferenceStateSchema = withParser(Schema.Literal('timer-recovery-backed', 'observe-only-not-needed', 'manual-required', 'unavailable', 'not-claimed'));
export const V08SupportedAdapterAuditReferenceStateSchema = withParser(Schema.Literal('audit-reference-backed', 'manual-required', 'unavailable', 'not-claimed'));
export const V08SupportedAdapterRefusalReasonSchema = withParser(Schema.Literal('none', 'manual-artifact-required', 'target-unavailable', 'not-claimed-boundary', 'unsupported-platform', 'permission-or-dependency-degraded'));
...
export const V08SupportedAdapterRuntimeProofEntrySchema = withParser(V08SupportedAdapterRuntimeProofEntryBaseSchema.pipe(Schema.filter((entry) => supportedAdapterRuntimeProofEntryIsHonest(entry) ||
...
export const V08SupportedAdapterRuntimeProofReadModelSchema = withParser(Schema.Struct({
    schemaVersion: ParentContractSchemaVersionSchema,
...
}).pipe(Schema.filter((readModel) => new Set(readModel.entries.map((entry) => entry.proofEntryId)).size === readModel.entries.length ||
    'Expected V0.8 supported adapter runtime proof entry ids to be unique')));
function supportedAdapterRuntimeProofEntryIsHonest(entry) {
    if (supportedAdapterRuntimeProofEntryHasClaimUpgrade(entry)) {
        return false;
    }
    return supportedAdapterRuntimeProofEntryStateMatchesProof(entry);
}
function supportedAdapterRuntimeProofEntryStateMatchesProof(entry) {
...
function supportedAdapterRuntimeProofImplementedBoundaryIsHonest(entry) {
...
function supportedAdapterRuntimeProofManualRequiredIsHonest(entry) {
    return entry.adapterResult === 'manual-proof-required' && entry.manualProofRequirements.length > 0;
}
function supportedAdapterRuntimeProofUnavailableIsHonest(entry) {
    return (entry.adapterResult === 'target-unavailable' &&
        entry.platformSupportState === 'unavailable-on-target' &&
        entry.manualProofRequirements.length > 0);
}
function supportedAdapterRuntimeProofNotClaimedIsHonest(entry) {
    return entry.adapterResult === 'not-claimed' && entry.manualProofRequirements.length > 0;
}
function supportedAdapterRuntimeProofUnsupportedIsHonest(entry) {
    return (entry.adapterResult === 'unsupported-platform' &&
        entry.platformSupportState === 'unsupported-platform' &&
        entry.manualProofRequirements.length > 0);
}
function supportedAdapterRuntimeProofDegradedIsHonest(entry) {
    return (entry.adapterResult === 'degraded-permission-or-dependency' &&
        entry.platformSupportState === 'degraded' &&
        entry.refusalReason === 'permission-or-dependency-degraded' &&
        entry.manualProofRequirements.length > 0);
}
function supportedAdapterRuntimeProofEntryHasClaimUpgrade(entry) {
    return [
...
export const V08SupportedAdapterRuntimeBoundary = {
    WindowsAppGameOwnedProcessTimeLimit: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-app-game-owned-process-time-limit'),
    WindowsNetworkFlowObservePolicyHandoff: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-network-flow-observe-policy-handoff'),
    WindowsBroadInstalledAppBlockingManualGate: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-broad-installed-app-blocking-manual-gate'),
    WindowsHostNetworkDomainBlockingManualGate: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-host-network-domain-blocking-manual-gate'),
    WindowsBroadInstalledAppArtifactStatus: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-broad-installed-app-artifact-status'),
    WindowsHostNetworkDomainArtifactStatus: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-host-network-domain-artifact-status'),
    WindowsManagedBrowserArtifactStatus: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-managed-browser-artifact-status'),
    WindowsManagedExactActiveTabNotClaimed: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-managed-exact-active-tab-not-claimed'),
    WindowsAdapterPermissionDependencyDegraded: V08SupportedAdapterRuntimeBoundarySchema.parse('windows-adapter-permission-dependency-degraded'),
...
export const V08SupportedAdapterRuntimeProofReadModel = V08SupportedAdapterRuntimeProofReadModelSchema.parse({
    ...
    entries: [
        entry({
            ...
            claimBoundary: 'App/game support is limited to owned-process identity, app-session evidence, timer state, audit refs, and recoverable expiry; it is not broad installed-app blocking.',
            fallbackBehavior: 'Targets without process/session identity or timer custody return manual-required or degraded instead of escalating to broad block.',
        }),
        entry({
            ...
            manualProofRequirements: [],
            claimBoundary: 'Network/domain support is observe-only policy handoff over stored flow evidence; it is not DNS, VPN, packet, or host filter enforcement.',
            fallbackBehavior: 'Network controls without a host filter adapter report manual-required for enforcement while preserving observe-only evidence refs.',
        }),
        manualEntry('windows-broad-installed-app-blocking-manual-gate', V08SupportedAdapterRuntimeBoundary.WindowsBroadInstalledAppBlockingManualGate, 'windows', 'broad-installed-app-blocking', 'insufficient-for-broad-target', ['same app identity proof', 'host block apply artifact', 'rollback artifact', 'audit custody artifact'], 'Broad installed-app blocking remains manual-required because scoped process/timer proof does not prove package-wide blocking.', 'The runtime refuses broad app blocking claims until target host apply, rollback, and audit artifacts exist.'),
        manualEntry('windows-host-network-domain-blocking-manual-gate', V08SupportedAdapterRuntimeBoundary.WindowsHostNetworkDomainBlockingManualGate, 'windows', 'host-network-domain-blocking', 'insufficient-for-broad-target', ['host DNS or filter apply artifact', 'rollback artifact', 'audit custody artifact'], 'Host network/domain blocking remains manual-required because flow evidence and policy handoff are not filter apply proof.', 'The runtime refuses network/domain blocking claims until a host filter or DNS adapter proves apply and rollback.'),
        artifactStatusEntry('windows-broad-installed-app-artifact-status', V08SupportedAdapterRuntimeBoundary.WindowsBroadInstalledAppArtifactStatus, 'broad-installed-app-artifact-status', 'insufficient-for-broad-target', [
            'same-identity app package evidence',
            'adapter apply result',
            'adapter rollback result',
            'audit custody event',
            'manual review after artifact gate',
        ], 'Windows app artifacts can make a broad-app target ready for manual review only; they do not prove broad installed-app blocking.', 'Missing, mismatched, or uncustodied app artifacts stay refused and complete artifact sets remain manual-review-only.'),
        artifactStatusEntry('windows-host-network-domain-artifact-status', V08SupportedAdapterRuntimeBoundary.WindowsHostNetworkDomainArtifactStatus, 'host-network-domain-artifact-status', 'insufficient-for-broad-target', [
            'network/domain filter apply result',
            'network/domain filter rollback result',
            'audit custody event',
            'manual review after artifact gate',
        ], 'Windows network/domain artifacts can make a host-filter target ready for manual review only; they do not prove DNS, VPN, packet, or domain blocking.', 'Missing, mismatched, or uncustodied network artifacts stay refused and complete artifact sets remain manual-review-only.'),
        artifactStatusEntry('windows-managed-browser-artifact-status', V08SupportedAdapterRuntimeBoundary.WindowsManagedBrowserArtifactStatus, 'managed-browser-artifact-status', 'insufficient-for-broad-target', ['managed-browser exact URL evidence', 'audit custody event', 'manual review after artifact gate'], 'Windows managed-browser artifacts can make exact-URL control ready for manual review only; they do not prove active-tab enforcement.', 'Missing, mismatched, or uncustodied managed-browser artifacts stay refused and complete artifact sets remain manual-review-only.'),
        entry({
            ...
            claimBoundary: 'Exact active-tab enforcement is not claimed by supported app/game or network observe-only runtime proof.',
            fallbackBehavior: 'The runtime may report managed-session or process fallback states, but exact active-tab enforcement remains not-claimed.',
        }),
        entry({
            ...
            claimBoundary: 'Supported-boundary adapters can degrade when permissions or dependencies are missing; degraded state is not enforcement success.',
            fallbackBehavior: 'The runtime emits degraded capability and keeps evidence capture or observe-only paths available where possible.',
        }),
        entry({
        ...
        mobileManualEntry('android-mobile-control-manual-gate', V08SupportedAdapterRuntimeBoundary.AndroidMobileControlManualGate, 'android', ['device-owner or managed-profile artifact', 'UsageStats artifact', 'accessibility or VPN/DNS artifact']),
        mobileManualEntry('ios-mobile-control-manual-gate', V08SupportedAdapterRuntimeBoundary.IosMobileControlManualGate, 'ios', ['Family Controls entitlement artifact', 'DeviceActivity artifact', 'Network Extension artifact']),
    ],
});
function manualEntry(proofEntryId, runtimeBoundary, platform, adapterCapability, targetIdentityState, manualProofRequirements, claimBoundary, fallbackBehavior) {
    return entry({
        proofEntryId,
        runtimeBoundary,
        platform,
...
function artifactStatusEntry(proofEntryId, runtimeBoundary, adapterCapability, targetIdentityState, manualProofRequirements, claimBoundary, fallbackBehavior) {
    return entry({
        proofEntryId,
        runtimeBoundary,
        platform: 'windows',
...
function mobileManualEntry(proofEntryId, runtimeBoundary, platform, manualProofRequirements) {
    return manualEntry(proofEntryId, runtimeBoundary, platform, 'mobile-child-control-adapter', 'unsupported-platform-target', manualProofRequirements, 'Mobile child control remains manual-required and is not proved by Windows host supported-boundary adapters.', 'Mobile targets keep privileged platform states manual-required until real mobile artifacts exist.');
}
...
export const decodeV08SupportedAdapterRuntimeProofEntry = Schema.decodeUnknownSync(V08SupportedAdapterRuntimeProofEntrySchema);
export const decodeV08SupportedAdapterRuntimeProofReadModel = Schema.decodeUnknownSync(V08SupportedAdapterRuntimeProofReadModelSchema);
//# sourceMappingURL=v0-8-supported-adapter-runtime-proof.js.map