...
export const V08EnforcementIntegrityRuntimeAuditSurfaceSchema = withParser(Schema.Literal('app-game-time-limit', 'managed-browser-session', 'unmanaged-browser-process-fallback', 'network-domain-observe-only', 'host-network-domain-filter', 'notification-delivery', 'integrity-heartbeat', 'tamper-uninstall-signal', 'mobile-child-control'));
export const V08EnforcementIntegrityRuntimeAuditResultSchema = withParser(Schema.Literal('succeeded', 'failed', 'unavailable', 'expired', 'rolled-back', 'superseded', 'no-op', 'manual-required', 'unsupported', 'observe-only'));
export const V08EnforcementIntegrityRuntimeAuditExecutionSchema = withParser(Schema.Literal('executed-supported-boundary', 'dry-run-no-adapter-execution', 'rejected-before-adapter', 'manual-required-no-execution', 'observe-only-no-execution', 'unavailable-no-execution', 'unsupported-no-execution', 'recovery-needed-no-execution'));
export const V08EnforcementIntegrityRuntimeAuditIntentStateSchema = withParser(Schema.Literal('validated', 'observe-only', 'rejected-invalid', 'rejected-stale', 'rejected-wrong-device', 'rejected-unsupported'));
export const V08EnforcementIntegrityRuntimeAuditTimerStateSchema = withParser(Schema.Literal('active-timer-backed', 'expired-backed', 'cancelled-backed', 'rollback-backed', 'recovery-needed', 'not-applicable', 'unavailable'));
export const V08EnforcementIntegrityRuntimeAuditRollbackStateSchema = withParser(Schema.Literal('not-needed', 'rollback-token-backed', 'rollback-completed', 'rollback-required', 'manual-required', 'unavailable'));
export const V08EnforcementIntegrityRuntimeAuditChildStateSchema = withParser(Schema.Literal('status-ref-backed', 'reason-ref-backed', 'approval-intent-backed', 'manual-required', 'unsupported', 'not-claimed'));
export const V08EnforcementIntegrityRuntimeAuditIntegrityStateSchema = withParser(Schema.Literal('running', 'permission-missing', 'adapter-unavailable', 'stale-heartbeat', 'service-stopped', 'uninstall-detection-manual-required', 'tamper-signal-manual-required', 'anti-tamper-not-claimed', 'not-applicable'));
export const V08EnforcementIntegrityRuntimeAuditAuditStateSchema = withParser(Schema.Literal('audit-backed', 'audit-required', 'manual-required', 'unavailable', 'not-claimed'));
...
export const V08EnforcementIntegrityRuntimeAuditEntrySchema = withParser(V08EnforcementIntegrityRuntimeAuditEntryBaseSchema.pipe(Schema.filter((entry) => integrityAuditEntryIsHonest(entry) ||
...
export const V08EnforcementIntegrityRuntimeAuditReadModelSchema = withParser(Schema.Struct({
    schemaVersion: ParentContractSchemaVersionSchema,
    ...
    notificationProviderStatusBoundary: V08NotificationProviderStatusBoundaryReadModelSchema,
}).pipe(Schema.filter((readModel) => new Set(readModel.entries.map((entry) => entry.auditEntryId)).size === readModel.entries.length ||
    'Expected V0.8 enforcement integrity runtime audit entry ids to be unique')));
function integrityAuditEntryIsHonest(entry) {
...
function integrityAuditEntryHasClaimUpgrade(entry) {
    return [
...
const AllowedExecutionsByResult = {
    succeeded: ['executed-supported-boundary'],
    expired: ['executed-supported-boundary'],
    'rolled-back': ['executed-supported-boundary'],
    superseded: ['executed-supported-boundary'],
    'no-op': ['dry-run-no-adapter-execution', 'rejected-before-adapter'],
    failed: ['rejected-before-adapter'],
    'observe-only': ['observe-only-no-execution'],
    'manual-required': ['manual-required-no-execution'],
    unavailable: ['unavailable-no-execution', 'recovery-needed-no-execution'],
    unsupported: ['unsupported-no-execution'],
};
function integrityAuditExecutionMatchesResult(entry) {
    return executionIsAllowed(AllowedExecutionsByResult[entry.result], entry.execution);
}
function executionIsAllowed(allowedExecutions, execution) {
    return allowedExecutions.some((allowedExecution) => allowedExecution === execution);
}
function integrityAuditStateHasRequiredRefs(entry) {
...
function integrityAuditNonClaimStateIsExplicit(entry) {
...
const SourceReadModelIds = {
    ...
    NotificationProviderStatusBoundary: 'v0-8-notification-provider-status-boundary',
};
export const V08EnforcementIntegrityRuntimeAuditReadModel = V08EnforcementIntegrityRuntimeAuditReadModelSchema.parse({
    ...
    notificationProviderStatusBoundary: V08NotificationProviderStatusBoundaryReadModel,
    entries: [
        supportedEntry({
        ...
        manualEntry({
            ...
            boundary: 'Host network/domain filtering remains manual-required until apply, rollback, and audit artifacts exist.',
        }),
        unavailableEntry({
        ...
        manualEntry({
            ...
            boundary: 'Tamper/uninstall is represented as detectable/manual-required state only; no stealth, persistence, or anti-tamper hardening is claimed.',
        }),
    ],
});
function supportedEntry(input) {
    return entry({
...
function noExecutionEntry(input) {
    return entry({
        auditEntryId: input.auditEntryId,
...
function unsupportedEntry(input) {
    return entry({
        auditEntryId: input.auditEntryId,
        surface: input.surface,
...
export const decodeV08EnforcementIntegrityRuntimeAuditEntry = Schema.decodeUnknownSync(V08EnforcementIntegrityRuntimeAuditEntrySchema);
export const decodeV08EnforcementIntegrityRuntimeAuditReadModel = Schema.decodeUnknownSync(V08EnforcementIntegrityRuntimeAuditReadModelSchema);
//# sourceMappingURL=v0-8-enforcement-integrity-runtime-audit.js.map
