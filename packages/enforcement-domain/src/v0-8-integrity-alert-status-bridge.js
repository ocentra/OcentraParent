...
export const V08IntegrityAlertStateSchema = withParser(Schema.Literal('permission-loss', 'stale-heartbeat', 'stopped-or-removed', 'tamper-manual-required'));
export const V08IntegrityAlertParentVisibleStatusSchema = withParser(Schema.Literal('permission-action-required', 'agent-heartbeat-stale', 'agent-stopped-or-removed', 'tamper-review-required'));
export const V08IntegrityAlertNotificationIntentStateSchema = withParser(Schema.Literal('intent-created', 'manual-review-required'));
...
export const V08IntegrityAlertStatusBridgeEntrySchema = withParser(V08IntegrityAlertStatusBridgeEntryBaseSchema.pipe(Schema.filter((entry) => integrityAlertStatusBridgeEntryIsHonest(entry) ||
...
export const V08IntegrityAlertStatusBridgeReadModelSchema = withParser(Schema.Struct({
    schemaVersion: ParentContractSchemaVersionSchema,
...
}).pipe(Schema.filter((readModel) => new Set(readModel.entries.map((entry) => entry.bridgeEntryId)).size === readModel.entries.length ||
    'Expected V0.8 integrity alert/status bridge entry ids to be unique'), Schema.filter((readModel) => integrityAlertStatusBridgeCoversRequiredStates(readModel.entries) ||
...
function integrityAlertStatusBridgeEntryIsHonest(entry) {
    if (integrityAlertStatusBridgeEntryHasClaimUpgrade(entry)) {
        return false;
    }
    return (entry.deliveryState === 'not-delivered-provider-not-configured' &&
...
function integrityAlertStatusBridgeEntryHasClaimUpgrade(entry) {
    return [
...
function integrityAlertStatusBridgeManualProofMatchesState(entry) {
...
function integrityAlertStatusBridgeCoversRequiredStates(entries) {
    const states = new Set(entries.map((entry) => entry.integrityAlertState));
    return ['permission-loss', 'stale-heartbeat', 'stopped-or-removed', 'tamper-manual-required'].every((state) => states.has(state));
}
...
export const V08IntegrityAlertStatusBridgeReadModel = V08IntegrityAlertStatusBridgeReadModelSchema.parse({
    ...
    entries: [
        entry({
            ...
            boundary: 'Permission loss is parent-visible unavailable status with notification intent and audit refs; provider delivery remains unconfigured and unclaimed.',
        }),
        entry({
            ...
            boundary: 'Stale heartbeat is a degraded integrity status and alert intent; it is not anti-tamper hardening or provider-delivery proof.',
        }),
        entry({
            ...
            boundary: 'Stopped or removed service state is represented as an auditable parent status and alert intent; it does not install persistence or anti-removal behavior.',
        }),
        entry({
            ...
            boundary: 'Tamper/uninstall remains manual-required evidence review only; no stealth, persistence, privilege escalation, or anti-tamper resistance is claimed.',
        }),
    ],
});
...
export const decodeV08IntegrityAlertStatusBridgeEntry = Schema.decodeUnknownSync(V08IntegrityAlertStatusBridgeEntrySchema);
export const decodeV08IntegrityAlertStatusBridgeReadModel = Schema.decodeUnknownSync(V08IntegrityAlertStatusBridgeReadModelSchema);
//# sourceMappingURL=v0-8-integrity-alert-status-bridge.js.map