...
const EffectStatusPatterns = [
  ...
  [/not a generic third-party API|not-implemented|future|relay|Find My|lost mode|MDM|supervision|iOS.*limits/iu, 'future-gap'],
  [/denied|revok|permission|background|foreground|precise|approximate|reduced-accuracy|service disabled|entitlement/iu, 'permission-required'],
  [/manual|required setup|manual-required|user-entered|parent must|child must respond|consent|disclosure/iu, 'manual-required'],
  [/stale|offline|battery|throttle|low power|airplane|no signal|approximate|weak|limited|degrade|not instant|may fail/iu, 'degraded'],
  [/current location|fresh fix|live tracking|continuous|geofence|arrival|departure|dwell|policy decision|exact|coordinates|proof/iu, 'proof-required'],
  [/history|retention|custody|audit|report|map|delete|export|summary|label|source|timestamp|accuracy/iu, 'already-represented'],
...
const RuntimeOwnerPatterns = [
  ...
  [/history|retention|custody|export|delete|parent-owned storage|report|map|journal|query store/iu, 'parent-owned-storage'],
  [/location API|GPS|GNSS|Wi-Fi|cellular|Bluetooth|fused provider|geofence|background|foreground|permission|OS|adapter|lost mode|MDM/iu, 'os-adapter'],
...
const CapabilityRequirementPatterns = [
  [/current location|fresh fix|live|coordinate|GPS|GNSS|fused provider/iu, 'location-provider-freshness-accuracy-proof'],
...
const ProofRequirementPatterns = [
  [/policy decision|Enforce location-based policy|Missing proof/iu, 'Location-based policy decisions require fresh location evidence, typed parent rule, deterministic child-agent evaluation, and explicit ask/warn/report fallback.'],
  [/current location|fresh fix|live tracking|continuous|exact|last known/iu, 'Current or live location claims require source, permission, freshness, accuracy, custody, and adapter-state proof; last-known evidence must be labeled stale when applicable.'],
  [/geofence|arrival|departure|dwell|radius/iu, 'Geofence decisions require region shape, radius, schedule, transition type, freshness, and fallback proof.'],
  [/history|retention|custody|export|delete|storage/iu, 'Location history requires local custody, retention, deletion, and parent-reveal audit proof before display or export.'],
  [/permission|background|foreground|precise|approximate|disclosure/iu, 'Location controls require explicit OS permission state, precision state, and child-facing disclosure evidence.'],
] as const satisfies readonly PatternValue<string>[];

const SourceFallbackPatterns = [
  [/Portal-side policy evaluation/iu, 'Portal renders authoring metadata and reports only; child-agent/local runtime owns location policy evaluation.'],
  [/last known|current location|live map|continuous/iu, 'Show freshness, accuracy, and source labels; never imply stale last-known evidence is current live location.'],
  [/history|storage|Ocentra-hosted|retention|custody/iu, 'Keep location history local or parent-owned by default; never silently upload location history to Ocentra-hosted storage.'],
  [/geofence|arrival|departure|dwell/iu, 'When geofence proof is missing or throttled, degrade to ask, warn, report, or manual check-in instead of claiming enforcement.'],
] as const satisfies readonly PatternValue<string>[];

const EffectStatusFallbacks = {
  'permission-required': 'Disable strict tracking until required OS permission, precision, and disclosure proof are present.',
  ...
  'proof-required': 'Require evidence proof before strict alerts or policy effects; otherwise fall back to observe, ask, warn, or report.',
...
export function trackingCapabilityRequirementFor(
  sectionTitle: string,
  groupTitle: string,
  sourceText: string
): string {
...
export function trackingFallbackFor(
  effectStatus: TrackingControlEffectStatus,
  sourceText: string
): string {
  return (
