/* thin adapter helpers for Rust-seeded network control catalog metadata */
...
const PolicyLanePatterns = [
  ...
  [/block|enforce|firewall|wfp|packet filter|vpn|proxy|tunnel|adapter|rollback|strict action|terminate|router/iu, 'enforcement'],
  [/evidence|dns|domain|ip|port|protocol|process|flow|metadata|exact URL|encrypted|https|indicator|attribution/iu, 'evidence'],
...
const EffectStatusPatterns = [
  ...
  [/manual-required|manual required|admin|privilege|service installation|driver|mdm|supervision|entitlement|router api|WFP|Windows Filtering Platform|always-on|lockdown|force all traffic/iu, 'manual-required'],
  ...
  [/exact URL|path\/query|HTTPS|decrypted|payload|page body|chat content|search terms|form values|cookies|tokens|credentials|proof|evidence id|confidence|must cite|not proof/iu, 'proof-required'],
...
const RuntimeOwnerPatterns = [
  ...
  [/firewall|WFP|packet filter|VPN|proxy|router|DNS|resolver|Network Extension|DevicePolicyManager|VpnService|adapter|process|endpoint|ETW|IP Helper/iu, 'os-adapter'],
...
const ProofRequirementPatterns = [
  [/exact URL|path\/query|active tab|page title|download source/iu, 'Exact URL evidence requires managed browser, explicit URL filter, or proxy proof; passive network flow metadata is not enough.'],
  [/decrypted|payload|page body|chat content|search terms|form values|cookies|tokens|credentials|packet payload/iu, 'Network controls must not collect decrypted content or payload fields; use metadata evidence only.'],
  [/block|firewall|WFP|packet filter|VPN|router|always-on|lockdown|force all traffic|strict/iu, 'Strict network enforcement requires parent rule, evidence reference, real adapter result, audit event, and rollback or unavailable state.'],
  [/domain|DNS|IP|port|protocol|flow|process|indicator|attribution|confidence|evidence id/iu, 'Network claims require stored metadata evidence with source, freshness, confidence, and custody labels.'],
] as const satisfies readonly PatternValue<string>[];

const SourceFallbackPatterns = [
  [/exact URL|path\/query|active tab|page title/iu, 'Hide or disable exact URL controls unless managed browser, explicit URL filter, or proxy proof exists.'],
  [/decrypted|payload|page body|chat content|search terms|form values|cookies|tokens|credentials/iu, 'Never collect decrypted content or sensitive payload fields in the network-control catalog.'],
] as const satisfies readonly PatternValue<string>[];

const EffectStatusFallbacks = {
  'manual-required': 'Show manual-required until setup, privileges, and adapter proof exist; compile observe or unavailable fallback.',
  degraded: 'Render degraded state and keep unsupported behavior out of compiled enforcement plans.',
  'proof-required': 'Require evidence proof before strict effect; otherwise fall back to observe, warn, ask, or unavailable.',
