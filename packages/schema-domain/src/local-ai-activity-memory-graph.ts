...
export const LocalAiActivityMemoryTimeRangeSchema = withParser(
  Schema.Struct({
    ...
    Schema.filter(
      (range) =>
        Date.parse(range.observedUntil) >= Date.parse(range.observedFrom) ||
        'Expected memory graph time range to be ordered'
    )
  )
);

export const LocalAiActivityMemoryGraphNodeSchema = withParser(
  Schema.Struct({
...
export const LocalAiActivityMemoryGraphEdgeSchema = withParser(
  Schema.Struct({
    graphId: LocalAiActivityMemoryGraphIdSchema,
    ...
    Schema.filter((edge) => edge.fromNodeId !== edge.toNodeId || 'Expected memory graph edge endpoints to differ'),
    Schema.filter(
      (edge) =>
        edge.observedUntil === null ||
...
export const LocalAiActivityMemoryGraphQuerySchema = withParser(
  Schema.Struct({
...
export const LocalAiActivityMemoryGraphReadInputSchema = withParser(
  Schema.Struct({
...
export const LocalAiActivityMemoryGraphReadResultSchema = withParser(
  Schema.Struct({
    query: LocalAiActivityMemoryGraphQuerySchema,
    ...
    Schema.filter((result) => result.returnedNodeCount === result.nodes.length || 'Expected returned node count match'),
    Schema.filter((result) => result.returnedEdgeCount === result.edges.length || 'Expected returned edge count match'),
    Schema.filter((result) => result.edges.every((edge) => edgeReferencesReturnedNodes(edge, result.nodes)))
  )
);
...
function edgeReferencesReturnedNodes(
  edge: LocalAiActivityMemoryGraphEdge,
  nodes: readonly LocalAiActivityMemoryGraphNode[]
): boolean {
