...
const LocalAiModelArtifactText = NonEmptyStringSchema.pipe(
  Schema.filter((candidate) => isOpaqueLocalModelRef(candidate, LOCAL_MODEL_ARTIFACT_REF_PREFIX) || 'Expected local model artifact ref')
);
const LocalAiModelManifestText = NonEmptyStringSchema.pipe(
  Schema.filter((candidate) => isOpaqueLocalModelRef(candidate, LOCAL_MODEL_MANIFEST_REF_PREFIX) || 'Expected local model manifest ref')
);
...
export const LocalAiModelManifestIntegrityStateSchema = withParser(
  Schema.Literal('unavailable', 'unchecked', 'verified', 'manifest-missing', 'checksum-mismatch', 'signature-invalid', 'corrupted')
);
export const LocalAiModelDownloadStatusSchema = withParser(
  Schema.Literal('download-disabled', 'download-not-requested', 'download-in-progress', 'download-complete', 'download-failed')
);
export const LocalAiModelCacheUnavailableReasonSchema = withParser(
  Schema.Literal('model-source-unconfigured', 'artifact-not-installed', 'manifest-unavailable', 'download-disabled', 'cache-storage-unavailable', 'integrity-unverified', 'corruption-detected')
);
export const LocalAiModelCacheStorageErrorCodeSchema = withParser(
  Schema.Literal('cache-root-unavailable', 'manifest-read-failed', 'artifact-read-failed', 'metadata-write-disabled', 'storage-permission-denied', 'quota-unavailable')
);
export const LocalAiModelCacheCorruptionReasonCodeSchema = withParser(
  Schema.Literal('manifest-missing', 'checksum-mismatch', 'signature-invalid', 'artifact-missing', 'manifest-artifact-mismatch', 'unknown-integrity')
);

function isOpaqueLocalModelRef(candidate: unknown, prefix: typeof LOCAL_MODEL_ARTIFACT_REF_PREFIX | typeof LOCAL_MODEL_MANIFEST_REF_PREFIX): boolean {
  if (typeof candidate !== 'string' || LOCAL_MODEL_UNSAFE_REF_PATTERNS.some((pattern) => pattern.test(candidate))) return false;
...
export const LocalAiModelCacheStatusSchema = withParser(
  LocalAiModelCacheStatusBaseSchema.pipe(
    Schema.filter((status) => localAiModelCacheStatusIsConsistent(status) || 'Expected consistent local model cache status')
  )
);

function localAiModelCacheStatusIsConsistent(status: LocalAiModelCacheStatusCandidate): boolean {
  if (status.downloadEnabled !== (status.downloadStatus !== 'download-disabled')) return false;
  if (status.sourcePolicy === 'unavailable') {
    return status.cacheState === 'unavailable' && status.cacheHealth === 'unavailable' && status.manifestIntegrity === 'unavailable' && status.manifestRef === null && status.cacheByteSize === 0 && status.unavailableReason !== null;
  }
  if (status.cacheState === 'cache-ready') {
    return status.cacheHealth === 'healthy' && status.manifestIntegrity === 'verified' && status.manifestRef !== null && status.cacheByteSize > 0 && status.unavailableReason === null && status.storageError === null && status.corruptionReason === null;
  }
  return true;
}
