...
import {
  ScreenPolicyEvidenceChainWithDefaultsSchema,
...
function activityScreenPolicyEvidenceChainIsCoherent(row: {
  readonly policyDecisionRef: unknown;
  readonly policyAction: unknown;
  readonly policyReasonCodes: unknown;
  readonly parentRuleRefs: unknown;
  readonly localModelRuntimeRefs: unknown;
  readonly parentExplanationRefs: unknown;
  readonly explanationReasons: unknown;
  readonly deletionReasons: unknown;
}): boolean {
  return ScreenPolicyEvidenceChainWithDefaultsSchema.safeParse({
  ...
  Schema.Struct({
    ...
    rows: Schema.Array(
      Schema.Struct({
        ...
        Schema.filter(
          (row) =>
            activityScreenPolicyEvidenceChainIsCoherent(row) ||
            'Expected Activity screen policy evidence chain fields to remain coherent'
        )
      )
    ),
  })
