...
export const decodeSocialAndroidNativeAppCapabilityMatrix = (input: unknown) =>
  SocialAndroidNativeAppCapabilityMatrixSchema.parse(input);
...
function androidPackageVisibilityRowIsHonest(row: SocialAndroidNativeAppCapabilityRowCandidate): boolean {
  return (
    (row.capabilityState === 'app-level-capable-with-proof' &&
      row.proofState === 'existing-schema-domain-proof-ref' &&
