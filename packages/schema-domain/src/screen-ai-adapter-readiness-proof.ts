...
const V08SupportedAdapterCapabilitySchema = withParser(
  Schema.Literal(
...
const V08SupportedAdapterRuntimeStateSchema = withParser(
  Schema.Literal('implemented-boundary', 'manual-required', 'unavailable', 'not-claimed', 'unsupported', 'degraded')
);

const V08SupportedAdapterResultSchema = withParser(
  Schema.Literal(
...
const V08SupportedAdapterPlatformSupportStateSchema = withParser(
  Schema.Literal('supported-on-windows', 'manual-required', 'unavailable-on-target', 'unsupported-platform', 'degraded')
);

const V08SupportedAdapterTargetIdentityStateSchema = withParser(
  Schema.Literal(
...
const V08SupportedAdapterRollbackReferenceStateSchema = withParser(
  Schema.Literal('timer-recovery-backed', 'observe-only-not-needed', 'manual-required', 'unavailable', 'not-claimed')
);

const V08SupportedAdapterAuditReferenceStateSchema = withParser(
  Schema.Literal('audit-reference-backed', 'manual-required', 'unavailable', 'not-claimed')
);

const V08SupportedAdapterRefusalReasonSchema = withParser(
  Schema.Literal(
    'none',
