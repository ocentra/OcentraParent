...
describe('agent protocol defaults', () => {
  it('exposes the core protocol transport defaults', () => {
    ...
    expect(AgentProtocolDefaults.Target.LocalNetworkWindowsAgent.route).toBe('local-network');
  });
  ...
  it('preserves shared LAN, policy, and network value catalogs', () => {
    ...
    expect(AgentProtocolDefaults.NetworkRemoteDeliveryStatus.CrossProcessReplayCursorRef).toBe(
      'network.remote-delivery.cross-process-replay-cursor.10r'
    );
    expect(AgentProtocolDefaults.NetworkLinuxNftablesLabStatus.TableName).toBe('ocentra_parent_lab_row42a');
  });
});
