...
describe('policy control approval notification boundary', () => {
  it(
    'covers preview pending approved denied modified expired and replay-rejected approval queue states',
    assertQueueStates
  );
  it(
    'keeps preview-only assistant drafts confirmation-gated and approvals override-backed',
    assertConfirmationAndOverrideRules
  );
  it(
    'rejects rows that incorrectly claim parent review provider delivery or override creation',
    assertInvalidApprovalRows
  );
});
...
function assertConfirmationAndOverrideRules() {
...
function assertInvalidApprovalRows() {
  const previewOnly = entryFor('policy-control-preview-only-assistant-draft');
  const approved = entryFor('policy-control-approved-bonus-time');
  const denied = entryFor('policy-control-denied-temporary-override');
