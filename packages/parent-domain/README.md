...
- App/game source-gated policy preview timer service-readiness response
  consumer parent-surface read-model handoff contracts that consume
  parent-surface handoff rows and keep future parent-surface read-model proof
  refs visible without claiming response consumer implementation,
  parent-surface read-model runtime/persistence, parent-surface rendering,
  portal rendering, service runtime, adapter dispatch, or platform enforcement.
...
- App/game policy preview handoff and timer-status proofs remain read-only
  ...
  read-model proof refs visible while keeping parent-surface read-model
  runtime/persistence, parent-surface rendering, portal rendering, service
  runtime, and adapter dispatch unclaimed.
  Timer service-readiness response consumer parent-surface handoff rows consume
  response-consumer handoff rows and keep future parent-surface proof refs
  visible while keeping parent-surface rendering, portal rendering, service
  ...
  Timer service-readiness response consumer parent-surface status read-model
  parent-surface read-model service read API response handoff rows consume
  service read API handoff rows and keep future service read API response proof
  visible while keeping package exports, service read-model runtime emission,
  service event runtime emission, service events, read APIs, read API
  responses, response consumers, service runtime, protocol, rendering, portal
  rendering, persistence, and adapter dispatch unclaimed.
  Timer service-readiness response consumer parent-surface status read-model
  parent-surface read-model service read API response consumer handoff rows
  consume service read API response handoff rows and keep future response
  consumer proof visible while keeping package exports, service read-model
  runtime emission, service event runtime emission, service events, read APIs,
  read API responses, response consumers, service runtime, protocol, rendering,
  portal rendering, persistence, and adapter dispatch unclaimed.
