...
function rejectsRuntimeExecutionOverclaims(): void {
  it('rejects implemented public runtime runner status backend upload and legal execution states', () => {
    ...
    expect(ProductionSupportPublicationExecutionStatusRowSchema.safeParse({ ...runbookRow, publicRuntimeState: 'executed' }).success).toBe(false);
    expect(ProductionSupportPublicationExecutionStatusRowSchema.safeParse({ ...runbookRow, publicationRunnerState: 'implemented' }).success).toBe(false);
    expect(ProductionSupportPublicationExecutionStatusRowSchema.safeParse({ ...runbookRow, statusBackendState: 'executed' }).success).toBe(false);
    expect(ProductionSupportPublicationExecutionStatusRowSchema.safeParse({ ...uploadRow, supportBackendUploadState: 'executed' }).success).toBe(false);
    expect(ProductionSupportPublicationExecutionStatusRowSchema.safeParse({ ...legalRow, legalExecutionState: 'executed' }).success).toBe(false);
  });
}
