...
it('defines the reduced browser forest and keeps AI questions outside the browser surface', () => {
  ...
  expect(BrowserPolicyQuestions.filter((question) => question.surface === 'ai').map((question) => question.id)).toEqual(
    ['A1', 'A2', 'A3']
  );
  expect(
  ...
  expect(BrowserPolicyQuestionnaireCompactOrder).toHaveLength(45);
  expect(BrowserPolicyQuestions.find((question) => question.id === '1.1')?.options.map((option) => option.id)).toEqual([
    'off',
    'on',
    'paused',
    'emergency-allow',
...
it('reveals branch cards from selected parent decisions', () => {
  const base: BrowserPolicyAnswerMap = {
    ...
    '3.1': ['managed-all'],
    '4.1': ['block-launch'],
    '5.1': ['exact-url', 'search-terms', 'video', 'downloads'],
