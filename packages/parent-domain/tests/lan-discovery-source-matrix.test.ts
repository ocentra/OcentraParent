...
function registerSourceMatrixSchemaTests(): void {
  it('accepts an honest matrix covering all 20 LAN workpacks', () => {
    ...
    expect(parsed.sourceRows.find((row) => row.source === 'windows-neighbor-table')?.canAssignChildProfile).toBe(
      false
    );
    expect(parsed.sourceRows.find((row) => row.source === 'signed-child-agent-hello')?.canConfirmChildAgent).toBe(
      true
    );
  });
...
function workpack(
  ...
  return {
    ...
    discoveryState,
    proofState,
    runtimeOwner: proofState === 'manual-required' || proofState === 'not-implemented' ? 'manual-proof' : 'rust-service-read-model',
    status,
