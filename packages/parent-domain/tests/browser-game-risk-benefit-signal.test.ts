...
} from '../src/browser-game-risk-benefit-signal';
...
function acceptsCandidateSignalSet() {
  ...
  expect(parsed.schemaVersion).toBe('browser-game-risk-benefit-signal-contract');
  expect(parsed.recommendedPolicyInput).toBe('ask-parent-candidate');
...
function acceptsUnknownSignalRows() {
  expect(
    BrowserGameRiskSignalSchema.safeParse(
      riskSignal({
        signalId: 'browser-game-risk-signal-manual',
        kind: 'unknown-risk',
        severity: 'unknown',
...
function signalSet(overrides = {}): BrowserGameRiskBenefitSignalSet {
  return {
    schemaVersion: 'browser-game-risk-benefit-signal-contract',
    ...
    recommendedPolicyInput: 'ask-parent-candidate',
    confidence: 'medium',
    degradedState: 'none',
    uncertaintyReasons: [],
...
function riskSignal(overrides = {}) {
  return {
    signalId: 'browser-game-risk-signal-ugc',
