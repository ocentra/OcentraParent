...
import {
  ...
  buildAppGameSourceGatedPolicyPreviewTimerServiceReadinessHandoff,
} from '../src/app-game-source-gated-policy-preview-timer-service-readiness-handoff';
...
describe('app/game source-gated policy preview timer service-readiness handoff', () => {
  it('projects parent-surface rows into service read-api readiness states', () => {
    const handoff = buildServiceReadinessHandoff();
    ...
    expect(handoff.rows.map((row) => row.serviceReadinessHandoffState)).toEqual([
  ...
  it('adds service-readiness refs only after parent-surface proof is required', () => {
    const handoff = buildServiceReadinessHandoff();
    ...
    expect(handoff.rows[0]?.serviceReadinessProofRequired).toBe(true);
    expect(handoff.rows[0]?.serviceReadApiProofRequired).toBe(true);
    expect(handoff.rows[0]?.serviceReadApiImplemented).toBe(false);
    expect(handoff.rows[1]?.serviceReadApiProofRequired).toBe(false);
  });

  it('rejects service API, UI, timer, adapter, and count overclaims', () => {
    const handoff = buildServiceReadinessHandoff();

    expect(
      AppGameSourceGatedPolicyPreviewTimerServiceReadinessHandoffSchema.safeParse({
        ...handoff,
