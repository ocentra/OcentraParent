...
import {
  AppGameTimerServiceResponseConsumerParentSurfaceHandoffSchema,
  buildAppGameTimerServiceResponseConsumerParentSurfaceHandoff,
} from '../src/app-game-timer-service-response-consumer-parent-surface-handoff';
import { AppGameTimerServiceResponseConsumerParentSurfaceHandoffState } from '../src/app-game-timer-service-response-consumer-parent-surface-handoff-rules';
import { ParentContractSchemaVersion } from '../src/reference-primitives';

const UpstreamServiceResponseConsumerHandoffPath = new URL(
...
const ParentSurfaceHandoffOptions = {
  ...
  generatedAt: '2026-06-06T15:10:00Z',
  sourceContractRefs: [
...
it('projects service response consumer rows into parent-surface handoff states', () => {
  ...
  expect(handoff.rows.map((row) => row.serviceResponseConsumerParentSurfaceHandoffState)).toEqual([
...
it('rejects parent-surface rendering, runtime, adapter, raw-source, and count overclaims', () => {
  ...
  expect(
    AppGameTimerServiceResponseConsumerParentSurfaceHandoffSchema.safeParse({
      ...handoff,
...
function buildParentSurfaceHandoff() {
  return buildAppGameTimerServiceResponseConsumerParentSurfaceHandoff(
    ParentSurfaceHandoffOptions,
    readUpstreamServiceResponseConsumerHandoff()
  );
}

function readUpstreamServiceResponseConsumerHandoff() {
  return AppGameSourceGatedPolicyPreviewTimerServiceReadinessResponseConsumerParentSurfaceStatusReadModelParentSurfaceReadModelServiceReadApiResponseConsumerHandoffSchema.parse(
    JSON.parse(readFileSync(UpstreamServiceResponseConsumerHandoffPath, 'utf8'))
  );
}
