...
import { TrackingLocationAiAnalysisResultSchema, TrackingPolicySchemaVersion } from '../src/tracking-location-policy';
import {
  TrackingPolicyCompilerRequestSchema,
  compileTrackingPolicyDecision,
  type TrackingPolicyCompilerRequest,
  type TrackingPolicyCompilerResult,
} from '../src/tracking-location-policy-compiler';
...
describe('tracking location policy compiler runtime', () => {
...
function registerActionCompilationCases() {
  it('compiles every tracking policy action from parent rules without provider or enforcement claims', () => {
...
function registerRequestValidationCases() {
  it('rejects non-manual compiler requests without evidence and live grants without runtime grant data', () => {
    const missingEvidence = TrackingPolicyCompilerRequestSchema.safeParse({
      ...requestFor('notify-parent', { alertId: 'tracking-alert-no-evidence' }),
      evidenceReferences: [],
    });
    const missingGrantData = TrackingPolicyCompilerRequestSchema.safeParse(
...
function compile(
  ...
  options: Partial<TrackingPolicyCompilerRequest> & {
    readonly ruleOverrides?: Partial<TrackingPolicyRule>;
  } = {}
): TrackingPolicyCompilerResult {
  return compileTrackingPolicyDecision(requestFor(ruleAction, options));
}
...
function requestFor(
  ...
  return TrackingPolicyCompilerRequestSchema.parse(requestShapeFor(ruleAction, options));
}
...
function expectAction(
  result: TrackingPolicyCompilerResult,
  action: TrackingPolicyCompilerResult['decision']['action'],
