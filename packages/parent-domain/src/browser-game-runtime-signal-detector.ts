...
type BrowserGameRuntimeSignalStatusValidator = (signal: BrowserGameRuntimeSignalRowCandidate) => boolean;

const BrowserGameRuntimeSignalStatusValidators = {
  'detected-shape': detectedRuntimeSignalRowIsHonest,
  'candidate-shape': candidateRuntimeSignalRowIsHonest,
  'manual-required': degradedRuntimeSignalRowIsHonest,
  unavailable: degradedRuntimeSignalRowIsHonest,
} satisfies Record<BrowserGameRuntimeSignalRowCandidate['status'], BrowserGameRuntimeSignalStatusValidator>;
...
function browserGameRuntimeSignalRowIsHonest(signal: BrowserGameRuntimeSignalRowCandidate): boolean {
  ...
  return BrowserGameRuntimeSignalStatusValidators[signal.status](signal);
}

function detectedRuntimeSignalRowIsHonest(signal: BrowserGameRuntimeSignalRowCandidate): boolean {
  return (
...
function candidateRuntimeSignalRowIsHonest(signal: BrowserGameRuntimeSignalRowCandidate): boolean {
  return signal.confidence !== 'high' && signal.signalKind !== 'unknown' && signal.sourceKind !== 'unavailable';
}

function degradedRuntimeSignalRowIsHonest(signal: BrowserGameRuntimeSignalRowCandidate): boolean {
  return signal.confidence !== 'high' && runtimeSignalHasDegradedMarker(signal);
}

function runtimeSignalHasDegradedMarker(signal: BrowserGameRuntimeSignalRowCandidate): boolean {
  return (
    signal.signalKind === 'unknown' ||
    signal.sourceKind === 'manual-review-ref' ||
    signal.sourceKind === 'unavailable' ||
