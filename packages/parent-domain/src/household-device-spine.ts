...
import { DeviceRuntimeRoleStateSchema, DeviceRuntimeRouteStateSchema } from './device-roles';
...
export const HouseholdDeviceRoleSchema = withParser(
...
const HouseholdLanDeviceHardwareProfileSchema = Schema.Struct({
  ...
  cpuModel: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), { default: () => null }),
  cpuCores: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), { default: () => null }),
  memoryTotal: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), { default: () => null }),
  gpuModel: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), { default: () => null }),
  gpuDriver: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), { default: () => null }),
  gpuMemory: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), { default: () => null }),
  nvidiaSmi: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), { default: () => null }),
});
...
export const HouseholdLanDeviceRefSchema = withParser(
  Schema.Struct({
    ...
    ipAddress: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), {
      default: () => null,
    }),
    macAddress: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), {
      default: () => null,
    }),
    ...
    agentStatus: Schema.optionalWith(Schema.Union(NonEmptyHouseholdDeviceText, Schema.Null), {
      default: () => null,
    }),
    hardwareProfile: Schema.optionalWith(Schema.Union(HouseholdLanDeviceHardwareProfileSchema, Schema.Null), {
      default: () => null,
    }),
  })
);
...
const HouseholdDeviceBaseSchema = Schema.Struct({
  ...
  roleBadges: Schema.Array(HouseholdDeviceRoleSchema),
  ...
  HouseholdDeviceBaseSchema.pipe(
    Schema.filter(
      (device) =>
        ...
        'Expected child-agent devices to expose child-agent roles and control surfaces and non-child-agent devices to stay non-enrollable'
    )
  )
);

export const HouseholdDeviceSpineReadModelSchema = withParser(
  Schema.Struct({
    schemaVersion: LanPairingSchemaVersionSchema,
    generatedAt: LanPairingTimestampSchema,
    devices: Schema.Array(HouseholdDeviceSpineEntrySchema).pipe(
      Schema.filter(
        (devices) =>
          new Set(devices.map((device) => device.canonicalDeviceId)).size === devices.length ||
          'Expected one canonical row per physical household/LAN device'
      )
    ),
  })
);
...
function householdDeviceSpineEntryIsConsistent(device: HouseholdDeviceCandidate): boolean {
  ...
  return (
    ...
    device.enrollable &&
    device.policyTargetSurfaces.includes('devices') &&
    device.policyTargetSurfaces.includes('policy') &&
    device.policyTargetSurfaces.includes('browser') &&
    device.policyTargetSurfaces.includes('app') &&
    device.policyTargetSurfaces.includes('screen') &&
...
export type HouseholdDeviceRole = Infer<typeof HouseholdDeviceRoleSchema>;
...
export type HouseholdDeviceSpineReadModel = Infer<typeof HouseholdDeviceSpineReadModelSchema>;
