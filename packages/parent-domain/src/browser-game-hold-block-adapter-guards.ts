...
import {
  ...
  type BrowserGameHoldBlockAction,
...
function expectedReasonForManagedActionIsPresent(plan: BrowserGameHoldBlockAdapterPlanGuardInput): boolean {
  ...
  if (plan.requestedAction === 'hold-until-parent-approval') {
    return plan.reasonCodes.includes('policy-candidate-ask-parent') && plan.fallbackAction === 'show-approval-page';
  }
...
function browserGameHoldBlockAdapterPlanClaimsRuntime(plan: BrowserGameHoldBlockAdapterPlanGuardInput): boolean {
  return (
    plan.rawUrlIncluded ||
    plan.rawPageBodyIncluded ||
    plan.rawGamePayloadIncluded ||
    plan.childCookieSessionReused ||
    plan.unmanagedBrowserExactUrlClaimed ||
    plan.browserMutationExecutedClaimed ||
    plan.renderedChildPageClaimed ||
    plan.notificationDeliveredClaimed ||
    plan.finalPolicyDecisionClaimed ||
    plan.timeLimitAppliedClaimed ||
    plan.cloudFrameAnalysisClaimed ||
    plan.nativeGameControlClaimed ||
    plan.enforcementClaimed
  );
}
