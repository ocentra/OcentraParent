...
export const ProductionSupportPublicationExecutionStatusRowSchema = withParser(
  Schema.Struct({
    ...
    Schema.filter(
      (row) =>
        row.sourceContractState === 'source-contract-ready' &&
        row.statusContractState === 'status-contract-ready' ||
...
>;
export type ProductionSupportPublicationExecutionLifecycleStatus = (typeof RequiredPublicationExecutionStatusLifecycleStates)[number];
...
): Record<ProductionSupportPublicationExecutionStatusTarget, Record<ProductionSupportPublicationExecutionLifecycleStatus, number>> {
