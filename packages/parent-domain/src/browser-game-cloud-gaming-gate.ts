...
function browserGameCloudGateRequestActionIsHonest(request: BrowserGameCloudGateRequestCandidate): boolean {
  if (request.actionCandidate === 'ask-parent-candidate') {
    return (
  ...
  if (request.actionCandidate === 'block-candidate') {
    return (
  ...
  if (request.actionCandidate === 'time-limit-candidate') {
    return (
      request.policyCandidateRef !== null &&
  ...
  if (request.actionCandidate === 'allow-window-candidate') {
    return (
      request.policyCandidateRef !== null &&
...
function browserGameCloudGateRequestClaimsRuntime(request: BrowserGameCloudGateRequestCandidate): boolean {
  return (
    request.rawCloudTitleStored ||
    request.rawStreamFrameStored ||
    request.cloudStreamFrameAnalysisClaimed ||
    request.perGameCloudTitleClaimed ||
    request.nativeGameControlClaimed ||
    request.nativeLauncherControlClaimed ||
    request.gameChatContentClaimed ||
    request.accountOrPurchaseFlowClaimed ||
    request.notificationDeliveredClaimed ||
    request.uiRenderedClaimed ||
    request.childNotifiedClaimed ||
    request.finalPolicyDecisionClaimed ||
