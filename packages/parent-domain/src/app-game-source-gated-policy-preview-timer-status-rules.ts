...
export const appGameSourceGatedPolicyPreviewTimerStatusCountsMatch = (
  ...
  status.timerRuntimeProofRequiredCount ===
    status.rows.filter(
      (row) =>
        row.timerStatusState === AppGameSourceGatedPolicyPreviewTimerStatusState.TimerRuntimeProofRequired &&
  ...
  status.sourceFreshnessProofRequiredCount ===
    status.rows.filter(
      (row) =>
        row.timerStatusState === AppGameSourceGatedPolicyPreviewTimerStatusState.SourceFreshnessProofRequired &&
        !row.timerRuntimeProofRequired &&
        row.sourceFreshnessProofRequired &&
        !row.compilerDecisionProofRequired &&
        !row.timerScheduled
  ...
  status.compilerDecisionProofRequiredCount ===
    status.rows.filter(
      (row) =>
        row.timerStatusState === AppGameSourceGatedPolicyPreviewTimerStatusState.CompilerDecisionProofRequired &&
        !row.timerRuntimeProofRequired &&
        !row.sourceFreshnessProofRequired &&
        row.compilerDecisionProofRequired &&
        !row.timerScheduled
