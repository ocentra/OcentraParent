...
import { LocalAiEvidenceContextSummarySchema, LocalAiRejectedFieldSchema, LocalAiStoredEvidenceContextBuildInputSchema, } from '@ocentra-parent/ai-domain/local-ai-context';
import { selectLocalAiEvidenceContextInput } from './local-ai-context-selection';
...
const HostedCustodySummary = decodeContextSummary('ocentra-hosted non-activity metadata cannot source child-activity evidence');
...
function selectedInputFor(parsed, selection) {
    return {
        ...parsed,
        request: {
            ...parsed.request,
...
function contextForSelection(selectedInput, selection) {
    if (selectedInput.evidenceReferences.length === 0) {
        return null;
    }
    return contextForInput(selectedInput, {
...
function hostedCustodyResult(parsed, selection) {
    return resultFor(parsed, 'rejected', null, [EvidenceReferencesField], [], selection.forbiddenCustodyReferences.map((reference) => reference.evidenceRefId), HostedCustodySummary, HostedCustodyGate);
}
function unallowedCustodyResult(parsed, selection) {
    return resultFor(parsed, 'rejected', null, [EvidenceReferencesField], selection.missingEvidenceKinds, selection.degradedSourceRefs, UnallowedCustodySummary, UnallowedCustodyGate);
}
function insufficientResult(parsed, selection) {
    return resultFor(parsed, 'insufficient', null, [], selection.missingEvidenceKinds, [], NoEvidenceSummary, InsufficientEvidenceGate);
}
function partialResult(parsed, selection, context) {
    return resultFor(parsed, 'partial', context, selection.unallowedCustodyReferences.length > 0 ? [EvidenceReferencesField] : [], selection.missingEvidenceKinds, selection.degradedSourceRefs, selection.unallowedCustodyReferences.length > 0 ? UnallowedCustodySummary : StoredEvidenceSummary, PartialContextGate);
}
export function buildLocalAiEvidenceContext(input) {
...
//# sourceMappingURL=local-ai-context-builder.js.map