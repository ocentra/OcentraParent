...
import { LocalAiCapabilityFlagSchema, LocalAiDegradedStateSchema, LocalAiModelIdSchema, LocalAiModelReferenceSchema, LocalAiModelLoadStateSchema, LocalAiPromptVersionSchema, LocalAiProviderIdSchema, LocalAiResourceClassSchema, LocalAiRuntimeReferenceIdSchema, LocalAiTimestampSchema, LocalAiUnavailableReasonSchema, } from './local-ai-primitives';
export * from './local-ai-model-artifacts';
export const LocalAiProviderPrivacyModeSchema = withParser(Schema.Literal('local-only'));
export const LocalAiAdapterBoundarySchema = withParser(Schema.Literal('status-only', 'local-adapter-unavailable', 'local-adapter-ready'));
export const LocalAiExecutionStateSchema = withParser(Schema.Literal('disabled', 'dry-run-ready', 'running', 'failed'));
export const LocalAiProviderSourceSchema = withParser(Schema.Literal('unavailable', 'local-config', 'local-model-cache', 'os-capability-probe'));
export const LocalAiAdapterProbeStateSchema = withParser(Schema.Literal('probe-unavailable', 'probe-ready', 'probe-failed'));
export const LocalAiProviderConfigurationStateSchema = withParser(Schema.Literal('local-provider-unconfigured', 'local-provider-configured', 'local-provider-config-invalid'));
export const LocalAiAdapterReadinessStateSchema = withParser(Schema.Literal('adapter-not-ready', 'adapter-ready', 'adapter-readiness-invalid'));
...
export const LocalProviderAdapterProbeSchema = withParser(LocalProviderAdapterProbeBaseSchema.pipe(Schema.filter((probe) => localProviderAdapterProbeReadinessIsConsistent(probe) ||
    'Expected local provider adapter readiness to match execution permission')));
function localProviderAdapterProbeReadinessIsConsistent(probe) {
    if (probe.readinessState === 'adapter-not-ready') {
        return probe.executionAllowed === false;
    }
    if (probe.readinessState === 'adapter-ready') {
        return (probe.executionAllowed === true &&
...
export const LocalProviderCapabilitySchema = withParser(Schema.Struct({
    providerId: LocalAiProviderIdSchema,
...
export const LocalAiModelRequestMetadataSchema = withParser(Schema.Struct({
    providerId: LocalAiProviderIdSchema,
    modelId: LocalAiModelIdSchema,
    promptVersion: LocalAiPromptVersionSchema,
}));
//# sourceMappingURL=local-ai-runtime.js.map