...
import { LocalAiDegradedStateSchema, LocalAiModelIdSchema, LocalAiModelReferenceSchema, LocalAiProviderIdSchema, LocalAiResourceClassSchema, LocalAiRuntimeReferenceIdSchema, LocalAiTimestampSchema, LocalAiUnavailableReasonSchema, } from './local-ai-primitives';
...
export const LocalAiProviderSchedulerLifecycleSchema = withParser(Schema.Literal('idle', 'running', 'queued', 'degraded', 'unavailable'));
export const LocalAiProviderSchedulerJobClassSchema = withParser(Schema.Literal('child-safety', 'parent-assistant', 'parent-report'));
export const LocalAiProviderSchedulerJobStatusSchema = withParser(Schema.Literal('accepted', 'running', 'queued', 'degraded', 'unavailable', 'complete'));
export const LocalAiProviderSchedulerQueueSchema = withParser(Schema.Struct({
...
export const LocalAiProviderSchedulerStatusSchema = withParser(LocalAiProviderSchedulerStatusBaseSchema.pipe(Schema.filter((status) => localAiProviderSchedulerStatusIsConsistent(status) ||
...
export const LocalAiProviderSchedulerDecisionSchema = withParser(LocalAiProviderSchedulerDecisionBaseSchema.pipe(Schema.filter((decision) => localAiProviderSchedulerDecisionIsConsistent(decision) ||
    'Expected local AI provider scheduler decision to expose selected runtime, queue position, or unavailable reason')));
function localAiProviderSchedulerStatusIsConsistent(status) {
...
function unavailableStatusIsConsistent(status) {
    return (status.currentJobClass === null &&
        totalQueuedJobs(status.queue) === 0 &&
        status.unavailableReason !== null &&
        status.degradedState === 'provider-unavailable');
}
function idleStatusIsConsistent(status) {
    return (status.currentJobClass === null &&
        totalQueuedJobs(status.queue) === 0 &&
        status.unavailableReason === null &&
        status.degradedState === 'none');
}
function localAiProviderSchedulerDecisionIsConsistent(decision) {
...
function unavailableDecisionIsConsistent(decision) {
    return (decision.selectedRuntimeReferenceId === null &&
        decision.queuePosition === null &&
        decision.unavailableReason !== null &&
        decision.duplicateRuntimeBlocked === false);
}
function queuedDecisionIsConsistent(decision) {
    return (runtimeDecisionHasSelectedRuntime(decision) &&
        decision.queuePosition !== null &&
        runtimeDecisionBlocksDuplicateRuntime(decision));
}
function activeRuntimeDecisionIsConsistent(decision) {
    return (runtimeDecisionHasSelectedRuntime(decision) &&
        decision.queuePosition === null &&
        runtimeDecisionBlocksDuplicateRuntime(decision));
}
function degradedDecisionIsConsistent(decision) {
    return runtimeDecisionHasSelectedRuntime(decision) && runtimeDecisionBlocksDuplicateRuntime(decision);
}
function runtimeDecisionHasSelectedRuntime(decision) {
    return decision.selectedRuntimeReferenceId !== null && decision.unavailableReason === null;
}
function runtimeDecisionBlocksDuplicateRuntime(decision) {
    return decision.duplicateRuntimeBlocked;
}
function totalQueuedJobs(queue) {
    return queue.childSafetyQueued + queue.parentAssistantQueued + queue.parentReportQueued;
}
//# sourceMappingURL=local-ai-provider-scheduler.js.map