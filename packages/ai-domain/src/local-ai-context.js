...
import { ChildProfileReferenceSchema, FamilyReferenceSchema, ParentDeviceReferenceSchema, ParentEvidenceReferenceSchema, } from '@ocentra-parent/family-domain/references';
...
import { LocalAiCapabilityFlagSchema, LocalAiConfidenceSchema, LocalAiEvaluationRequestIdSchema, LocalAiPromptVersionSchema, LocalAiRuntimeReferenceIdSchema, LocalAiTimestampSchema, } from './local-ai-primitives';
...
}).pipe(Schema.filter((reference) => reference.sourceEvidenceReferences.length > 0 || 'Expected local AI context evidence to cite stored evidence')));
export const LocalAiParentRuleContextRefSchema = withParser(Schema.Struct({
...
}).pipe(Schema.filter((reference) => reference.targetEvidenceRefs.length > 0 || 'Expected parent rule context to cite target evidence refs')));
export const LocalAiEvidenceContextBuildRequestSchema = withParser(Schema.Struct({
...
export const LocalAiEvidenceContextValidationSummarySchema = withParser(Schema.Struct({
...
export const LocalAiEvidenceContextSchema = withParser(Schema.Struct({
    schemaVersion: ParentContractSchemaVersionSchema,
...
export const LocalAiEvidenceContextBuildResultSchema = withParser(Schema.Struct({
    schemaVersion: ParentContractSchemaVersionSchema,
    requestId: LocalAiEvaluationRequestIdSchema,
...
export const LocalAiStoredEvidenceContextBuildInputSchema = withParser(Schema.Struct({
    contextId: LocalAiEvidenceContextIdSchema,
...
//# sourceMappingURL=local-ai-context.js.map