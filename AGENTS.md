...
Before editing or committing, run `npm run lanes:status`, `npm run lanes:guard`, `npm run hub:status`, and `npm run hub:guard` from the checkout you are using. Each active worktree lane must be recorded in the repo-owned `.hub/lane-ledger.json` and `docs/hub/lane-ledger.md`, with lane id, branch, remote branch, PR, status, last validation, merge state, next action, and cleanup safety. Live Codex coordination transport does not belong in the product repo. `.hub/state/**` is ignored and must not be committed. During migration, live hub commands read the legacy external root configured in `.hub/hub.config.json`; the target architecture is OcentraHub, an external append-only event ledger documented in `docs/hub/ocentra-hub-event-ledger.md`. Actual worktree checkouts may live outside the repo. The pre-commit hook runs both guards automatically.

No Codex lane work is complete unless it updates `.hub/lane-ledger.json` and `docs/hub/lane-ledger.md`, or explicitly reports why no ledger update was needed. Use `npm run hub:lane-ledger:audit` or `powershell -ExecutionPolicy Bypass -File scripts/hub/audit-lanes.ps1` on any PC that has Ocentra Parent worktrees before declaring a worktree safe to delete, parked, merged, or lost.

When using Codex from more than one PC, sync the external hub transport before coordination or lane work. During migration, that means ensuring the configured legacy hub root is current; after OcentraHub lands, use the OcentraHub sync command for the `ocentra-parent` hub. Only actual worktree folders may remain outside the repo. Do not let two PCs actively edit the same lane at the same time; sync, claim, and report through the external hub before continuing.
