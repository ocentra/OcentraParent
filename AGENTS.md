...
Before editing or committing, run `npm run lanes:status`, `npm run lanes:guard`, `npm run hub:status`, and `npm run hub:guard` from the checkout you are using. Each active worktree lane must be recorded in the repo-owned `.hub/lane-ledger.json` and `docs/hub/lane-ledger.md`, with lane id, branch, remote branch, PR, status, last validation, merge state, next action, and cleanup safety. Operational Codex coordination state defaults to repo-local `.hub/state/worktree-lanes.json` and `.hub/state/ocentra-parent-hub`; actual worktree checkouts may live outside the repo. Do not put authoritative Ocentra Parent Codex state under user-global `.codex` unless an explicit `OCENTRA_PARENT_LANE_LEDGER` or `OCENTRA_PARENT_HUB_ROOT` override is being used for a deliberate temporary recovery. The pre-commit hook runs both guards automatically.
...
When using Codex from more than one PC, run `npm run hub:state:sync` before coordination or lane work and `npm run hub:state:sync -- -Commit` after semantic hub changes. Only actual worktree folders may remain outside the repo. Do not let two PCs actively edit the same lane at the same time; sync, claim, report, and commit repo-owned hub state first.
