...
const scannerPath = fileURLToPath(new URL('../../scripts/security/scan-staged-secrets.mjs', import.meta.url));
...
test('repository secret scan allows environment templates while still scanning their contents', () => {
  withTempRepo((root) => {
    ...
    assert.match(result.stdout, /Ocentra Enforcer secrets passed/u);
  });
});
