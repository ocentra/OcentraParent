...
import { evaluateReleaseVersionPolicy, isReleaseSemver } from '../../scripts/release/version-policy.mjs';
...
function withVersionWorkspace(testBody) {
  ...
  try {
    ...
    mkdirSync(join(root, 'platforms', 'ios', 'OcentraChildAgent.xcodeproj'), { recursive: true });
...
test('release version policy accepts aligned workspace versions', () => {
  withVersionWorkspace((root) => {
    ...
    writeFileSync(
      join(root, 'platforms', 'ios', 'OcentraChildAgent.xcodeproj', 'project.pbxproj'),
