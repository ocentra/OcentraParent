...
describe("billing api contract", () => {
  ...
  it("returns distinct referral abuse contracts for device-farm rejection and payment-method manual review", async () => {
    ...
    const deviceFarmContract = BillingReferralInviteResultSchema.parse(
      await readJson<unknown>(deviceFarm.response),
    );
    ...
    assert.equal(
      paymentMethodContract.rejectionReason,
  ...
  it("returns partial refund, invoice-not-found rejection, and reconciliation visibility contracts", async () => {
    ...
    assert.equal(
      missingInvoiceRefundContract.refundState,
      "manual-review-required",
    );
    assert.equal(
      missingInvoiceRefundContract.rejectionReason,
      "invoice-not-found",
    );
    ...
    const reconciliationContract =
      BillingSupportAdminReconciliationSummarySchema.parse(
        await readJson<BillingReconciliationContractResponse>(
  ...
  it("returns shared support/admin read contracts for account search, invoice search, disputes, referrals, and audit timelines", async () => {
    ...
    const auditContract = BillingSupportAdminAuditEventsResponseSchema.parse(
