...
function ensureRuntimeDevVars(): {
  ...
  if (existsSync(runtimeDevVarsPath)) {
    const parsed = parseDotEnvFile(readFileSync(runtimeDevVarsPath, "utf8"));
  ...
  writeFileSync(runtimeDevVarsPath, buildRuntimeDevVarsContents(), "utf8");
  return {
    created: true,
...
function currentRuntimeStripeWebhookSecret(): string {
  return currentRuntimeDevVar(
    "STRIPE_WEBHOOK_SECRET",
    localRuntimeSecrets.stripeWebhookSecret,
  );
}
    ...
    process.platform === "win32"
      ? spawn(
          ...
          [
            ...
            "/s",
            "/c",
            [wranglerCommand, ...wranglerArgs].map(quoteWindowsArgument).join(" "),
          ],
          {
            cwd: cloudflareDir,
...
describe("wrangler local runtime", () => {
  ...
  it("executes real hosted checkout and portal writes with durable audit persistence", async () => {
    ...
    const firstCheckoutResponse = await fetch(
      `${runtime.baseUrl}/auth/billing/checkout`,
      checkoutRequestInit,
    );
    ...
    const replayCheckoutResponse = await fetch(
      `${runtime.baseUrl}/auth/billing/checkout`,
      checkoutRequestInit,
    );
    ...
    const auditResponse = await fetch(
      `${runtime.baseUrl}/admin/billing/audit?q=runtime-`,
      {
        headers: {
          authorization: "Bearer parent:admin-agent",
          "x-ocentra-role": "admin",
    ...
    const checkoutEvents =
      auditBody.results?.filter(
        (row) => row.eventId === "billing-checkout-session:runtime-checkout-session-proof",
      ) ?? [];
    const portalEvents =
      auditBody.results?.filter(
        (row) => row.eventId === "billing-portal-session:runtime-portal-session-proof",
      ) ?? [];
  ...
  it("executes a real payment change-plan write with durable-object idempotency", async () => {
    ...
    assert.ok(
      cancelledStatusBody.warnings.includes("cancellation-scheduled-period-end"),
    );
  });
  ...
  it("processes real signed Stripe failure and dispute lifecycle events through the worker runtime", async () => {
    ...
    const graceSnapshotResponse = await fetch(
      `${runtime.baseUrl}/auth/billing/entitlement-snapshot`,
      {
        headers: {
          authorization: "Bearer parent:demo-maxed",
          "x-ocentra-trusted-device": "true",
    ...
    const disputeOpenSignature = await createStripeSignature(
      disputeOpenPayload,
      currentRuntimeStripeWebhookSecret(),
    );
    ...
    const openedDisputeResponse = await fetch(
      `${runtime.baseUrl}/admin/billing/disputes?q=dp_runtime_maxed`,
      {
        headers: {
          authorization: "Bearer parent:admin-agent",
          "x-ocentra-role": "admin",
    ...
    const disputeWonSignature = await createStripeSignature(
      disputeWonPayload,
      currentRuntimeStripeWebhookSecret(),
    );
    ...
    const recoveredDisputeResponse = await fetch(
      `${runtime.baseUrl}/admin/billing/disputes?q=dp_runtime_maxed`,
      {
        headers: {
          authorization: "Bearer parent:admin-agent",
  ...
  it("processes real support and admin payment operations through the worker runtime", async () => {
    ...
    const adminAccountsResponse = await fetch(
      `${runtime.baseUrl}/admin/billing/accounts?q=manual-review`,
      {
        headers: {
          authorization: "Bearer parent:admin-agent",
          "x-ocentra-role": "admin",
    ...
    const manualInvoiceEvents =
      auditBody.results?.filter(
        (row) => row.eventId === "billing-manual-invoice:runtime-manual-invoice-proof",
      ) ?? [];
    const refundEvents =
      auditBody.results?.filter((row) => row.eventId === "billing-refund:runtime-refund-proof") ?? [];
    const reconciliationEvents =
      auditBody.results?.filter(
        (row) => row.eventId === "billing-reconciliation:runtime-reconciliation-proof",
      ) ?? [];
    assert.equal(auditResponse.status, 200);
  ...
  it("processes real signed non-stripe provider lifecycle transitions through the worker runtime", async () => {
    ...
    const googleSignature = await createHmacSignature(
      googlePayload,
      currentRuntimeDevVar(
        "GOOGLE_PLAY_SERVICE_ACCOUNT_REF",
        localRuntimeSecrets.googlePlayServiceAccountRef,
      ),
    );
    ...
    const appleResponse = await fetch(`${runtime.baseUrl}/webhooks/apple`, {
      method: "POST",
      headers: {
        authorization: `Bearer ${currentRuntimeDevVar(
          "APPLE_STORE_KEY_REF",
          localRuntimeSecrets.appleStoreKeyRef,
        )}`,
      },
      body: applePayload,
    });
    ...
    const auditResponse = await fetch(
      `${runtime.baseUrl}/admin/billing/audit?q=runtime_`,
      {
        headers: {
          authorization: "Bearer parent:admin-agent",
          "x-ocentra-role": "admin",
  ...
  it("processes real referral invite writes and direct license decisions through the worker runtime", async () => {
    ...
    const adminReferralsBeforeResponse = await fetch(
      `${runtime.baseUrl}/admin/billing/referrals`,
      {
        headers: {
          authorization: "Bearer parent:admin-agent",
          "x-ocentra-role": "admin",
    ...
    assert.ok(
      referralsAfterBody.invites.some(
        (row) =>
          row.invitedIdentifier === "runtime-family@example.com" &&
          row.inviteState === "invite-created",
      ),
    );
    ...
    assert.equal(
      adminReferralAfter?.invitedFamilies,
      (adminReferralBefore?.invitedFamilies ?? 0) + 1,
    );
