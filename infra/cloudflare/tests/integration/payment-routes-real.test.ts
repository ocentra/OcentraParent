...
describe("real payment worker routes", () => {
  ...
  it("returns referral state and supports explicit invite creation", async () => {
    ...
    const updatedReferralsBody = BillingReferralSummarySchema.parse(
      await readJson<unknown>(updatedReferrals.response),
    );
  ...
  it("keeps referral abuse outcomes explicit and non-queued for device-farm, payment-method, and generic fraud-review cases", async () => {
    ...
    const deviceFarmBody = BillingReferralInviteResultSchema.parse(
      await readJson<unknown>(deviceFarmInvite.response),
    );
    ...
    assert.equal(
      paymentMethodBody.rejectionReason,
  ...
  it("queues accepted change-plan and cancel reconciliation actions with real route summaries", async () => {
    ...
    const updatedSnapshotBody = await readJson<BillingEntitlementSnapshotResponse>(
      updatedSnapshot.response,
    );
    ...
    assert.ok(
      cancelledStatusBody.warnings.includes("cancellation-confirmed-in-grace"),
    );
  });
  ...
  it("deduplicates repeated billing write requests through durable-object idempotency", async () => {
    ...
    assert.equal(
      referralSummary.invites.filter(
        (inviteSummary) => inviteSummary.invitedIdentifier === "dedupe-family@example.com",
      ).length,
      1,
    );
  });
  ...
  it("queues referral, manual-invoice, refund, and reconciliation actions with billing-safe payloads", async () => {
    ...
    const manualInvoiceStatusBody = await readJson<BillingStatusResponse>(
      manualInvoiceStatus.response,
    );
    ...
    assert.equal(
      manualInvoiceStatusBody.manualInvoiceState?.invoiceState,
    ...
    const manualInvoiceInvoicesBody = await readJson<BillingInvoicesResponse>(
      manualInvoiceInvoices.response,
    );
    ...
    const refundBody = BillingSupportAdminRefundResultSchema.parse(
      await readJson<unknown>(refund.response),
    );
    ...
    const refundedSnapshotBody = await readJson<BillingEntitlementSnapshotResponse>(
      refundedSnapshot.response,
    );
  ...
  it("keeps partial refund, invoice-not-found rejection, and reconciliation visibility explicit", async () => {
    ...
    const partialRefundInvoicesBody = await readJson<BillingInvoicesResponse>(
      partialRefundInvoices.response,
    );
  ...
  it("supports manual invoice, admin invoice search, disputes, referrals, refunds, reconciliation, and audit surfaces", async () => {
    ...
    const accountsBody = BillingSupportAdminAccountsResponseSchema.parse(
    ...
    assert.equal(
      ...
      accountsBody.results.filter((row) => row.manualRequired).length,
    );
    ...
    const disputesBody = BillingSupportAdminDisputesResponseSchema.parse(
    ...
    const referralsBody = BillingSupportAdminReferralsResponseSchema.parse(
    ...
    const auditBody = BillingSupportAdminAuditEventsResponseSchema.parse(
