...
the existing encrypted journal/store/read-model path. It does not prove registry
crawling, Store package enumeration, portal evidence/classifier rendering, local
model quality/provider execution, policy evaluator consumption, or platform
enforcement. Live process snapshots now
...
- [ ] Running and foreground session evidence. Contract and local
      ...
      shortcut inventory rows with hashed source refs. Portal freshness polish,
      richer source subscriptions, registry/Store inventory, and policy
      integration remain.
