...
The app/game policy preview handoff proof consumes compiled dry-run decisions
and produces parent-domain preview rows only, and the service read-model proof
adds a payload bridge for the existing generic policy preview read model.
Native-game service promotion still waits on persisted source target kind. It
does not run the policy evaluator, render authoring UI, start timers, deliver
child notifications, dispatch adapters, or prove platform enforcement.
