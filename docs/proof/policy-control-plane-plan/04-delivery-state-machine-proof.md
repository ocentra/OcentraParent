...
- `crates/policy-control-core/src/policy_delivery/adapter_execution.rs` accepts an opaque execution capability for acknowledged, applied, and rolled-back states. The capability has private fields, no `Deserialize` implementation, and no public mint; until a trusted dependency-owned adapter exists, this path cannot be invoked by production callers.
...
This proof closes the delivery state-machine contract on policy-owned surfaces only. It proves queue identity compatibility and deterministic identity derivation as separate contracts, and proves that public production seams fail closed rather than fabricating execution authority. WP04 remains runtime-blocked until a trusted domain- or enforcement-owned adapter performs the real side effect and mints the opaque capability. It does not claim portal rendering, shared event transport mechanics, or enforcement runtime execution.
