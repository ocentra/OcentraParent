...
| `policy-delivery.rollback-receipt-required` | `delivery_rollback_with_valid_context_still_requires_trusted_execution_receipt` drives a compiled source document through the child-policy queue handoff, supplies valid rollback reason and prior-state context, and proves the receipt-aware transition rejects the rollback without a trusted adapter execution receipt. |
