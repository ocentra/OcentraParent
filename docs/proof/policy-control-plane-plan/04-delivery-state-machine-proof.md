...
| `policy-delivery.rollback-receipt-required` | `delivery_rollback_with_valid_context_still_requires_trusted_execution_receipt` supplies valid rollback reason and prior-state context through the child-policy handoff, then proves the transition fails closed without an execution receipt. |
