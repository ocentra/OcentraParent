...
## WP04 vertical slice note

- `compiled_delivery_execution_receipt_and_rollback_chain_validates_end_to_end` proves the typed policy compile -> delivery -> adapter execution receipt -> rollback chain stays explicit, validates the rollback receipt evidence, and returns a rolled-back record that still carries the delivered rollback reference.
- `rolled_back_execution_receipt_requires_rollback_reference_state` and `generic_stored_rolled_back_receipt_is_rejected_as_unauthenticated` remain the negative guards for missing rollback evidence and unauthenticated stored rollback hydration.
