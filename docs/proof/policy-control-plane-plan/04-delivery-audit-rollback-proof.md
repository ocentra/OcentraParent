...
- The receipt matrix proves acknowledged, applied, and rolled-back evidence is tied to delivery, household, source document, target, attempt, sequence, audit, reason, and rollback provenance. Missing, stale, or mismatched evidence fails closed; an exact same-sequence transition plus full stored receipt retry returns `Duplicate` without advancement.
- Receipt-required delivery records store adapter evidence, and record validation rejects missing or record-mismatched evidence. Equality is not authentication: generic record deserialization rejects every `Applied` payload even when all caller-supplied record and receipt fields match.
- `PolicyDeliveryAdapterExecution` is opaque, non-deserializable, and has no public mint. WP04 also defines no authenticated persisted-`Applied` capability. Trusted initial execution and persisted `Applied` rehydration are therefore unsupported and not proven.
...
This proof covers audit-linkage, rollback-linkage, receipt evidence validation, exact replay, legacy compatibility, and redaction on policy-owned delivery surfaces only. It does not claim trusted adapter minting, real enforcement side effects, authenticated persistence, persisted `Applied` rehydration, portal audit presentation, or external log pipeline behavior.
