...
Historical run context: 2026-07-18, branch `codex/network-wp01-foundation-contract-eventing`, base commit `2973ce0c9` before this packet's commit.

## Current-main integration status

On 2026-07-31 this receipt's source/test/doc packet was integrated into
`codex/network-wp01-foundation-20260731` from current `origin/main`, with the
remaining `IpOnly` attribution review defect repaired. Current validation is
pending the coordinated `Cargo.lock` update required by the new
`agent-core -> network-core` and `agent-protocol` test dependencies. The
historical commands below are retained as source evidence only; they do not
prove the current-main packet until the focused commands are rerun and their
new artifact pointers are recorded.
