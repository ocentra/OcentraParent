...
| 30   | Dead-letter record and event                                                   | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `DeadLetterEvent`, `dead_letter_recorded_event_type()`, queue policy proof harness                                                                                                   | Dead letters carry explicit reason and convert to a typed dead-letter domain event; public API exposes a validated `EventType`, not a raw `&str`.                                                                             |
...
| 52   | Vite/TypeScript typed-intent-only boundary                                     | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/52-ui-typed-intent-boundary/proof-summary.json`, `eventing-ui-typed-intent-boundary-proof.mjs`                                                           | Portal outbound messages are typed `AgentCommandEnvelope` values, inbound service events parse as read models, command inventory keeps events as result metadata, and source gate rejects portal event-bus/publish ownership. |
...
| 59   | Weak-network-evidence cannot publish enforcement command                       | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/59-weak-network-evidence-command-routing/proof-summary.json`, `manual_required_network_evidence_does_not_publish_enforcement_command` test               | Weak or unavailable network evidence filters enforcement command/result phases before publish, keeps review/audit/read-model phases visible, and keeps `adapter_action_executed=false`.                                       |
| 60   | AI cannot publish enforcement command                                          | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/60-61-command-boundary/proof-summary.json`, `eventing-command-boundary-proof.mjs`                                                                        | Parent-assistant/AI command mapping and service AI router do not target enforcement action handlers.                                                                                                                          |
| 61   | Portal/UI cannot publish enforcement command                                   | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/60-61-command-boundary/proof-summary.json`, `apps/portal/src/transport.ts`, portal transport test                                                        | Portal transport rejects direct enforcement action commands before WebSocket serialization; current portal command inventory contains no direct enforcement action commands.                                                  |
| 62   | Network event proof artifacts linked back to eventing plan                     | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/62-network-proof-links/proof-summary.json`, `test-results/eventing-network-runtime-proof/proof.json`, `eventing-network-runtime-proof.mjs`               | Harness writes a plan-facing proof summary that links eventing row 62, completed row 59, network row 10, runtime proof artifacts, partial rows 57-58, and no-claim boundaries without claiming network row 10 complete.       |
| 63   | Type-safety and validation source gate                                         | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/63-type-safety-source-gate/proof-summary.json`, `eventing-type-safety-source-gate-proof.mjs`                                                             | Eventing public identifiers and error identity fields use validated newtypes; stored JSON is wrapped by `StoredEventPayload`; source gate rejects public raw JSON/string constants, `Uuid`, and raw domain identifier fields. |
...
| 74   | Bus shutdown, drain, dead-letter, and test clear lifecycle                     | [x]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/74-production-shutdown/proof-summary.json`, `eventing-production-shutdown-proof.mjs`; `output/eventing-plan-proof/74-lifecycle-clear/proof-summary.json` | Production shutdown drains dispatchable queued work, dead-letters remaining queued work, cancels pending requests, rejects new work, and preserves explicit test-only drop/reset proof.                                       |
