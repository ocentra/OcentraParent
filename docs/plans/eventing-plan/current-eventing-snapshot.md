...
- There is no Unity/TypeScript lineage compatibility suite.
- There is no event topology manifest for publisher/subscriber/orphan health.
- There is no generated event contract registry documentation.
- There is no manual clock implementation or deterministic timeout proof.
- There is no explicit duplicate subscriber policy implementation.
- There is no shutdown/drain/dead-letter/test-clear lifecycle implementation.
- There is no source gate preventing payload-carried deferred/completion,
  cancellation, resource, service, or cleanup handles.
- There is no proof that the reusable crate avoids hidden global singleton bus
  ownership.
