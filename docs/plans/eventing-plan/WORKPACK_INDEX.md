...
| ready — production source accepted; tests/proof open | [11 Type Safety And Ownership Hardening](workpacks/11-type-safety-and-ownership-hardening.md) | 63-68        | P2/P3 hardening                | Through `fa1230661`, live/stored decode revalidates identity, request completion is associated-response typed, unsupported journal idempotency fails closed, and action replay consumes a journal-minted non-cloneable authority. Existing replay tests still use the retired API; expected negative/audit test source, focused execution, and retained proof remain open. |
| open — missing harness and canonical proof root | [12 Rollout Proof And PR Gate](workpacks/12-rollout-proof-and-pr-gate.md)                     | Main gates   | route gate                     | `scripts/test/eventing-rollout-proof.mjs` is absent; canonical proof root is `output/eventing-plan-proof/12-rollout-proof-and-pr-gate/`. |
| open — code complete; validation/proof open | [13 Test Folder Layout Regression Audit](workpacks/13-test-folder-layout-regression-audit.md) | fresh audit  | P2_GENERIC_CRATE_RUNTIME       | Moved external test layout is present, but fresh validation/proof is absent; the `contract` Cargo harness is required. |
