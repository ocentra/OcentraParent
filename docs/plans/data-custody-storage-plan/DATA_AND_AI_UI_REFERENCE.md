...
# Data And AI UI Reference

Status: reference-only migrated product/UI requirements. This document is not
an executable workpack and carries no implementation, test, proof, checklist,
READY, PR_READY, or DONE claim. Requirements execute only through the named
Data runtime, parent bridge, Portal, and AI owner workpacks below.
...
## Executable Owner Route

This reference preserves product requirements; it does not own their source or
completion evidence.

Data execution is routed through:

- `WP-data-custody-storage-plan-08-parent-storage-settings-apply-flow` for the
  typed parent storage settings, preview, confirmation, and apply boundary;
- `WP-data-custody-storage-plan-11-runtime-composition-and-custody-mount` for
  owner-derived custody composition and fail-closed runtime mounting;
- `WP-parent-desktop-runtime-package-plan-06-parent-local-service-route-bridge`
  for the authenticated parent local-service crossing;
- `WP-portal-ux-household-surfaces-plan-01-service-backed-shell-and-navigation`
  for route and target navigation; and
- `WP-portal-ux-household-surfaces-plan-12-reports-notifications-and-custody`
  for the final parent-visible Data and custody surface.

AI execution is routed through:

- `WP-ai-plan-39-device-hardware-model-fit-lane`;
- `WP-ai-plan-40-model-catalog-artifact-integrity-lane`;
- `WP-ai-plan-41-llama-gguf-runtime-packaging-lane`;
- `WP-ai-plan-42-inference-settings-template-governance-lane`;
- `WP-ai-plan-43-ai-activity-portal-surface-lane`;
- `WP-ai-plan-44-provider-api-authorization-custody-lane`; and
- `WP-ai-plan-45-remote-redacted-report-assistant-lane`.

Only those executable owners may define code/test roots, dependencies, proof,
or completion. This reference must not inherit the Data plan's generic proof
directory or appear in the engineering graph READY queue.
