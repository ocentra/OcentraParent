...
- [x] Nearby-place evidence includes query radius, distance, provider,
      ...
      P1 provider-adapter proof:
      `output/tracking-plan-proof/20-google-places-and-poi-provider-adapter/07-nearby-place-proof.json`.
...
| Nearby-place evidence carries provider/radius/category/ambiguity          | P1_FIXTURE_SIMULATION        | P1_FIXTURE_SIMULATION | simulated          | `output/tracking-plan-proof/19-nearby-place-provider-abstraction/`; `output/tracking-plan-proof/20-google-places-and-poi-provider-adapter/07-nearby-place-proof.json`                                                                                                                                     | live provider credentials, production provider setup, provider delivery, UI, and physical-device proof remain pending                                              |
...
| 19   | Nearby-place provider abstraction                | [x] Contract proof complete            | P1_FIXTURE_SIMULATION | P1_FIXTURE_SIMULATION | simulated            | `output/tracking-plan-proof/19-nearby-place-provider-abstraction/`; `output/tracking-plan-proof/20-google-places-and-poi-provider-adapter/07-nearby-place-proof.json`                                                                                                                                     | live provider credentials, production provider setup, provider delivery, UI, and physical-device proof remain pending                                               |
| 20   | Google Places and POI provider adapter           | [x] P1 provider adapter proof complete | P1_FIXTURE_SIMULATION | P1_FIXTURE_SIMULATION | simulated            | `output/tracking-plan-proof/20-google-places-and-poi-provider-adapter/07-nearby-place-proof.json`; `output/tracking-plan-proof/20-google-places-and-poi-provider-adapter/13-security-negative-proof.log`                                                                                                  | live provider credentials, production provider setup, provider delivery, UI, and physical-device proof remain pending                                               |
...
- [x] Nearby-place/AI safety analysis contracts have focused contract proof;
      WP20 now has P1 provider-adapter proof for bounded Google Places nearby
      requests, minimal production field masks, deterministic category mapping,
      ambiguity, and unavailable-provider degradation. Live provider
      credentials, production provider setup, provider delivery, and UI remain
