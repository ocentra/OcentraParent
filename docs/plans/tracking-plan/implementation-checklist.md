...
| Tracking service read-model command                                       | P2_HOSTED_CI                 | P2_HOSTED_CI          | proved             | `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/14-retention-delete-proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/18-service-read-model-proof.json`                                                                                                  | narrow portal summary consumption and service-backed retention-delete tombstone replay proved; broader product read models, full child UI, and platform replay proof remain pending           |
...
| Tracking WSL/local replay proof                                           | P3_LOCAL_DEV_MACHINE         | P3_LOCAL_DEV_MACHINE  | proved             | `output/tracking-plan-proof/wsl-local-replay/proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/19-wsl-local-replay-proof.json`; `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/17-wsl-local-proof.json`                                            | Android/iOS physical-device, authority, hosted full UI/accessibility, provider delivery, and production proof remain separate                                                                 |
| Tracking hosted parent-route service citations                            | P2_HOSTED_CI                 | P2_HOSTED_CI          | proved             | `output/tracking-plan-proof/30-parent-and-child-ui-ux-surfaces/17-hosted-ui-proof.json`; `output/tracking-plan-proof/30-parent-and-child-ui-ux-surfaces/12-playwright-proof.log`; `test-results/tracking-plan-hosted-ui-proof/accessibility-summary.json`                                                   | seeded real-service parent-route proof only; child UI, physical-device screenshots, full parent/child UI, authority proof, and production pilot remain pending                              |
...
| 32   | Journal SQLite and read-model proof              | [ ] P2 service plus P3 WSL proof partial     | P3_LOCAL_DEV_MACHINE  | P3_LOCAL_DEV_MACHINE  | proved               | `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/10-journal-sqlite-proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/14-retention-delete-proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/18-service-read-model-proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/19-wsl-local-replay-proof.json`; `output/tracking-plan-proof/30-parent-and-child-ui-ux-surfaces/17-hosted-ui-proof.json` | narrow portal summary consumption, service-backed retention-delete tombstone replay, WSL/local replay, and hosted parent-route citations proved; broader read models, full child UI, platform physical proof, and production proof remain pending |
| 33   | Proof gates fixtures rollout and PR gate         | [x] Pre-device plus WSL and hosted UI gates complete | P3_LOCAL_DEV_MACHINE  | P3_LOCAL_DEV_MACHINE  | proved               | `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/proof-summary.json`; generated runtime artifact `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/00-run-metadata.json`; `output/tracking-plan-proof/pre-device-gap-closure/proof-summary.json`; `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/17-wsl-local-proof.json`; `output/tracking-plan-proof/30-parent-and-child-ui-ux-surfaces/17-hosted-ui-proof.json` | full child UI, Android/iOS physical-device, authority, provider delivery, and production proof remain pending                                                                                                                         |
...
`11-ui-snapshots/policy-tracking-parent-fixture.png`. The hosted proof command
`node scripts/test/tracking-plan-hosted-ui-proof.mjs` captures desktop/mobile
screenshots and accessibility summary for the parent `policy-tracking` route
rendering seeded real-service citations; child UI, physical-device screenshots,
full parent/child UI, authority proof, and production proof remain pending.
...
product read models and full child UI remain pending.
...
authority, hosted UI/accessibility, provider-delivery, or production proof.
...
- [x] Tracking service read-model command has P2 proof for SQLite tracking rows,
      ...
      replay with deleted evidence citation summaries, narrow parent portal
      summary consumption, and hosted parent-route service-citation rendering;
      broader read models, full child UI, and platform replay proof remain
      pending.
- [ ] Tracking UI/UX is not product-complete. A P1 parent portal tracking-state
      fixture, local parent-route screenshot, and P2 hosted parent-route
      screenshot/accessibility proof for service-backed citations exist; child
      UI, full live parent/child UI, physical-device screenshots, authority
      proof, and production proof remain pending.
...
- [x] WSL/local replay proof exists in
      ...
      physical-device, authority, hosted full UI accessibility,
      provider-delivery, and production proof as separate gates.
- [x] Product capability checklist row update is queued through the hub
      ...
      proof, hosted parent-route screenshot/accessibility proof, and remaining
      Android foreground/background location, iOS simulator, physical-device,
      authority, child UI, provider-delivery, and production gaps.
...
- [ ] Full child and physical-device screenshots are stored under the assigned
      proof root. The runtime proof command captures the local parent fixture
      screenshot at
      `output/tracking-plan-proof/30-parent-and-child-ui-ux-surfaces/11-ui-snapshots/policy-tracking-parent-fixture.png`.
- [x] Hosted parent-route screenshots and accessibility summary are stored for
      seeded real-service tracking citations:
...
- [ ] Physical-device screenshots are stored under the assigned proof root.
