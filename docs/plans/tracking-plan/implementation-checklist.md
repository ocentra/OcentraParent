...
| Tracking WSL/local replay                                                 | P3_LOCAL_DEV_MACHINE         | P3_LOCAL_DEV_MACHINE  | proved             | `test-results/tracking-plan-wsl-local-proof/proof.json`; `output/tracking-plan-proof/wsl-local-replay/proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/19-wsl-local-replay-proof.json`; `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/17-wsl-local-proof.json` | WSL2/Ubuntu, linked-worktree Git mapping, lockfile-declared Linux optional dependency availability, contract build, JS service read-model proof, and Rust core read-model test are proved; physical-device and product claims remain separate |
...
| 32   | Journal SQLite and read-model proof              | [ ] P2 service proof partial       | P2_HOSTED_CI          | P3_LOCAL_DEV_MACHINE  | proved               | `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/10-journal-sqlite-proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/18-service-read-model-proof.json`; `output/tracking-plan-proof/32-journal-sqlite-and-read-model-proof/19-wsl-local-replay-proof.json`                                                                                      | narrow portal summary consumption and WSL local service/read-model replay are proved; deletion/tombstone replay, richer read models, full UI, hosted UI/accessibility, and platform replay proof remain pending |
| 33   | Proof gates fixtures rollout and PR gate         | [x] Pre-device proof gate complete | P2_HOSTED_CI          | P3_LOCAL_DEV_MACHINE  | proved               | `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/proof-summary.json`; generated runtime artifact `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/00-run-metadata.json`; `output/tracking-plan-proof/pre-device-gap-closure/proof-summary.json`; `output/tracking-plan-proof/33-proof-gates-fixtures-rollout-and-pr-gate/17-wsl-local-proof.json` | WSL/local proof harness is proved for local replay; hosted full UI/a11y, live UI, platform-local, physical-device, authority, and production proof remain pending                                               |
...
WSL/local replay now has an evidence-backed local harness through
`npm run test:tracking-plan-wsl-local-proof`, written to
`output/tracking-plan-proof/wsl-local-replay/`. On the current Windows linked
worktree it proves WSL2/Ubuntu availability, a mapped Git dir, contract build,
the JS service read-model proof, and Rust core tracking read-model tests. The
harness probes for the lockfile-declared Linux Rolldown optional dependency and
repairs the local WSL `node_modules` state when needed. WSL/local replay is
proved for this local replay path, while product claims stay blocked until
mobile, hosted UI, physical-device, authority, and production proof exist.
...
- [x] Product capability checklist row update is queued through the hub
      ...
      production gaps.
- [x] WSL/local replay proof harness exists in
      `scripts/test/tracking-plan-wsl-local-proof.mjs`; the current artifact is
      P3 local replay proof because the harness records WSL2/Ubuntu, mapped
      Git, contract build, JS service proof, and Rust core read-model proof.
