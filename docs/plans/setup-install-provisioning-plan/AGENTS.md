...
Context: parents start at `family.ocentra.ca`; the site is informational by default and must not collect child activity data. Registration/login starts an account/household flow, then routes parents to the right installer, child-device install, permissions, pairing, and readiness status.
...
- `parent-desktop-runtime-package-plan` owns package build, signing, update, rollback, and installer artifact proof.
