...
| source accepted / tests-validation-proof deferred | [WP07 First-Run Setup UI And State Machine](workpacks/07-first-run-setup-ui-and-state-machine.md) | bounded Rust source packet accepted; 14/14 historical proof rows are not refreshed by this source wave | `docs/expectations/family-setup.md`, `docs/expectations/portal.md` | `output/setup-install-provisioning-plan-proof/07-first-run-setup-ui-and-state-machine/` |
