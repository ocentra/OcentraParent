...
Known gaps/manual-required states: real LAN/device-trust proof remains owned by `docs/plans/lan-plan`; redacted pairing-log and bootstrap-log artifacts are blocker files, not green proof; no physical-device LAN proof, service restart proof, or broader portal runtime proof is claimed here.
