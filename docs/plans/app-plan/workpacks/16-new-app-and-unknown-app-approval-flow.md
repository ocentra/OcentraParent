...
- `apps/portal/src/*approval*` or live activity surfaces when assigned.
- `fixtures/app/policy/ask_unknown_apps.json`

## Tests And Proof

- New inventory app creates new_app_detected candidate.
- Unknown runtime process creates new_app_launched candidate.
- Parent approval request includes evidence refs and expiry.
- Allow once expires; allow always persists with audit.
- Parent block returns manual_required if no adapter proof exists.
- Approval survives restart/replay.
