...
- [x] Before-state source snapshot recorded in `output/app-plan-proof/14-app-read-models-and-service-events/00-source-snapshot.md`.
...
- [x] Raw evidence artifacts captured where applicable: inventory, runtime, foreground, launcher, daily rollup, and SQLite/read-model rows are covered by the shared app/game projection proof; policy, approval, authority-tier, and enforcement results are N/A for this read-model-only slice.
...
- [x] Manual platform proof captured for any claim stronger than observe-only; no stronger claim was made, so the proof pack records N/A.
