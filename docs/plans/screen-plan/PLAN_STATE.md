...
Retention sweeper: expired encrypted queue record removed and visible as expiredDeleted row; no parent retention UI/cloud policy claim. WP22 additionally has a real Windows raw-temp deletion proof and focused queue/sweeper/protocol gates; portal unit contracts pass, while the shared E2E runner times out in unrelated network coverage before the screen screenshot artifact.
...
- [22 Deletion And Retention Proof](workpacks/22-deletion-and-retention-proof.md) - 5/6 checked, 1 open (rendered portal screenshot).
