...
| Encrypted temporary queue custody | P3 proved | `output/screen-plan-proof/real-capture/manual-parent-test-active-window/03-encrypted-queue.ndjson` | Queue proof is local harness path, not service scheduler integration. |
...
| Timed two-frame cadence harness | P3 proved | `output/screen-plan-proof/real-capture/trigger-matrix/proof-summary.json` | Does not claim product scheduler or disabled-setting suppression yet. |
