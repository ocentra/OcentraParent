...
| Windows service WinRT OCR runtime          | P3 real OCR service proved  | `output/screen-ai-pipeline-proof/service-winrt-ocr/proof-summary.json`                                                                                                                                                                                         | Proves timed cadence service capture from a live public Wikipedia Chrome window through the encrypted queue, Windows WinRT OCR, `localOcr` Activity Screen read model with bounded OCR snippets/redaction-note array shape, queue drain, and temp image deletion; no sensitive-redaction-on-public-source, production OCR quality, authenticated/social, cross-platform, enforcement, live view, or raw retention claim.            |
| OCR sensitive text redaction proof         | P2 contract + portal intent | `output/screen-plan-proof/19-sensitive-text-and-redaction-model/proof-summary.json`; `output/screen-plan-proof/19-sensitive-text-and-redaction-model/portal-intent-proof-summary.json`; `output/screen-ai-pipeline-proof/service-winrt-ocr/proof-summary.json` | Proves parent-controlled OCR snippet limits, disabled OCR text state, credential-like suppression, PII-like redaction, no raw text retention, no raw image retention, no remote AI, parent portal intent rendering of redacted OCR snippets/redaction notes, and service persistence of bounded OCR snippets/redaction-note shape. A real service-emitted portal screenshot with actually redacted sensitive text remains separate. |
| OCR Tesseract baseline readiness           | P1 readiness/unavailable    | `output/screen-plan-proof/34-ocr-tesseract-baseline/proof-summary.json`                                                                                                                                                                                        | Verifies current upstream Tesseract project/docs/license direction and records this Windows lane lacks `tesseract` on `PATH`; no extraction, packaging, quality, CPU/memory/runtime, or PaddleOCR comparison claim.                                                                                                                                                                                                                 |
