...
| Screen parent portal summary UI            | P2 route/build proof        | `output/screen-plan-proof/screen-parent-portal-summary-ui/proof-summary.json`                      | Proves the dedicated Screen Analysis route consumes service-backed Activity Screen read-model rows and exposes capability, queue, summary/category, confidence, model/runtime, custody/deletion, policy, audit/evidence, and not-claimed enforcement details. Fresh desktop/mobile screenshots remain separate.                                          |
...
- [x] Screen Analysis route renders a parent summary UI from service-backed
      Activity Screen rows with capability, queue, model, confidence, custody,
      policy, audit, evidence refs, and not-claimed enforcement details.
