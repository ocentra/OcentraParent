...
- [x] Before-state source snapshot recorded in `output/browser-plan-proof/20-windows-applocker-app-control-proof/00-source-snapshot.md`.
...
- [x] Raw evidence artifacts captured where applicable: this workpack adds read-model/proof-state representation, not live AppLocker/WDAC event capture.
...
- [x] Security/no-claim negative proof captured where applicable: no unmanaged exact URL claim, no AppLocker/WDAC launch prevention claim, no policy creation/update/rollback claim, and no AI direct enforcement.
