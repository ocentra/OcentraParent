...
- [x] Before-state source snapshot recorded in `output/browser-plan-proof/10-tab-evidence-mapper/00-source-snapshot.md` or explicit docs-only N/A reason.
- [x] Contracts updated first where this workpack changes behavior.
- [x] Rust/service parity updated only after contracts exist; no portal UI change was made.
- [x] Raw evidence artifacts captured where applicable: bridge/CDP payloads, managed session state, unmanaged process rows, journal entries, SQLite/read-model rows, policy decisions, and action results.
...
- [x] UI snapshots are not applicable; `ui-not-applicable.md` records why.
