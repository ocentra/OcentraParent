...
- [x] Before-state source snapshot recorded in `output/browser-plan-proof/24-rollout-checklist-and-pr-gate/00-source-snapshot.md`.
- [x] Contracts updated first where this workpack changes behavior; WP24 is a rollout gate and adds no new runtime contract.
- [x] Rust/service/portal parity updated only after contracts exist; no Rust/service/portal runtime code changed for WP24.
- [x] Raw evidence artifacts captured where applicable: completed base workpack proof paths are summarized in the main checklist.
