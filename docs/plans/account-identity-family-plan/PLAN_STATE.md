...
```text
Plan route: upgraded
Execution-grade workpacks: WP01 has a docs-only provider/custody proof pack on disk; WP08 has a tracked durable Rust-authority manifest under `docs/proof/account-identity-family-plan/08-rust-schema-workers-d1-runtime-migration/`; WP02, WP03, WP04, WP05, and WP07 have prior complete proof roots on disk; WP06 is reopened for final aggregation after WP08 plus Cloudflare WP06/WP08 handoffs
Implementation: central-schema migration is in progress/current for shared account/family shapes; helper/projection implementation exists in family-domain, setup-domain, family-identity-core, and provisioning-core; WP08 verifies only the Rust authority/generated-edge slice; Cloudflare-owned account-identity adapter/runtime and D1/DO/KV schema/migration proof remain open
