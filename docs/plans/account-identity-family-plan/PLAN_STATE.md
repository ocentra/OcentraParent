...
Implementation: central-schema migration is in progress/current for shared account/family shapes; helper/projection implementation exists in family-domain, setup-domain, family-identity-core, and provisioning-core; WP08 verifies only the Rust authority/generated-edge slice. A 2026-08-16 code-drafted `AccountIdentityAuthorityHandoff` now publishes the provider-subject mapping plus existing account/household/member/device/session authority vocabulary through `crates/schema` and `packages/schema-domain`; its focused checks and proof are deferred. Merged #632 adds an optional/manual-required Cloudflare D1 provider-subject mapping adapter, while external provider verification, account runtime routes, and D1/DO/KV schema/migration proof remain open
...
Cloudflare control-plane runtime/schema:
  #632 merged a narrow optional `ACCOUNT_IDENTITY_D1` provider-subject mapping adapter after provider verification; it does not establish token verification, account/session routes, household authority, a deployed/migrated D1 schema, Durable Object coordination, or production Worker readiness. Those runtime/persistence boundaries remain open here.
...
- WP08's new `AccountIdentityAuthorityHandoff` schema/generator slice is code-drafted at `crates/schema/src/account_identity_authority.rs` and `packages/schema-domain/src/account-identity-authority.ts`; its focused Rust/TypeScript checks and retained proof are deferred and the prior durable manifest does not validate this follow-up. Cloudflare WP06's typed consumer is code-drafted at `infra/cloudflare/src/account-identity-d1-adapter.ts`; it never persists the optional authority snapshot and has no public route.
- Cloudflare WP06 now supplies only a narrow optional/manual-required `ACCOUNT_IDENTITY_D1` provider-subject mapping adapter backed by the generated Account WP08 handoff. No Cloudflare-owned token-verification route, account-family authority binding, deployed/migrated schema proof, Durable Object coordination, or runner proof exists; the adapter is not runtime authority.
