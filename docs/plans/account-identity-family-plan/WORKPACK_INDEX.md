...
| validation / production source accepted / route-store tests and proof open | [WP03 Session Token Lifecycle](workpacks/03-session-token-lifecycle.md) + [current boundary addendum](workpacks/03-current-boundary-addendum.md) | 14/14 historical proof; Cloudflare browser session migrations/store/routes now mapped, expected route/store/request-safety tests and proof remain open | `RESEARCH_AND_DECISIONS.md`, `crates/family-identity-core/src/session_lifecycle.rs`, `infra/cloudflare/src/storage/account-browser-session-store.ts`, `infra/cloudflare/src/auth/browser-session-routes.ts` | `output/account-identity-family-plan-proof/03-session-token-lifecycle/` |
