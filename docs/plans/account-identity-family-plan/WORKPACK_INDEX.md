...
| blocked / production source reviewed / P0 trusted-device repair and route/runtime composition open | [WP03 Session Token Lifecycle](workpacks/03-session-token-lifecycle.md) + [current boundary addendum](workpacks/03-current-boundary-addendum.md) | 14/14 historical proof; isolated Cloudflare browser-session migrations/store/routes, Firebase/JWKS inputs, and Rust session repository are positive source topology only; provider bearer identity is projected to `trustedDevice: true` without a request-bound owner/device credential, session routes are unbound and return 501/manual-required, production auth/D1/migration mounting is unproven, the Rust repository has no non-owner caller, and four expected Cloudflare runtime test/proof roots remain absent | `RESEARCH_AND_DECISIONS.md`, `crates/family-identity-core/src/session_lifecycle.rs`, `crates/family-identity-core/src/session_lifecycle_repository.rs`, `infra/cloudflare/migrations/account-identity/0007_account_browser_session_custody_hardening.sql`, `infra/cloudflare/src/storage/account-browser-session-store.ts`, `infra/cloudflare/src/auth/browser-session-routes.ts`, `infra/cloudflare/src/providers/firebase-auth-jwks.ts` | `output/account-identity-family-plan-proof/03-session-token-lifecycle/` |
| source and six expected test roots reviewed / owner adapters and execution open | [WP04 Invites Recovery Lifecycle](workpacks/04-invites-recovery-lifecycle.md) | 13/13 historical proof; strict durable repository, rate/replay custody, private owner receipts, typed handoff source, and all six expected behavioral test roots reviewed; shipped identity/membership/support/Data owners, focused execution, and current proof open | `docs/expectations/family-setup.md`, `docs/expectations/data-custody.md` | `output/account-identity-family-plan-proof/04-invites-recovery-lifecycle/` |
...
production owner adapters, remaining runtime handoffs, WP02/WP03/WP05/WP08
expected-test writing, focused execution of WP04's reviewed six-root test
packet, validation, and current proof are still open.
