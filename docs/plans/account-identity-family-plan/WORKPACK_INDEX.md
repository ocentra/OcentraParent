...
| planned / normal completion gated / tests, adapter, and Cloudflare composition open | [WP09 Account Issuer Key Custody And Cloudflare Handoff](workpacks/09-account-issuer-key-custody-and-cloudflare-handoff.md) | New Account-owned durable issuer/key registry, authenticated producer binding, startup reload/recovery, and sealed WP08 handoff route; all mapped source/test roots absent | `PLAN_STATE.md`, WP08 sealed transport contract, `docs/engineering-graph/code-map.json` | `docs/proof/account-identity-family-plan/09-account-issuer-key-custody-and-cloudflare-handoff/` |
...
WP09 is the Account-owned durable issuer and signing-key custody route. It owns monotonic public-key registry/revocation, an authenticated Account producer service-binding adapter, startup reload/recovery, and the typed handoff over WP08's existing sealed contract. Its only direct dependency is Account WP08 as a reviewed-implementation prerequisite; normal completion remains gated on WP08 DONE. The five planned source roots and four expected test roots are absent. This route does not claim Cloudflare, Device Trust, Account WP02, Account WP05A, runtime readiness, READY, or DONE.
