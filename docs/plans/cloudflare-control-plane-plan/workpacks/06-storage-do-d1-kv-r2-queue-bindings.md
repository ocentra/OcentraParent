...
- Required direct migration-test validation: `npm --prefix infra/cloudflare exec -- node --import tsx --test tests/integration/account-identity-d1-migration.test.ts`; retain its result separately so the aggregate integration script cannot omit it.
...
- `npm --prefix infra/cloudflare exec -- node --import tsx --test tests/integration/account-identity-d1-migration.test.ts` required direct test; retain its result in `03-account-identity-d1-migration-test.md` and do not let `test:integration` substitute for it
