...
| WP01 | `npm --prefix packages/logging-domain run build`; `npm --prefix infra/cloudflare install --ignore-scripts --no-audit --no-fund --no-package-lock`; `npm --prefix infra/cloudflare ls wrangler @cloudflare/workers-types`; `npm --prefix infra/cloudflare run lint`; `npm --prefix infra/cloudflare run test:unit`; `npm run lint:architecture -- --files infra/cloudflare` | `docs/proof/cloudflare-control-plane-plan/01-cloudflare-module-scaffold/` |
