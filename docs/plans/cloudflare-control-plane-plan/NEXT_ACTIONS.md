...
| 06 | WP06 account-identity storage binding/migration | code drafted; proof deferred; blocked on Account WP08 contract and WP01 environment | `infra/cloudflare/src/env.ts`, both Wrangler configs, isolated D1 migration, and migrated-schema store | Consume the Account WP08 canonical handoff later; apply the isolated account migration only after dependency restoration and retain its direct result. Keep account DO/KV absent/manual-required, do not borrow `BILLING_D1`, and do not add the DTO adapter in this slice. | The exact migration proof identifies the selected account binding and directory for Cloudflare WP08 and Account WP06; no account/runtime/deployment closure. |
