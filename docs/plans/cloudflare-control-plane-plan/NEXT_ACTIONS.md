...
| 07 | Local dev, seeding, and fixtures | proof-produced / acceptance-open | `LOCAL_DEV_AND_SEEDING_MODEL.md` | Route the committed and current-head-validated WP07 packet through the owning checklist/index acceptance path while keeping production deployment, WP12 handoff, downstream payment acceptance, and manual authority states open. | The WP07 packet is accepted and consumed by the owning checklist/index without promoting local preflight, seed, structured-log, or bounded Wrangler evidence into production, WP12, or payment readiness. |
