...
| 06 | WP06 account-identity storage binding/migration | code drafted; proof deferred; provider verification/manual-required | `infra/cloudflare/src/account-identity-d1-adapter.ts`, `src/storage/account-identity-store.ts`, both Wrangler configs, and isolated D1 migration | Consume the generated Account WP08 handoff through the typed adapter; apply the isolated account migration only after dependency restoration and retain its direct result. Keep account DO/KV and provider verification manual-required, do not borrow `BILLING_D1`, and keep the adapter off public routes. | The exact migration proof identifies the selected account binding and directory for Cloudflare WP08 and Account WP06; no account/runtime/deployment closure. |
