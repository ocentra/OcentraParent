...
| 06 | WP06 account-identity storage binding/migration | code drafted; proof deferred; provider verification/manual-required | `infra/cloudflare/src/storage/account-identity-store.ts`, both Wrangler configs, and isolated D1 migration | Apply the isolated account migration only after dependency restoration and retain its direct result. Keep account DO/KV and provider verification manual-required, do not borrow `BILLING_D1`, and do not treat the uncalled store as a runtime handoff. | The exact migration proof identifies the selected account binding and directory for Cloudflare WP08 and Account WP06; no account/runtime/deployment closure. |
