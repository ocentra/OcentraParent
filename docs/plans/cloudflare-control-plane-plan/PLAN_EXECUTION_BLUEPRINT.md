...
| 01. Module scaffold | WP01 | `infra/cloudflare/package.json` | Plan route is selected | Module tree and current concentrated source shape are documented honestly | `docs/proof/cloudflare-control-plane-plan/01-cloudflare-module-scaffold/` (durable WP01 exception; raw output stays ignored) | Remove unsupported files or mark them manual-required |
