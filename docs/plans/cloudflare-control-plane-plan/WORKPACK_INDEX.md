...
| reviewed source present / mutation mount and expected tests open | [WP06 Storage DO D1 KV R2 Queue Bindings](workpacks/06-storage-do-d1-kv-r2-queue-bindings.md) | D1 read/current-authority plus Account-owned create/CAS/revoke source retained; mutation runtime composition, migration/tests/proof open | `STORAGE_BINDING_MODEL.md`; Account WP08 `v0.7`; Account WP02 target authority; isolated account D1 migration/config | no tracked root |
...
WP06 blocks storage/coordination/queue claims. After Account WP08 and target-aware Account WP02, it owns the authoritative Account D1 create/update/revoke/currentness/CAS path and Firebase/provider-to-sealed-authority caller. Those source owners now exist and the verifier reaches current-authority resolution, but Account-owned mutations are not mounted in the Worker entrypoint. Caller-supplied trust facts are rejected, higher capability/lease/step-up authority remains unavailable, and runtime composition, migration execution, expected tests, proof, deployment, and normal DONE semantics remain open.
