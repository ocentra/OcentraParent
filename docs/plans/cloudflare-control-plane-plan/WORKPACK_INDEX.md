...
| reviewed safe read/manual boundary; Account transport source mapped; private verifier/mount and expected tests open | [WP06 Storage DO D1 KV R2 Queue Bindings](workpacks/06-storage-do-d1-kv-r2-queue-bindings.md) | D1 read/current-authority and parameterless mutation readiness retained; writer create/CAS/revoke stays producer-gated; Account transport exists, authenticated adapter/key custody, private verifier/mount, migration/tests/proof open | `STORAGE_BINDING_MODEL.md`; Account WP08 producer transport; Account WP02 target authority; isolated account D1 migration/config | no tracked root |
...
WP06 blocks storage/coordination/queue claims. After Account WP08 and target-aware Account WP02, it owns the authoritative Account D1 create/update/revoke/currentness/CAS path and Firebase/provider-to-sealed-authority caller. The runtime is intentionally only a safe verified-provider read/manual boundary: Account-owned writer mutations remain gated by an unconstructible producer, and no scalar mutation API is exposed. The exact missing route is Account WP02/WP08 signed/sealed current-authority producer transport plus Worker verifier. Caller-supplied trust facts are rejected, higher capability/lease/step-up authority remains unavailable, and migration execution, expected tests, proof, deployment, and normal DONE semantics remain open.
