...
| private decoder/verifier source accepted; durable Account binding and expected tests open | [WP06 Storage DO D1 KV R2 Queue Bindings](workpacks/06-storage-do-d1-kv-r2-queue-bindings.md) | D1 read/current-authority and parameterless mutation readiness retained; writer create/CAS/revoke stays producer-gated; bounded decoder/private one-shot verifier source exists, but Account service-binding adapter/key registry, runtime mount, migration/tests/proof remain open | `STORAGE_BINDING_MODEL.md`; WP06 producer-consumer contract; Account WP08 producer transport; Account WP02 target authority; isolated account D1 migration/config | no tracked root |
...
WP06 blocks storage/coordination/queue claims. After Account WP08 and target-aware Account WP02, it owns the authoritative Account D1 create/update/revoke/currentness/CAS path and Firebase/provider-to-sealed-authority caller. The bounded decoder and private one-shot Worker verifier are source-accepted, but the runtime remains a safe verified-provider read/manual boundary until the Account-owned service-binding adapter and durable public-key registry are authenticated. Only then may the verified handoff reach the existing D1 writer's same-transaction currentness/revocation/CAS recheck. Caller-supplied trust facts, Firebase authority substitution, environment keys, and fixture keys are rejected; higher capability/lease/step-up authority remains unavailable, and migration execution, expected tests, proof, deployment, and normal DONE semantics remain open.
