...
Use this file to point at the active workpack, not to store proof.

| Checklist row | Owning workpack | Close when |
| --- | --- | --- |
| CF-00 | WP00 | Games parity map records keep/adapt/strip decisions. |
| CF-01 | WP01 | Module tree exists and no-claim scaffold is explicit. |
| CF-02 | WP02 | Wrangler envs, bindings, and secret names are documented. |
| CF-03 | WP03 | Entry guard chain and scheduled hook shape are explicit. |
| CF-04 | WP04 | Route manifest and domain contract ownership are explicit. |
