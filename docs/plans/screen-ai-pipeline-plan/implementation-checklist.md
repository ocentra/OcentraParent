...
| Live operator YouTube ordinary video     | P3 local operator proved | `output/screen-ai-pipeline-proof/live-operator/proof-summary.json`                          | Proves one real operator-supplied YouTube URL opened in a headful browser, recorded page readiness evidence, captured the active local window, ran local VLM, produced `video`/`warn`, and deleted raw image material. The remaining eight live operator rows are not complete.              |
...
- [x] Partial live operator proof completed for ordinary YouTube video:
