...
| Live operator YouTube/native/protected rows | P3 local operator proved | `output/screen-ai-pipeline-proof/live-operator/proof-summary.json`                          | Proves a real operator-supplied YouTube URL opened in a headful browser, recorded page readiness evidence, captured the active local window, ran local VLM, produced `video`/`warn`, and deleted raw image material. The same manifest run also proves a real native Notepad app row as `productivity`/`allow` and a protected-surface degraded row with no raw image, AI, or policy claim. The remaining six live operator rows are not complete. |
...
- [x] Partial live operator proof completed for ordinary YouTube video, native app, and protected/unsupported state:
