...
```text
AI is local child-device safety first.
...
Enforcement adapters consume policy decisions, not raw AI output.
...
```mermaid
flowchart TD
  Capture["Browser, apps, games, tracking, LAN, network, and screen capture slices"] --> Journal["Encrypted local journal"]
  Journal --> Store["SQLite read models"]
  Store --> Context["Local AI evidence context builder"]
  ...
  Runtime["Local model/runtime status and provider capability"] --> Context
  Context --> Queue["AI job queue and provider router"]
  Queue --> Deterministic["Deterministic classifier lane"]
  Queue --> TextModel["Local text LLM lane"]
  Queue --> Ocr["OCR lane"]
  Queue --> Vlm["Guided VLM lane"]
  Deterministic --> Result["Schema-valid AI result"]
  TextModel --> Result
  Ocr --> Result
  Vlm --> Result
  Result --> Policy["Deterministic parent-policy evaluator"]
  ...
  Remote["Parent-approved remote assistant, V4 only"] -.-> Portal
```
...
| 07   | [AI job queue contract](workpacks/07-ai-job-queue-contract.md)                                                   | AI jobs are bounded, prioritized, cancellable, auditable, and source-referenced.                               |
| 08   | [AI provider routing contract](workpacks/08-ai-provider-routing-contract.md)                                     | Deterministic, local text LLM, OCR, VLM, and remote parent-assistant routes are explicit.                      |
...
| 32   | [Family AI hub and remote assistant boundary](workpacks/32-family-ai-hub-and-remote-assistant-boundary.md)       | Remote/API AI is parent-approved explanation/report help, not child safety authority.                          |
...
| 47   | [Performance resource battery proof lane](workpacks/47-performance-resource-battery-proof-lane.md)               | AI jobs respect resource limits, queue backpressure, foreground safety, and battery/thermal states.            |
