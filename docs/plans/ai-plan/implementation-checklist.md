...
| Screen AI policy integration                      | P3 contract proved  | `output/ai-plan-proof/screen-ai-policy-integration-proof/proof-summary.json`                                                           | Proves existing local AI safety result and policy decision contracts treat screen AI as evidence input, not policy authority: stricter parent block and time-limit rules override schema-valid local screen AI allow/low-confidence allow outputs while preserving AI result refs, evidence refs, rule refs, dry-run state, and disabled enforcement handoff. It does not execute a model, rerun capture, render portal UI, or dispatch enforcement.                                        |
...
- [x] screen AI policy integration proof artifacts under
      `output/ai-plan-proof/screen-ai-policy-integration-proof`.
- [x] screen AI policy integration proof run:
      `node --check scripts/test/screen-ai-policy-integration-proof.mjs` and
      `node scripts/test/screen-ai-policy-integration-proof.mjs`.
