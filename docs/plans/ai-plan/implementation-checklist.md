...
| Parent-rule context builder                       | P3 contract proved  | `output/ai-plan-proof/local-ai-parent-rule-context-proof/proof-summary.json`                                                           | Proves grounded parent-rule refs are selected only when their target evidence survives local AI context filtering, and ungrounded or missing parent-rule context degrades with `parent-rule-missing`. It does not execute a model, prove model quality, use remote/API AI, grant policy authority, dispatch enforcement, or retain raw evidence.                                                                                                                                                             |
...
- [x] Parent-rule context builder implemented with proof that ungrounded or
      missing parent-rule context degrades before local AI input can claim
      readiness.
...
- [x] local AI parent-rule context proof artifacts under
      `output/ai-plan-proof/local-ai-parent-rule-context-proof`.
- [x] local AI parent-rule context proof run:
      `node --check scripts/test/local-ai-parent-rule-context-proof.mjs` and
      `node scripts/test/local-ai-parent-rule-context-proof.mjs`.
