...
| Local text output parser                          | P3 contract proved  | `output/ai-plan-proof/local-ai-text-output-parser-proof/proof-summary.json`                                                            | Proves generic local text model output is parsed into a policy-eligible `LocalAiSafetyResult` only when schema, local runtime, provider, model, prompt, evidence, and parent-rule refs match the adapter proof. Malformed, remote, mismatched, or manual-required output is rejected before policy eligibility without retaining raw model output, executing a model, claiming model quality, policy authority, UI, or enforcement.                                                                          |
| Local text parser read model                      | P3 contract proved  | `output/ai-plan-proof/local-ai-text-parser-read-model-proof/proof-summary.json`                                                        | Projects generic local text parser proof rows into parent-visible ready and manual-required read-model rows while preserving parser, runtime, provider, model, prompt, evidence, parent-rule, and source-proof refs. It does not execute a model, prove model quality, retain raw model output, render portal UI, grant policy authority, use remote/API AI, or dispatch enforcement.                                                                                                                        |
| Local text parser policy handoff                  | P3 contract proved  | `output/ai-plan-proof/local-ai-text-parser-policy-handoff-proof/proof-summary.json`                                                    | Proves only ready generic local text parser read-model rows can create dry-run `PolicyDecision` rows with disabled enforcement handoff and local AI result refs, while rejected or manual parser rows remain manual-required and policy-ineligible. It does not execute a model, prove model quality, retain raw model output, render portal UI, grant policy authority, use remote/API AI, or dispatch enforcement.                                                                                         |
...
- [x] Output parser implemented for generic local text output without
      model-execution, remote/API, policy-authority, enforcement,
      production-model-quality, or raw model-output retention claims.
- [x] Text parser read-model projection implemented for ready and
      manual-required parent-visible rows without model-execution, remote/API,
      raw output retention, policy-authority, enforcement, portal UI, or
      production-model-quality claims.
- [x] Text parser policy handoff implemented so only ready parser rows create
      dry-run policy decisions; rejected/manual rows remain manual-required
      without model-execution, remote/API, raw output retention,
      policy-authority, enforcement, portal UI, or production-model-quality
      claims.
...
- [x] local text output parser proof artifacts under
      `output/ai-plan-proof/local-ai-text-output-parser-proof`.
- [x] local text output parser proof run:
      `node --check scripts/test/local-ai-text-output-parser-proof.mjs` and
      `node scripts/test/local-ai-text-output-parser-proof.mjs`.
- [x] local text parser read-model proof artifacts under
      `output/ai-plan-proof/local-ai-text-parser-read-model-proof`.
- [x] local text parser read-model proof run:
      `node --check scripts/test/local-ai-text-parser-read-model-proof.mjs` and
      `node scripts/test/local-ai-text-parser-read-model-proof.mjs`.
- [x] local text parser policy handoff proof artifacts under
      `output/ai-plan-proof/local-ai-text-parser-policy-handoff-proof`.
