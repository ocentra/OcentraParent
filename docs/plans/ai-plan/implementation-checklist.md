...
| Activity Screen degraded AI surface               | P3 contract proved  | `output/ai-plan-proof/activity-screen-ai-degraded-surface-proof/proof-summary.json`                                                    | Proves exported Activity Screen read-model contracts can surface local OCR unavailable and local VLM degraded rows with runtime/model/template refs, deleted or unavailable image custody, no raw image retention, and no policy eligibility. It does not execute OCR/VLM inference, prove production model quality, render portal UI, grant policy authority, or dispatch enforcement.                                                                                                                      |
...
- [x] Screen OCR/VLM degraded states visible.
...
- [x] Activity Screen degraded AI surface proof artifacts under
      `output/ai-plan-proof/activity-screen-ai-degraded-surface-proof`.
- [x] Activity Screen degraded AI surface proof run:
      `node --check scripts/test/activity-screen-ai-degraded-surface-proof.mjs`
      and `node scripts/test/activity-screen-ai-degraded-surface-proof.mjs`.
