# WP152 - Timer parent preference setup provider delivery manual-required handoff

<!-- agent-capsule -->

> Agent Capsule
> Plan: `app-game-plan`
> Doc: `WP152 - Timer parent preference setup provider delivery manual-required handoff`
...
provider-delivery readiness to a provider-delivery attempt boundary. This is a
manual-required handoff row that proves the service can expose the next provider
step without executing provider delivery.

## Implementation

- `packages/agent-protocol-domain` adds provider-delivery attempt refs/status
  and claimed flags to the accepted parent preference setup request result
  schema.
- `crates/agent-protocol` mirrors the result fields and adds provider-delivery
  attempt status/suffix constants.
- `crates/agent-service` persists a provider-delivery manual-required attempt
  audit row after provider-delivery readiness persists.
- `packages/portal-domain` and `apps/portal` render and test provider-delivery
  attempt refs/status in accepted command-result details.
- App/game docs record that this is a manual-required handoff only.
