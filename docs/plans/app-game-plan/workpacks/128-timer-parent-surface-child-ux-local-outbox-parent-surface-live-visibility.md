...
> Agent Capsule
> Plan: `app-game-plan`
> Doc: `WP128 Timer Parent-Surface Child UX Local Outbox Parent Surface Live Visibility`
...
surfacing child UX local outbox parent-surface intent readiness through the live
timer parent-surface protocol, service read model, and parent portal summary.

## Scope

- Extend the shared timer parent-surface read model with child UX parent-surface
  intent counts and reference ids.
- Mirror the contract in Rust protocol types.
- Derive the live service fields from replayed child UX local artifact/action
  result rows.
- Render the parent-surface intent aggregates in the existing App/Game Sessions
  timer parent-surface panel.
...
- No raw private source rows, private diagnostics, raw target values, screenshots,
  reports, or sensitive child evidence in the portal summary.
