...
Add a parent-domain service response-consumer parent-surface status read-model handoff that consumes WP110 service response-consumer parent-surface status handoff rows and records which native app/native game rows still require parent-surface status read-model proof before parent-visible status read-model consumption can be claimed.
...
- [x] Add parent-domain service response-consumer parent-surface status read-model handoff schemas, builder, no-claim flags, and state rules.
- [x] Add focused parser/builder tests using the real WP110 proof output.
...
- `output/app-game-plan-proof/111-timer-service-response-consumer-parent-surface-status-read-model`
