...
proof at `output/app-game-plan-proof/203-app-game-windows-local-policy-evidence-proof/`;
...
absent upstream proof refs (WP195/WP205), absent
...
| WP32-WP48 | Existing service activity-surface composition carries staged evidence/identity/authority/classifier refs into app-use/games read models. The mounted WP48 portal dashboard consumes `sourceStatusRows` and renders source counts, freshness, capability, timestamp, and evidence summaries. Canonical `ca8c1ae76` adds the real focused intent test for fresh/manual, unavailable, stale, degraded, hostile-metadata, evidence, and redaction behavior. | The refs are projection evidence, not trusted policy/compiler/evaluator authority. Test execution and retained proof remain open. |
...
- WP48 and WP63 are no longer source/test-writing gaps. The tracked dashboard
  consumes `sourceStatusRows`; the split source-panel intent groups App use/Game
  rows; the mounted SVG renders source freshness metrics; and canonical
  `ca8c1ae76` adds their shared real focused behavior test. Execution, retained
  proof, and all source-authority, policy, delivery, and adapter claims remain
  open.
...
- The child UX scheduler bridge now maps reopened honest local-outbox rows into the shared Rust `due-local` record, preserves source/scheduler/evidence refs, blocks non-queued states, fails closed on unsafe claims, and persists conservative proof rows atomically with restart/idempotency/conflict tests. Its focused 13+15 tests, full 128-test contract target, strict Clippy, architecture, and retained [WP59 proof manifest](../../proof/app-game-plan/WP59_NOTIFICATION_SCHEDULER_BRIDGE_PROOF.md) are green. Production scheduling, retry/quiet-hours execution, provider/preference execution, receipts, service composition, and UI remain open.
- The child UX provider preflight now accepts only persisted identity-bound `due-local` scheduler rows, preserves scheduler/outbox/evidence/policy/audit refs, generates distinct adapter/credential/smoke-proof requirements, and keeps manual/dead-letter or unsafe rows blocked. Its focused 4+5 tests, full 128-test contract target, strict Clippy, architecture, and retained [WP61 proof manifest](../../proof/app-game-plan/WP61_NOTIFICATION_PROVIDER_PREFLIGHT_PROOF.md) are green. Provider execution, credential custody, receipts, retry/quiet-hours runtime, cloud routing, service composition, and UI remain open.
...
- The child UX preference preflight now maps persisted due-local scheduler rows to explicit parent preference, notification frequency, and quiet-hours requirements, keeps manual/dead-letter rows blocked, and rejects unpersisted, mismatched, claimed, or duplicate requirements. Its focused 6-test filter, scheduler bridge tests, full 128-test contract target, strict Clippy, architecture, and retained [WP62 proof manifest](../../proof/app-game-plan/WP62_NOTIFICATION_PREFERENCE_PREFLIGHT_PROOF.md) are green. Preference mutation, quiet-hours execution, delivery, receipts, parent UI, and child delivery remain open.
