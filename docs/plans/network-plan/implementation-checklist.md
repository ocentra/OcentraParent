...
| 10   | NetworkActivityEvent contracts and reusable Rust eventing consumption                                                     | [~]    | E-D        | `codex/eventing-network-runtime-implementation` | `output/eventing-plan-proof/57-network-workpack-10-reusable-crate/proof-summary.json`, `crates/agent-core/src/network_event_runtime*.rs`, `scripts/test/eventing-network-runtime-proof.mjs` | Reusable in-process eventing spine now covers typed publish, no-subscriber queue/drain, and local typed request-response; full TS parity, protocol export parity, retry/backpressure depth, and broker/family-hub delivery remain. |
