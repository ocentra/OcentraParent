...
publish enforcement events. The Rust eventing implementation should expose:

- protocol-owned event type constants or enums;
- `NetworkEventEnvelope` with event id, observed time, source, target handler,
  retry metadata, and payload;
- sync/async subscription traits or channels;
- publish result equivalent to success, failure, handled, queued, expired, or
  dropped;
- oneshot/deferred request-response handling;
- queue/retry/TTL/in-flight behavior;
...
- `NetworkEventBusCapability`
- `NetworkEventBusDeliveryResult`
