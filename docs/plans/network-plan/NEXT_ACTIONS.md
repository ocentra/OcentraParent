...
6. [WP04 Cross Slice Cascade And Parent Surface](workpacks/04-cross-slice-cascade-and-parent-surface.md) - the fabricated production bridge/pipeline is deleted; in the later test phase delete or rewrite every fake-path test, then implement only shipped owner-backed cascade/AI/policy/notification/custody handoffs.
