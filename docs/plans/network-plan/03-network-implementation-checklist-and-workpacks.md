...
10. NetworkActivityEvent contracts and handlers that consume the reusable Rust
    eventing plan instead of creating a network-only bus.
...
- [ ] Network event bus contract exists in source.
...
- Network event routing should reuse the Ocentra Games eventing-domain semantics
  through the reusable Rust eventing plan instead of inventing unrelated or
  network-only bus behavior.
