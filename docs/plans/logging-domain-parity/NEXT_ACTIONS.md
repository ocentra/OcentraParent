...
The current WP04 integration code is `296d25b2d`: its regenerated proof root covers canonical operation-record encoding, atomic no-replace fallback publication, conservative Unix journal-cache invalidation, 48 all-feature logging-core tests, direct agent-service dev-log proof, and unchanged TypeScript fixture parity. WP04 remains scoped to local Rust helper/direct-consumer behavior.
```
