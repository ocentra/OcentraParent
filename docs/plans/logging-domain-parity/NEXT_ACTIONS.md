...
### 0. Accepted source-wave redaction truth sync

Status:

```text
Rust owns the exact 18-key sensitive-key policy and generates the checked-in
TypeScript artifact. The TypeScript sanitizer, Logger serialization path, and
portal compatibility fallback all use that policy; no alternate local policy
is accepted.
```

The source mapping is recorded in the affected workpacks and graph. Tests,
focused validation, proof, and external composition remain deferred.
