...
WP04 has complete recorded local proof evidence regenerated against source commit `261e6dfb1cdc58424156e3bbba9bc4fd45fc44c8`; its generated output root is intentionally ignored and is not expected in a fresh checkout. WP03, WP06, WP07, WP08, and WP10 have the separately documented proof states for their current checkout.
...
WP04 Rust logging core: complete-proven at 12/12 against source commit `261e6dfb1cdc58424156e3bbba9bc4fd45fc44c8`; explicit operation-identity semantics remain intact, partial-tail recovery is bounded, atomic final markers repair incomplete writes, committed retries verify exact record bytes, operation markers compact to a durable `commits.state` journal excluded from NDJSON discovery, compacted-journal directory mutations are flushed around commit-marker deletion, compacted lookups index prior journal bytes once and retain bounded path/hot-marker caches with fixed Bloom membership, non-locking producer rows remain intact beside operation-aware atomic appends and through failed-sync rollback, cleanup waits for the stream lock and syncs owning directories after deletion, operation-state directory creation is parent-synced, CR/LF operation IDs are rejected, replay validates every custody field including createdAt through the custody digest and re-syncs existing metadata directory entries, hard-link and crash-safe copy fallback preserve immutable publication, stale primary temporaries recover on the first attempt, Windows runs a real parent-directory durability flush, extended UNC paths are preserved, real subprocess publication/replay/conflict custody is covered, adversarial normalized redaction is proved through persisted NDJSON and artifact metadata, the agent-service dev-log target runs 3 tests, checked-in CI requires the all-features logging-core gate, both normal and all-features gates are recorded, and all five ignored proof artifacts were regenerated against that source tree.
