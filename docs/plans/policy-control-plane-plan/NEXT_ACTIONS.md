...
- WP05 production-code checkpoint: the Rust resolution command/service and delivery-binding validation exist, but the portal callback has no rendered caller and parent actor/provider context is not injected from the account/identity owner; notification-provider dispatch is also absent. The next legal slice is Rust-owned typed staging/preview after that context handoff, with validation/tests deferred and no completion claim.
