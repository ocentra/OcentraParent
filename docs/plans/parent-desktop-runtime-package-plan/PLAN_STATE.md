...
Status: WP03 retains an independently accepted desktop-shell source slice, but its live service-readiness claim is blocked by WP06. WP06 is blocked in the production-source phase because the shipped local WebSocket session is unauthenticated; typed response identity/freshness/deadline checks do not replace owner-issued admission authority. Account currently routes that ownership through WP03, but the provider-independent parent-local capability must be decomposed from WP03's downstream Cloudflare/Protected/Parent cycle before a valid graph edge and implementation can land. WP01, WP04, WP08, and WP09 retain their earlier bounded closure; WP02 remains validation/repair-blocked; remaining implementation and proof for later workpacks remain open. Protected WP01's neutral CNG/TPM/FFI/core foundation is integrated at canonical `a6d7d9adf`; the protected preflight always fails closed with `ExternalProvisioningRequired`, while WP02/WP03/WP04 own the split enrollment, monotonic, and client-transport routes. WP12 is planned/source-authorable for a narrow package-only source slice; its normal derived state remains blocked until real installer/package evidence exists.
...
protected-capability-custody-foundation-plan:
  Protected WP01 owns the neutral private core/FFI/protocol foundation and
  opaque broker boundary. Protected WP02 owns the fixed Enrollment/SCM/TPM
  owner transaction and provisioner source, WP03 owns monotonic/anti-rollback
  currentness, and WP04 owns the client anchor/fixed-pipe transport. Parent
  WP12 owns only the parent-side MSI/WiX, fixed owner-approved binary
  invocation, build wiring, and package lifecycle contract; it never accepts
  raw `authValue` or caller authority.
...
WP06 retains historical proof under `output/parent-client-runtime-distribution-plan-proof/06-parent-local-service-route-bridge/`, but that proof predates the discovered unauthenticated transport boundary and cannot be revalidated until Account WP03 authority and WP06 authenticated composition are implemented and tested.
...
- The parent local-service route bridge has Rust-owned contract shape and historical unavailable/timeout/local-target coverage under `output/parent-client-runtime-distribution-plan-proof/06-parent-local-service-route-bridge/`; it does not have accepted authenticated runtime proof.
...
- The desktop production health path uses the configured local agent's typed `AgentHealthCheck` / `AgentHealthReported` WebSocket response and validates correlation, schema, expected service/portal peers, `online=true`, `transport=websocket`, and deadline. However, agent-service accepts a missing `Origin`, announces connection readiness before authentication, and can report health `Ready` while authentication is `Unauthenticated`; parent runtime uses `ws://` with a caller-supplied origin. The typed envelope is therefore not parent authority.
...
- Parent local-service route bridge retains explicit Rust-owned route snapshot, timeout degradation, and setup separation, but authenticated reachability is blocked on Account WP03's opaque owner-issued current/revocable session capability and an owner-bound handshake.
...
| WP03 desktop shell | `apps/parent-desktop/src-tauri/src/main.rs` -> `lib::run` -> Tauri commands | Shell is reachable. Its package/source slice is accepted, but live service readiness is not: WP06's transport session remains unauthenticated despite structural response binding and a hard deadline. Signing, production update/rollback, setup, and child authority remain open. | Shell source accepted; live service readiness blocked on WP06 and Account WP03. |
...
| WP06 local-service bridge | Tauri/dev-web route commands -> `parent-runtime-core::load_parent_route_snapshot` / action dispatch -> typed agent-service WebSocket loaders | Real read-model and command paths exist, and structural response/degradation checks fail closed. The WebSocket transport is still unauthenticated: missing `Origin` is accepted, readiness precedes authentication, health can be `Ready`/`Unauthenticated`, and parent runtime supplies its own `ws://` origin. | REPAIR/BLOCKED; Account WP03 authority and authenticated composition are missing before tests. |
...
| WP12 protected broker provisioner | Planned/source-authorable parent-side MSI/WiX/custom-action/build/lifecycle roots under scripts/release/windows/parent-protected-custody/ and its package test roots; it invokes the WP02-owned BIN-only provisioner | No real custom action, package invocation, protected enrollment, or lifecycle test family is present. The WP02-owned BIN has no library/public API and must remain fail-closed; it accepts no caller/MSI-provided path, index, policy, auth, identity, or success input. External platform authority/owner handoff remains unavailable. Existing child-agent WiX is not a substitute. The zero-argument provisioner remains ExternalProvisioningRequired/manual-required; no service-start/readiness claim is implied. | Planned/source-authorable for the bounded package slice; normal completion remains blocked on real package invocation/test/proof evidence after consuming the WP01 neutral foundation, Protected WP03 monotonic/package boundary, and Parent WP03 package identity contract. WP12 produces the installed broker/provisioner artifact that Protected WP04 later consumes; WP04 is not a WP12 input. |
...
- Parent-side protected broker/provisioner MSI/WiX, fixed WP02-owned BIN-only
  provisioner invocation, custom-action, build, and upgrade/rollback/uninstall
  contract is routed as WP12; its package source, installer ceremony, tests,
  and proof remain absent. The narrow package-source slice is authorizable,
  while normal completion remains blocked. WP01's neutral foundation and
  current protected preflight do not establish enrollment or protected
  readiness.
...
- WP06 parent local-service route bridge is not source-complete: Account WP03 must issue an opaque current/revocable parent-local session capability, and WP06 must bind it to transport admission/readiness before tests, proof, or completion. Setup, child runtime distribution, and package/release readiness remain separate non-claims.
...
## WP12 installer-only routing refresh — 2026-08-25

The parent-side Protected route remains package/invocation lifecycle only. Its
narrow source slice is authorizable, while normal derived state remains
blocked. The owned roots are:
...
WP12 may schedule and package the fixed Protected owner-approved provisioner,
and own install/repair/upgrade/rollback/uninstall outcomes. It cannot own the
Protected Cargo source, mint authority, accept raw `authValue` or caller
identity/path/index/policy/generation/lease/capability/success inputs, or
claim readiness from package success. The existing zero-argument provisioner
continues to return `ExternalProvisioningRequired`/manual-required; no service
start/readiness claim is added. Source, tests, proof, signing, and runtime
availability remain open; normal derived completion stays blocked and this
routing refresh does not mark WP12 READY or DONE.
