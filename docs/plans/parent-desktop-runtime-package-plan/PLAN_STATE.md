...
- The desktop production health path now uses the configured local agent's typed `AgentHealthCheck` / `AgentHealthReported` WebSocket response; raw TCP listener acceptance is no longer treated as runtime readiness. This phase does not rerun or promote validation/proof.
...
| WP03 desktop shell | `apps/parent-desktop/src-tauri/src/main.rs` -> `lib::run` -> Tauri commands | Shell is reachable. Service readiness previously trusted any TCP listener; corrected to typed health response through the WP06 slice. Signing, production update/rollback, setup, and child authority remain open. | Accepted WP03/WP06 health correction. |
