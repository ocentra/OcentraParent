...
protected-capability-custody-foundation-plan:
  Protected WP01 owns private core/FFI enrollment acceptance, TPM policy and
  non-exportable-handle validation, exact registry/SCM/peer authority, and
  opaque broker outcomes. Parent WP12 owns only the parent-side MSI/WiX,
  elevated custom-action/provisioner invocation, build wiring, and package
  lifecycle contract; it never accepts raw `authValue` or caller authority.
...
| WP12 protected broker provisioner | Planned parent-side MSI/WiX/custom-action/build/lifecycle roots under `scripts/release/windows/parent-protected-custody/` | No package-side provisioner source, real custom action, protected enrollment, or lifecycle test family is present. Existing child-agent WiX is not a substitute. | Routing-only; blocked on Protected WP01 owner contract and real package/test/proof evidence. |
...
- Parent-side protected broker/provisioner MSI/WiX, custom-action, build, and
  upgrade/rollback/uninstall contract is now routed as WP12; its source,
  installer ceremony, tests, and proof remain absent.
