...
| source identity corrected / tests and release authority open | [WP06 Child iOS Capability Package](workpacks/06-child-ios-agent-capability-package.md) | Smoke/workflow consumers, Apple signing/provisioning, physical-device launch, TestFlight/App Store authority, and retained proof remain open; Core Location remains Tracking WP11/WP12 after trusted ingress. | Child identity build/smoke and simulator/device limit tests. | Capability-only; no daemon, tracking handoff, or runtime parity. | Source reviewed; expected-test wave next. |
