...
- the next live WP03 gaps are now above this slice: broader parent-surface consumers outside the owner package that still duplicate or bypass the shared compiler artifact contract, and route/proof closure
...
1. Continue `policy-control-plane-plan` `WP04` delivery/ack/audit, because the WP08 event model is now green and the next open policy-control workpack should consume the stabilized event/audit contract.
2. Continue `policy-control-plane-plan` `WP05` ask-parent overrides after delivery is in place, because the same event and audit primitives feed the override lifecycle.
3. Continue `policy-control-plane-plan` `WP06` rollout proof and route gate once the remaining core authority flows are less partial.
4. Leave the broader plan follow-ons until the remaining policy-control workpacks are narrowed further.
