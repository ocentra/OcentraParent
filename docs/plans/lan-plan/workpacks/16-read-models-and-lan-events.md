...
- [ ] The add-device read model now carries a `lanDiscoverySourceMatrix` snapshot that maps all 20 LAN plan workpacks and concrete discovery sources into typed implemented, partial, manual-required, and not-implemented statuses.
- [ ] Activity/Network diagnostics render source-matrix rows for LAN workpacks, implemented source proof, weak-source fencing, and matrix generation time from the service-backed read model.
