...
| `07` | validation | accepted source; expected tests plus packet/manual proof open | Canonical `ffaade68a` carries the recurring passive runtime with one absolute collection-cycle deadline, per-receive remaining-time enforcement, Windows sub-millisecond handling, and panic-safe socket-timeout restoration. The new slow-trickle, zero-deadline, timeout-restoration, six-listener rotation, and cancellation tests are not written or run; physical packet/platform proof also remains open. |
