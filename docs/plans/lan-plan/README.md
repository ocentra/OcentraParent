...
- [ ] `docs/product-capability-checklist.md` still needs the matching LAN
      source-matrix proof note. It was not edited in this branch because the
      path is actively locked by codex-a; the required update is to point the
      Remote/LAN/mobile LAN row at the source-matrix proof script, proof JSON,
      and screenshot artifacts above while keeping physical household proof
      manual-required.
