...
Status: engineering-grade monetization spec is complete, current execution is green across the shared Cloudflare test/deploy/probe surfaces, WP00 remains `blocked / proof-required`, WP01 through WP04 have declared proof paths but the checkout output tree does not currently carry the physical proof bundles, and payment remains blocked only by the absent upstream handoff proof plus the account/trusted-device/deployment custody gates.
...
WP03 has a declared Rust-owned webhook lifecycle proof path at `output/payment-subscription-plan-proof/03-subscription-webhook-lifecycle/`.
WP04 has a declared Rust-owned entitlement-delivery proof path at `output/payment-subscription-plan-proof/04-entitlement-delivery-gates/`.
...
- WP03 is closed as `done / proof-required`: `crates/billing-core` now carries provider channel, payload-parse, idempotency, replay/order, retry, dead-letter, reconciliation, and test/live boundary truth with real unit coverage under `crates/billing-core/tests/unit/**`. TypeScript does not own this webhook lifecycle contract.
- WP04 is closed as `done / proof-required`: `crates/entitlement-core` now owns signed entitlement snapshot derivation, provider input-only boundary, referral seat recalculation, fixed snapshot-model fields, and the snapshot-to-device gate bridge with real unit coverage under `crates/entitlement-core/tests/unit/**`. TypeScript remains proof-consumer only for this slice.
...
| PSP-017 | architecture-closed / spec-complete / wp00-proof-required / execution-green | Proof matrix, route sync, and exact assertion scope are documented, but WP00 is still missing the Cloudflare handoff proof in this checkout, so payment remains proof-required rather than runtime-blocked. | Proof paths, assertion matrix, validation logs, route docs, a generated WP00 handoff proof, and the upstream Cloudflare handoff all agree; upstream blockers are either cleared or explicitly accepted. |
