...
- Stripe is the default web-first subscription provider.
- Razorpay is the India-native card/UPI/subscription adapter.
- PayPal is the secondary wallet/subscription adapter.
- Apple and Google are store-billing adapters for mobile distribution constraints.
- Manual invoice is a support/admin path for special cases.
