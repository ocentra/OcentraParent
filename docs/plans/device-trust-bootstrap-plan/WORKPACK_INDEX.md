...
| partial / source repair candidate, expected tests open | [WP06 Recovery Reset Re-Pair](workpacks/06-recovery-reset-re-pair.md) | raw installation-generation re-pair is removed; encrypted bundle construction fails closed without key custody; import requires exact current tombstone cursor and same-operation revalidation; public restore/readiness seams are crate-private. No encrypted custody owner, durable revocation cursor owner, authorized re-pair producer, real executor, shipped caller, tests, or proof exists. Storage source overlaps Data WP05 candidate `3def622df` and is not integration-ready | `RECOVERY_RESET_MODEL.md`, `LOCAL_KEY_SEALING_MODEL.md` | `output/device-trust-bootstrap-plan-proof/06-recovery-reset-re-pair/` |
