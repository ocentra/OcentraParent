...
| partial / Windows-only unmerged vertical slice | [WP02 Local Key Sealing](workpacks/02-local-key-sealing.md) | parent-desktop command consumes a native staged ceremony; Windows command-path test present; workpack remains open | `LOCAL_KEY_SEALING_MODEL.md`, `PLATFORM_KEY_CUSTODY_MATRIX.md` | `output/device-trust-bootstrap-plan-proof/02-local-key-sealing/` |
