...
| --- | --- | ---: | --- | --- |
| blocked / lifecycle source retained; unsafe owner-side fence withdrawn | [WP01 Device Trust Source Of Truth](workpacks/01-device-trust-source-of-truth.md) | accepted lifecycle/current-binding source remains; rejected residual `5f0280f711` is quarantine evidence only and no participant/test root is accepted. Protected Device-owned receipts, Account WP05A caller, migration/retention owners, expected tests, proof, and completion are absent | `DEVICE_TRUST_MODEL.md`, `RESEARCH_AND_UI_GUIDANCE.md` | `output/device-trust-bootstrap-plan-proof/01-device-trust-source-of-truth/` |
| partial / Windows custody source hardened; real negative tests written but unexecuted; downstream of WP01 | [WP02 Local Key Sealing](workpacks/02-local-key-sealing.md) | Canonical `22815871c` bounds recovery scans, rejects unsafe custody entries, validates filename/generation bindings, and adds DPAPI-context plus staged-handle replay/restart negatives. `require_authenticated_parent_authority()` remains permanently unavailable, no ceremony issuer or custody-to-lifecycle startup composition reaches it, and no test execution/end-to-end platform proof is claimed | `LOCAL_KEY_SEALING_MODEL.md`, `PLATFORM_KEY_CUSTODY_MATRIX.md` | `output/device-trust-bootstrap-plan-proof/02-local-key-sealing/` |
