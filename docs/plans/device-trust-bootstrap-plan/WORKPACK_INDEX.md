...
| partial / candidate source wave, expected tests open | [WP05 Entitlement Device License](workpacks/05-entitlement-device-license.md) | candidate signed envelope/verifier with authority-generation/channel and weak-key binding, durable monotonic snapshot/revocation custody, crate-private opaque package/billing/device/live-identity and trusted-time owner seams, verifier-receipt-only cache mutation, and a child-runtime library consumer with manual-required public startup only are present; no concrete owner composition, real issuer/key/package/currentness/grace-policy/re-resolution/handle-safe-custody owners, service startup caller, signed delivery caller, independent review, expected tests, and proof remain open | `ENTITLEMENT_DEVICE_LICENSE_MODEL.md` | `output/device-trust-bootstrap-plan-proof/05-entitlement-device-license/` |
...
WP05's signed authority source packet and child-runtime consumer are a
candidate/unreviewed source wave; WP06's fail-closed restore boundary and
WP07's durable removal/readiness boundary are also source-only, not
test/proof/completion claims. WP05 still requires external
issuer/key/package/currentness/trusted-time/grace-policy/revocation and
handle-safe-custody owners and remains manual-required at startup. WP03 remains
