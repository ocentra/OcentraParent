...
| partial / candidate source wave, expected tests open | [WP05 Entitlement Device License](workpacks/05-entitlement-device-license.md) | candidate signed envelope/verifier with authority-generation/channel and weak-key binding plus fail-closed durable revocation custody are present; authority verification remains crate-private, snapshot-cache ingestion has no writer, and the child-runtime entitlement module is not exported. No concrete owner composition, real issuer/key/package/currentness/grace-policy/re-resolution/handle-safe-custody owners, child entitlement action owner/service startup caller, signed delivery caller, independent review, expected tests, and proof remain open | `ENTITLEMENT_DEVICE_LICENSE_MODEL.md` | `output/device-trust-bootstrap-plan-proof/05-entitlement-device-license/` |
...
consumer are a candidate/unreviewed source wave; WP06's fail-closed restore
