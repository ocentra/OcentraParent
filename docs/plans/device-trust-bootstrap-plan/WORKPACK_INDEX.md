...
| partial / candidate source wave, expected tests open | [WP05 Entitlement Device License](workpacks/05-entitlement-device-license.md) | candidate signed envelope/verifier with authority-generation/channel and weak-key binding, durable monotonic snapshot/revocation custody, crate-private opaque package/billing/device/live-identity owner seams, and a child-runtime library consumer with manual-required public startup only are present; no concrete owner composition, real issuer/key/package/currentness/re-resolution/revocation owners, service startup caller, signed delivery caller, independent review, expected tests, and proof remain open | `ENTITLEMENT_DEVICE_LICENSE_MODEL.md` | `output/device-trust-bootstrap-plan-proof/05-entitlement-device-license/` |
...
test/proof/completion claims. WP05 still requires
external issuer/key/package/currentness/revocation owners and remains
manual-required at startup. WP03 remains BLOCKED in the default graph on
