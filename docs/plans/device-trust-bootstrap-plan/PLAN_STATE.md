...
- A narrow Windows adapter now consumes the distinct `SealParentDeviceTrust` parent-presence capability and derives its sealed subject/device from that verified ceremony. Unseal requires a runtime-owned current-lifecycle authority source; no deserializable authority snapshot is accepted. This is still partial adapter proof, not full device-trust lifecycle closure.
...
- The repo has a narrow execution-grade Windows DPAPI key-sealing adapter only. It fails closed when current device authority, the ceremony-derived sealed identity binding, DPAPI, or the local Windows machine binding is unavailable; Android, Linux, macOS, iOS, recovery, reinstall lifecycle custody, correlated custody-event delivery, and complete lifecycle integration remain open.
