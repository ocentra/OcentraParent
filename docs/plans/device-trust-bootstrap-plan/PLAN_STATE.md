...
| WP02 | `crates/storage-custody-core` has Windows DPAPI/registry-epoch source, `crates/family-identity-core` has current-authority and lifecycle-activation seams, and `crates/parent-runtime-core` plus the parent desktop have an opaque staged-handle facade and mounted custody-sealing command. Canonical `22815871c` adds bounded recovery enumeration, reparse/non-file/oversize rejection, filename/generation binding checks, and real DPAPI-context plus staged-handle replay/restart negative test source. | `require_authenticated_parent_authority()` is permanently unavailable before custody mutation; no ceremony issuer or custody-to-lifecycle startup composition reaches the source. Windows custody-open platform failures are typed unavailable; unsupported non-Windows startup is typed manual-required; the later authenticated-parent gate remains manual-required. The command does not perform lifecycle activation. The new tests are written but unexecuted; non-Windows custody and end-to-end sealing remain unavailable/manual-required, and older stale lifecycle/custody tests are not proof. |
