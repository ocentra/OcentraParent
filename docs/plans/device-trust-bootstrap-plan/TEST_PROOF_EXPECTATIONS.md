...
WP01's attempted runtime-fence participant and its expected test root were
withdrawn after review found caller-mintable committed rows, no explicit
existing-database migration, and unbounded startup scanning. There is currently
no approved participant test root. After protected Device-owned receipt
custody, explicit migration/retention ownership, and the real Account WP05A
caller are stable, a new test contract must be designed and reviewed before it
is mapped here. Do not use the quarantined remote packet as test evidence.
