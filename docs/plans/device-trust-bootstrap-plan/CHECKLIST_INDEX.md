...
| DTB-06 | Recovery and re-pair model defined | 06-recovery-reset-re-pair | [ ] source repair candidate only: raw generation-only re-pair is removed; encrypted bundle construction and missing/mismatched current tombstone cursor fail closed; the dead apply/authority seam is unconditionally blocked until an owner-bound cursor token can be reread at apply. Encrypted key custody, durable revocation owner, authorized re-pair ceremony, real executor/caller, expected tests, proof, and acceptance remain open; storage files overlap Data WP05 candidate `3def622df` | `output/device-trust-bootstrap-plan-proof/06-*` |
