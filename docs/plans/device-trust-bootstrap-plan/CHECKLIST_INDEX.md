...
| DTB-05 | Device-bound entitlement model defined | 05-entitlement-device-license | [ ] candidate/unreviewed source packet: signed envelope/verifier, owner-controlled trusted-time/currentness seam, verifier-receipt-only durable cache mutation, monotonic revocation custody, and child-runtime consumer are present; no concrete owner composition, issuer key custody, installed-package authority, billing/currentness/grace-policy owner, handle-safe cache custody, signed delivery caller, expected tests, proof, CI, independent review, and completion remain open | `output/device-trust-bootstrap-plan-proof/05-*` |
