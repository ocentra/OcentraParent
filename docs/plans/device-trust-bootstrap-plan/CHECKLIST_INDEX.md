...
| DTB-05 | Device-bound entitlement model defined | 05-entitlement-device-license | [ ] candidate/unreviewed source packet: signed envelope/verifier, owner-controlled trusted-time/currentness seam, fail-closed monotonic revocation custody, and no exported child-runtime entitlement consumer are present; no public capability unlock/consume route or snapshot-cache ingestion writer exists. Concrete owner composition, issuer key custody, installed-package authority, billing/currentness/grace-policy owner, handle-safe cache custody, signed delivery caller, child entitlement action owner, expected tests, proof, CI, independent review, and completion remain open | `output/device-trust-bootstrap-plan-proof/05-*` |
