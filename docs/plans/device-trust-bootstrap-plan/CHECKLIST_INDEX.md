...
| DTB-06 | Recovery and re-pair model defined | 06-recovery-reset-re-pair | [ ] source repair candidate only: raw generation-only re-pair is removed; encrypted bundle construction, current tombstone cursor, same-operation preflight revalidation, and crate-private restore/readiness boundaries fail closed. Encrypted key custody, durable revocation owner, authorized re-pair ceremony, real executor/caller, expected tests, proof, and acceptance remain open; storage files overlap Data WP05 candidate `3def622df` | `output/device-trust-bootstrap-plan-proof/06-*` |
