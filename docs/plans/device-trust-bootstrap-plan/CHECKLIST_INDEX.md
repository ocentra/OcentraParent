...
| DTB-05 | Device-bound entitlement model defined | 05-entitlement-device-license | [ ] independently reviewed source repairs; expected tests open: signed envelope/verifier, owner-controlled trusted-time/currentness seam, typed Grace restriction, read-only signed revocation custody, and no exported child-runtime entitlement consumer are present; no public capability unlock/consume route or cache mutation writer exists. Concrete owner composition, issuer key custody, installed-package authority, billing/currentness/grace-policy owner, handle-safe cache custody, signed delivery caller, child entitlement action owner, expected tests, proof, CI, and completion remain open | `output/device-trust-bootstrap-plan-proof/05-*` |
| DTB-06 | Recovery and re-pair model defined | 06-recovery-reset-re-pair | [ ] independently reviewed source repair integrated through `f656a80a1`: raw generation-only re-pair is removed; encrypted bundle construction and missing/mismatched current tombstone cursor fail closed; the dead apply/authority seam is unconditionally blocked until an owner-bound cursor token can be reread at apply. Encrypted key custody, durable revocation owner, authorized re-pair ceremony, real executor/caller, expected-test rewrites, proof, and acceptance remain open; Data WP05 candidate `e91bb3de1` must rebase and preserve this boundary | `output/device-trust-bootstrap-plan-proof/06-*` |
