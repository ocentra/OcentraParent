...
| DTB-02 | Platform key sealing matrix defined | 02-local-key-sealing | [ ] partial: Windows custody source now has an installed Tauri command that accepts only an opaque staged-ceremony reference and generates trust material inside the native facade. The command is reachable and returns typed rejected/manual-required state, but no record-backed ceremony issuer is mounted and the custody adapter remains fail-closed until authenticated parent authority is available. DPAPI ciphertext, registry-epoch binding, atomic record rollback, and unsupported-platform manual-required behavior remain source boundaries only; Android/Linux/iOS/macOS, recovery/re-pair, focused command/custody execution, proof, and merged CI remain open | local untracked `output/device-trust-bootstrap-plan-proof/02-*` plus visible source/tests and current CI after merge |
