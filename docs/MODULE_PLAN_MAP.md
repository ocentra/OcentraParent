...
| `apps/local-api` | `child-agent-runtime-distribution-plan` | `agent-protocol-domain` and service routes |

## TypeScript Packages

| Module | Primary plan route | Secondary route |
| --- | --- | --- |
| `packages/schema-domain` | `setup-install-provisioning-plan` | all contract-bearing plans |
| `packages/endpoint-domain` | `child-agent-runtime-distribution-plan` | `lan-plan`, `payment-subscription-plan`, `remote-access-plan` |
| `packages/agent-protocol-domain` | `child-agent-runtime-distribution-plan` | all service-backed feature plans |
| `packages/text-domain` | `portal-ux-household-surfaces-plan` | `policy-control-plane-plan` |
| `packages/portal-domain` | `portal-ux-household-surfaces-plan` | all portal-rendered feature plans |
...
| `packages/event-domain` | `eventing-plan` | all event-producing feature plans |
| `packages/logging-domain` | `logging-domain-parity` | all proof-bearing plans |
...
| `packages/lan-domain` | `lan-plan` | `remote-access-plan`, `ai-plan` |
