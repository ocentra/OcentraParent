...
- The agent accepts, emits, stores, replays, queries, and reports activity using
  schema-versioned shapes shared with TypeScript and Rust.
...
- In scope for V0.2 through V0.5: journal record metadata, activity event
  ...
  status, recent activity summaries, portal intent/query envelopes, service
  health, copy/debug payloads, and typed failure reasons.
...
- The portal must validate service payloads; the service must validate portal
  intents; journal replay must validate persisted events.
...
- TypeScript valid and invalid parser tests for every new or changed contract.
...
The contract can be used from TypeScript and Rust without string guessing, and
tests would fail if a field name, enum value, schema version, parent-visible
claim, persisted payload, or branded primitive changed incorrectly.
