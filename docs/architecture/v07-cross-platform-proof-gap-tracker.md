...
package inspection plus a glibc blocker, and Android emulator package smoke.
Those results do not replace controlled privileged OS, household two-device
LAN, package/autostart/reboot, signing, store, entitlement, or physical
mobile-device proof.
...
| Linux CI                                | Run `26415925682` proves shared TypeScript/Rust mechanics, real portal-to-Rust E2E, DEB build, DEB install/remove smoke, and artifact upload on hosted Linux for `b9ed9dc`.                                                                                                                                 | Record unsupported desktop capture, network visibility, service-manager, and enforcement states separately. Use WSL or Docker where CI cannot represent local assumptions.                                                                                                                                | `ci-mechanical-proof` for shared mechanics and package preview; `not-yet-proven` for platform adapters.                                                                                                   | Primary or Linux portability owner.                                                                                                  |
| Linux WSL/Docker                        | Worker B ran WSL Ubuntu 22.04 `npm run test:pre-ai-proof` successfully and inspected the DEB metadata/contents. Docker CLI is not installed on the Windows proof host.                                                                                                                                      | Resolve package runtime baseline: extracted DEB launch on WSL Ubuntu 22.04 failed before binding because the artifact requires `GLIBC_2.39` and the host has glibc `2.35`. Repeat on Ubuntu 24.04+ or rebuild against the intended baseline.                                                              | `ci-mechanical-proof` for WSL pre-AI proof and DEB inspection; `blocked` for Ubuntu 22.04 package launch; `unavailable` for Docker on this host; desktop capability claims stay `not-yet-proven`.         | Linux portability/package owner.                                                                                                     |
...
- Every platform row separates CI/package mechanics from manual OS/device
  behavior.
- Current run `26415925682` and prior PR #96 run `26401270250` are referenced as
  CI-mechanical proof only.
