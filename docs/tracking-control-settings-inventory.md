...
0001. Enable device location features?
...
0002. What location posture should this device use?
...
0003. Where should location behavior execute?
...
0004. Represent: `service-disabled`
...
0005. Represent: `manual-required`
...
0006. Represent: `offline-last-known-only`
...
0007. Represent: `battery-throttled`
...
0008. Represent: Battery saver, low-power modes, radio state, and network reachability can degrade tracking
...
0009. Represent: Last agent heartbeat
...
0010. Represent: Last location sample
...
0011. Represent: Last successful parent sync
...
0012. Represent: Battery percentage and charging state where available
...
0013. Represent: Low-power/battery-saver state where available
...
0014. Represent: Pending upload count
...
0015. Represent: Parent marks device missing
...
0016. Represent: Device contact, battery, and network state become prominent
...
0017. Represent: Tracking a powered-off or offline device
...
0018. Represent: Show degraded/offline/manual-required states instead of disabled-looking mystery failures
...
0019. Represent: last-known only
...
0020. Represent: missing device mode
...
0021. Represent: Latitude, longitude, altitude, heading, speed, and bearing where exposed
...
0022. Represent: Horizontal and vertical accuracy
...
0023. Represent: Provider kind: GPS/GNSS, Wi-Fi, cellular, IP, Bluetooth/beacon, fused provider, user-entered default, or unknown
...
0025. Represent: Clear separation between Ocentra evidence and OS account features
...
0026. Represent: Raw child activity evidence is local-first by default
...
0027. Represent: [`docs/expectations/policy.md`](expectations/policy.md)
...
0028. When can live tracking run?
   - settingId: `live.mode`
...
0029. What is the maximum live session duration?
...
0030. How often should live tracking request updates?
...
0031. What should happen when battery is low?
...
0032. Represent: The parent wants low-noise confirmation instead of a live map
...
0033. Represent: Capability matrix row | Capability=Foreground live tracking | Mobile child agent=Yes, while app/session active | Desktop/laptop child agent=Sometimes, while app/service active | Required layer=OS location API | Important limit=Requires visible use or active session semantics
...
0034. Represent: Capability matrix row | Capability=Background live tracking | Mobile child agent=Platform-dependent | Desktop/laptop child agent=Limited | Required layer=Background execution plus permission | Important limit=Throttled, battery-sensitive, and often entitlement-bound
...
0035. Represent: Parent opens live map
...
0036. Represent: Child is travelling between known places
...
0037. Represent: Child missed an expected arrival
...
0038. Represent: Device is marked missing
...
0039. Represent: Parent explicitly starts a temporary safety session
...
0040. Represent: A policy rule asks for short-term verification after a geofence miss
...
0041. Represent: Session id, child id, device id, requester, and reason code
...
0042. Represent: Requested accuracy: approximate, precise, or best-available
...
0043. Represent: Requested cadence: one-shot, on-change, interval, high-accuracy burst, or geofence-only
...
0044. Represent: Maximum duration and auto-stop reason
...
0045. Represent: Permission requirement and user-visible disclosure state
...
0046. Represent: Delivery path: local, LAN, authenticated relay, parent cache, or parent-owned storage
...
0047. Represent: Audit events for start, update, degrade, stop, and parent reveal
...
0048. Represent: OSes may throttle background updates
...
0049. Represent: Apps may need foreground service notification, background mode, entitlement, device-owner state, MDM supervision, or explicit user permission
...
0050. Represent: High accuracy increases battery use
...
0051. Represent: Indoor GPS may fail or fall back to Wi-Fi/cell/IP estimates
...
0052. Represent: A child can turn off device location services, revoke permission, uninstall the app where allowed, power off the device, or lose network
...
0053. Represent: Live map updates should never imply that a stale point is still current
...
0054. Represent: Low battery: reduce cadence and explain degraded state
...
0055. Represent: Live map temporarily active
...
0056. Represent: Battery throttled -> reduce cadence and record degraded state
...
0057. Represent: Continuous tracking has battery impact and should be cadence-limited
...
0058. Represent: Parent starts a time-limited live tracking session
...
0059. Represent: Agent sends repeated updates while online and permitted
...
0060. Represent: Session has a visible reason, duration, and audit trail
...
0061. Represent: Location permission
...
0062. Represent: Runtime update path
...
0063. Represent: Local/LAN/relay delivery path
...
0064. Represent: Battery-aware cadence
...
0065. Represent: High sensitivity and battery impact
...
0066. Represent: Show live tracking only when a fresh update path is available
...
0067. Represent: Show child-facing disclosure mode when background/live tracking is enabled
...
0068. Represent: temporary live map
...
0069. Enable parent-defined places?
...
0070. What minimum radius should place geofences use?
...
0071. What if geofence monitoring is unavailable?
...
0072. Which geofence transitions should be used?
...
0073. Represent: Arrival and departure audit
...
0074. Represent: Geofence alerts are not instant and may be throttled
...
0075. Represent: Capability matrix row | Capability=Geofence enter/exit | Mobile child agent=Android/iOS with limits | Desktop/laptop child agent=Weak or app-running-only on desktop | Required layer=OS geofence/region monitoring | Important limit=Delayed events, radius limits, count limits, false exits
...
0076. Represent: Capability matrix row | Capability=Dwell alerts | Mobile child agent=Android/iOS with limits | Desktop/laptop child agent=Weak | Required layer=Geofence plus dwell support | Important limit=Not immediate; large radius may be required
...
0077. Represent: Capability matrix row | Capability=Parent arrival/departure alerts | Mobile child agent=Yes, with geofence/check-in evidence | Desktop/laptop child agent=Limited | Required layer=Geofence or sampled evidence | Important limit=Notifications must minimize sensitive detail
...
0078. Represent: `location-summary`: derived day/trip/place summary with redacted detail
...
0079. Represent: `geofence-transition`: arrival, departure, dwell, missed arrival, or stale state
...
0080. Represent: Place/geofence audit: medium retention
...
0081. Represent: Arrived at school during a schedule
...
0082. Represent: Left school before dismissal
...
0083. Represent: Did not arrive at practice by a time
...
0084. Represent: Stayed near home after bedtime
...
0085. Represent: Notify if device leaves a travel corridor
...
0086. Represent: Region id, label token, latitude, longitude, radius, schedule, transition type, dwell duration, and expiration
...
0087. Represent: Minimum radius and maximum count per platform
...
0088. Represent: Proof requirement: platform geofence, sampled location, check-in, or manual
...
0089. Represent: Fallback when geofence monitoring is unavailable
...
0090. Represent: Debounce/noise control to avoid alert storms
...
0091. Represent: Geofence events can be delayed
...
0092. Represent: Small radii can be unreliable
...
0093. Represent: Wi-Fi, cell, and GPS availability affect transition quality
...
0094. Represent: Some platforms wake apps for geofence events; others only work while the app is running
...
0095. Represent: Geofence count limits require compile-time validation
...
0096. Represent: Dwell events are useful to reduce alert noise but can delay notifications
...
0097. Represent: Visualize accuracy radius when useful
...
0098. Represent: Distinguish live, last-known, check-in, geofence, and manual/default points
...
0099. Represent: Arrivals/departures by place
...
0100. Represent: Missed arrival/departure alerts
...
0101. Represent: Arrival/departure alerts enabled
...
0102. Represent: Geofence unavailable -> use scheduled check-in or sampled location if allowed
...
0103. Represent: Region monitoring only while the app is running and the system is awake, according to Apple geofence documentation
...
0104. Represent: Geofencing API with platform count, permission, delay, and radius limits
...
0106. Represent: Arrival/departure alerts unless separately enabled
...
0107. Represent: Parent defines places and schedules
...
0108. Represent: Agent records geofence or sampled evidence
...
0109. Represent: Notifications are sent only through alert rules
...
0110. Represent: Geofence or sampled-location capability
...
0111. Represent: Debounce/noise controls
...
0112. Represent: Custody and retention settings
...
0113. Represent: Instant transitions
...
0114. Represent: Small-radius precision
...
0115. Represent: Location evidence can contribute to local policy, such as "ask if leaving school early" or "notify if not at practice by 18:00"
...
0116. Represent: Show exact coordinate reveal separately from summary/place reporting
...
0117. Represent: Show geofence alerts as delayed/coarse arrival/departure evidence
...
0118. Represent: geofence arrival/departure
...
0119. How should check-ins work?
...
0120. Should check-ins include location?
...
0121. When is a check-in unanswered?
...
0122. Which child responses are allowed?
...
0123. Which location events should notify a parent?
...
0124. What location detail may appear in push/email/SMS bodies?
...
0125. Represent: The child device is on a constrained platform
...
0126. Represent: The agent can send an ask/confirm prompt without tracking movement all day
...
0127. Represent: A check-in response without a fresh coordinate is still useful
...
0128. Represent: A fresh coordinate without child response should be labeled as location-only
...
0129. Represent: Unanswered check-ins should produce a notification intent only through an explicit alert rule
...
0130. Represent: Sensitive child notes should not appear in third-party notification previews
...
0131. Represent: Parent can request a child response
...
0132. Represent: Unanswered check-ins can drive explicit notification rules
...
0134. Represent: Remote relay and notifications must minimize child details
...
0135. Represent: [`docs/expectations/notifications.md`](expectations/notifications.md)
...
0136. Represent: Show check-in as a separate workflow from tracking
...
0138. Represent: location-based policy alerts
...
0139. Represent: GPS/GNSS: best outdoors, weaker indoors, battery-sensitive
...
0140. Represent: IP address: coarse and often wrong for household, VPN, carrier-grade NAT, or corporate networks
...
0141. Represent: Bluetooth/beacon: local proximity, not global location
...
0142. Represent: Manual/default location: a fallback, not current device proof
...
0143. Represent: Fused provider: OS/provider chooses from several sources
...
0145. Represent: `altitudeAccuracyMeters` when known
...
0148. Represent: `isUserEnteredDefault`
...
0149. Represent: `isSimulatedOrDeveloperMode` if detectable
...
0150. Represent: Device offline -> show last contact and queue parent request
...
0151. Represent: Parent relay unavailable -> continue local policy and send when reachable
...
0152. Represent: Typed policy target
...
0153. Represent: Explicit fallback when proof is stale or unavailable
...
0154. Represent: Local child-agent evaluation
...
0155. Represent: Portal-side policy evaluation
...
0156. Represent: Guessing current location from stale last-known evidence
...
0157. Show last known location on the map?
...
0158. When should a point become stale?
...
0159. What should the UI show when location is stale?
...
0160. Represent: Last known location is not proof of current location
...
0161. Represent: Offline devices can only report last known location and last contact time
...
0162. Represent: Capability matrix row | Capability=Family map | Mobile child agent=Yes from latest evidence | Desktop/laptop child agent=Yes from latest evidence | Required layer=Query/read model plus map rendering | Important limit=Map must label freshness and accuracy
...
0163. Represent: Capability matrix row | Capability=Enforce location-based policy | Mobile child agent=Possible after proof | Desktop/laptop child agent=Limited | Required layer=Local policy plus location evidence | Important limit=Missing proof must degrade to ask/warn/report
...
0164. Represent: `freshnessSeconds`
...
0165. Represent: Network type and reachability summary
...
0166. Represent: Offline: show last contact time and last known point
...
0167. Represent: Show freshness on every point
...
0168. Represent: Let parents reveal exact coordinates only when the data scope permits it
...
0169. Represent: Keep summaries useful without requiring raw trail exposure
...
0170. Represent: Recent location status by child/device
...
0171. Represent: Check-in timeline
...
0172. Represent: Device offline and battery timeline
...
0173. Represent: Current location unavailable -> show last known location with timestamp
...
0174. Represent: Do not assume Windows service behavior maps to macOS
...
0175. Represent: Fused Location Provider for last known, current, and periodic updates
...
0176. Represent: Show newest location evidence when available
...
0177. Represent: Do not run continuous tracking
...
0179. Represent: Real-time movement
...
0180. Represent: Agent tries to provide current or last known location
...
0181. Represent: Evidence freshness and accuracy thresholds
...
0182. Represent: Parent surfaces author rules and view reports; they do not execute child capture or policy
...
0183. Represent: [`docs/product-roadmap.md`](product-roadmap.md)
...
0184. Represent: Show last known location separately from current/live location
...
0185. What location permission is required?
...
0186. What should happen if permission is missing?
...
0187. Allow approximate location when precise is not granted?
...
0188. What should the child device disclose?
...
0189. Represent: Permission state: denied, foreground-only, background, approximate, precise, reduced-accuracy, supervised/managed, or unknown
...
0190. Represent: Device state: online, offline, low power, battery saver, airplane mode, no signal, service disabled, or adapter unavailable
...
0191. Represent: Background location is unavailable or not appropriate
...
0192. Represent: `ready-precise-background`
...
0193. Represent: `ready-foreground-only`
...
0194. Represent: `ready-approximate-only`
...
0195. Represent: `permission-required`
...
0196. Represent: `background-permission-required`
...
0197. Represent: Location permission is user-visible and revocable
...
0198. Represent: Background location is a separate capability from foreground location
...
0199. Represent: Approximate/reduced accuracy must be represented separately from precise location
...
0200. Represent: Capability matrix row | Capability=One-time current location | Mobile child agent=Yes, with permission | Desktop/laptop child agent=Sometimes, with permission | Required layer=OS location API | Important limit=Fresh fix may fail indoors, offline, or with service off
...
0201. Represent: Capability matrix row | Capability=Last known location | Mobile child agent=Yes, if provider cache exists | Desktop/laptop child agent=Sometimes | Required layer=OS location API or local cache | Important limit=May be stale, null, approximate, or user-entered
...
0203. Represent: Raw precise trail: disabled by default unless explicitly enabled
...
0204. Represent: Accuracy, freshness, and permission state
...
0206. Represent: Foreground/when-in-use permission
...
0207. Represent: Background/always permission
...
0208. Represent: Approximate or reduced-accuracy permission
...
0209. Represent: Precise/full-accuracy permission
...
0210. Represent: OS location service disabled
...
0211. Represent: Device policy allowed, denied, or user-in-control
...
0212. Represent: Supervised/device-owner/MDM-only state
...
0213. Represent: Unknown or stale state
...
0214. Represent: Wi-Fi: strong for urban/indoor approximate positioning when databases and nearby access points are available
...
0217. Represent: Permission/service state
...
0218. Represent: Online with permission: show fresh or actively updating state
...
0219. Represent: Online without permission: show permission-required state and last known point
...
0220. Represent: Service disabled: show location-service-disabled, not "tracking failed"
...
0221. Represent: Distinguish approximate from precise
...
0222. Represent: Location permission health
...
0223. Represent: Location controls disabled
...
0225. Represent: Background location enabled by parent and OS permission
...
0226. Represent: Last-known report only because device is offline
...
0227. Represent: Location unavailable because permission/service is off
...
0228. Represent: Background permission missing -> offer foreground/check-in mode
...
0229. Represent: Precise permission denied -> use approximate-only rules or mark precise rules unavailable
...
0230. Represent: `Windows.Devices.Geolocation.Geolocator` for one-time and continuous location where the app has permission
...
0231. Represent: Wi-Fi BSSID access increasingly tied to precise-location consent
...
0232. Represent: Location can be approximate, IP-derived, stale, manually configured, or unavailable
...
0233. Represent: A background Windows service cannot assume Store-app-style foreground consent semantics without implementation proof
...
0234. Represent: TCC permissions, background execution, launchd behavior, signing, and notarization matter
...
0235. Represent: IP/Wi-Fi based approximate location if the service/provider is available
...
0236. Represent: Browser geolocation with user permission for web surfaces, separate from the child agent
...
0237. Represent: Permission agents and desktop portals vary
...
0238. Represent: Foreground location with `ACCESS_COARSE_LOCATION` and/or `ACCESS_FINE_LOCATION`
...
0239. Represent: Background location with `ACCESS_BACKGROUND_LOCATION` when core functionality and Play policy allow it
...
0240. Represent: Approximate vs precise permission state
...
0241. Represent: Foreground service and visible notification for long-running location use where required
...
0242. Represent: Background location is restricted and must be core to the app
...
0243. Represent: Background updates are throttled
...
0244. Represent: Approximate permission may be the only granted precision
...
0245. Represent: Users can change permission, precision, and location service settings
...
0246. Represent: Background location with required mode, authorization, and user disclosure
...
0247. Represent: Always/background location has explicit prompts and disclosure requirements
...
0248. Represent: Show freshness, accuracy, source, permission, and custody
...
0249. Represent: Background permission
...
0250. Represent: Agent may include current location if permission allows
...
0251. Represent: Foreground prompt/notification
...
0252. Represent: Optional location permission
...
0253. Represent: Current location when permission/service is unavailable
...
0254. Represent: Existing permission or platform-specific lost-device capability
...
0255. Represent: Show approximate/reduced accuracy as a first-class state
...
0256. Represent: Keep permission state close to every control
...
0257. Represent: Timestamp from the platform provider and ingest timestamp from Ocentra
...
0258. Represent: `platform-unsupported`
...
0259. Represent: Capability matrix row | Capability=Lost-device location | Mobile child agent=OS/product-specific | Desktop/laptop child agent=OS/product-specific | Required layer=OS lost mode/Find My/device management | Important limit=Not a generic third-party API on every platform
...
0260. Represent: Cellular: useful wide-area estimate on mobile devices
...
0261. Represent: Platform unsupported -> show unavailable/manual-required, not a fake toggle
...
0262. Represent: Windows location service using GPS, Wi-Fi, cell towers, and IP where available
...
0263. Represent: Windows privacy settings and MDM/Policy CSP for whether Windows apps may access location
...
0264. Represent: Child-agent service/contact state independent of location service state
...
0265. Represent: Desktop/laptop hardware may have no GPS or cellular radio
...
0266. Represent: Product claims need real Windows adapter proof, not only a contract
...
0267. Represent: Core Location authorization and location updates
...
0268. Represent: Reduced/full accuracy state
...
0269. Represent: Find My Mac as an Apple account feature, not a generic third-party tracking API
...
0270. Represent: MDM/device management posture for managed-device features where available
...
0271. Represent: Lost-device and Find My behavior should be described as OS/account feature context unless Ocentra has an approved API and proof
...
0272. Represent: GeoClue over D-Bus on desktops that ship/configure it
...
0273. Represent: Manual/default location fallback
...
0274. Represent: No universal Linux live-location stack exists across distros
...
0275. Represent: Headless/service deployments may have no useful location provider
...
0276. Represent: Product claims must name distro/service assumptions and real proof
...
0277. Represent: Device owner/profile owner policy only after real Android Enterprise or device owner proof
...
0278. Represent: Geofencing responsiveness can be delayed
...
0279. Represent: Store policy matters for a child-agent product
...
0280. Represent: Core Location When In Use and Always authorization
...
0281. Represent: Reduced/full accuracy state
...
0282. Represent: Standard, significant-change, visit, and region monitoring where permitted
...
0283. Represent: Family Sharing/Find My location sharing as an Apple user feature, not an Ocentra-owned raw telemetry API
...
0284. Represent: Supervised MDM Lost Mode device location for managed/supervised devices only
...
0285. Represent: Third-party apps cannot silently access general Find My data
...
0286. Represent: Region monitoring has platform limits and is not instant
...
0287. Represent: MDM device location is tied to Lost Mode and supervision requirements
...
0288. Represent: Entitlements, App Store review, Family Controls, and device supervision affect what is shippable
...
0289. Represent: Android and iOS capability claims are currently scaffold/manual-required until real device proof exists
...
0290. Represent: Platform claims must distinguish implemented, scaffold-only, unavailable, degraded, manual-required, and not-implemented states
...
0291. Represent: [`docs/expectations/platforms.md`](expectations/platforms.md)
...
0292. Represent: Custody label: child-local, parent-device cache, LAN-live, parent-owned storage, Ocentra-hosted non-activity metadata, or unavailable
...
0294. Represent: Missed check-in investigation
...
0295. Represent: Device lost/stolen review
...
0296. Represent: Safety explanation after an alert
...
0297. Represent: Export/delete/retention flows
...
0298. Represent: Parent-owned storage and local/LAN operation are the normal custody boundary
...
0299. Represent: Every strict policy or alert must carry evidence source, timestamp, accuracy, custody, and adapter state
...
0300. Represent: Capability matrix row | Capability=Location history | Mobile child agent=Yes, if locally journaled | Desktop/laptop child agent=Yes, if locally journaled | Required layer=Agent journal/query store | Important limit=Retention/custody/delete controls required
...
0301. Represent: Capability matrix row | Capability=Exact continuous movement trail | Mobile child agent=Sometimes, but expensive and sensitive | Desktop/laptop child agent=Usually no | Required layer=Frequent location updates | Important limit=Battery, consent, and retention concerns
...
0302. Represent: Capability matrix row | Capability=Remote live location away from LAN | Mobile child agent=Future, via relay or parent storage | Desktop/laptop child agent=Future | Required layer=Authenticated relay/sync | Important limit=Ocentra must not become default location-history store
...
0303. Represent: `location-point`: individual timestamped point with accuracy and provider
...
0304. Represent: `check-in`: child response, optional location point, note, and prompt reason
...
0305. Represent: `device-contact`: last online, last sync, battery, and network state
...
0306. Represent: `audit-only`: policy decision or parent action without raw coordinate
...
0307. Represent: Live point stream: short retention
...
0308. Represent: Policy/audit references: longer retention
...
0309. Represent: Parent-exported report: parent-chosen retention
...
0310. Represent: Evidence source and adapter version
...
0311. Represent: Custody and retention labels
...
0312. Represent: Redaction/deletion state
...
0313. Represent: Parent reveal/audit trail for sensitive exact coordinates
...
0314. Represent: Adapter error: show degraded status with audit reference
...
0315. Represent: Record parent reveal, export, delete, and retention actions
...
0316. Represent: Exportable parent report with custody labels
...
0317. Represent: Raw location evidence lives on the child device
...
0318. Represent: Parent surfaces read local/LAN, parent cache, parent-owned storage, or cloud relay through typed contracts
...
0319. Represent: Ocentra-hosted services may route minimal notification or relay metadata, but they must not retain raw location history by default
...
0320. Represent: Retention policy per data class
...
0321. Represent: Delete expired raw points
...
0322. Represent: Keep redacted summaries only if parent policy allows
...
0323. Represent: Export/delete flows that name data classes and destinations
...
0324. Represent: Audit for parent reveal, export, sync, delete, policy change, and strict alert action
...
0325. Represent: Policy decision or parent action
...
0326. Represent: Evidence reference
...
0327. Represent: Location source and adapter state
...
0328. Represent: Accuracy/freshness
...
0330. Represent: Retention class
...
0331. Represent: Notification intent reference where alerts were sent
...
0332. Represent: Failure/degraded reason
...
0333. Represent: Child-device agents own capture, journal, query, local AI, policy, and enforcement paths
...
0334. Represent: [`docs/expectations/data-custody.md`](expectations/data-custody.md)
...
0335. Represent: [`docs/expectations/sync-export.md`](expectations/sync-export.md)
...
0336. Represent: Keep custody, retention, and delete/export status visible for location history
...
0337. Represent: Every strict action should have an audit path: detected state, parent rule, mechanism, outcome, timestamp, accuracy, freshness, custody, and evidence reference
...
0338. Represent: parent-owned history/report export
