...
0001. Enable network management?
...
0002. What should happen to network activity?
...
0003. How should network management run on this device?
...
0004. Represent keep local-network exceptions for printers, LAN pairing, parental devices, and trusted home services?
...
0005. Represent school/home subnet where the parent explicitly allows discovery;?
...
0006. How should local-network exceptions behave?
...
0007. Which local exceptions should be available?
...
0008. Represent lAN versus internet classification;?
...
0009. Represent page title or active tab;?
...
0010. Represent page body, chat content, search terms, form values, cookies, tokens, or credentials;?
...
0011. Represent specific video or post within a CDN-backed service;?
...
0012. Represent which tab caused a network request;?
...
0013. Represent child intent or safety classification without a separate policy/AI contract?
...
0015. Represent local agent service;?
...
0016. Represent lAN parent controller;?
...
0018. Represent public internet;?
...
0020. Represent remote rule update delivery;?
...
0021. Represent control of arbitrary child internet traffic;?
...
0025. Represent eTW for network event streams if loss/decode/privilege states are typed;?
...
0026. Represent product claims should follow real host proof, not contract presence?
...
0027. Represent consumer child-agent claims must stay behind Apple-approved capabilities?
...
0028. Represent desktop foreground and app identity vary;?
...
0029. Represent claims need distro-specific validation?
...
0030. Represent usage Stats, accessibility, or browser/app-specific integrations for foreground/app context when explicitly approved?
...
0031. Represent decrypted content;?
...
0032. Represent exact exception scope;?
...
0033. Represent risk of broad private-network allow rules?
...
0034. Represent [`docs/expectations/policy.md`](expectations/policy.md)?
...
0035. Represent [`docs/product-roadmap.md`](product-roadmap.md)?
...
0036. Represent keep LAN exceptions visible beside strict rules?
...
0038. Represent local network exceptions;?
...
0039. Represent local IP and port;?
...
0040. Represent remote IP and port;?
...
0041. Represent transport protocol;?
...
0043. Represent process id and process identity where available;?
...
0044. Represent dNS/domain attribution where available;?
...
0045. Represent loopback service ports used by the child-device agent;?
...
0046. Represent lAN pairing ports between parent controller and child agent;?
...
0047. Represent multicast and broadcast protocols that are needed for device discovery?
...
0048. What network evidence may rules use?
...
0049. What proof is enough for network decisions?
...
0050. What if network proof is unavailable?
...
0051. What must network rules never collect?
...
0052. How should domain controls work?
   - settingId: `dns.mode`
...
0053. Which domain attribution sources may be used?
...
0054. How should encrypted DNS be handled?
...
0055. What network targets should rules match?
...
0056. What actions can network rules take?
...
0057. How should conflicting network rules resolve?
...
0058. Which network report fields should parents see?
...
0059. Show unknown and ambiguous states?
...
0060. Represent iP, port, protocol, and process rules;?
...
0061. Represent unusual new destination reporting;?
...
0062. Represent local AI/policy digests with evidence ids;?
...
0063. Represent app/browser correlation when the flow also has process or managed-browser evidence?
...
0064. Represent exact URL path or query in normal HTTPS;?
...
0065. Represent Detect remote port capability status.
...
0066. Represent Detect protocol capability status.
...
0067. Represent Cloud relay control capability status.
...
0068. Represent LAN exception capability status.
...
0069. Represent Suspicious indicator report capability status.
...
0070. Represent endpoint snapshots for local and remote IP/port/protocol;?
...
0071. Represent tCP and UDP owner PID tables where supported;?
...
0072. Represent managed resolver logs where Ocentra controls the resolver path;?
...
0073. Represent managed browser URL evidence when there is an explicit join to flow evidence?
...
0075. Represent `domain-candidate`;?
...
0076. Represent `domain-ambiguous`;?
...
0078. Represent `dns-unavailable`;?
...
0080. Represent `encrypted-content-unavailable`;?
...
0081. Represent `process-attributed`;?
...
0082. Represent `process-unknown`;?
...
0083. Which process opened network connections?
...
0084. Which destination did this process contact?
...
0085. Did an unmanaged browser or unknown app create traffic?
...
0086. Did a known app suddenly contact a new destination?
...
0087. Represent exact browser URL;?
...
0091. Represent captive portal or public Wi-Fi;?
...
0092. Represent new destination for child/device/process;?
...
0093. Represent high-volume unknown process;?
...
0094. Represent repeated connection failures;?
...
0095. Represent dNS mismatch or excessive DNS churn;?
...
0096. Represent dNS unavailable while traffic continues;?
...
0097. Represent direct IP traffic to public internet;?
...
0098. Represent unusual port or protocol for a child device;?
...
0099. Represent domain/IP reputation category when the category source is explicit;?
...
0100. Represent lAN scan-like pattern;?
...
0101. Represent traffic from an unmanaged browser or unsupported app;?
...
0102. Represent parent-friendly domain rules;?
...
0103. Represent broad device or profile coverage when DNS path is controlled;?
...
0104. Represent useful reporting for domains and categories;?
...
0105. Represent works even when the app is not a browser if it uses the managed resolver?
...
0106. Represent several domains can resolve to one IP, and one domain can resolve to many IPs;?
...
0107. Represent apps can use hard-coded IPs;?
...
0108. Represent dNS control does not see HTTPS path/query;?
...
0109. Represent report query routing;?
...
0110. Represent device reachability metadata;?
...
0111. Represent stateless report compilation from parent-authorized sources?
...
0112. Represent domain rules must tolerate ambiguous or unavailable domain evidence;?
...
0113. Represent eCH reduces SNI-based classification, so DNS/resolver or browser evidence becomes more important;?
...
0114. Represent cDN-backed sites need domain/category evidence, not IP-only overclaiming?
...
0115. Represent iP Helper endpoint snapshots for TCP/UDP owner PID state;?
...
0116. Represent process/window evidence for app/browser correlation;?
...
0117. Represent managed browser evidence for exact URL/tab state?
...
0118. Represent process observation with platform permissions;?
...
0119. Represent browser-managed evidence for exact URL/tab state;?
...
0120. Represent network Extension, System Extension, TCC, MDM, signing, notarization, and App Store review affect what is shippable;?
...
0121. Represent process and socket inspection through procfs/netlink where permitted;?
...
0122. Represent managed browser evidence for exact URL/tab state?
...
0123. Represent managed DNS, app restrictions, or package lifecycle controls where the device management posture permits;?
...
0124. Represent exact URL in arbitrary mobile browsers is not generally reliable from network metadata;?
...
0125. Configure screen time frameworks.
...
0126. Represent web domain shielding through managed settings where allowed;?
...
0127. Represent network Extension content filter or URL filter paths where entitlement and deployment permit;?
...
0128. Represent mDM/supervision for stronger managed-device content filtering;?
...
0129. Represent app/category/domain tokens rather than raw browser history in Screen Time flows?
...
0130. Represent third-party apps do not get general packet inspection or arbitrary exact URL telemetry from other apps;?
...
0131. Represent entitlements, review, supervision, and deployment model determine capability;?
...
0132. Represent web domain shielding is not the same as full browser history capture;?
...
0133. Represent parent iOS app claims and child iOS agent claims must stay separate?
...
0134. Represent record and summarize network metadata;?
...
0135. Represent show process, destination, protocol, port, DNS/domain, volume, and capability states where available;?
...
0136. Represent classify suspicious indicators in report-only mode?
...
0137. Represent exact URL rules;?
...
0138. Represent guaranteed process attribution on every platform?
...
0139. Represent use managed browser evidence for exact domain/origin when available;?
...
0140. Represent use DNS/network attribution with confidence when browser evidence is absent?
...
0141. Represent full URL path/query control;?
...
0142. Represent can break legitimate infrastructure;?
...
0143. Represent can miss app semantics;?
...
0144. Represent can be too broad for CDN-backed services?
...
0145. Represent allow specific local services, subnets, protocols, or Ocentra pairing traffic while internet rules remain strict?
...
0146. Represent last used evidence;?
...
0147. Represent network flow evidence is local-first metadata, not decrypted content?
...
0148. Represent browser URL/tab evidence remains the exact URL source?
...
0149. Represent lAN and cloud relay are typed control/report paths, not default hosted child evidence stores?
...
0150. Represent [`docs/architecture/network-flow-evidence-capture.md`](architecture/network-flow-evidence-capture.md)?
...
0151. Represent [`docs/expectations/network-flow-evidence.md`](expectations/network-flow-evidence.md)?
...
0152. Represent show exact URL controls only when managed browser or explicit URL-filter capability is available?
...
0153. Represent show domain rules as domain evidence, not exact URL evidence?
...
0155. Represent iP/port/protocol rules;?
...
0156. Represent managed browser for exact URL rules;?
...
0157. Represent interface, route, LAN/WAN, VPN, proxy, or tunnel indicators;?
...
0158. Represent allow, block, or rate-limit traffic by process, IP, port, protocol, domain, or category where supported;?
...
0159. Represent force or configure DNS, proxy, VPN, firewall, WFP, packet filter, or router policy;?
...
0160. Represent terminate a process after a network policy decision;?
...
0161. Represent local printer, NAS, DNS resolver, router, or media device;?
...
0162. What should happen to VPN/proxy/tunnel indicators?
...
0163. Which tunnel indicators should count?
...
0164. Represent domain allow/block rules;?
...
0165. Represent vPN/proxy/tunnel indicators;?
...
0166. Represent Detect remote IP capability status.
...
0167. Represent Attribute process capability status.
...
0168. Represent Attribute exact URL capability status.
...
0169. Represent Block IP capability status.
...
0170. Represent Block exact URL capability status.
...
0171. Represent Block process network capability status.
...
0172. Represent Block protocol or port capability status.
...
0173. Represent Detect VPN/proxy/tunnel capability status.
...
0174. Represent Force all traffic through adapter capability status.
...
0175. Represent Router-level control capability status.
...
0176. Represent firewall, VPN, WFP, packet filter, router, or proxy observations;?
...
0177. Represent `adapter-unavailable`;?
...
0178. Represent `adapter-permission-required`?
...
0179. Did a child-controlled process use VPN/proxy/tunnel-like behavior?
...
0180. Represent category enforcement without a parent-authored rule?
...
0181. Represent private address through VPN;?
...
0182. Represent router/cloud relay metadata?
...
0183. Represent vPN, proxy, Tor-like, tunnel, MASQUE-like, or unknown adapter indicator;?
...
0184. Represent apps can use DNS-over-HTTPS or DNS-over-TLS unless the platform routes or blocks those paths;?
...
0185. Represent dNS control usually cannot identify a local process unless an endpoint adapter joins it?
...
0186. Represent strong local enforcement on supported platforms;?
...
0187. Represent can block non-browser apps;?
...
0188. Represent can enforce LAN/internet route and port rules;?
...
0189. Represent can overblock shared infrastructure;?
...
0190. Represent does not decrypt HTTPS;?
...
0191. Represent may be bypassed by other privileged network layers if not properly installed;?
...
0192. Represent must be performance-tested and rollback-capable?
...
0193. Represent can cover many apps on mobile and desktop;?
...
0194. Represent can centralize DNS/domain policy;?
...
0195. Represent can implement always-on or lockdown modes on platforms that support them;?
...
0196. Represent can provide flow counters and route indicators?
...
0197. Represent requires visible setup and platform permission;?
...
0198. Represent can conflict with school/work VPNs;?
...
0199. Represent may not reveal original process or exact URL;?
...
0200. Represent always-on/lockdown claims require platform proof?
...
0201. Represent explicit domain allow/block;?
...
0202. Represent central policy for apps that honor proxy settings;?
...
0203. Represent potential full URL control for plain HTTP or explicitly managed browser/proxy integrations?
...
0204. Represent many apps ignore system proxy settings;?
...
0205. Represent hTTPS path/query remains hidden without TLS interception, which is out of current Ocentra Parent scope;?
...
0206. Represent certificates, trust, and privacy risk are high if interception is introduced later;?
...
0207. Represent qUIC and direct sockets can bypass unless separately controlled?
...
0208. Represent home-wide DNS, IP, port, or device rules;?
...
0209. Represent useful for IoT and guest devices;?
...
0210. Represent usually cannot see local process, active tab, foreground app, or child profile;?
...
0211. Represent device identity can be ambiguous with MAC randomization, NAT, VPN, or shared devices;?
...
0212. Represent remote/off-home traffic is not covered;?
...
0213. Represent router vendor APIs vary widely;?
...
0214. Represent router proof is separate from Windows endpoint proof?
...
0215. Represent exact URL rules require managed browser, explicit URL filter, or proxy proof;?
...
0216. Represent iP block rules should warn about shared infrastructure risk;?
...
0217. Represent doH/DoT policy needs a specific resolver-control or network-blocking posture;?
...
0218. Represent qUIC blocking can force TCP fallback for some browsers/sites, but that is a compatibility decision and not a content-inspection feature;?
...
0219. Represent block only where a platform adapter proves support?
...
0220. Represent allow or block `example.invalid`;?
...
0221. Represent allow or block subdomains;?
...
0222. Represent block a category when the category source is explicit;?
...
0223. Represent block a known resolver, VPN gateway, proxy, or tunnel host;?
...
0224. Represent block an app from making network connections to a domain/IP/port?
...
0225. Represent allow or block a specific page path;?
...
0226. Represent handle query strings;?
...
0227. Represent classify a specific video page;?
...
0228. Represent block a browser download source URL?
...
0229. Represent windows Firewall for IP, port, protocol, service, and application rules;?
...
0230. Represent windows Filtering Platform for future observation and enforcement adapters;?
...
0231. Represent wFP, ETW, firewall, and service paths may require admin rights and careful installer/service setup;?
...
0232. Represent broad domain/network blocking is still manual-required until a real adapter proof exists;?
...
0233. Represent network Extension content filter or app proxy paths where entitled and approved;?
...
0234. Represent dNS/proxy/VPN settings when allowed by setup?
...
0235. Represent do not assume Windows WFP or process-control semantics map to macOS;?
...
0236. Represent nftables/netfilter, iptables compatibility, or distro firewall managers;?
...
0237. Represent dNS/proxy/VPN controls;?
...
0238. Represent router/server Linux is not the same as a child desktop agent;?
...
0239. Represent vpnService for an app-owned VPN path;?
...
0240. Represent always-on VPN and lockdown when user, device owner, or profile owner setup permits;?
...
0241. Represent a normal app cannot broadly firewall every other app without a VPN-style or device-management boundary;?
...
0242. Represent per-app VPN and always-on behavior need device proof and UX setup;?
...
0243. Represent school/work VPN conflicts need explicit handling?
...
0244. Represent dHCP/device identity;?
...
0245. Represent dNS resolver policy;?
...
0246. Represent firewall/IP/port rules;?
...
0247. Represent lAN device grouping;?
...
0248. Represent parent-owned router API integration?
...
0249. Represent weak process/child attribution;?
...
0250. Represent vendor-specific APIs;?
...
0251. Represent mAC randomization and device sharing;?
...
0252. Represent no off-home coverage;?
...
0253. Represent no exact URL path/query without explicit proxy/filter integration?
...
0254. Represent network blocking;?
...
0255. Represent allow, warn, ask, limit, or block domains and subdomains where a domain-control layer is available;?
...
0256. Represent cDN-safe IP blocking;?
...
0257. Represent process attribution from router or DNS-only data?
...
0258. Represent allow or block remote IPs, CIDRs, local/remote ports, and transport protocols through firewall/WFP/VPN/router/packet-filter adapters?
...
0259. Represent optionally require managed network path where supported?
...
0260. Represent ocentra knows tunneled destinations or content?
...
0261. Represent child-device agent applies platform network control after a typed policy decision references stored evidence or a parent-authored target?
...
0262. Represent adapter capability proof;?
...
0263. Represent policy decision;?
...
0264. Represent visible unsupported/degraded state?
...
0265. Represent windows is first, but broad network/domain blocking remains manual-required until real OS adapter proof exists?
...
0266. Represent portal UI authors rules and shows capability states. It does not run capture, policy evaluation, enforcement, timers, OS commands, or scripts?
...
0267. Represent [`docs/expectations/enforcement.md`](expectations/enforcement.md)?
...
0268. Represent show IP-only, domain-ambiguous, DNS-unavailable, encrypted-content-unavailable, process-unknown, adapter-unavailable, and stale states directly?
...
0269. Configure show capability status beside each strict action.
...
0270. Represent vPN/proxy/tunnel handling;?
...
0271. Represent strict network enforcement where proven;?
...
0272. Represent router or cloud relay options only where separately configured and proven?
...
0273. Represent timestamps, connection counts, duration, and bytes sent/received where the adapter can prove them;?
...
0275. What should network budgets count?
...
0276. Represent bandwidth and connection-count summaries;?
...
0277. Represent Bandwidth budget capability status.
...
0278. Represent Time budget capability status.
...
0279. Configure flow byte budget.
...
0280. Configure connection-count budget.
...
0281. Configure network-active time.
...
0282. Configure foreground-correlated time.
...
0283. Configure schedule budget.
...
0284. Represent traffic during blocked schedule or after budget exhaustion?
...
0285. Represent can enforce local network access and internet schedules?
...
0286. Represent endpoint snapshots can miss short-lived flows and may not provide bytes;?
...
0287. Represent bandwidth/time schedules;?
...
0288. Represent apply budgets to flow bytes, connection counts, network-active duration, or foreground-correlated duration according to the selected evidence source?
...
0289. Represent counted evidence type;?
...
0291. Represent degraded behavior when counters are missing;?
...
0292. Represent whether background traffic counts?
...
0293. Represent keep process, domain, IP, port, protocol, VPN/proxy/tunnel, LAN exception, and bandwidth/time budget rules as separate target types?
...
0294. Represent bandwidth and network-active time budgets;?
...
0295. Represent warn, ask parent, or report instead of blocking;?
...
0296. Represent may need admin rights, service installation, signed drivers, system extensions, MDM, or entitlement approval;?
...
0297. Represent parent approval requests;?
...
0299. Represent observe, warn, ask, block, or require approval for VPN/proxy/tunnel indicators;?
...
0304. Represent top ports/protocols;?
...
0305. Represent new destinations;?
...
0306. Represent bandwidth summaries;?
...
0307. Represent time-window summaries;?
...
0308. Represent vPN/proxy/tunnel indicators;?
...
0309. Represent lAN exceptions used;?
...
0310. Represent blocked/allowed/warned/asked decisions;?
...
0311. Represent enforcement failures and unavailable states;?
...
0312. Represent source/custody labels?
...
0313. Represent live local child agent;?
...
0314. Represent live LAN child agent;?
...
0315. Represent child-device encrypted journal;?
...
0316. Represent child-device SQLite query store;?
...
0317. Represent parent-device cache;?
...
0318. Represent parent-owned export;?
...
0319. Represent parent-authorized relay;?
...
0320. Represent ocentra-hosted non-activity metadata;?
...
0322. Represent raw flow evidence retention;?
...
0323. Represent domain summary retention;?
...
0324. Represent bandwidth summary retention;?
...
0325. Represent policy/audit retention;?
...
0326. Represent exported report retention;?
...
0327. Represent deletion and expiry behavior;?
...
0328. Represent whether redacted summaries survive raw evidence deletion?
...
0332. Represent adapter id and capability state;?
...
0333. Represent action requested;?
...
0335. Represent rollback or expiry state;?
...
0336. Represent parent approval or override reference;?
...
0338. Represent timestamp and source?
...
0339. Represent keep custody labels close to reports, AI summaries, exports, and parent assistant surfaces?
...
0340. Represent evidence id, source id, capability state, custody state, and retention state?
...
0341. Represent per-process DNS attribution when the source is host-level DNS cache only;?
...
0342. Represent Attribute domain capability status.
...
0343. Represent Block domain capability status.
...
0344. Represent Audit enforcement result capability status.
...
0345. Represent dNS client cache or observed resolver events;?
...
0346. Represent dNS answers can be cached before policy changes;?
...
0347. Represent can produce auditable adapter results?
...
0348. Represent must not export child activity to Ocentra-hosted infrastructure by default;?
...
0349. Represent storage of child network evidence by default;?
...
0350. Represent exact network observations unless the child agent uploaded parent-authorized typed summaries under an explicit custody setting?
...
0351. Represent dNS client cache or DNS event observation;?
...
0352. Represent dNS cache is host-level unless a stronger source proves per-process attribution;?
...
0354. Represent rollback or expiry path;?
...
0355. Represent policy can consume stored network summaries and unusual indicators only after they are journaled and queryable?
...
0356. Represent enforcement is scaffold/protocol/audit work unless a real platform adapter proof exists?
...
0357. Represent [`docs/expectations/data-custody.md`](expectations/data-custody.md)?
...
0358. Configure require proof for enforcement claims.
...
0359. Represent require managed browser/network path;?
...
0360. Represent configuration profiles or MDM for stronger managed-device cases;?
...
0361. Represent privilege and service installation differ by distro;?
...
0362. Represent devicePolicyManager controls for device-owner or profile-owner deployments;?
...
0363. Represent [`docs/managed-unmanaged-browser.md`](managed-unmanaged-browser.md)?
   ...
   - proofRequirement: none
   - sourceDocument: docs/network-control-capability-guide.md
   - sourceLine: 797; sourceText: [`docs/managed-unmanaged-browser.md`](managed-unmanaged-browser.md)
   - acceptedOptions: Enabled | Disabled
   - helperText: network-control-capability-registry
