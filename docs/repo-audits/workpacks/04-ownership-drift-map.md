...
| `packages/parent-domain/src/policy.ts` | broad parent frontage | frontage | `packages/schema-domain/src/policy.ts` | Policy authority now lives in `@ocentra-parent/schema-domain/policy`, so parent-domain should not be treated as the policy owner surface. | keep as frontage only |
