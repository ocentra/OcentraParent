...
| `app-game-plan` | Accepted WP10 source / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, schema, tracked vendor portal surface, portal | WP16's cohesive dashboard and WP48/WP63 source-freshness panel are mounted by the production portal route; the earlier "no cohesive/rendered surface" gaps were false. WP99-WP101 are Rust-owned in the existing `app_game_source_gated_policy_preview_timer_followthrough` source and contract test. WP100/WP101's retired parent-domain roots were false gaps. WP102 is explicitly no-code/superseded because WP103 owns the real Rust protocol plus agent-service parent-surface read model and tests. | WP16/WP48/WP63 still lack focused intent/state/empty/stale/degraded/hostile-metadata tests, and Game budgets has no service-backed row. WP100/WP101 focused execution, proof, product runtime, and parent rendering remain open. WP103 execution/proof and the live linkage from the bounded contract chain into product runtime remain open. | Continue the remaining real AppGame production gaps without duplicating mounted portal source or recreating parent-domain. Write the mapped portal UI test family in the later test wave, then run focused tests only after the source wave. |
