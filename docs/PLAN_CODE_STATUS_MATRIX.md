...
| `cloudflare-control-plane-plan` | 13 | 0 | 13 | 0 | Fully mapped from live source/tests. WP05 includes the reviewed Firebase verifier source. WP06 now has reviewed read/current-authority, Account-owned writer, and provider caller source, but mutation runtime composition and complete expected tests remain open; the other runtime/deployment workpacks retain concrete Phase 1 gaps. |
...
1. **Complete the account-authority runtime chain**: Firebase subject verification, Account WP08, target-aware Account WP02, and Cloudflare WP06 read/current-authority plus Account-owned mutation source now exist and fail closed. The mutation producer is not mounted, higher authority composition is unavailable, and migration, expected tests, retained proof, deployment, and full runtime reachability remain open. Supply those seams before WP03's native parent authority, Cloudflare WP08 test proof, and Account WP06 aggregation.
