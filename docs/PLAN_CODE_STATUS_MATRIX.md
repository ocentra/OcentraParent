...
| Data custody/storage | 11 | The shipped child-service startup opens the durable journal/effect/tombstone stores, recovers them before readiness, and routes `PublishStorageCustody` through dynamic Device Trust readiness into `ChildStorageCustodyRuntime::execute`. | WP05 has typed bundle/preflight/integrity/manual readiness but no parent-local/provider writer or retriever, scheduler, cryptographic byte verifier, restore/migration/apply/rollback/idempotency runtime. WP09 routes byte custody; WP10 routes orchestration and producer handoffs. | Real internal custody lifecycle; WP09/WP10 source not implemented, trusted external authority composition, expected tests, proof, and DONE remain open. |
