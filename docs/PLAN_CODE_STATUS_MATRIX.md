...
| `protected-capability-custody-foundation-plan` | Independently reviewed source integrated / Phase 1 incomplete | `protected-capability-custody-core`, `protected-capability-custody-protocol`, `protected-capability-custody-broker`, `protected-capability-custody-client` | Active source now provides the separate broker/client/protocol topology, bounded shared wire contract, narrow sealed core admission facade, startup preflight, dynamic enrolled-client pipe ACL construction, and split immutable-enrollment/runtime registry custody. It fails closed before custody state or service availability when protected peer admission is unavailable. | Safe pinned Windows process/token observation, exact registry owner/DACL/parent-chain verification, a non-restorable monotonic provider, immutable broker/SCM enrollment, a real caller, all expected tests, proof, and runtime availability remain absent. | Implement those dependency-owned adapters plus installer/SCM enrollment and the first real caller; then write the core, protocol, broker, and client expected tests before focused execution, repo-wide Enforcer, proof, precommit, one PR, CI, and promotion. |
...
| `protected-capability-custody-foundation-plan` | 1 | 0 | 1 | 0 | Independently reviewed core/protocol/broker/client source, manifests, targets, active workspace membership, and the narrow sealed facade are integrated. The route remains validation/open because protected Windows process/token/registry/monotonic/installer owners, a real caller, every expected test, runtime availability, proof, PR, merge, and DONE are absent. |
...
| `protected-capability-custody-core` + protocol/broker/client packages | Account WP05A, Device Trust WP01/WP03, Data Custody | The isolated-process topology, shared wire contract, and narrow core facade are integrated source, but deliberately unavailable. Protected Windows process/token/registry/monotonic authority, installer/SCM enrollment, a real caller, tests, and proof must exist before consumers can claim a protected operation. |
