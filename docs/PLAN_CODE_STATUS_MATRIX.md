...
1,257 edges, 24 plans, and 687 workpacks. It maps 3,365 implementation files and
1,153 test files: 573 workpacks have both code and tests, 12 are source-only, 8
are tests-only, and 94 currently have no source or test files. Of those 94, 73
are expected no-code coordination/reference packets, 8 expect tests only, and
13 require missing code-and-tests roots. No workpack has unknown ownership.
Topology expectations match for 638/687 workpacks. Derived state is 256
planned, 187 blocked, 3 ready, 4 active, 236 in validation, and 1 done.
...
`npm run graph:matrix -- --json` is the complete 684-workpack table; the reviewed
