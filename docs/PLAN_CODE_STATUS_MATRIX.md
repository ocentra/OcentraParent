...
| `v0-8-enforcement-control-plan` | Foundation | `child-enforcement-core`, `policy-control-core`, `agent-core`, `agent-service`, schema | Contract/action-state surfaces and a managed-browser profile/launch boundary exist. | Live audit 2026-07-23: browser policy targets are manual-required and service execution implements only owned-process termination; managed-browser proof/read models overstate execution. No browser action receipt, rollback, audit trace, or parent-visible result is proven. | Implement one trusted managed-session adapter vertical slice: authority/bridge validation -> bounded action -> signed/bound receipt and journal -> rollback -> visible status; keep exact URL and unmanaged browsers out of scope. |
