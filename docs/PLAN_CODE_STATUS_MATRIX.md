...
| `app-game-plan` | Accepted WP10 source / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, schema, tracked vendor portal surface, portal | WP16's cohesive dashboard is mounted by the production portal route and renders service/source/capability/evidence state; its earlier "no cohesive surface" gap was false. WP99-WP101 are Rust-owned in the existing `app_game_source_gated_policy_preview_timer_followthrough` source and contract test. WP100/WP101's retired parent-domain roots were false gaps. WP102 is explicitly no-code/superseded because WP103 owns the real Rust protocol plus agent-service parent-surface read model and tests. | WP16 still lacks focused intent/state/hostile-metadata tests, and Game budgets has no service-backed row. WP100/WP101 focused execution, proof, product runtime, and parent rendering remain open. WP103 execution/proof and the live linkage from the bounded contract chain into product runtime remain open. | Continue the remaining real AppGame production gaps without duplicating WP16 or recreating parent-domain. Write WP16's mapped UI test family in the later test wave, then run mapped focused tests only after the source wave. |
