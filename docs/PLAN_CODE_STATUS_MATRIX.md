...
The graph validates at 703 nodes and 705 edges, with 24 migration/dependency
...
| App | `app-core` 3/5 plus shared agent-service integration and the versioned runtime-decision contract slice. | WP01 runtime-decision envelope/tuple boundary is code/test/proof/CI/main-merge complete; app identity/evidence/session/service path remains open. |
...
| App Plan WP01 Contract Boundary And Effect Schemas | Schema-domain build/type-check and contract tests 11/11; focused Rust app tests pass; scoped architecture and Enforcer guard pass; durable proof retained in `docs/proof/app-plan/slice-01-app-runtime-decision.md`; PR #643 CI run `31366692141` and main merge `47a2ac717` complete. | Contract slice is code/test/proof/CI/main-merge complete; installed inventory, process/foreground capture, service/portal integration, policy, and platform paths remain open. |
