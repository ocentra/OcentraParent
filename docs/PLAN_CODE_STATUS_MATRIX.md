...
| Eventing | 13 | 10/0/1/2/0 | 182/111 | 0/2/1/0/9/1 | WP11 production source is integrated at `4aaddb425` with 24 mapped implementation files and all reviewed production callers migrated; expected negative/audit test source remains open, while WP10/WP12 are blocked. |
...
| Data custody/storage | 9 | Encrypted local queue/journal, ActivityStore projections, deletion/outbox, and custody read models provide real local persistence effects. | User-authorized export/delete/sync custody worker, durable cross-boundary lifecycle, and end-to-end source-to-custody ownership remain incomplete. The remote `ec129d668` draft was rejected because it has no shipped caller, accepts publicly mintable authority without current-state freshness/revocation, and journals intent without executing local custody. | Incomplete; local custody foundations only. Rejected draft remains quarantined remotely. |
...
| LAN | 26 | Agent-service discovery/pairing commands and parent read paths produce bounded network observations and fail-closed state. | WP26 has a partial custody/registry/transport draft but still needs a shipped signed-child beacon ingress, durable receipt/idempotency custody, selected non-revoked route composition, and private Eventing WP10 authority handoff. It is blocked on Device Trust WP01 registration and WP03 one-time `RegisterLanSignerAnchor` authorization; physical proof remains later. | Blocked; no service route or completion claim. |
...
- Device Trust: WP01 persistent registration route → WP03 one-time parent
  ceremony, after the real enrollment/presence owner is assigned through graph
  routing. WP02 is conditional only for a demonstrated private-key/install
  custody requirement.
...
| Cloudflare control plane | 13 | 10/2/0/1/0/0 | 186 / 63 | 13 / 13 | Fully code-mapped; WP00-WP02 and WP04 are Phase 1 complete, WP06 has an independently accepted bounded source adapter/auth chain, and the remaining workpacks retain concrete code/test gaps. WP06 migration/tests/proof remain open. |
...
| Logging domain parity | 10 | 5/0/0/0/5/0 | 127 / 49 | 10 / 10 | Fully code-mapped; the accepted source wave updates WP02/WP04 redaction authority and WP03/WP08 serialization routing, while all test/proof/composition gaps remain open. |
| Network | 8 | 6/1/0/0/1/0 | 372 / 97 | 8 / 8 | Fully code-mapped; WP05 and WP08 are Phase 1 complete for their bounded scopes, while WP01-WP04 and WP06-WP07 retain canonical-contract, live-runtime, production-wiring, or executable-harness gaps. WP04 is blocked behind Eventing WP09 and its other reviewed owners. |
| Parent desktop/runtime package | 11 | 4/0/0/0/7/0 | 526 / 182 | 11 / 11 | Fully code-mapped; WP01, WP04, WP05, and WP09 are Phase 1 complete for their bounded scope, while seven workpacks retain concrete runtime or expected-test gaps. |
...
| Setup/install/provisioning | 7 | 0/1/0/0/6/0 | 571 / 197 | 7 / 7 | Fully code-mapped; six workpacks have no Phase 1 writing gap, while WP07 remains a static unavailable-state panel rather than the required live first-run state machine. |
...
| V0.8 enforcement | 20 | 13/1/0/0/6/0 | 922 / 496 | 20 / 20 | Fully code-mapped; 7 workpacks have no Phase 1 writing gap in their bounded scope, while 13 retain concrete runtime, surface, lifecycle-test, or executable-harness gaps. |
...
| WP11 Type Safety And Ownership Hardening | EventEnvelope/store/decode and focused round-trip/version-skew tests exist. | **Implementation-ready; incomplete** | `EventEnvelope<E>` remains unconstrained at the struct boundary; stored decode does not revalidate aggregate/idempotency keys; malformed-payload, tamper, mutation, lock/await negatives and retained proof are missing. |
...
WP10 is blocked on LAN WP26, WP11 is implementation-ready but incomplete, WP12
...
WP01-WP07 and WP09 retain concrete runtime or expected-test gaps. WP01 now has
...
| WP02 TypeScript Logging Package Parity | Thirty-eight implementation files and twelve tests cover explicit parent scopes, core logger/config, bridge transport/server, NDJSON writers, DuckDB ingest/query, app logs, wipe/retention helpers, package exports, scripts, invalid payloads, fixture parity, and database/query behavior; accepted source now consumes the generated Rust-owned 18-key policy. | **Source present; validation open** | Focused TypeScript parity tests, proof root, and checklist closeout remain deferred; no completion claim. |
| WP03 Parent Logging Architecture And Routing | Seven implementation and four test files implement the portal-domain bridge-first logger with compatibility fallback, thin portal wrapper, live bridge receiver, logging-core-backed agent-service writer, structured startup diagnostics, and portal/Rust route tests; the fallback sanitizes before JSON serialization through the shared generated-policy sanitizer. | **Source present; validation/proof open** | Focused route validation/proof and external portal/service composition remain deferred; no completion claim. |
| WP04 Rust Logging Core Crate | Fifty-two Rust/fixture implementation files and seventeen tests cover typed events/levels/sources/fields, NDJSON durability and recovery, artifacts, redaction, path safety, diagnostics/run records, snapshots, concurrency, subprocess recovery, TypeScript fixture parity, and the agent-service consumer; accepted source owns the exact 18-key policy and generator. | **Source present; validation open** | Cargo, clippy, consumer, fixture parity tests, proof root, and checklist closeout remain deferred; no completion claim. |
...
| WP08 Logger Instrumentation And Adoption | Twelve implementation and six test files cover registered TypeScript source/context/file metadata, the portal shared logger path, Rust logging-core delegation, structured agent-service startup fields, evidence run/command identities, storage/query preservation, and selected CLI/MCP visibility; Logger and portal fallback sanitize before serialization using the generated policy. | **Source present; adoption/validation open** | No checker or negative test prevents new raw console logging or ad hoc JSON log writers on touched production surfaces, health/runtime diagnostic adoption remains narrower than the target, and tests/proof/external composition remain deferred. |
...
**Logging Domain Parity source-wave result (2026-08-17):** all 10/10
workpacks retain reviewed code/test ownership. The accepted source wave
centralizes the exact 18-key sensitive-key policy in Rust, generates the
checked-in TypeScript artifact, and routes both Logger and portal fallback
serialization through the generated-policy sanitizer. WP02/WP04 remain
source-present; WP03/WP08 remain bounded partial-proof/source-routing states;
WP01/WP05-WP07/WP09-WP10 retain their existing states. Tests, proof,
checklist, DONE, and external composition claims are explicitly deferred.
...
| --- | --- | --- | --- | --- | --- |
| `account-identity-family-plan` | Foundation | `family-identity-core`, `provisioning-core`, `entitlement-core`, `schema`, narrow `infra/cloudflare` D1 and Firebase adapters | Family identity has 13 source / 7 test files; setup and signed-entitlement paths exist. The integration branch now adds a bounded Firebase token/JWKS subject verifier with strict issuer/audience/time/claim checks and fail-closed cache expiry. It does not accept caller household/child/device headers as authority. | Account WP08 Rust schema/account-authority proof is merged. External subject verification source exists, but the Worker deliberately returns `manual-required` until Cloudflare WP06 supplies server-derived binding context; provider tests, D1/DO/KV migration proof, Cloudflare WP08 runner proof, deployment, and account runtime routes remain open. | Write the provider and negative boundary tests, implement WP06's server-owned account/family/device binding and migration seam, then complete WP08 proof and Account WP06 aggregation. Any missing input remains a payment/policy/remote/device-trust scheduling block. |
...
| `cloudflare-control-plane-plan` | Fully audited / Phase 1 incomplete | `infra/cloudflare`, account/billing contracts, portal consumer boundary | All 13 workpacks have reviewed code/test ownership. WP05 now owns a real Firebase subject-verification source packet: bounded JWT/JWKS parsing, WebCrypto RS256 verification, timeout/backoff, expired-cache rejection, and Worker mounting are present. | WP05 still lacks expected tests and cannot mint parent/family/device authority; it returns `manual-required` until WP06 supplies server-derived binding context. Durable DO state, account migration isolation, persisted local seeding, a true portal consumer smoke, deployment/rollback automation, and proof remain open. | Write WP05 provider/negative tests, then implement WP06 binding/migration ownership before broad Cloudflare tests or proof regeneration; follow with focused module gates, Enforcer, retained proof, and the payment handoff. |
...
| `device-trust-bootstrap-plan` | Fully audited / Phase 1 incomplete | `family-identity-core`, `storage-custody-core`, `parent-runtime-core`, `entitlement-core`, `child-enforcement-core`, `schema` | All 9 workpacks have reviewed code/test ownership. Durable lifecycle/parent-presence libraries, a Windows DPAPI custody slice, signed step-up proof contracts, entitlement/recovery contracts, and honest tamper-status rows are real; the integration branch now exposes immutable current-signer authority state from Rust. | The accessor slice is source-only: expected tests and shipped composition remain open. Eight workpacks still lack required production composition, cross-platform custody, live approval/QR authority, trusted entitlement verification, actual restore/re-pair mutation, child tamper/uninstall execution, or a route-gate verifier. | Write/repair WP01 tests around the current-signer authority surface, then complete WP01's live trust-owner composition and proceed to WP02/WP03 platform custody and authority. |
| `eventing-plan` | Integration | `ocentra-eventing`, `agent-protocol`, `agent-core`, `agent-service`, portal host bridge | WP06 journal/replay, topology, version-skew, and typed handoff surfaces are retained. WP09 Phase 1 production code and expected tests are written. The integration branch now includes WP11's explicit serde bounds and typed payload decode, removing the generic `EventEnvelope<E>` E0283 ambiguity. | WP11 remains READY because only the source repair is integrated; expected tests, focused validation, proof, and acceptance are still open. WP09 whole-plan integration/CI/merge remains open; WP10 is blocked on LAN WP26; WP12 lacks its harness/root and WP13 lacks current validation/proof. | Write the exact WP11 compile/serde/ownership tests next, then complete focused gates. Continue LAN WP26 -> Eventing WP10 and finish WP13/WP12 in dependency order. |
...
| --- | ---: | ---: | ---: | ---: | --- |
| `account-identity-family-plan` | 8 | 5 | 3 | 0 | WP01 remains partial. Firebase subject-verification source is now integrated, but expected tests, server-derived family/device binding, runtime routes, deployment/migration, and WP01 acceptance remain open. Account WP06 still aggregates Account WP08 plus Cloudflare WP06/WP08 evidence. |
...
| `device-trust-bootstrap-plan` | 9 | 0 | 9 | 0 | Fully mapped from live code/tests. WP01's current-signer accessor source is integrated, but its expected-test and shipped-composition gaps remain; WP01-WP07 and WP09 are still open, while WP08 is complete only for its bounded research/test scope. |
| `eventing-plan` | 6 | 1 | 5 | 0 | Six selectable workpacks: WP06 is the one doc-closed row; WP09-WP13 are open. WP11's source ambiguity repair is integrated but tests/validation/proof remain open; WP10 is blocked on LAN WP26, WP12 lacks its harness/root, and WP13 lacks current validation/proof. |
