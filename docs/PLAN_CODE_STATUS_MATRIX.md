...
`docs/plans/tracking-plan/CODE_AUDIT.md`. The 684-row `graph:matrix` remains the
...
code map, not from plan checkboxes. The graph currently contains 708 nodes, 832
edges, 23 plans, and 684 workpacks. It maps 3,272 implementation files and 1,152
test files: 587 workpacks have both code and tests, 5 are source-only, 8 are
tests-only, and 84 currently have no source or test files. Of those 84, 73 are
expected no-code coordination/reference packets, 7 expect tests only, and 4
require missing code-and-tests roots. The latter set includes Data WP09, WP10,
and WP11. No workpack has unknown ownership. Topology expectations match for
650/684 workpacks. Derived state is 356 planned, 48 blocked, 2 ready, 8 active,
269 in validation, and 1 done.
...
contract; 683/684 still lack recorded reviewed completion evidence.
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| Account identity/family | 8 | 0 | 6 | 0 | 0 | 2 | 0 | WP08 now maps the crate-private Rust producer transport at `c5ed3ce5c`; WP02 target-aware source remains reviewed and keeps actor separate from target. | Producer wire/parser/canonical/signature/time negatives; actor/target substitution; caller-trust rejection; reload/concurrent transition and audited support cases. | Sealed WP08 authority, local SQLite repository/CAS, producer transport source, bounded target-aware resolver, and Cloudflare current-authority lookup are real; authenticated producer adapter/key custody and Cloudflare mount are absent. | Add Account signer/key custody and authenticated adapter, then complete Cloudflare private verifier/mount and the WP02/WP06 expected-test packets. |
| Cloudflare control plane | 13 | 9 | 3 | 0 | 0 | 1 | 0 | WP06 has D1 writer/currentness/CAS source and verified-provider caller source; Account WP08 transport is mapped but no private verifier/service-binding consumer is mounted. | Private transport verification and subject binding; D1 migration/write/CAS concurrency, stale generation/session, revocation, restart, exact conflict classification, caller reachability, and manual-required configuration cases. | Firebase verification, ordered migrations, D1 read/current-authority resolution, Account-owned mutation source, and Rust transport source exist; only current-authority read is runtime-reachable. Capability/lease/step-up and signer/key custody remain unavailable. | Resolve the private verifier/service-binding file boundary, then mount through the existing caller/runtime and recheck D1 currentness before writing the complete WP06 tests. Migration execution, proof, deployment, and DONE stay later. |
...
| --- | ---: | ---: | ---: | ---: | --- |
| Account identity/family | 8 | 7/0/0/1/0 | 104/32 | 0/6/0/0/2/0 | WP02 target-aware action authority source is reviewed and integrated; Cloudflare composition, expected tests, proof, and DONE stay open. |
...
| Cloudflare control plane | 13 | 9/0/1/3/0 | 37/72 | 9/3/0/0/1/0 | Firebase verifier/read source, target-aware Account WP02 identity, Account-owned D1 writer/currentness/revocation/CAS, and provider caller source are retained. Current-authority resolution is reachable; mutation composition, higher authority, expected tests, proof, migration execution, deployment, and DONE remain open. |
...
| --- | ---: | --- | --- | --- |
| Account identity/family | 8 | Family/setup/provisioning helpers remain reachable. Accepted source through `e69acf279` adds Rust-owned schema validation, sealed Account capability, local SQLite repository/CAS, session and invite/recovery records, Cloudflare D1 read adapter/migrations, and Account-bound billing/support consumers. Reviewed source at `86caae334` and `7934fb41b` adds the bounded target-aware resolver and migrates the storage-custody consumer. | Cloudflare/provider composition and the complete WP02 expected-test packet remain unresolved; the legacy raw evaluator must not be used as runtime authority. | WP02 bounded source is reviewed. Cloudflare/runtime composition, tests, proof, and DONE remain open. |
...
| `device-trust-bootstrap-plan` | Accepted multi-workpack source / Phase 1 incomplete | `family-identity-core`, `storage-custody-core`, `parent-runtime-core`, `entitlement-core`, `child-runtime`, Android child bridge, `child-enforcement-core`, `schema`, parent desktop Tauri shell | Accepted source through `68717b5b7` preserves owner-resolved current device/signer authority, unsigned entitlement projection, fail-closed restore authorization, durable trust-bound child removal/readiness, and the WP02 desktop opaque-handle command mount. Public/caller-minted household and restore authority paths were removed. | Real platform/passkey enrollment, mounted ceremony issuer, entitlement issuer/revocation, encrypted restore executor/key custody, parent transport/platform removal, expected tests, and shipped composition remain open. No proof or DONE follows from source acceptance. | Finish the repository source wave, then write the complete WP01/WP05/WP06/WP07 expected-test delta before focused crate/domain execution and Enforcer. |
