...
| `app-game-plan` | Accepted WP10 source / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, schema, portal | WP99 and WP100 are Rust-owned in the existing `app_game_source_gated_policy_preview_timer_followthrough` source and contract test. WP100's retired parent-domain package/test roots were a false gap; its bounded Phase 1 shapes, builder, and real test are mapped. | WP101-WP102 still carry stale/absent parent-domain expectations pending their own owner decisions. WP100 focused execution, proof, product runtime, and parent rendering remain open. | Audit WP101 then WP102 against the Rust-first owner before adding any source; do not recreate parent-domain business logic. Run the WP100 focused test only in the later test phase. |
