...
code map, not from plan checkboxes. Fresh `graph:matrix -- --json` and
`graph:report -- --json` output records 707 nodes, 826 edges, 23 plans, and 683
workpacks. The reviewed workpack maps contain 3,245 implementation-file entries
and 1,152 test-file entries: 587 workpacks have both code and tests, 5 are
source-only, 8 are tests-only, and 83 currently have no source or test files.
Of those 83 no-source rows, 73 explicitly expect no code; three still require
code-and-tests and seven still require tests-only roots. No workpack has unknown
ownership. Topology expectations match for 651/683 workpacks.
Derived state is 354 planned, 46 blocked, 3 ready, 8 active, 271 in
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| Account identity/family | 8 | 0 | 6 | 0 | 0 | 2 | 0 | WP02 target-aware source is reviewed: opaque current Account authority keeps the parent actor separate from the target child/profile/device and the storage-custody consumer uses it. Cloudflare now resolves verified-provider current authority through the bounded Account caller. | Actor/target mismatch and substitution for all affected actions; caller-trust rejection; correct parent `ViewChildStatus`; reload/concurrent transition and audited support cases. | Sealed WP08 authority, local SQLite repository/CAS, the bounded target-aware resolver, and Cloudflare current-authority lookup are real. Cloudflare create/CAS/revoke source is not mounted in the Worker entrypoint. | Complete the remaining Account/Cloudflare runtime composition source, then write the full WP02/WP06 expected-test packets. |
| Cloudflare control plane | 13 | 9 | 3 | 0 | 0 | 1 | 0 | WP06 now has `account-identity-authority-writer.ts` and `account-identity-authority-caller.ts`: authoritative create/currentness/CAS/revoke source plus verified-provider current-authority composition exist. The mutation producer is not mounted in `src/index.ts`. | Migration/write/CAS concurrency, stale generation and session, revocation, provider/account mismatch, restart, exact conflict classification, caller reachability, and manual-required configuration cases. | Firebase verification, ordered migrations, D1 read/current-authority resolution, and Account-owned mutation source exist; only current-authority resolution is runtime-reachable. Capability/lease/step-up authority remains unavailable. | Mount mutations through the owning Account runtime boundary, then write the complete WP06 expected tests. Migration execution, proof, deployment, and DONE stay later. |
...
| --- | ---: | ---: | ---: | ---: | --- |
| Account identity/family | 8 | 7/0/0/1/0 | 178/32 | 0/6/0/0/2/0 | WP02 target-aware action authority source is reviewed and integrated; Cloudflare composition, expected tests, proof, and DONE stay open. |
| AI | 48 | 44/0/0/4/0 | 201/98 | 46/0/0/0/2/0 | All 48 lack retained/referenced completion proof; mapped code is not whole-chain acceptance. |
...
| Child-agent runtime distribution | 11 | 10/0/0/1/0 | 83/17 | 2/9/0/0/0/0 | Distribution/runtime proof remains unreferenced across the plan. |
| Cloudflare control plane | 13 | 9/0/1/3/0 | 40/72 | 9/3/0/0/1/0 | Firebase verifier/read source, target-aware Account WP02 identity, Account-owned D1 writer/currentness/revocation/CAS, and provider caller source are retained. Current-authority resolution is reachable; mutation composition, higher authority, expected tests, proof, migration execution, deployment, and DONE remain open. |
| Data custody/storage | 11 | 8/0/0/3/0 | 234/16 | 1/3/2/0/5/0 | WP06's Rust plus generated TypeScript custody edge is source-accepted, but no report/query consumer reaches it and its stale/unwritten expected tests remain open. WP05 backup/migration and WP08 confirmation authority remain source gaps; WP09/WP10 are explicit no-source runtime routes, and WP07 is blocked on the Account WP04/WP05 trusted export/delete chain. Tests, proof, and DONE remain open. |
| Device trust bootstrap | 9 | 6/0/3/0/0 | 132/36 | 0/3/0/0/6/0 | WP03 is blocked with two missing planned owners. The route is Account WP08 -> target-aware Account WP02 -> Cloudflare WP06 -> WP03, alongside WP01 foundation. WP02 now has a mounted parent-desktop opaque-handle custody-sealing command whose success is explicitly custody-sealed-only; no record-backed issuer reaches custody, Windows custody-open platform failures are unavailable, and unsupported non-Windows startup is manual-required. Expected tests, platform callers, proof, lifecycle activation, and completion remain open. |
...
| Parent desktop/runtime package | 11 | 10/0/0/1/0 | 133/32 | 4/0/0/2/5/0 | Signed package/update/rollback and retained release proof remain open. |
...
| Screen AI pipeline | 10 | 7/0/0/3/0 | 157/43 | 9/1/0/0/0/0 | One blocked workpack and unresolved policy/custody authority chain. |
| Screen | 43 | 29/3/0/11/0 | 262/80 | 25/0/0/0/18/0 | Three source-only mappings plus platform/runtime/custody gaps. |
...
| --- | ---: | --- | --- | --- |
| Account identity/family | 8 | Family/setup/provisioning helpers remain reachable. The canonical reconciled source retains Rust-owned schema validation, sealed Account capability, local SQLite repository/CAS, session and invite/recovery records, Cloudflare D1 read adapter/migrations, and Account-bound billing/support consumers. Reviewed source at `86caae334` and `7934fb41b` adds the bounded target-aware resolver and migrates the storage-custody consumer. The six patch-unique commits on remote WP02 `35edb2830` are superseded/rejected and are not integration evidence. | Cloudflare/provider composition and the complete WP02 expected-test packet remain unresolved; the legacy raw evaluator must not be used as runtime authority. | WP02 bounded source is reviewed. Cloudflare/runtime composition, tests, proof, and DONE remain open. |
...
| Cloudflare control plane | 13 | Worker route dispatcher reaches billing/admin/webhook boundaries; Firebase verification source, Account migrations, the D1 read adapter, and target-aware Account WP02 identity source exist. | WP06 lacks authoritative Account D1 create/update/revoke/currentness/CAS and the shipped provider-to-target-aware sealed-authority caller. Capability/lease/step-up authority remains unavailable; the migration is unapplied, production configuration remains manual-required, and expected tests/proof/deployment are open. | WP06 implementation-only source is authorized; normal readiness and DONE remain blocked. |
| Data custody/storage | 11 | The shipped child-service startup opens the durable journal/effect/tombstone stores, recovers them before readiness, and routes `PublishStorageCustody` through dynamic Device Trust readiness into `ChildStorageCustodyRuntime::execute`; the shared WP06 query-custody boundary is source-present but has no report/query consumer. | Default composition deliberately installs manual-required custody authority. The real missing chain is Account WP08 current authority → WP04 correlated export/delete lifecycle → WP05 account/session/device/capability/lease composer → a private child-runtime adapter. WP05 backup/migration source and the WP09/WP10 parent-local/restore runtime routes remain open; WP06's stale Rust tests and unwritten TypeScript contract test remain open. | Real internal custody lifecycle; WP06 shared source edge is accepted without runtime reachability, expected tests, proof, or DONE. |
| Device trust bootstrap | 9 | Typed challenge/presence/identity contracts and bounded atomic receipt/challenge custody exist. The canonical reconciled source retains WP01 current binding, unsigned entitlement projection, fail-closed restore authority, and durable trust-bound removal readiness. The two patch-unique commits on remote WP01 `914d06b6a` are superseded/rejected because their authority path was forgeable and are not integration evidence. | WP03 lacks target-aware `RegisterLanSignerAnchor` authority and a shipped parent runtime; no passkey/native signer, durable sign counter, authoritative Account/Device Trust currentness composition, expected-test wave, or LAN handoff exists. Device Trust WP02's Windows custody remains a separate conditional route behind unavailable authority. | Blocked on Account WP08 -> Account WP02 -> Cloudflare WP06, alongside WP01 foundation; two planned WP03 source roots are missing and no runtime/DONE claim exists. |
...
`npm run graph:matrix -- --json` is the complete 683-workpack table; the reviewed
...
| --- | ---: | ---: | ---: | ---: | --- |
| Account identity/family | 8 | 0/6/0/0/2/0 | 325 / 82 | 8 / 8 | Fully code-mapped. WP02/WP08 have reviewed implementation completion evidence; WP03-WP05 have accepted partial source but retain production composition gaps. All eight still retain expected-test, proof, or aggregate-gate gaps. |
| AI | 48 | 46/0/0/0/2/0 | 121 / 39 | 48 / 48 | Fully code-mapped; 11 workpacks are Phase 1 complete for bounded scope and 37 retain concrete production-code or expected-test gaps. |
| App/game | 220 | 126/2/0/2/90/0 | 609 / 62 | 220 / 220 | Fully code-mapped; 169 bounded packets have current code plus expected tests, 19 are reviewed no-code packets, and 32 retain concrete production-code or expected-test gaps. |
| App | 95 | 13/0/0/0/82/0 | 186 / 67 | 95 / 95 | Fully code-mapped; 81 bounded workpacks have no Phase 1 source/expected-test writing gap and 14 retain concrete compiler, durability, runtime, UI, notification, performance, or followthrough gaps. |
| Browser | 30 | 30/0/0/0/0/0 | 74 / 13 | 30 / 30 | Fully code-mapped; 14 executable workpacks are Phase 1 complete for bounded scope, 10 retain concrete code/test gaps, and six imported packets are reference-only. |
| Child-agent runtime distribution | 11 | 2/9/0/0/0/0 | 158 / 16 | 11 / 11 | Fully code-mapped; WP01, WP02, WP05, WP06, and WP09 are Phase 1 complete for their bounded scope, while six workpacks retain runtime, lifecycle-test, handoff, or release-gate gaps. The new service/JNI startup is real but does not close Android transport, supervision, identity, health-consumption, or device-proof work. |
| Cloudflare control plane | 13 | 9/3/0/0/1/0 | 198 / 63 | 13 / 13 | Fully code-mapped; WP00-WP02 and WP04 are Phase 1 complete, WP06 has an independently accepted bounded source adapter/auth chain, and the remaining workpacks retain concrete code/test gaps. WP06 migration/tests/proof remain open. |
| Data custody/storage | 11 | 1/3/2/0/5/0 | 993 / 482 | 11 / 11 | Fully code-mapped. WP06's Account-authority-derived Rust/generated custody edge is source-accepted, but no report/query consumer reaches it and its stale/unwritten expected tests remain open. WP05 and WP08 are legal remaining source packets; WP09/WP10 are explicit no-source runtime routes, and WP07 is dependency-blocked on the Account authority chain. No workpack is DONE, and all current expected-test/proof acceptance remains open. |
| Device trust bootstrap | 9 | 0/3/0/0/6/0 | 761 / 143 | 9 / 9 | Fully code-mapped; all nine remain graph-open. Canonical reconciled source covers WP01 current binding, WP05 unsigned entitlement, WP06 fail-closed restore authority, and WP07 durable removal/readiness. Remote WP01 `914d06b6a` is superseded/rejected because its authority path was forgeable and is excluded from current evidence. WP02 maps Windows custody plus a mounted parent-desktop opaque-handle custody-sealing command; its success does not activate lifecycle, the authority issuer and custody gate remain fail-closed, and stale lifecycle/custody tests are excluded from current evidence. External ceremony/provider/platform callers, expected tests, lifecycle activation, and proof remain open. |
| Eventing | 13 | 0/3/1/0/8/1 | 829 / 490 | 13 / 13 | Fully code-mapped. WP06 is graph-done. WP08 is source-hardened but blocked on Account, Tracking, Policy, Enforcement, and Child Runtime owners plus missing source/test roots. WP09 is validation/integration-open; WP11 remains READY for the later expected-test phase; WP10/WP12 are blocked and WP13 remains validation/proof-open. |
...
| Parent desktop/runtime package | 11 | 4/0/0/2/5/0 | 586 / 183 | 11 / 11 | Fully code-mapped; WP01, WP04, WP05, and WP09 are Phase 1 complete for their bounded scope, while seven workpacks retain concrete runtime or expected-test gaps. |
| Payment/subscription | 13 | 4/5/0/4/0/0 | 75 / 39 | 13 / 13 | Fully code-mapped; accepted production source closes four major authority/durability defects, while Account/provider/issuer dependencies and the complete expected-test delta remain open. |
| Policy control plane | 8 | 1/4/0/0/3/0 | 1037 / 480 | 8 / 8 | Fully code-mapped; all eight workpacks retain production reachability or expected-test gaps. WP01 is blocked on the reviewed Cloudflare and Device Trust owners, and WP03's deterministic compiler remains library code without a shipped authoritative-source-to-domain caller. |
| Portal UX/household surfaces | 20 | 15/0/0/0/5/0 | 1102 / 531 | 20 / 20 | Fully code-mapped; 9 workpacks have no Phase 1 writing gap in their bounded scope, while 11 retain concrete product-code or expected-test gaps. |
...
| Screen AI pipeline | 10 | 9/1/0/0/0/0 | 131 / 33 | 10 / 10 | Fully code-mapped; prerequisite routing is the only bounded Phase 1 row without a writing gap. WP02-WP10 retain production-composition, authority, durability, custody-negative, performance-test, or missing executable-harness gaps. |
| Screen | 43 | 25/0/0/0/18/0 | 118 / 26 | 43 / 43 | Fully code-mapped; 9 of 40 executable workpacks are complete for bounded Phase 1 code/expected-test writing, 31 retain concrete gaps, and three imported reference packets own no executable code. |
| Setup/install/provisioning | 7 | 0/2/0/0/5/0 | 671 / 198 | 7 / 7 | Fully code-mapped; five workpacks are in validation and two are blocked. WP07 has a reachable Rust-owned 15-row fail-closed setup matrix, a canonical 13-variant LAN classifier, and non-LAN dispatch rejection; evaluator/actions remain deliberately not run, LAN is observation-only, and real authority inputs, the state machine/completion guard, and current expected tests remain open. |
...
| V0.8 enforcement | 20 | 13/1/0/0/6/0 | 952 / 496 | 20 / 20 | Fully code-mapped; 7 workpacks have no Phase 1 writing gap in their bounded scope, while 13 retain concrete runtime, surface, lifecycle-test, or executable-harness gaps. |

The repository-wide reviewed ownership map now covers all 683 workpacks,
...
retains the canonical reconciled Account source. The six patch-unique commits
on `origin/codex/account-wp02-source-wave` at `35edb2830` are
superseded/rejected and are not integration evidence. The graph maps the
canonical source deltas and records reviewed
...
in any implementation total. The remote `35edb2830` packet is also
superseded/rejected and must not be revived. The remaining Account gaps are the shipped
...
| WP06 Storage DO D1 KV R2 Queue Bindings | Env/config ownership, billing D1/KV/R2 read-model code, Worker DO/queue paths, and five existing tests cover binding names, privacy constraints, seed/read behavior, idempotent writes, enqueue failure, and dead-letter capture. The independently accepted source packets add the binding-specific account migration, canonical Account WP08 `v0.7` D1 read/current-authority adapters, Account-owned create/CAS/revoke source, mounted verified-provider current-authority resolution, production rejection of local fixture auth and missing queue secrets, and deterministic schema-contract build order. | **Incomplete** | The mutation producer is not mounted in the Worker entrypoint; capability/lease/step-up authority remains unavailable. The account migration has not run, and the new writer/caller/current-authority paths lack focused expected tests and retained proof. Account DO/KV remain absent, and `BillingControlDO`/`ReferralControlDO` still keep idempotency in an in-memory `Map` rather than `DurableObjectState.storage`, so restart durability is not implemented or tested. |
...
| Workpack | Reviewed live code/test evidence | Phase 1 | Concrete code/test gap |
| --- | --- | --- | --- |
| WP01 Device Trust Source Of Truth | Thirty-eight mapped implementation files and twelve existing tests cover the lifecycle repository, transition journal/sidecar generations, current device/signer binding, parent-presence custody, restart/replay, schema/path integrity, opaque references, and fail-closed Eventing delivery. The canonical reconciled source retains the owner-resolved current-binding surface; remote WP01 `914d06b6a` is superseded/rejected because its authority path was forgeable and is not integration evidence. Public household signer/verifier mint paths are removed and independent source review found no remaining internal P0/P1. | **Foundation/source accepted; expected tests and authority bridge open** | No shipped platform/passkey ceremony issuer or complete product composition owns bootstrap-through-revoke/reset/re-pair state. Existing tests predate parts of the accepted surface; the complete expected-test migration, focused validation, proof, caller integration, platform custody, Account WP08 -> target-aware Account WP02 -> Cloudflare WP06 current-authority bridge, and DONE remain open. |
...
the WP01/WP05/WP06/WP07 production surfaces; remote WP01 `914d06b6a` is
superseded/rejected because its authority path was forgeable and is excluded
from integration evidence. Every workpack still retains
...
| Data custody/storage | 11 | 1/3/3/0/4/0 | 974 | 482 |
...
| `cloudflare-control-plane-plan` | Fully audited / Phase 1 incomplete | `infra/cloudflare`, account/billing contracts, portal consumer boundary | All 13 workpacks have reviewed code/test ownership. WP05 has real Firebase subject verification, the accepted Account/Payment source adds WP06's current-authority D1 adapter plus ordered Account/Payment migration source and authority-bound billing/support consumers, and the Account WP03 overlay adds browser-session custody migrations/store/routes composed through the final WP06 caller/verifier result seam. | WP03 route/store/request-safety/integration tests, migration execution, live D1/Worker execution, durable DO state, persisted local seeding, portal smoke, deployment/rollback automation, and proof remain open. | Finish provider/account route composition and remaining runtime source, then write WP03 plus WP05/WP06 expected tests before broad Cloudflare validation or proof regeneration; follow with focused module gates, Enforcer, retained proof, and payment handoff. |
| `data-custody-storage-plan` | Accepted multi-workpack source / Phase 1 incomplete | `schema`, `storage-custody-core`, `child-runtime`, `ocentra-evidence`, `ocentra-eventing` | WP01's Rust-owned matrix, WP02 scope authority, WP03 manifest custody, WP04 child-owned durable tombstone/effect lifecycle, WP05 import integrity, WP06's Account-authority-derived request/row boundary plus generated TypeScript edge, and WP07 internal service command/recovery source are present; WP08 remains the confirmation-authority route, while WP09/WP10 are explicit no-source parent-local/restore runtime routes. | The Account WP08 → WP04 → WP05 trusted authority chain and private child-runtime adapter are absent; WP05 backup/migration, WP08 confirmation authority, and WP09/WP10 runtime source remain open; WP06 has no report/query consumer and its stale/unwritten expected tests remain open; no current proof/DONE. | Finish the remaining source owners first, then write the complete expected-test delta, run focused crate/domain gates, repo-wide Enforcer, proof, precommit, and one PR/CI cycle. |
| `device-trust-bootstrap-plan` | Accepted multi-workpack source / Phase 1 incomplete | `family-identity-core`, `storage-custody-core`, `parent-runtime-core`, `entitlement-core`, `child-runtime`, Android child bridge, `child-enforcement-core`, `schema`, parent desktop Tauri shell | Canonical reconciled source preserves owner-resolved current device/signer authority, unsigned entitlement projection, fail-closed restore authorization, durable trust-bound child removal/readiness, and the WP02 desktop opaque-handle command mount. The two unique commits on remote Device WP01 `914d06b6a` are superseded/rejected because their authority path was forgeable and are not integration evidence. | Real platform/passkey enrollment, mounted ceremony issuer, entitlement issuer/revocation, encrypted restore executor/key custody, parent transport/platform removal, expected tests, and shipped composition remain open. No proof or DONE follows from source acceptance. | Finish the repository source wave, then write the complete WP01/WP05/WP06/WP07 expected-test delta before focused crate/domain execution and Enforcer. |
...
| `screen-plan` | Fully audited / Phase 1 incomplete | `screen-capture-adapter`, `agent-protocol`, `agent-core`, `agent-service`, `screen-ai-core`, `screen-live-view-core`, schema, Android agent, portal | All 43 imported packets have reviewed ownership: 40 executable workpacks plus three reference-only packets. WP26 disclosure source, WP32 structured-first router source at canonical `b4c3a921b`, and WP33 managed-browser CDP source are mapped as source-only. | Thirty-one executable workpacks retain code/test gaps. WP26 lacks child delivery/authority bridge; WP32 lacks a real managed-browser producer/caller and extraction/fallback/receipt tests; WP33 lacks trigger integration and CDP bounds/custody tests. Platform tests, durable replay, policy refs, live-view, OCR/VLM/detector/rollout harnesses also remain open. | Reconcile WP01/WP02 first, then close contracts/scope/platform safety (WP03/WP05-WP14), write and run the WP26/WP32/WP33 test wave, and continue queue/runtime composition (WP16-WP21) before policy/live/AI/rollout packets. Do not regenerate proof until those Phase 1 gaps close. |
| `setup-install-provisioning-plan` | Accepted fail-closed source / Phase 1 incomplete | `provisioning-core`, `child-runtime`, `parent-runtime-core`, parent desktop, generated bridge, `portal-domain`, portal | All 7 workpacks have reviewed ownership. WP05's readiness model is real; WP07's `crates/parent-runtime-core/src/setup_first_run.rs` is reachable and projects a Rust-owned 15-row `not-run`/`manual-required` matrix while preserving LAN as observation-only and rejecting non-LAN dispatch. | Authenticated owner inputs for all readiness authorities, the live state machine/actions/completion guard, and current Rust/portal/E2E expected tests remain open. Historical proof is stale. | Bind real typed owner inputs first, finish the whole WP07 state-machine source packet, then write all WP07 expected tests before any focused execution or proof. |
...
| --- | ---: | ---: | ---: | ---: | --- |
| `account-identity-family-plan` | 8 | 5 | 3 | 0 | Canonical reconciled Account source includes sealed current authority, durable CAS repository, lifecycle records, D1 adapter/migrations, Account-bound consumers, browser-session custody, and target-aware WP02 resolution. Remote Account WP02 `35edb2830` is superseded/rejected, not integrated. Exact tests, migration execution, provider/account runtime proof, remaining handoffs, deployment/proof, and Account WP06 aggregation remain open. |
...
| `data-custody-storage-plan` | 8 | 0 | 8 | 0 | Current graph/topology keeps every executable workpack open. Source has advanced through WP06's Rust/generated custody edge, but no report/query consumer, tests, retained proof, checklist reconciliation, the Account WP08 → WP04 → WP05 composition chain, WP05 backup/migration behavior, and WP08 confirmation authority remain incomplete; WP06's stale/unwritten expected tests are deferred. |
| `device-trust-bootstrap-plan` | 9 | 0 | 9 | 0 | Canonical source retains WP01/WP05/WP06/WP07 fail-closed source, but remote Device WP01 `914d06b6a` is superseded/rejected and not integrated. The plan has zero graph-DONE workpacks; expected tests, external authority/composition, focused validation, and proof remain open. |
