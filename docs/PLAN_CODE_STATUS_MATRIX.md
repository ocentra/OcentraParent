...
| `app-game-plan` | Accepted WP10 source / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, schema, portal | WP99 retains only `parent_surface_status.rs` plus its existing contract test. WP100-WP102 planned parent-domain package/test roots are absent and now form a reviewed source-order chain only. | WP100-WP102 remain blocked/validation with no package source, tests, proof, or readiness claim. | Land the actual Rust-to-edge/package owner decision before any parent-domain source packet; then write the named tests and proof. |
