...
| `app-game-plan` | Accepted WP10 source / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, schema, portal | WP99-WP101 are Rust-owned in the existing `app_game_source_gated_policy_preview_timer_followthrough` source and contract test. WP100/WP101's retired parent-domain package/test roots were false gaps; their bounded Phase 1 shapes, builders, and real test are mapped. | WP102 still carries a stale/absent parent-domain expectation pending its own owner decision. WP100/WP101 focused execution, proof, product runtime, and parent rendering remain open. | Audit WP102 against the Rust-first owner before adding any source; do not recreate parent-domain business logic. Run the mapped focused tests only in the later test phase. |
