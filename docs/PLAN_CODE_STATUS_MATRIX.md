...
| `cloudflare-control-plane-plan` | Fully audited / Phase 1 incomplete | `infra/cloudflare`, account/billing contracts, portal consumer boundary | All 13 workpacks have reviewed code/test ownership. WP05 has real Firebase subject verification plus a separate provider-webhook boundary: Stripe verification is real, while non-Stripe providers remain explicit manual-required/unavailable. The accepted Account/Payment source adds WP06's current-authority D1 adapter plus ordered Account/Payment migration source and authority-bound billing/support consumers, and the Account WP03 overlay adds browser-session custody migrations/store/routes composed through the final WP06 caller/verifier result seam. | WP03 route/store/request-safety/integration tests, WP05 webhook/auth tests, migration execution, live D1/Worker execution, durable DO state, persisted local seeding, portal smoke, deployment/rollback automation, and proof remain open. Admin/support and family/device authority remain unavailable. | Finish provider/account route composition and remaining runtime source, then write WP03 plus WP05/WP06 expected tests before broad Cloudflare validation or proof regeneration; follow with focused module gates, Enforcer, retained proof, and payment handoff. |
...
| `cloudflare-control-plane-plan` | 13 | 0 | 13 | 0 | Fully mapped from live source/tests. WP05 includes the reviewed Firebase verifier source. WP06 remains a safe verified-provider read/manual runtime boundary, while Account WP03 now maps browser-session custody source through that final caller/verifier result seam. WP03's exact route/store/request-safety/integration tests, the missing Account WP02/WP08 signed/sealed producer transport plus Worker verifier, migration execution, proof, deployment, and runtime composition remain open; the other runtime/deployment workpacks retain concrete Phase 1 gaps. |
