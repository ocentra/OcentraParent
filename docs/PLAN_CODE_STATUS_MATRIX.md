...
WP04, and Cloudflare WP02. Their bounded production source and expected real
test source are checked in; none of the new tests has been executed in this
code-first phase. Protected WP04 still depends on external enrollment,
hardware-monotonic, Parent WP12 lifecycle, and owner-caller composition. Data
WP06 and Child WP10 were inspected and stopped without changes because a legal
Account current-authority fixture/runtime owner and Device Trust binding owner,
...
- Historical Protected Custody WP01 source was integrated at `1b46b5935` across
  four active packages (core, protocol, broker, client); startup failed closed
  before state or listener readiness, and the adapter/owner gaps recorded at
  that snapshot remain historical. The current WP04 source audit is recorded
  below at `cbd974291`; installer/SCM enrollment, a real caller, and expected
  tests remain open.
...
The integrated source checkpoint `a78d8f831` now covers the complete shared
WP06 Rust and generated TypeScript custody edge: current Account-authority-
derived opaque source capability, session expiry, exact actor-role binding,
page size `<= 100`, and requested/source/next cursor plus unique stable-sort,
row, and source-ref continuity. No shipped report/query consumer reaches this
boundary. The three existing Rust harnesses are stale against the current API
or canonical sample, and the TypeScript contract test path is expected but
unwritten. No tests, build, proof, precommit, CI, or PR validation was run;
WP06 remains open and is not DONE or PR_READY. The exact 20 newly integrated
production/generated files, retained WP06 ownership roots, and four deferred
test paths are mapped in
`docs/engineering-graph/code-map.json` and the WP06 workpack.

### Protected Custody WP04 source audit refresh — 2026-08-26

The fresh Windows lane audit at canonical source `cbd974291` confirms the
complete 21-root WP04 client/broker anchor packet is present and registered.
The Windows FFI owns the unsafe boundary and provides retained limited-query /
synchronize process handles, process epoch/liveness/image observations,
process and impersonated-thread token handles, bounded SID/integrity/session
and TokenGroups observations, and `LookupAccountNameW` service-SID resolution
from the retained SCM service name. Private core admission retains and
revalidates those observations; broker service startup performs peer admission
before creating the fixed listener or reporting `Running`; the client checks
the kernel-reported server PID/session before bootstrap and broker-hello
acceptance. No sysinfo/path/caller-identity substitute or public adapter
constructor is present.

Focused Linux and Windows-target library checks, per-crate formatting, and
scoped source-shape/no-test-doubles/validation-bypass/reexport guards pass.
The architecture policy reports the expected missing organized test scaffolds.
This refresh records source and validation truth only: external enrollment and
monotonic owners, the owner-bound production caller, three WP04 transport
tests, proof, runtime availability, and DONE remain open.
...
validation / done. Those state counts are routing/scheduling metadata only;
they are never a code, test, runtime, proof, or completion score.

### Current Account -> Cloudflare -> Device source frontier

The full-word state columns below are routing state only, not completion. The
code/test/runtime columns are the current live-source finding for the three
touched plans.

| Plan | Workpacks | Planned | Blocked | Ready | Active | Validation | Done | Actual production-code intent gap | Expected-test-source gap | Runtime / caller reachability | First coherent source packet |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| Account identity/family | 10 | 0 | 8 | 0 | 0 | 2 | 0 | WP09 durable issuer/key-registry/outbox core is reviewed at `4f6245e51`, but the protected signer, binding authenticator, delivery owner, and production lifecycle caller are absent; WP05A's multi-owner coordinator/participants also remain missing. | Seven WP09 custody/registry/adapter/runtime roots; WP02 actor/target substitution; WP03/WP04/WP05 restart, concurrency, replay, authority, and support-owner cases. | Sealed WP08 authority, local SQLite custody, target-aware WP02 resolution, and Account WP09 fail-closed core are real. No production caller can install its private adapters or deliver to Cloudflare, and Cloudflare cannot verify the outer wire/current key record. | Complete the Account WP09 protected-signer/delivery/runtime packet, then the Cloudflare private issuer consumer/mount, before the consolidated expected-test wave. |
| Cloudflare control plane | 13 | 0 | 11 | 0 | 0 | 2 | 0 | WP06 has D1 writer/currentness/CAS, verified-provider caller, bounded decoder, and private WP08 inner-wire verifier source. Account WP09 lacks protected signer/binding/delivery/runtime, and Cloudflare lacks issuer transport/current-key consumer/runtime mount. | Nine WP06 roots covering outer-wire/key-registry/runtime, subject binding, D1 migration/write/CAS concurrency, stale generation/session, rotation/revocation, restart, exact conflict classification, caller reachability, and manual-required configuration. | Firebase verification, ordered migrations, D1 current-authority resolution, Account-owned mutation source, WP08 wire, and WP09 fail-closed core exist; only current-authority read is runtime-reachable. Capability/lease/step-up and authenticated producer delivery remain unavailable. | Complete Account WP09's three producer roots, then Cloudflare WP06's three consumer/runtime roots; write the nine-test packet only after that coherent handoff. Migration execution, proof, deployment, and DONE stay later. |
| Device trust bootstrap | 9 | 0 | 3 | 0 | 0 | 6 | 0 | WP03 lacks `parent_step_up_target_authority.rs` and `parent_step_up_runtime.rs`; bounded custody cannot authorize `RegisterLanSignerAnchor` or bind live Account and Device Trust currentness. | Actor/target mismatch, stale/revoked Account/Device Trust, provider mismatch, sign-counter/nonce replay, restart reuse, signature failure, and runtime-bypass cases. | Receipt/challenge custody exists, but no shipped parent runtime, passkey/native signer, authoritative target resolver, or LAN handoff is reachable. | After Cloudflare WP06, land target-aware ceremony composition and the parent-runtime caller together; LAN remains downstream. |
...
| Protected capability custody | 5 | 0/4/0/0/1 | 159/0 | 0/0/0/0/4/0 | WP01 remains the neutral fail-closed foundation; WP02/WP03 are external-owner/currentness blocked, while WP04's bounded Windows anchor/transport source is present and WP05 remains source-authorable. Installer/SCM enrollment, signer/store provider, owner-bound callers, expected tests, proof, and runtime availability remain open. |
...
| Parent desktop/runtime package | 12 | 10/0/0/2/0 | 136/32 | 0/10/0/0/2/0 | WP06 is production-source blocked on Account WP03's missing opaque current/revocable parent-session capability and authenticated handshake; typed response checks over the current unauthenticated WebSocket do not establish readiness. WP12 remains a bounded installer/package source-authorable route; normal package/protected dependencies, tests, signing, runtime, and retained release proof remain open. |
...
| --- | ---: | --- | --- | --- |
| Account identity/family | 10 | Family/setup/provisioning helpers, sealed WP08 authority, target-aware WP02 resolution, durable invite/recovery custody, and WP09's SQLite issuer/key-registry/outbox core are present. | WP09 has no protected signer, binding authenticator, delivery owner, production caller, or authenticated current-key handoff; WP05A's multi-owner coordinator/participants and the consolidated expected-test wave also remain missing. | WP09 is a reviewed fail-closed core, not a complete producer runtime. Cloudflare composition, tests, proof, and DONE remain open. |
...
| Browser | 30 | Windows inventory, unmanaged discovery, CDP target-list parsing, service events, and portal status paths are reachable. WP06 profile storage is intentionally unavailable and cannot produce Ready/Deleted. | Protected profile owner/custody, handle-bound platform mutation/recovery, a real profile-store caller, owner-bound WP07 launch, active-focus authority, trusted intervention delivery, and native action receipts are missing. | Incomplete/blocked; the accepted WP06 packet is a safety correction, not managed-profile runtime. |
...
| Cloudflare control plane | 13 | Worker routes reach billing/admin/webhook boundaries; Firebase verification, Account migrations, D1 read/writer/currentness/CAS, target-aware Account WP02 source, bounded decoder, and private WP08 inner-wire verifier exist. | Account WP09 lacks protected signer/delivery/runtime; Cloudflare lacks issuer transport/current-key/runtime mount. Capability/lease/step-up authority remains unavailable; migration is unapplied, production configuration stays manual-required, and nine expected tests/proof/deployment are open. | WP06 implementation is blocked on Account WP09's missing reviewed producer roots; normal readiness and DONE remain blocked. |
| Data custody/storage | 12 | The shipped child-service startup opens the durable journal/effect/tombstone stores, recovers them before readiness, and routes `PublishStorageCustody` through dynamic Device Trust readiness into `ChildStorageCustodyRuntime::execute`; the shared WP06 query-custody boundary is source-present but has no report/query consumer. | Default composition deliberately installs manual-required custody authority. The missing path is now split honestly: WP05 owns base backup/import contracts and ledgers; WP09 owns provider/byte capability; WP10 owns restore/producer orchestration; and WP11 must mount those owners behind the trusted Account/custody chain in parent runtime. WP09, WP10, and WP11 have no production source or expected tests yet, while WP06's stale Rust tests and unwritten TypeScript contract test remain open. | Real internal custody lifecycle and WP05 base source; WP09/WP10/WP11 runtime ownership, expected tests, proof, and DONE remain open. |
| Device trust bootstrap | 9 | Typed challenge/presence/identity contracts and bounded atomic receipt/challenge custody exist. Accepted source through `68717b5b7` also retains WP01 current binding, unsigned entitlement projection, fail-closed restore authority, and durable trust-bound removal readiness. | WP03 lacks target-aware `RegisterLanSignerAnchor` authority and a shipped parent runtime; no passkey/native signer, durable sign counter, authoritative Account/Device Trust currentness composition, expected-test wave, or LAN handoff exists. Device Trust WP02's Windows custody remains a separate conditional route behind unavailable authority. | Blocked on Account WP08 -> Account WP02 -> Cloudflare WP06, alongside WP01 foundation; two planned WP03 source roots are missing and no runtime/DONE claim exists. |
...
| LAN | 26 | Agent-service discovery/pairing commands and parent read paths produce bounded network observations and fail-closed state. | WP26 has a partial custody/registry/transport draft but still needs a shipped signed-child beacon ingress, durable receipt/idempotency custody, selected non-revoked route composition, and private Eventing WP10 authority handoff. It is ordered after Account WP08 -> target-aware Account WP02 -> Cloudflare WP06 -> Device Trust WP03 one-time `RegisterLanSignerAnchor` authorization and consumes WP01 current binding/revocation; physical proof remains later. | Blocked; no service route or completion claim. |
...
| Parent desktop/runtime package | 11 | Tauri route commands and provisioning/install-state contracts are reachable from the desktop shell; WP06 response envelopes have useful structural identity/freshness/deadline validation. | The local WebSocket session is unauthenticated: missing `Origin` is accepted, readiness precedes authentication, health can be `Ready`/`Unauthenticated`, and parent runtime supplies its own `ws://` origin. Account WP03's opaque current/revocable parent-session authority and owner-bound handshake are missing, alongside signed package/build/update/rollback, integrity, installer, and release owners. | WP06 production source blocked on Account WP03 authority; runtime distribution remains incomplete. |
...
- Device Trust: Account WP08 canonical binding → Account WP02 target-aware
  authority → Cloudflare WP06 authoritative writer/provider caller → WP03
  one-time parent ceremony, with WP01 as the
...
`npm run graph:matrix -- --json` is the complete 687-workpack table; the reviewed
...
| Browser | 30 | 9/21/0/0/0/0 | 66 / 13 | 30 / 30 | Fully topology-mapped. WP06 is source-only with five protected-owner/platform/caller roots and five expected tests missing; WP07 and WP09 retain explicit owner-bound launch/target gaps. Eight packets are expected no-code routing/reference rows. |
...
| Data custody/storage | 12 | 1/5/1/0/5/0 | 993 / 482 | 9 / 12 | WP01-WP08 and the reference-only Data/AI UI packet have reviewed topology. WP05 base source is present but retains planned test obligations; WP06 has no report/query consumer. WP09 provider capability and WP10 restore/producer orchestration have implementation-only gates that remain blocked on Account WP05's missing durable participant/CAS source; WP11 remains blocked on those source packets. WP07/WP08 remain dependency-blocked. No workpack is DONE, and all current expected-test/proof acceptance remains open. |
| Device trust bootstrap | 9 | 0/2/1/0/6/0 | 553 / 143 | 9 / 9 | Fully code-mapped; all nine remain graph-open. Accepted source through `68717b5b7` covers WP01 current binding, WP05 unsigned entitlement, WP06 fail-closed restore authority, and WP07 durable removal/readiness. WP02 maps Windows custody plus a mounted parent-desktop opaque-handle custody-sealing command; its success does not activate lifecycle, the authority issuer and custody gate remain fail-closed, and stale lifecycle/custody tests are excluded from current evidence. External ceremony/provider/platform callers, expected tests, lifecycle activation, and proof remain open. |
| Eventing | 13 | 0/3/1/0/8/1 | 826 / 490 | 13 / 13 | Fully code-mapped. WP06 is graph-done. WP08 is source-hardened but blocked on Account, Tracking, Policy, Enforcement, and Child Runtime owners plus missing source/test roots. WP09 is validation/integration-open; WP11 remains READY for the later expected-test phase; WP10/WP12 are blocked and WP13 remains validation/proof-open. |
| LAN | 26 | 0/1/0/0/25/0 | 320 / 60 | 26 / 26 | The 25 pre-existing workpacks are code-mapped and 21 have bounded Phase 1 code/expected tests written. WP07 has accepted absolute-deadline source but lacks its timing/lifecycle test delta; WP16, WP20, and WP25 retain integrated-validation or executable-verifier gaps. WP26 is code-mapped as a partial draft but remains blocked on Account WP08 -> target-aware Account WP02 -> Cloudflare WP06 current authority and Device Trust WP03 ceremony, with no completion claim. A selected platform sealing/lifecycle-revocation path also requires the reviewed Device Trust WP02 gate; the default non-sealing route does not force it. |
...
| WP07 Managed Chromium Launcher | Core launch planning and private authority remain, while canonical `f80b47c6a` removes unreachable service launch state, env/dev profile authority, placeholder polling, and dead Screen composition. Production status is manual-required. | **Blocked / fail-closed source baseline accepted** | No owner-issued retained runtime exists. Existing service tests use stale helper arities/private launch fields, and `crates/agent-service/tests/integration/browser_managed_runtime.rs` is absent. Write the real owner lifecycle first, then the full test family. |
...
| WP09 CDP Version And Target Adapter | Version/target parsing and private core capture authority remain tab-list-only with active state Unknown; the dead service Browser-to-Screen path is removed. | **Blocked on WP07** | No retained-launch production caller, same-launch target binding, or Screen-owned handoff exists. `crates/agent-core/tests/integration/browser_bridge_managed_launch.rs` is absent and existing private-boundary tests need migration. |
...
| WP07 Passive Discovery Listeners | Passive ARP, DHCP, mDNS, SSDP, WSD, LLMNR, NetBIOS, beacon, and allowed SNMP observations feed hint-only runtime state. Canonical `ffaade68a` adds one absolute recurring-cycle deadline, per-receive remaining-time enforcement, Windows sub-millisecond handling, and panic-safe socket-timeout restoration. | **Accepted source; expected tests open** | Write slow-trickle and zero-deadline timing, success/error/unwind timeout restoration, six-listener rotation, and cancellation tests. Focused execution and physical packet proof follow later. |
...
| WP26 Signed Child Beacon Ingress And Household Mesh Authority Handoff | Partial custody/registry/transport draft; no shipped ingress, organized real-ingress tests, or current proof root is claimed. | **Blocked** | Account WP08 -> target-aware Account WP02 -> Cloudflare WP06 current-authority resolution and Device Trust WP03 one-time `RegisterLanSignerAnchor` authorization are missing, along with real signed child/runtime ingress, durable custody/restart recovery, W15/W18/W19 authority composition, and the private Eventing WP10 handoff. If the platform sealing/lifecycle-revocation path is selected, the reviewed Device Trust WP02 gate must also complete before this consumer route can proceed; the non-sealing route does not force Device Trust WP02. |

**LAN Phase 1 result:** 26 rows are now routed: 13 document-closed and 13
open. Five Phase 1 gaps are explicit: WP07 deadline/lifecycle tests, WP16
cross-process delivery, WP20 aggregate verifiers, WP25 integrated rollout
delivery, and the new WP26 signed child beacon/household authority handoff.
WP26 is the LAN dependency that blocks Eventing WP10.
Manual household/router/platform artifacts remain Phase 3 and do not reopen the
21 code-and-expected-test-complete rows.
...
| WP13 iOS ReplayKit Adapter Plan Proof | No mapped ReplayKit production source exists; the Child iOS capability shell is not capture evidence. | **Incomplete** | No ReplayKit/broadcast-extension capture implementation or test target exists. |
...
| WP32 Browser Structured Extraction Before Screenshot | Reviewed source now spans the CDP structural probe, opaque producer binding, redacted/digest-only transport, frozen-capture guards, service handoff, and neutral router input. Three of six source rows are implemented. | **Incomplete** | The producer deliberately exposes no title/meta/accessibility/visible text without a browser/policy safe-disclosure owner; no executable service composition invokes the route; `no_screen_needed`, five mapped focused test roots, proof, and CI remain open. |
...
| WP37 Household Mesh Screen Analysis Queue | Contract/proof-shaped redacted-summary validation and ordered phase definitions exist, but the public arbitrary-string input and in-memory `without_owner_handlers()` publisher are not production authority. | **Blocked / source-incomplete** | `agent-service` startup has no household-mesh producer/consumer and row-ready is explicit `RuntimeOwnerUnavailable`; claim/lease IDs are synthesized in process-local state, and no authenticated durable provider transfer, owner-issued result receipt, restart/replay custody, or complete screen-specific startup composition exists. AI WP07/WP08/WP32 provide contract/boundary ownership, Eventing WP10 is blocked on LAN WP26, and no indexed owner supplies the missing composition. |
...
| WP08 Parent Runtime Integration | Existing tracking, policy, child-domain, and enforcement helpers remain, and the service now has a narrow typed parent-intent marker route. Malformed input is rejected; valid input is manual-required with journal, publication, event id, and child transport all unclaimed. | **Source hardened / functionally blocked** | No authenticated session producer, dependency-backed parent-runtime composer, canonical Tracking/Policy consumer dispatch, or child receipt/replay path exists. Planned source is `crates/parent-runtime-core/src/parent_runtime_intent_ingress.rs`; the service-security and parent-runtime integration test roots are absent. |
...
| WP05 Export Import Backup Recovery | The fail-closed schema/storage/parent-runtime packet now covers import integrity, backup schedule/job state, restore/migration ledgers, restart reconciliation, rollback validation, and Eventing/outbox seams. | **Base source accepted; composition/tests blocked** | No production caller implements the Account/provider/key/producer ports; all five expected runtime test roots are absent; blocked-restore source reports false/false while a stale test expects true/true; and the sealed import-custody port needs an owner-side adapter. Continue Account WP05 -> Data WP09 -> WP10 -> WP11 -> caller before the complete test wave. |
| WP06 Report Query Custody | Account-authority-derived opaque Rust query-source custody, request/row validators, pagination/continuity enforcement, and generated TypeScript contract/rules are integrated. | **Source accepted; expected tests open** | No shipped report/query consumer reaches the shared boundary. `crates/schema/tests/contract/report_query_custody.rs`, `crates/schema/tests/contract/report_query_custody_generated.rs`, and `crates/storage-custody-core/tests/unit/report_query_custody.rs` are stale against the current API/canonical sample; `packages/schema-domain/tests/contract/report-query-custody.test.ts` is expected but unwritten. Downstream report/notification/AI/portal execution stays adjacent-owned. |
...
**Data Custody source-wave result:** all 11 workpacks retain reviewed ownership,
but no DONE claim follows. WP02/WP03/WP04 Rust source is accepted, WP06's
Account-authority-derived Rust/generated custody edge is accepted with no
runtime consumer, WP05 retains backup/migration source gaps, WP08 retains its
confirmation-authority gap, WP09/WP10 remain explicit no-source runtime routes,
and WP07 retains trusted composition/external-caller gaps. WP06's stale Rust
harnesses and unwritten TypeScript contract test remain expected-test debt.
Expected-test source, focused execution, proof, precommit, CI, and PR remain
deliberately later phases. No Phase 2 passing-test or Phase 3 proof claim is
inferred from this ownership audit.
...
#### Account WP03 production source overlay - 2026-08-18
...
No test source was added or executed in the source-only packet. The provider
caller consumes Cloudflare WP06's final discriminated verification result and
the browser-session route re-resolves current Account authority from D1 when no
provider token is present. WP06 mutation readiness remains parameterless and
manual-required; no caller-minted authority or migration/deployment claim is
inferred.
...
| WP06 Storage DO D1 KV R2 Queue Bindings | Env/config ownership, billing D1/KV/R2 paths, binding-specific Account migration, D1 read/current-authority adapters, Account-owned create/CAS/revoke source, verified-provider current-authority resolution, and canonical `da84e6ee3`'s bounded JSON decoder/private one-shot Account producer verifier are source-present. | **Private verifier source accepted; runtime/tests blocked** | Account still provides no durable signer/public-key registry or authenticated Cloudflare service binding, so mutation composition is unmounted/manual-required. All six expected authority tests, migration execution, retained proof, deployment, capability/lease/step-up authority, and DONE remain open. Account DO/KV remain absent, and billing/referral DO idempotency still lacks restart-durable storage proof. |
...
| WP01 Product Pricing Entitlement | `entitlement-core` owns checked base-child plus active-referral plus paid-seat derivation, rejects zero base/overflow, and emits an explicitly unsigned projection; Rust schema/generated-edge contracts remain non-authoritative consumers. | **Production arithmetic source present; runtime ownership and expected tests incomplete** | No non-test caller, durable provider-owned billing/referral ledger, Account/Device-Trust composition, or real issuer/signer reaches the derivation. The focused derivation/negative test root and current proof root are absent; do not recreate `packages/billing-domain` or infer paid access from the unsigned projection. |
...
| Workpack | Reviewed live code/test evidence | Phase 1 | Concrete code/test gap |
| --- | --- | --- | --- |
| WP01 Device Trust Source Of Truth | Thirty-eight mapped implementation files and twelve existing tests cover the lifecycle repository, transition journal/sidecar generations, current device/signer binding, parent-presence custody, restart/replay, schema/path integrity, opaque references, and fail-closed Eventing delivery. The accepted continuation is integrated through `68717b5b7`; public household signer/verifier mint paths are removed and independent source review found no remaining internal P0/P1. | **Foundation/source accepted; expected tests and authority bridge open** | No shipped platform/passkey ceremony issuer or complete product composition owns bootstrap-through-revoke/reset/re-pair state. Existing tests predate parts of the accepted surface; the complete expected-test migration, focused validation, proof, caller integration, platform custody, Account WP08 -> target-aware Account WP02 -> Cloudflare WP06 current-authority bridge, and DONE remain open. |
| WP02 Local Key Sealing | Eighteen mapped implementation files expose Windows DPAPI/registry-epoch source, the current-authority and lifecycle-activation seams, an opaque parent-runtime handle facade, and a parent-desktop custody-sealing command; one mapped plan test is document-only. | **Incomplete / manual-required** | `require_authenticated_parent_authority()` is permanently unavailable before custody mutation. No ceremony issuer or custody-to-lifecycle startup composition reaches the command; its success is custody-sealed-only and does not activate the family lifecycle. Windows custody-open platform failures are unavailable, unsupported non-Windows startup is manual-required, and current lifecycle/custody tests and synthetic probes are stale and not DPAPI proof. Android, Linux, iOS, and macOS custody plus encrypted recovery fallback remain absent. |
| WP03 Parent Step-Up Auth | Twelve mapped implementation files and four tests cover signed action/household/device/target-bound receipts, atomic ceremony custody, linked lifecycle validation, restart reconciliation, expiry, replay, trust-epoch changes, tampering, and schema round trips. | **Blocked / not implementation-authorized** | The only production verifier is the fail-closed unavailable verifier. No passkey/WebAuthn, biometric, or OS-native ceremony acquires the proof, no runtime caller consumes it as a live high-risk action boundary, and no durable one-shot replay owner exists. The reviewed edges do not authorize the bounded WP03 source packet until Cloudflare WP06 has reviewed authoritative caller/writer evidence. Account WP02 alone is the current authority-chain implementation entry; the route provides no ceremony authority, tests, proof, runtime reachability, or completion. |
| WP04 Phone QR Approval Bridge | Two mapped production Rust modules expose typed QR challenge/response contracts plus pre-verifier binding/time/replay-shape validation; the sole mapped test asserts wording in the plan model. | **Blocked / incomplete** | No production QR issuer, phone ceremony, signature verifier, nonce/challenge consumer, desktop/phone transport, or currentness/action consumer is composed. The verifier remains unavailable by default, `Fresh` is an untrusted response claim until nonce consumption, the mapped test is document-only, and the proof root is absent. |
| WP05 Entitlement Device License | Twenty-eight mapped production files cover the signed snapshot transport/issuer boundary, strict key/signature/account/household/device/package/currentness/revocation verification, read-only signed revocation cache, and the child-runtime preflight projection consumer; seven mapped tests include entitlement, schema/proof-shape, child-runtime, and plan-local coverage. | **Source-repair present; runtime blocked / expected tests open** | Signing, pinned-key, installed-package, Account/Device currentness, trusted-time/grace, revocation-delivery, handle-safe cache-custody, and child entitlement action owners are absent. The child-runtime consumer only receives verifier-owned context through trusted internal construction; wire deserialization resets it unavailable, and no non-test caller reaches the preflight. Billing-core unsigned projections and generated schema/proof tests are not Device Trust authority; existing mapped tests are stale or document-only, and the WP05 proof root is absent. |
...
WP03 remains blocked in the default graph and is not implementation-authorized
until Cloudflare WP06 has reviewed authoritative caller/writer evidence. Account
WP02 alone is the current authority-chain implementation entry; normal
READY/DONE state is unchanged. The plan's real Windows custody, lifecycle,
step-up-proof, entitlement, recovery-contract, and tamper-status
...
| WP06 Child iOS Capability Package | Rust owns the capability contract and generated TypeScript; the Xcode capability-only app and simulator ZIP builder now use canonical Child identity, and existing smoke/workflow wiring follows that identity. | **Incomplete** | The expected child capability identity test is not written. Workflow execution, physical-device provisioning/signing, supervision, store distribution, and background execution remain open or manual-required. |
...
workpacks now have reviewed code/test ownership. WP01, WP02, WP05, and WP09
are complete for code/expected-test writing within their bounded route,
Windows-harness, debug-emulator, and matrix scopes. WP03, WP04, WP06,
WP07, WP08, WP10, and WP11 retain concrete identity-test, lifecycle-test, runtime,
...
| Data custody/storage (pre-WP11 historical row) | 11 | 1/3/3/0/4/0 | 974 | 482 |
...
| App/game | 220/220 reviewed maps; generic activity/read-model roots remain distinct from performance-health ownership. WP99 retains only bounded Rust parent-surface status source; WP100-WP102 have no current parent-domain package/test roots. | WP59 scheduler, WP60 metadata-history, and WP61 persisted provider-preflight bridges remain separate bounded work; WP100-WP102 are blocked/validation by absent package roots and reviewed source-order edges only. |
| App | `app-core`, `app-game-core`, `agent-protocol`, `agent-core`, `agent-service`, schema/generated contracts, parent runtime, and portal remain mapped. App WP26 performance-health source/proof/test roots are absent and remain blocked behind AppGame WP27's validation-only generic observation roots. | No performance-health ownership or proof is inferred from generic activity/read-model code. See `docs/plans/app-plan/CODE_AUDIT.md`; Phase 2 tests, Enforcer, proof, and release acceptance remain separate. |
| Browser | `browser-core` plus agent core/protocol/service policy and adapter surfaces. | Inventory/status source exists, but WP06 storage deliberately fails closed; protected profile custody, a real caller, WP07 owner-bound launch, adapter result, rollback, and parent-visible managed state are not a closed product path. |
...
| Device trust WP02 Windows local custody | [#623](https://github.com/ocentra/OcentraParent/pull/623) merged as `46bb53da4d0dfbdd8d1b40937abfd67262aac8c3`, but the current integration truth is source-only: `require_authenticated_parent_authority()` is permanently unavailable before mutation. | Historical CI did not prove a reachable DPAPI path. No current desktop/native mount, ceremony issuer, custody-to-lifecycle startup caller, or fresh runtime proof is claimed; stale lifecycle/custody tests are not DPAPI evidence. | Windows source remains manual-required. Cross-platform custody, recovery/reset/re-pair, QR approval, and child tamper/uninstall proof remain open. |
...
| Device Trust WP02 local key sealing | Superseded historical row: the current integration contains Windows DPAPI/registry source, current-authority/lifecycle seams, and an opaque parent-runtime facade, while `require_authenticated_parent_authority()` remains permanently unavailable. | Blocked/manual-required. The old PR/CI and synthetic parent-presence probes do not prove DPAPI, desktop dispatch, lifecycle activation, or revocation; the required proof root remains absent. | Select the conditional platform route only after a real ceremony issuer, desktop/native mount, custody-to-lifecycle startup caller, and fresh Windows runtime proof are owned; otherwise keep Device Trust WP02 out of the default Account WP08 -> target-aware Account WP02 -> Cloudflare WP06 -> WP03 -> LAN/child route. |
...
| --- | --- | --- | --- | --- | --- |
| `account-identity-family-plan` | Accepted multi-workpack source / Phase 1 incomplete | `family-identity-core`, `schema`, `schema-domain`, `infra/cloudflare`, provisioning and billing consumers | All eight workpacks are mapped at 207 implementation / 82 test files. Accepted source through `e69acf279` adds strict schema validation, a sealed current Account capability, durable repository/CAS/invariants, lifecycle records, D1 adapter and ordered `0001`-`0004` migrations, plus Account-bound billing/support consumers. The WP03 overlay adds the browser-session migrations/store/routes and final WP06 caller/verifier composition. | WP03 route/store/request-safety tests, migration execution, live D1/Worker runtime, and retained proof remain open. Invite/recovery orchestration, Device Trust/remote/export/delete handoffs, the remaining expected-test wave, and deployment remain open. | Finish the remaining source composition seams first; then write the WP03 route/store/request-safety/integration tests and the broader WP02-WP05/WP08 expected-test wave, run focused tests/Enforcer, and only afterward regenerate Cloudflare WP08 and Account WP06 proof. |
...
| `app-plan` | Fully audited / Phase 1 incomplete | `app-core`, `app-game-core`, `agent-protocol`, `agent-core`, `agent-service`, schema/generated contracts, `parent-runtime-core`, portal-domain, portal | App WP26's four performance-health source/proof/test roots are absent; its reviewed-implementation dependency on AppGame WP27 remains explicit, while WP27 is validation-only over generic activity/read-model observation roots. | Performance-health source/tests/proof remain missing; generic app/game activity roots do not close that gap. | Implement the missing WP26 owner after the WP27 reviewed observation boundary is accepted; then write focused tests/proof. |
| `app-game-plan` | Accepted WP10 source / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, schema, tracked vendor portal surface, portal | WP16's cohesive dashboard and WP48/WP63 source-freshness panel are mounted by the production portal route; the earlier "no cohesive/rendered surface" gaps were false. WP99-WP101 are Rust-owned in the existing `app_game_source_gated_policy_preview_timer_followthrough` source and contract test. WP100/WP101's retired parent-domain roots were false gaps. WP102 is explicitly no-code/superseded because WP103 owns the real Rust protocol plus agent-service parent-surface read model and tests. | WP197's first Docker-host preflight source packet was independently rejected: mutable environment/config can redirect the daemon, executable identity is not retained across spawn, Unix unwind can leak the process group, unauthenticated LAN requests can trigger probes, and daemon text parsing is too permissive. WP16/WP48/WP63 tests, Game budgets, WP100/WP101 runtime/proof, and WP103 live linkage also remain open. | Repair and re-review WP197's real process/path/provenance/parser boundaries first. Continue the other production gaps without duplicating mounted portal source or recreating parent-domain; write the mapped test family only after the source wave. |
| `browser-plan` | Fully audited / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `browser-core`, schema-domain, portal-domain, portal, Android owned shell | All 30 imported packets have reviewed ownership. Thirteen executable workpacks have bounded code/expected tests written; contracts, Windows inventory, fail-closed managed-launch/CDP baselines, durable ingest/read models, policy manifest/compiler, and unmanaged detection are real. | WP06's first repair is rejected: plain JSON/path state is forgeable, custody is not retained, portable no-follow/TOCTOU coverage is incomplete, lifecycle mutation/recovery is inert, and no caller exists. Ten other workpacks still lack cross-platform inventory, active-focus evidence, portal tests, trusted action execution, live AppLocker, extension integration, health/load, or aggregate E2E. | Repair and independently re-review WP06's protected owner/handle/platform boundary first, then WP05/WP11 and WP17/WP19; write tests only after each source packet is accepted. |
| `child-agent-runtime-distribution-plan` | Source phase / Phase 1 incomplete | `child-runtime`, `child-runtime-android-bridge`, `agent-updater`, Rust schema/child-enforcement contracts, Android/iOS child apps, desktop package/service scripts, CI/release workflows | WP06's independent iOS project/product/bundle/release identity source packet is reviewed at `c71becbcfd4f07eb98a118f10dbf261320f6b54e`; capability contracts and package scaffolds exist. | WP06 tests, workflows, signing, device, and store proof remain open. WP10 still lacks the reviewed Device Trust WP01 startup binding, authenticated external ingress, and externally reachable health; its typed handoff has no production caller. WP08 has no Account-authority/platform cleanup caller or durable cleanup receipt path. WP09 does not consume the WP10 handoff. WP07 has only static manager declarations, and WP11 has no executable aggregate gate. WP02-WP05 retain trusted startup, health, lifecycle, and platform-callback gaps. | First source packet: integrate/review WP06's identity correction beside Device-Trust-WP01-gated WP10 startup/ingress/health/handoff composition. Then WP02-WP05, WP07, WP08, WP09, and WP11 in order; write the complete expected-test wave only after production source is present. |
| `cloudflare-control-plane-plan` | Fully audited / Phase 1 incomplete | `infra/cloudflare`, account/billing contracts, portal consumer boundary | All 13 workpacks have reviewed code/test ownership. WP02 now has independently accepted non-throwing production-origin validation at `7eabc9ff5`; WP05 has real Firebase subject verification plus a separate provider-webhook boundary; the accepted Account/Payment source adds WP06's current-authority D1 adapter and authority-bound consumers, and the Account WP03 overlay adds browser-session custody through the final WP06 caller/verifier seam. | WP02 still lacks its complete malformed-origin/wildcard/development test matrix. WP03 route/store/request-safety/integration tests, WP05 webhook/auth tests, migration execution, live D1/Worker execution, durable DO state, persisted local seeding, portal smoke, deployment/rollback automation, and proof remain open. Admin/support and family/device authority remain unavailable. | Finish remaining production runtime composition, then write WP02/WP03 plus WP05/WP06 expected tests before broad Cloudflare validation or proof regeneration; follow with focused module gates, Enforcer, retained proof, and payment handoff. |
| `data-custody-storage-plan` | Accepted multi-workpack source / Phase 1 incomplete | `schema`, `storage-custody-core`, `child-runtime`, `parent-runtime-core`, `ocentra-evidence`, `ocentra-eventing` | WP01's Rust-owned matrix, WP02 scope authority, WP03 manifest custody, WP04 child-owned durable tombstone/effect lifecycle, WP05 base backup/import contracts and ledgers plus the independently accepted rollback authority-binding validator, WP06's Account-issued authority-snapshot request/row boundary, all-seven-state and sealed-proof gates, source/proof page bounds, and Rust-generated TypeScript parity, and WP07 internal service command/recovery source are present. WP08 remains the confirmation-authority route. | The trusted Account/custody chain and private runtime adapters are absent. WP09 provider/byte capability, WP10 restore/producer orchestration, and WP11 parent-runtime composition/custody mount have explicit missing production and expected-test roots; WP05 still lacks concrete provider-neutral restore composition and five expected runtime tests. WP06 authority is snapshot-only, has no production report/query consumer or owner adapter, its three Rust harnesses need sealed-wrapper and boundary/page-size migration, the TypeScript contract test is absent, and no current proof/DONE exists. | Finish WP05 base obligations and external owner composition, then WP09 and WP10 in parallel, then WP11 composition. After all production source closes, write the complete expected-test delta, run focused crate/domain gates, repo-wide Enforcer, proof, precommit, and one PR/CI cycle. |
| `protected-capability-custody-foundation-plan` | Independently reviewed source integrated / Phase 1 incomplete | `protected-capability-custody-core`, `protected-capability-custody-protocol`, `protected-capability-custody-broker`, `protected-capability-custody-client`, `ocentra-protected-capability-custody-windows-ffi` | Active source provides the separate broker/client/protocol topology, bounded shared wire contract, narrow sealed core admission facade, startup preflight, dynamic enrolled-client pipe ACL construction, split immutable-enrollment/runtime registry custody, and the complete WP04 retained Windows process/token/service-SID anchor/transport packet. It fails closed before custody state or service availability when protected peer admission is unavailable. | External enrollment/SCM owner authority and protected registry/SCM provisioning/mutation, a non-restorable monotonic provider, owner-bound production caller, all expected tests, proof, and runtime availability remain absent. The private core/FFI path already verifies the exact enrolled registry owner, protected DACL/ACE set, and every pinned ancestor on open and revalidation. | Complete the dependency-owned enrollment/monotonic/package caller boundaries; then write the core, protocol, broker, and client expected tests against the real private seams before focused execution, repo-wide Enforcer, proof, precommit, one PR, CI, and promotion. |
| `device-trust-bootstrap-plan` | Accepted multi-workpack source / Phase 1 incomplete | `family-identity-core`, `storage-custody-core`, `parent-runtime-core`, `entitlement-core`, `child-runtime`, Android child bridge, `child-enforcement-core`, `schema`, parent desktop Tauri shell | Accepted source preserves owner-resolved current device/signer authority, unsigned entitlement projection, fail-closed restore authorization, durable trust-bound child removal/readiness, and the WP02 desktop opaque-handle command mount. The attempted WP01 runtime-fence participant was rejected and withdrawn because its same-user SQLite rows could mint committed outcomes, existing databases had no migration, and retention/startup scanning was unbounded. | Protected Device-owned receipt/key custody, a versioned migration, bounded retention/archive semantics, real Account WP05A participant composition, platform/passkey enrollment, entitlement issuer/revocation, encrypted restore executor/key custody, parent transport/platform removal, expected tests, and shipped composition remain open. No proof or DONE follows from source acceptance. | Finish the protected custody/migration/retention owners and remaining repository source wave first; then write the complete WP01/WP05/WP06/WP07 expected-test delta before focused crate/domain execution and Enforcer. |
...
| `lan-plan` | Fully audited / Phase 1 incomplete | `lan-core`, `agent-protocol`, `agent-service`, `parent-runtime-core`, parent desktop, portal-domain, portal | All 25 pre-existing workpacks have reviewed ownership, and new WP26 has an explicit routed owner boundary. Twenty-one rows have bounded code/expected tests written; WP07 now has accepted absolute-deadline runtime source but its expected timing/lifecycle tests remain open. | WP26 lacks the real signed-child ingress, durable receipt/idempotency custody, trust/route composition, and private Eventing WP10 handoff. WP07 lacks the shared-deadline, timeout-restoration, rotation, and cancellation test delta; WP16 lacks one integrated backend-to-Tauri-AppHandle-to-portal-listener regression. Six aggregate verifier commands are absent, and WP25 depends on those validation gaps; physical/manual artifacts remain Phase 3. | Keep source work on graph-authorized WP26; in the later test wave write WP07's full deadline/lifecycle family, add the WP16 integrated delivery regression, and restore or replace the six executable verifiers so WP25 can enter focused validation before physical proof. |
...
| `parent-client-runtime-distribution-plan` | WP03 shell source accepted; WP06 production source blocked / Phase 1 incomplete | Tauri parent desktop, hosted portal, `parent-runtime-core`, agent service/protocol, dev bridge, generated portal bridge, Android/iOS parent projects | WP03 retains a reachable desktop-shell source slice. WP06 has typed response identity/freshness validation, hard monotonic deadlines, serialized polling, and fail-closed dependency/LAN route state, but those checks run over an unauthenticated WebSocket and do not establish parent authority or readiness. Existing package and smoke foundations remain. | Account WP03 does not yet issue an opaque current/revocable parent-local bridge session capability or owner-bound handshake. Its provider-independent authority slice also needs decomposition from the current downstream Cloudflare/Protected/Parent cycle before the graph can record a valid hard edge. Agent-service accepts missing `Origin`, emits readiness before authentication, and can report health `Ready`/`Unauthenticated`; parent runtime uses caller-supplied `ws://` origin. Authenticated composition, the complete WP03/WP06 expected tests, historical-proof replacement, hosted authority, signing/store, updater/rollback, setup handoff, mobile/store proof, and aggregate release work remain open. | Decompose and review the Account parent-local authority slice first, then compose WP06 admission/readiness/health/route/action authentication as one source packet. Only after source acceptance write the full capability/origin/readiness/response/deadline/dependency test wave. |
...
| `browser-plan` | 24 | 0 | 24 | 0 | Fully mapped from live code/tests. Thirteen executable workpacks have no currently identified bounded Phase 1 writing gap; WP06's rejected owner/path/lifecycle packet moves it into the eleven rows with concrete source or expected-test gaps. Six additional imported packets are reference-only. |
| `child-agent-runtime-distribution-plan` | 11 | 0 | 11 | 0 | Source routing is reconciled against live code and the reviewed WP06 identity packet `c71becbcfd4f07eb98a118f10dbf261320f6b54e`: WP06 tests/workflows/signing/device/store proof remain open; WP10 has no trusted startup/ingress/health/updater caller; WP02-WP05, WP07-WP09, and WP11 retain source gaps. No expected-test source, focused execution, proof, or DONE is claimed. |
| `cloudflare-control-plane-plan` | 13 | 0 | 13 | 0 | Fully mapped from live source/tests. WP02's production-origin source defect is integrated, but its expected runtime-validation tests remain unwritten. WP05 includes the reviewed Firebase verifier source. WP06 remains a safe verified-provider read/manual runtime boundary, while Account WP03 maps browser-session custody through that final caller/verifier seam. WP03 tests, the missing Account WP02/WP08 producer transport plus Worker verifier, migration execution, proof, deployment, and runtime composition remain open. |
| `data-custody-storage-plan` | 9 | 0 | 9 | 0 | Current graph/topology keeps every executable workpack open. Source has advanced through WP05 base and WP06's all-seven-state sealed Rust/generated custody edge with source/proof page bounds, but WP06 authority remains snapshot-only and no report/query consumer, owner adapter, migrated Rust tests, TypeScript contract test, retained proof, or checklist acceptance exists. Trusted Account/custody composition and WP08 confirmation runtime are also incomplete; WP09 provider capability, WP10 restore/producer orchestration, and WP11 composition/custody mount remain explicit source-and-test gaps. |
| `protected-capability-custody-foundation-plan` | 1 | 0 | 1 | 0 | Independently reviewed core/protocol/broker/client/FFI source, manifests, targets, active workspace membership, and the narrow sealed facade are integrated. WP04's retained Windows process/token/service-SID anchor and fixed-pipe admission are source-present and focused host/Windows checks pass. The route remains validation/open because external enrollment/registry/monotonic/installer owners, an owner-bound caller, every expected test, runtime availability, proof, PR, merge, and DONE are absent. |
| `device-trust-bootstrap-plan` | 9 | 0 | 9 | 0 | Live source retains accepted WP01 lifecycle/current-binding plus WP05/WP06/WP07 packets, but the unsafe WP01 runtime-fence participant is withdrawn and the plan has zero graph-DONE workpacks. Protected Device-owned receipt custody, versioned migration, bounded retention, the production Account WP05A caller, expected participant test, and lifecycle test migration remain open. WP08 has bounded research/review only; external authority/composition, focused validation, proof, and completion remain open. |
...
| `lan-plan` | 26 | 13 | 13 | 0 | Twenty-six routed rows: 13 document-closed and 13 open. Five Phase 1 gaps remain explicit at WP07, WP16, WP20, WP25, and new WP26; WP07 has accepted source with expected deadline/lifecycle tests open, while WP26 is the signed-child beacon/household-mesh authority handoff required by Eventing WP10. |
...
| `parent-client-runtime-distribution-plan` | 11 | 5 | 6 | 0 | WP03 retains accepted shell/package source, but its live service readiness and WP06 are production-source blocked on Account WP03's missing opaque current/revocable parent-session authority and authenticated handshake. Historical WP06 proof and route-only tests do not close that gap. |
...
| `protected-capability-custody-core` + protocol/broker/client packages | Account WP05A, Device Trust WP01/WP03, Data Custody | The isolated-process topology, shared wire contract, narrow core facade, and WP04 retained Windows process/token/service-SID anchor are integrated source, but deliberately unavailable. Protected registry/monotonic authority, installer/SCM enrollment, an owner-bound caller, tests, and proof must exist before consumers can claim a protected operation. |
...
- WP06 is independent first source work. Its iOS project/product/bundle/release
  identity source packet is reviewed at `c71becbcfd4f07eb98a118f10dbf261320f6b54e`; tests, workflows, signing, device, and store proof remain open.
...
1. **Complete the account-authority runtime chain**: Firebase subject verification, Account WP08, target-aware Account WP02, and Cloudflare WP06's safe read/manual boundary now exist and fail closed. The exact missing seam is Account WP02/WP08 signed/sealed current-authority producer transport plus Worker verifier; no caller-scalar mutation API or higher authority composition is available. Migration, expected tests, retained proof, deployment, and full runtime reachability remain open. Supply that owner route before WP03's native parent authority, Cloudflare WP08 test proof, and Account WP06 aggregation.
...
| Source packet | Live code mapped | Honest remaining state |
| --- | --- | --- |
| Data Custody WP05 | `crates/parent-runtime-core/src/data_custody_restore_runtime_rollback_dispatch_validation.rs` | Rollback authority binding is real and independently accepted; concrete provider-neutral restore composition, Account/key/provider/producer owners, runtime reachability, five expected tests, focused validation, proof, and DONE remain open. |
| Cloudflare WP05 | `infra/cloudflare/src/auth/provider-webhook.ts` | Fake non-Stripe webhook verification is removed; Stripe verification is real, Razorpay/PayPal/Apple/Google remain manual-required/unavailable, admin/support authority remains unavailable, and tests/proof/deployment/runtime readiness remain open. |

This is a code/test topology refresh only. It does not promote either workpack
to READY or DONE and does not replace the later expected-test, Enforcer,
proof, precommit, CI, review, or merge gates.
...
| Protected capability custody | Partial/source accepted: WP01 four-package fail-closed core/protocol/broker/client and read-only preflight; WP04's 21-root fixed-pipe/client-anchor source is present. | WP02 external owner P0s remain absent; WP03 operational monotonic provider remains absent. WP04's complete retained process/token/service-SID anchor and fixed-pipe admission are source-present, with normal derived completion blocked. WP05 freezes self-contained TPM-native ECDSA P-256 v2: producer ocentra.account-authority-producer.v2; inner domain ocentra.account-authority-producer.signing.v2\\0; audience ocentra.account.authority.v2; algorithm ecdsa-p256-sha256-p1363; outer domain ocentra.account-issuer.transport.v2\\0; service ocentra.account-authority-producer.cloudflare.v2; algorithm-aware sha256:ecdsa-p256:<hex> IDs; canonical 65-byte SEC1; exact low-S 64-byte P1363. Protocol envelope kinds 6/7 are AccountIssuerRequest/AccountIssuerResponse carrying IssueCurrentAuthority/AcknowledgeReceipt operations; Verify is owner-local. Rust schema is crates/schema/src/account_identity_authority_producer_v2.rs; family retains its existing authority DTO and v1 historical parse/verifier path, not a v1 inner signature inside v2. Service-specific key custody is runtime-blocked: existing ACL is SYSTEM GenericAll, SCM has SID type only, while the WP04 TokenGroups and LookupAccountNameW service-SID observations are present; external provisioning creates/sets ACL and broker only revalidates. WP04 maps shared FFI/core observations and WP05 maps CNG descriptor revalidation. ring 0.17.14 verifies original bytes after explicit low-S precheck; sha2 is only for key-ID hashing; no p256/ecdsa dependency. Account SQLite remains authoritative key/receipt/outbox custody; D1 owns public currentness/CAS and inbound idempotency receipt only. Ed25519 v1 is verification-only history; unsupported TPM/manual enrollment fails closed. Attestation, rotation, recovery, provider binding, service-binding lineage repair, tests, proof, and DONE remain open. | Partial map, absent execution: WP01 source accepted; WP04 implementation roots are present; WP02 and WP03 split roots, WP05 Rust schema/owner/protocol/client/broker/FFI/facade/Cloudflare v2 roots, and the D1 .sql root are mapped, while all expected tests remain absent. | WP01 validation; WP02 operationally blocked/no source authorization; WP04 source-present but normal-blocked; WP05 source-authorable only with normal derived state blocked; no READY/DONE. |
...
| Parent desktop/runtime package | Partial: Tauri route commands, structural response checks, and provisioning/install-state contracts are reachable. | Account WP03 parent-session authority and WP06 authenticated WebSocket admission/readiness are absent; signed package/build/update/rollback, integrity verification, installer distribution, release callers, and WP12 package source also remain absent. | Partial/open; authenticated WP06 negatives and WP12 package test roots are absent. | WP06 blocked in production source; broader distribution/runtime ownership remains incomplete. |
