...
| `eventing-plan` | Integration | `ocentra-eventing`, `agent-protocol`, `agent-core`, `agent-service`, portal host bridge | WP06 journal/replay, topology, version-skew, and typed handoff surfaces are retained. WP09 Phase 1 production code and expected tests are written for exact-source ingestion, deterministic phase identity, recovered durable journaling, fail-closed recovery, reconciliation, projection-only reads, and real AI/portal direct-enforcement mutation negatives. Eight wrapped focused command families, 159 portal tests/type-check, changed-file Enforcer/architecture, normal pre-commit, accepted commits, and push pass; local ignored evidence is regenerated under the exact WP09 root. | WP09 whole-plan integration, CI, review, and merge remain open; rows 57-62 stay unchecked and Network WP04 stays locked. WP10 is blocked on LAN WP26's real ingress/custody/authority handoff. WP11 still needs its bounded type-safety implementation/tests; WP12 lacks its harness/root and WP13 lacks current validation/proof. | Run WP11 and LAN WP26 as the current READY code packets. Then finish WP10, WP13, and WP12 in dependency order while keeping WP09 integration acceptance honest. |
...
| `cloudflare-control-plane-plan` | 13 | 0 | 13 | 0 | Fully mapped from live source/tests. WP00-WP02 and WP04 are Phase 1 complete for code/expected-test writing; WP06 has a bounded source adapter/auth chain accepted without test/proof closure, and WP03 plus WP05/WP07-WP12 have concrete Phase 1 gaps. |
...
| `eventing-plan` | 6 | 1 | 5 | 0 | Six selectable workpacks: WP06 is the one doc-closed row; WP09-WP13 are open. WP09 awaits integration/CI/review/merge, WP10 is blocked on LAN WP26, WP11 is implementation-ready but incomplete, WP12 lacks its harness/root, and WP13 lacks current validation/proof. Seven historical rows are excluded and must not be rescheduled. |
...
1. **Complete the account-authority runtime chain**: Account WP08's bounded `v0.7` source and Cloudflare WP06's bounded durable adapter/auth source are independently accepted, but provider verification, migration, focused tests, retained proof, and runtime reachability remain open. Supply those seams before WP03's native parent authority, Cloudflare WP08 test proof, and Account WP06 aggregation. Do not restore removed TypeScript authority ownership or revive #604.
