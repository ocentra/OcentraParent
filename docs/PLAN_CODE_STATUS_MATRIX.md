...
retained proof exists. The graph maps WP01 as `planned` with implementation-only
authorization. The root Cargo metadata records staged protocol/broker/client
package registration, while real manifests/targets and the narrow core facade
remain implementation obligations. This does not create normal READY, runtime,
...
| `protected-capability-custody-foundation-plan` | 1 | 0 | 1 | 0 | Existing core source is mapped, but the staged protocol/broker/client manifests/targets, narrow core facade, and complete package-owned expected tests are absent. Graph state is planned with implementation-only authorization; normal READY, proof, PR, merge, and DONE remain open. |
...
| `protected-capability-custody-core` + staged protocol/broker/client packages | Account WP05A, Device Trust WP01/WP03, Data Custody | Neutral protected-capability custody is a source-order foundation only. The current core is a checked SQLite replica boundary; a real isolated authenticated Windows broker/client, shared wire contract, and narrow core facade must exist before consumers can claim protected operation. |
