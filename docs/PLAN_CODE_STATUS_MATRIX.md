...
| `device-trust-bootstrap-plan` | Accepted multi-workpack source / Phase 1 incomplete | `family-identity-core`, `storage-custody-core`, `parent-runtime-core`, `entitlement-core`, `child-runtime`, Android child bridge, `child-enforcement-core`, `schema`, parent desktop Tauri shell | Accepted source through `f5974c795` preserves owner-resolved current device/signer authority, adds the private durable action/target/signer/generation-bound WP01 runtime-fence participant with fail-closed recovery, and retains unsigned entitlement projection, fail-closed restore authorization, durable trust-bound child removal/readiness, and the WP02 desktop opaque-handle command mount. Public/caller-minted household, participant-handle, and restore authority paths are absent. | Real Account WP05A participant composition, participant schema-migration/startup ownership, platform/passkey enrollment, mounted ceremony issuer, entitlement issuer/revocation, encrypted restore executor/key custody, parent transport/platform removal, expected tests, and shipped composition remain open. No proof or DONE follows from source acceptance. | Finish the repository source wave and real participant caller/composition first; then write the complete WP01/WP05/WP06/WP07 expected-test delta before focused crate/domain execution and Enforcer. |
...
| `device-trust-bootstrap-plan` | 9 | 0 | 9 | 0 | Live source now includes accepted WP01/WP05/WP06/WP07 packets, with WP01's private durable runtime-fence participant integrated through `f5974c795`, but the plan has zero graph-DONE workpacks. The production Account WP05A caller, migration/startup ownership, expected participant test and lifecycle test migration remain open. WP08 has bounded research/review only; external authority/composition, focused validation, proof, and completion remain open. |
