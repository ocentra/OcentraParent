...
| `cloudflare-control-plane-plan` | 13 | 0 | 13 | 0 | Fully mapped from live source/tests. WP05 includes the reviewed Firebase verifier source. WP06 is a safe verified-provider read/manual runtime boundary: the verifier reaches current-authority resolution, writer mutations remain gated by an unconstructible AccountOwnedAuthorityProducer, and no caller-scalar mutation API exists. The exact missing Account WP02/WP08 signed/sealed producer transport plus Worker verifier, migration execution, complete expected tests, proof, deployment, and runtime composition remain open; the other runtime/deployment workpacks retain concrete Phase 1 gaps. |
