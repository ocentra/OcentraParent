...
| WP02 Local Key Sealing | Sixteen mapped implementation files expose Windows DPAPI/registry-epoch source, the current-authority and lifecycle-activation seams, and an opaque parent-runtime handle facade; one mapped plan test is document-only. | **Incomplete / manual-required** | `require_authenticated_parent_authority()` is permanently unavailable before custody mutation. No ceremony issuer, desktop/native command mount, or custody-to-lifecycle startup composition exists; current lifecycle/custody tests and synthetic probes are stale and not DPAPI proof. Android, Linux, iOS, and macOS custody plus encrypted recovery fallback remain absent. |
