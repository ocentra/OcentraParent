...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| Account identity/family | 8 | 0 | 6 | 0 | 0 | 2 | 0 | WP02 target-aware source is reviewed: opaque current Account authority keeps the parent actor separate from the target child/profile/device and the storage-custody consumer uses it. Cloudflare provider composition remains absent. | Actor/target mismatch and substitution for all affected actions; caller-trust rejection; correct parent `ViewChildStatus`; reload/concurrent transition and audited support cases. | Sealed WP08 authority, local SQLite repository/CAS, and the bounded target-aware resolver are real; Cloudflare has a read adapter, but no shipped provider-to-authority path exists. | Write the complete WP02 expected-test packet later; the next production source is Cloudflare WP06 authoritative writer/currentness/revocation/CAS plus provider caller. |
| Cloudflare control plane | 13 | 9 | 3 | 0 | 0 | 1 | 0 | WP06 lacks `account-identity-authority-writer.ts` and `account-identity-authority-caller.ts`: no authoritative create/update/revoke/currentness/CAS or provider-to-sealed-authority composition. | Migration/write/CAS concurrency, stale generation, revocation, provider/account mismatch, restart, caller reachability, and manual-required configuration cases. | Firebase verification, ordered migrations, and the D1 read adapter exist; Account authority remains `503` / `manual-required` and no authoritative writer is reachable. | After WP02 review, land the writer/currentness/revocation/CAS owner and shipped Firebase/provider caller as one WP06 source packet. |
...
| Cloudflare control plane | 13 | 9/0/1/3/0 | 37/72 | 9/3/0/0/1/0 | Firebase verifier/read source and target-aware Account WP02 identity source are retained. WP06 implementation-only source is authorized for the missing authoritative Account D1 writer/currentness/revocation/CAS and shipped provider caller; higher authority, tests, proof, migration execution, deployment, and DONE remain open. |
...
| WP06 Storage DO D1 KV R2 Queue Bindings | Env/config ownership, billing D1/KV/R2 read-model code, Worker DO/queue paths, and five existing tests cover binding names, privacy constraints, seed/read behavior, idempotent writes, enqueue failure, and dead-letter capture. The independently accepted source packet adds the binding-specific account migration, a canonical Account WP08 `v0.7` D1 read adapter, the mounted Firebase verifier source, production rejection of local fixture auth and missing queue secrets, and deterministic schema-contract build order. | **Incomplete** | The mounted Firebase verifier authenticates the provider subject but does not supply canonical household/device authority. The authoritative D1 writer/currentness/revocation/CAS path and provider-to-Rust sealed-authority composition remain missing; the account migration has not run, and the new read adapter has no focused tests or retained proof. Account DO/KV remain absent, and `BillingControlDO`/`ReferralControlDO` still keep idempotency in an in-memory `Map` rather than `DurableObjectState.storage`, so restart durability is not implemented or tested. |
