...
| `app-game-plan` | Accepted WP10 source / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, schema, portal | WP99-WP101 are Rust-owned in the existing `app_game_source_gated_policy_preview_timer_followthrough` source and contract test. WP100/WP101's retired parent-domain roots were false gaps. WP102 is explicitly no-code/superseded because WP103 owns the real Rust protocol plus agent-service parent-surface read model and tests. | WP100/WP101 focused execution, proof, product runtime, and parent rendering remain open. WP103 execution/proof and the live linkage from the bounded contract chain into product runtime remain open. | Continue the remaining real AppGame code/test gaps; do not recreate parent-domain or add a dead WP102 builder. Run mapped focused tests only in the later test phase. |
