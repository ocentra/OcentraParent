...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| Account identity/family | 8 | 0 | 6 | 0 | 0 | 2 | 0 | WP02 lacks `account_identity_target_authority.rs`: actor parent-controller and target child/profile/device are conflated; `same_family`, capability, and lease are caller facts; parent `ViewChildStatus` maps through child-device scope. | Actor/target mismatch and substitution for all affected actions; caller-trust rejection; correct parent `ViewChildStatus`; reload/concurrent transition and audited support cases. | Sealed WP08 authority and local SQLite repository/CAS are real; Cloudflare has a read adapter, but no shipped provider-to-target-aware authority path exists. | Implement the complete WP02 target-aware resolver behind the sealed WP08 boundary; no Cloudflare or Device code in that packet. |
...
| --- | ---: | ---: | ---: | ---: | --- |
| Account identity/family | 8 | 7/0/0/1/0 | 104/32 | 0/6/0/0/2/0 | WP02 is the first implementation-only packet: target-aware action authority is missing; sealed/local repository source remains, while Cloudflare composition, expected tests, proof, and DONE stay open. |
...
| Cloudflare control plane | 13 | 9/0/1/3/0 | 37/72 | 9/3/0/0/1/0 | Firebase verifier/read source is retained. WP06 is blocked behind target-aware Account WP02 and lacks authoritative Account D1 writer/currentness/revocation/CAS plus the shipped provider caller; tests, proof, migration execution, deployment, and DONE remain open. |
...
| --- | ---: | --- | --- | --- |
| Account identity/family | 8 | Family/setup/provisioning helpers remain reachable. Accepted source through `e69acf279` adds Rust-owned schema validation, sealed Account capability, local SQLite repository/CAS, session and invite/recovery records, Cloudflare D1 read adapter/migrations, and Account-bound billing/support consumers. | WP02 still conflates actor parent-controller and target child/profile/device for Pair/Register/Revoke/View/ChangePolicy/Remote actions; target-aware resolution, server-derived same-family/capability/lease/step-up, correct parent `ViewChildStatus`, provider composition, and expected tests remain unresolved. | WP02 is the first implementation-only source packet. Cloudflare/runtime composition, tests, proof, and DONE remain open. |
...
| Cloudflare control plane | 13 | Worker route dispatcher reaches billing/admin/webhook boundaries; Firebase verification source, Account migrations, and the D1 read adapter exist. | WP06 lacks authoritative Account D1 create/update/revoke/currentness/CAS and the shipped provider-to-target-aware sealed-authority caller. The migration is unapplied, production configuration remains manual-required, and expected tests/proof/deployment are open. | WP06 is blocked behind Account WP02; read-source presence is not runtime authority or DONE. |
