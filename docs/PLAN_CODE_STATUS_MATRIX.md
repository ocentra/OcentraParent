...
| Device trust bootstrap | 9 | 6/0/3/0/0 | 86/41 | 0/2/1/0/6/0 | WP01 repository source is independently accepted and integrated through `d8cd66ae9`; expected-test migration, shipped ceremony/platform custody, proof, and completion remain open. WP03's bounded custody/recovery source is implementation-only authorized against WP01, Account WP08, and Cloudflare WP06, while normal readiness and runtime authority remain blocked. |
...
| Device trust bootstrap | 9 | Typed challenge/presence/identity contracts and local bootstrap state are reachable in bounded flows. WP01's strict repository/source hardening is integrated and source-reviewed. | No shipped parent/device enrollment caller with sealed key custody, signed presence, revocation, and durable bootstrap handoff. Expected-test migration and functional validation remain open; WP03 still needs the real one-time ceremony. | Plan blocked; continue WP01 composition/test debt, then WP03 real one-time ceremony. WP02 is conditional only for demonstrated private-key/install custody. |
...
| --- | ---: | ---: | ---: | ---: | --- |
| Account identity/family | 8 | 0/6/0/0/2/0 | 190 / 82 | 8 / 8 | Fully code-mapped; WP08 is in validation and WP02 is implementation-authorized, while all eight retain Phase 1 production-source or expected-test gaps. |
...
| Data custody/storage | 9 | 1/0/0/1/7/0 | 690 / 408 | 9 / 9 | Fully code-mapped; WP04 and the source-only migrated UI reference are Phase 1 complete. Seven implementation workpacks remain incomplete. |
| Device trust bootstrap | 9 | 0/2/1/0/6/0 | 482 / 132 | 9 / 9 | Fully code-mapped; WP08 is complete for its research/test-only scope, while eight workpacks retain concrete code/test gaps. WP01's bounded repository source is integrated and source-reviewed but remains READY with expected tests and composition open. WP03's reviewed source topology includes atomic custody/recovery and lifecycle validation, but Account WP08, Cloudflare WP06, native authority, tests, proof, and caller integration remain open. |
...
| Logging domain parity | 10 | 5/0/0/0/5/0 | 131 / 49 | 10 / 10 | Fully code-mapped. Accepted source hardening now centralizes redaction, makes unsupported/reflection failures JSON-safe, routes the dev writer through the canonical owner, and closes realpath/symlink containment. WP01, WP04, and WP09 have no bounded writing gap; WP02, WP03, WP05-WP08, and WP10 still require expected-test or enforcement code. |
...
| Setup/install/provisioning | 7 | 0/1/0/0/6/0 | 573 / 198 | 7 / 7 | Fully code-mapped; six workpacks have no bounded Phase 1 writing gap. WP07 now has a reachable Rust-owned 15-row fail-closed setup matrix with evaluator/actions deliberately not run and LAN labelled observation-only; real authority inputs, the state machine/completion guard, and current expected tests remain open. |
...
in any implementation total. A live Cloudflare seam audit then proved WP06
cannot legally derive current member/role/device authority from the existing
crate-private WP08 port. The first source packet is therefore Account WP02's
sealed server-derived binding contract; Cloudflare WP06 follows with its durable
repository/runtime caller. The exact WP02-WP05 production and expected-test gaps
remain the rows above.
...
| Workpack | Reviewed live code/test evidence | Phase 1 | Concrete code/test gap |
| --- | --- | --- | --- |
| WP01 Device Trust Source Of Truth | Thirty-five mapped implementation files and twelve tests cover a typed durable lifecycle repository, platform-authority sidecar generations, pending/trusted/revoked/reset/re-pair transitions, redacted outbox events, explicit-path parent-presence custody, concurrency, restart/replay, schema/path integrity, opaque CSPRNG references, and fail-closed Eventing delivery. The five-file registration/lifecycle packet is integrated through `d8cd66ae9`, independently source-reviewed with no P0/P1, and passed focused source-format, architecture, Enforcer, diff, and guard checks. | **Source integrated; expected tests open** | The lifecycle and parent-presence APIs have no shipped production caller outside their crates/tests. Production parent-presence custody intentionally returns unavailable on every platform, and no live product composition owns bootstrap-through-revoke/reset/re-pair state. Expected-test migration, functional validation, proof, caller integration, platform custody, and full trust-source composition remain open. |
...
| --- | --- | --- | --- | --- | --- |
| `account-identity-family-plan` | Foundation | `family-identity-core`, `provisioning-core`, `entitlement-core`, `schema`, narrow `infra/cloudflare` D1 and Firebase adapters | Family identity has 13 source / 7 test files; setup and signed-entitlement paths exist. The integration branch now adds a bounded Firebase token/JWKS subject verifier with strict issuer/audience/time/claim checks and fail-closed cache expiry. It does not accept caller household/child/device headers as authority. | Account WP08 Rust schema/account-authority proof is merged. External subject verification source exists, but the Worker deliberately returns `manual-required`. A live seam audit proved Cloudflare WP06 cannot supply the missing context because the current WP08 cross-runtime handoff lacks current member/role/device-trust authority and its Rust read port is crate-private. Provider tests, durable migration, runner proof, deployment, and account runtime routes remain open. | First write Account WP02's sealed server-derived current binding over WP08 identities, then implement the Cloudflare WP06 durable adapter/runtime caller. After all source seams exist, write provider/repository/security tests, run Cloudflare WP08 proof, and aggregate Account WP06. Any missing input remains a payment/policy/remote/device-trust scheduling block. |
...
| `cloudflare-control-plane-plan` | Fully audited / Phase 1 incomplete | `infra/cloudflare`, account/billing contracts, portal consumer boundary | All 13 workpacks have reviewed code/test ownership. WP05 now owns a real Firebase subject-verification source packet: bounded JWT/JWKS parsing, WebCrypto RS256 verification, timeout/backoff, expired-cache rejection, and Worker mounting are present. | WP05 still lacks expected tests and cannot mint parent/family/device authority; it returns `manual-required`. WP06 cannot legally derive the missing binding until Account WP02 exposes a sealed current member/role/device contract. Durable DO state, account migration isolation, persisted local seeding, a true portal consumer smoke, deployment/rollback automation, and proof remain open. | Consume Account WP02's sealed binding in WP06, implement durable binding/migration ownership, then write WP05/WP06 expected tests before broad Cloudflare validation or proof regeneration; follow with focused module gates, Enforcer, retained proof, and the payment handoff. |
...
| `payment-subscription-plan` | Production source wave accepted / Phase 1 tests open | `billing-core`, `entitlement-core`, Rust billing schemas, Cloudflare Worker/DO, D1 provider mapping, portal consumer boundary | All 13 workpacks have reviewed ownership. The accepted source wave adds Account-composite provider identity, durable mutation leases/retry/outbox/CAS, stale cursor rejection, a fail-closed forward migration, and removes caller-minted signed entitlement authority. | Account WP02 migration ordering, real provider and entitlement issuer/verifier authority, regional/referral/parent/admin surfaces, stale signed-API tests, and the rollout verifier remain open. | Integrate Account authority, then write the complete Payment expected-test delta; only afterward run focused crate/domain validation. Proof and PR stay last. |
...
| `device-trust-bootstrap-plan` | 9 | 0 | 9 | 0 | Fully mapped from live code/tests. WP01's strict repository/current-signer source is independently accepted and integrated through `d8cd66ae9`, but expected-test migration and shipped composition remain open; WP01-WP07 and WP09 are still open, while WP08 is complete only for its bounded research/test scope. |
