...
| Network | 8 | Agent-service/agent-core discovery adapters and ActivityStore/network read models produce bounded visibility evidence. | The shipped read API currently invokes a proof builder that derives synthetic AI, policy, adapter, retention, deletion, and export references from an observation ID. No live analyzer-to-policy handoff, blocking adapter, durable rule authority, or provider/runtime execution exists. | Unsafe synthetic product-path claims remain; remove/fail closed before building the real chain. |
