...
| Device trust bootstrap | 9 | Typed challenge/presence/identity contracts and local bootstrap state are reachable in bounded flows. Accepted source through `68717b5b7` adds owner-resolved current binding, an unsigned entitlement projection, fail-closed restore authority, and durable trust-bound removal readiness. WP01 is foundation/source only. WP02's Windows DPAPI/registry source remains behind permanently unavailable `require_authenticated_parent_authority()` and has no mounted caller. | No shipped parent/device enrollment or passkey caller, authority issuer, entitlement issuer/revocation provider, restore executor, custody-to-lifecycle startup composition, parent transport, or platform removal owner. Current lifecycle/custody tests and synthetic probes are not DPAPI proof; the full expected-test wave and functional validation remain open. | Source checkpoint accepted; plan remains open/manual-required until Account WP08 -> Cloudflare WP06 current-authority, WP03 ceremony, conditional WP02 sealing/revocation, and post-WP03 LAN/child consumers are real and separately validated/proven. |
