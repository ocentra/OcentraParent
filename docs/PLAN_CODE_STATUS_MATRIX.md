...
| `data-custody-storage-plan` | Accepted multi-workpack source / Phase 1 incomplete | `schema`, `storage-custody-core`, `child-runtime`, `parent-runtime-core`, `ocentra-evidence`, `ocentra-eventing` | WP01's Rust-owned matrix, WP02 scope authority, WP03 manifest custody, WP04 child-owned durable tombstone/effect lifecycle, WP05 base backup/import contracts and ledgers, WP06's Account-authority-derived request/row boundary plus generated TypeScript edge, and WP07 internal service command/recovery source are present. WP08 remains the confirmation-authority route. | The trusted Account/custody chain and private runtime adapters are absent. WP09 provider/byte capability, WP10 restore/producer orchestration, and WP11 parent-runtime composition/custody mount have explicit missing production and expected-test roots; WP06 has no report/query consumer and its stale/unwritten expected tests remain open; no current proof/DONE. | Finish WP05 base obligations, then WP09 and WP10 in parallel, then WP11 composition. After all production source closes, write the complete expected-test delta, run focused crate/domain gates, repo-wide Enforcer, proof, precommit, and one PR/CI cycle. |
