...
retained proof exists. The graph maps WP01 as `validation` with
implementation-only authorization; this does not create normal READY, runtime,
...
| `protected-capability-custody-foundation-plan` | Source-only foundation route / implementation phase authorized | `protected-capability-custody-core`, future isolated Windows broker/client | Existing core source covers fail-closed binding, custody, path, platform, and checked-replica state. It is not externally constructible and has no broker/client, production caller, expected tests, or proof. | A separate authenticated broker process must own protected ACL/path/key/watermark/write-lease authority and opaque admission. In-process broker, same-process DPAPI, mutex/file-lock custody, caller attestation, fake/no-op adapters, normal READY, and DONE are forbidden. | Write the complete broker/client source first; then the seven expected tests, focused crate/platform/Enforcer gates, retained proof, repo-wide gate, one PR, CI, and promotion. |
...
| `protected-capability-custody-foundation-plan` | 1 | 0 | 1 | 0 | Existing core source is mapped, but the isolated broker/client source and all seven expected tests are absent. Graph state is validation with implementation-only authorization; normal READY, proof, PR, merge, and DONE remain open. |
...
| `protected-capability-custody-core` + future broker/client | Account WP05A, Device Trust WP01/WP03, Data Custody | Neutral protected-capability custody is a source-order foundation only. The current core is a checked SQLite replica boundary; a real isolated authenticated Windows broker/client must exist before consumers can claim protected operation. |
