...
| `app-game-plan` | Multi-packet source wave / Phase 1 incomplete | `agent-protocol`, `agent-core`, `agent-service`, `app-game-core`, `parent-runtime-core`, schema, tracked vendor portal surface, portal | WP16's cohesive dashboard and WP48/WP63 source-freshness panel are mounted by the production portal route; the earlier source gaps were false. WP99-WP101 are Rust-owned in the existing contract family, and WP102 is explicitly superseded by WP103's real Rust boundary. WP197 now has bounded, no-shell Docker CLI/daemon/context/image/container probes in the live platform-status service path with count-only parent rendering. | WP16/WP48/WP63 still lack focused intent/state/hostile-metadata tests, and Game budgets has no service-backed row. WP197's dedicated process-boundary test plus protocol/payload/parent fixture refresh, independent review, focused execution, and proof are open. WP100/WP101/WP103 runtime and proof gaps remain open. | Continue the seven remaining real AppGame production-source candidates without recreating parent-domain. Then write the complete mapped AppGame expected-test wave, run scoped tests/fixes and Enforcer, and defer retained proof until the source/test topology is complete. |
...
| `protected-capability-custody-foundation-plan` | Validation/source-only foundation route / implementation phase authorized | `protected-capability-custody-core`, graph-native protocol/broker/client workspace requirements | Existing core source covers fail-closed binding, custody, path, platform, and checked-replica state. It is not externally constructible. The required package manifests, active membership, targets, shared wire contract, narrow core admission facade, production caller, expected tests, and proof remain absent. | A separate authenticated broker process must own protected ACL/path/key/watermark/write-lease authority and opaque admission. In-process broker, same-process DPAPI, mutex/file-lock custody, caller attestation, fake/no-op adapters, normal READY, and DONE are forbidden. | Land real package topology and core facade first; then write the core-unit, protocol-contract, broker, and client expected tests, focused package/Enforcer gates, retained proof, repo-wide gate, one PR, CI, and promotion. |
...
| `protected-capability-custody-foundation-plan` | 1 | 0 | 1 | 0 | Existing core source is mapped, but the graph-required protocol/broker/client manifests, active membership, targets, narrow core facade, and complete package-owned expected tests are absent. Graph state is validation with implementation-only authorization; normal READY, proof, PR, merge, and DONE remain open. |
...
| `protected-capability-custody-core` + graph-required protocol/broker/client packages | Account WP05A, Device Trust WP01/WP03, Data Custody | Neutral protected-capability custody is a source-order foundation only. The current core is a checked SQLite replica boundary; a real isolated authenticated Windows broker/client, shared wire contract, and narrow core facade must exist before consumers can claim protected operation. |
