...
| `protected-capability-custody-foundation-plan` | Independently reviewed source integrated / Phase 1 incomplete | `protected-capability-custody-core`, `protected-capability-custody-protocol`, `protected-capability-custody-broker`, `protected-capability-custody-client`, `ocentra-protected-capability-custody-windows-ffi` | Active source provides the separate broker/client/protocol topology, bounded shared wire contract, narrow sealed core admission facade, startup preflight, dynamic enrolled-client pipe ACL construction, split immutable-enrollment/runtime registry custody, and the complete WP04 retained Windows process/token/service-SID anchor/transport packet. It fails closed before custody state or service availability when protected peer admission is unavailable. | External enrollment/SCM owner authority, exact registry owner/DACL/parent-chain verification, a non-restorable monotonic provider, owner-bound production caller, all expected tests, proof, and runtime availability remain absent. | Complete the dependency-owned enrollment/monotonic/package caller boundaries; then write the core, protocol, broker, and client expected tests against the real private seams before focused execution, repo-wide Enforcer, proof, precommit, one PR, CI, and promotion. |
