...
0002. What should happen to app activity?
...
0003. How should app management run on this device?
...
0004. How should installed app inventory be used?
...
0005. Which app inventory sources are allowed?
...
0006. Which identity fields may app rules use?
...
0007. What should happen when an app cannot be identified?
...
0008. Which runtime evidence sources may be used?
...
0009. What proof is enough for app rules?
...
0010. What if app proof is unavailable?
...
0011. Which duration should time budgets count?
...
0012. What must app controls never collect?
...
0013. What app targets should rules match?
...
0014. How strong must an app match be before strict action?
...
0015. Default rule for unknown apps?
...
0017. Default daily app time limit in minutes?
...
0018. What happens when app time runs out?
...
0019. Which app enforcement actions may run?
...
0020. What if a strict app action is unsupported?
...
0021. How long should the child get before strict action applies?
...
0022. Require rollback state for strict actions?
...
0023. How should app install and uninstall controls be handled?
...
0024. Which managed app lifecycle operations are allowed?
...
0025. Which app events require parent approval?
...
0026. What happens if the parent does not answer?
...
0027. Which app report fields should be visible?
...
0028. How long should raw app observations be kept?
...
0029. How long should app rollups be kept?
...
0030. Represent windows Win32 desktop app?
...
0031. Represent windows packaged app or Microsoft Store app?
...
0032. Represent macOS app bundle?
...
0033. Represent linux desktop app, package app, Flatpak, Snap, AppImage, or command-backed desktop entry?
...
0034. Represent android package?
...
0035. Represent iOS or iPadOS application selected through Apple-approved controls?
...
0036. Represent app installed by a supervised or managed device flow?
...
0037. Represent app allowlisted or denylisted by an OS application-control policy?
...
0038. Represent app represented by an opaque mobile platform token selected by a guardian?
...
0039. Represent app launched through an Ocentra-controlled shortcut, launcher, or policy adapter?
...
0040. Represent app process started by Ocentra and tracked with an owned process/session id?
...
0041. Represent a normal user-installed desktop app without Ocentra policy?
...
0042. Represent a portable executable?
...
0043. Represent a copied or renamed executable?
...
0044. Represent a helper process launched by a known app but not mapped to a supported app identity?
...
0045. Represent a mobile app that the platform does not expose through the approved parental, enterprise, or device-owner APIs?
...
0046. Represent an app running on an unsupported platform adapter?
...
0047. Represent package id, bundle id, AppUserModelID, package family name, desktop entry id, or application token?
...
0048. Represent executable path, file hash, publisher/signature, product name, or version?
...
0049. Represent process id and parent process id for a running observation?
...
0050. Represent window id and foreground state for active-use evidence?
...
0051. Represent installer/source reference and install state for inventory evidence?
...
0052. Represent app was observed?
...
0053. Represent app was running?
...
0054. Represent app was foreground-active where foreground proof exists?
...
0055. Represent app had a running or foreground duration inside a time window?
...
0056. Represent app identity, category, or unknown state was derived from specific evidence ids?
...
0060. Represent terminate owned or target process?
...
0061. Represent suspend, hide, shield, or block app where the platform supports it?
...
0062. Represent start, extend, expire, or roll back a time limit?
...
0063. Represent install or uninstall a managed app where the platform and custody model allow it?
...
0064. Configure inventory layer.
...
0068. Configure enforcement layer.
...
0070. Represent capability matrix row | Capability=Installed app inventory | Windows=Yes, partial by source | macOS=Yes, partial by source | Linux=Yes, partial by distro/desktop | Android=Yes, visibility-limited | iOS/iPadOS=Limited, token/MDM-managed paths | Required proof=Inventory adapter and source ids | Important limit=Inventory is not proof of current use?
...
0071. Represent capability matrix row | Capability=Package/process identity | Windows=Strong for observed processes/packages | macOS=Strong for bundles/processes | Linux=Varies by package and desktop entry | Android=Strong package id when visible | iOS/iPadOS=Opaque tokens or managed app metadata | Required proof=Identity fields plus confidence | Important limit=Renames, helpers, and wrappers reduce confidence?
...
0074. Represent capability matrix row | Capability=Running duration | Windows=Yes | macOS=Yes | Linux=Yes | Android=Usage-stat/session-dependent | iOS/iPadOS=Device Activity threshold-based | Required proof=Ordered observations/session model | Important limit=Gaps and restarts need stale handling?
...
0077. Represent capability matrix row | Capability=Launch app | Windows=Yes | macOS=Yes | Linux=Yes | Android=Yes, with package intents where allowed | iOS/iPadOS=Limited; open intents/managed flows | Required proof=Launch adapter result | Important limit=Launch does not imply ongoing control?
...
0081. Represent capability matrix row | Capability=Time-limit app use | Windows=Yes for app sessions and owned terminate | macOS=Possible with platform proof | Linux=Possible with platform proof | Android=Usage/DevicePolicy/Accessibility proof | iOS/iPadOS=Device Activity threshold/shield path | Required proof=Timer plus action/result audit | Important limit=Needs fallback when action cannot enforce?
...
0084. Represent capability matrix row | Capability=Child-facing message | Windows=Yes | macOS=Yes | Linux=Yes | Android=Yes | iOS/iPadOS=Shield UI where supported | Required proof=Local UI/notification/shield result | Important limit=Do not show parent diagnostics to child?
...
0085. Represent capability matrix row | Capability=Parent report | Windows=Yes | macOS=Yes | Linux=Yes | Android=Yes, if evidence exists | iOS/iPadOS=Yes, token/capability-limited | Required proof=Stored evidence and custody labels | Important limit=Reports must distinguish raw vs redacted fields?
...
0086. Represent capability matrix row | Capability=Audit/retention | Windows=Yes | macOS=Yes | Linux=Yes | Android=Yes | iOS/iPadOS=Yes | Required proof=Journal/query retention policy | Important limit=Local-first custody remains default?
...
0087. Represent app display name where safe?
...
0088. Represent package id, bundle id, package family name, desktop entry id, AppUserModelID, or app token?
...
0090. Represent version, publisher, signature, hash, install path, and executable path where available and policy permits?
...
0091. Represent category metadata from platform, catalog, desktop entry, app store metadata, or parent-maintained catalog?
...
0092. Represent install, update, uninstall, hidden, suspended, shielded, managed, unmanaged, unsupported, or permission-limited state where the platform exposes it?
...
0093. Represent it is partial on every platform?
...
0094. Represent it can miss portable apps, per-user installs, wrapped apps, web apps, and apps hidden by platform privacy?
...
0095. Represent it can report apps that are installed but never used?
...
0096. Represent mobile app lists can be package-visibility-limited, tokenized, supervised-only, or MDM-only?
...
0097. Represent inventory should never be used as proof of activity without runtime evidence?
...
0098. Represent process id, parent process id, executable path, process name, command-line handling status, user/session reference, and launch time where available?
...
0099. Represent publisher/signature/hash metadata where safe?
...
0100. Represent window id, title, active/foreground state, minimized/background state, and last foreground timestamp where available?
...
0101. Represent sessionization into running and foreground durations?
...
0102. Represent unknown, permission-limited, stale, unsupported, and adapter-error states?
...
0103. Represent process names can be renamed?
...
0104. Represent helper processes may not represent user-facing apps?
...
0105. Represent foreground window title may contain sensitive text and may be stale or misleading?
...
0106. Represent foreground proof does not reveal what happened inside the app?
...
0107. Represent background process duration is not the same as child attention?
...
0108. Represent elevated, protected, sandboxed, or cross-user processes can be unreadable or uncontrollable?
...
0110. How long was this app in foreground today?
...
0111. Did the time budget run out?
...
0112. Which evidence ids prove the count?
...
0123. Represent category is not content?
...
0124. Represent category confidence must be recorded?
...
0125. Represent parent rules decide actions. Category labels alone should not block?
...
0126. Represent some platforms expose categories as opaque tokens rather than raw identifiers?
...
0127. Represent unknown or ambiguous categories should degrade to observe, ask, or parent review according to explicit policy?
...
0128. Represent open an approved app?
...
0129. Represent relaunch a blocked/closed app later after a time budget resets?
...
0130. Represent open an app as part of an ask-parent approval?
...
0131. Represent prefer a managed app path or managed browser path for certain tasks?
...
0132. Represent launching an app does not guarantee it stays foreground?
...
0133. Represent launching an unmanaged app can move outside Ocentra control?
...
0134. Represent mobile launch behavior depends on platform foreground and intent rules?
...
0135. Represent stop an app after a block or time-limit decision?
...
0136. Represent stop an owned child process?
...
0137. Represent stop a target process when identity still matches the policy target?
...
0138. Represent record already-exited, target-changed, permission-limited, failed, or succeeded results?
...
0139. Represent termination can lose unsaved work?
...
0140. Represent target processes can exit and relaunch between detection and action?
...
0141. Represent parent/child UX should support grace periods, warnings, and ask-parent flows?
...
0142. Represent some platforms do not allow third-party apps to kill other apps?
...
0143. Represent protected, elevated, system, or different-user processes may be unavailable?
...
0144. Represent windows application control policy, AppLocker, WDAC/App Control for Business, or a narrower service adapter where proven?
...
0145. Represent macOS MDM profile, system extension, endpoint/security tooling, or managed app restriction where entitled and deployed?
...
0146. Represent linux policy, desktop/session integration, package-manager restriction, or service-level control where proven?
...
0147. Represent android DevicePolicyManager package hide/suspend, device owner/profile owner, managed configuration, or accessibility/VPN-adjacent UX where approved?
...
0148. Represent iOS/iPadOS Screen Time Family Controls, Managed Settings shields, Device Activity thresholds, or MDM restrictions where entitled/supervised?
...
0149. Represent broad app blocking is a privileged OS capability, not a normal UI toggle?
...
0150. Represent policy setup can require admin rights, device-owner enrollment, MDM, supervision, entitlements, app review, signing, or store distribution?
...
0151. Represent some systems support shielding/visibility restrictions rather than process termination?
...
0152. Represent platform APIs may expose opaque identifiers for privacy?
...
0153. Represent rollback and uninstall paths must be documented before strict policies ship?
...
0154. Represent app/session identity?
...
0155. Represent running or foreground duration proof?
...
0156. Represent schedule and budget state?
...
0157. Represent warning threshold and grace state?
...
0158. Represent parent approval or extension state?
...
0159. Represent enforcement fallback for unsupported action?
...
0160. Represent audit event for warning, timeout, action, failure, extension, and rollback?
...
0161. Represent a timer without action is report-only?
...
0162. Represent foreground time and running time should be separate settings?
...
0163. Represent cross-device time budgets need sync/custody rules?
...
0164. Represent mobile time-limit enforcement depends on platform-specific APIs?
...
0165. Represent windows MSI/MSIX/package manager or managed installer?
...
0166. Represent macOS installer/package/MDM managed app or declarative package management?
...
0167. Represent linux package manager, Flatpak, Snap, AppImage-managed wrapper, or desktop entry?
...
0168. Represent android package installer, device owner/profile owner, managed Play, or MDM?
...
0169. Represent iOS/iPadOS MDM managed app distribution and managed app removal?
...
0170. Represent ocentra must not remove personal apps unless a platform-approved managed path and parent/child custody model explicitly allow it?
...
0171. Represent store policies, signing, entitlements, user consent, device enrollment, and uninstall rights vary sharply?
...
0172. Represent install/uninstall actions must be audited separately from normal app observation?
...
0173. Represent known identity from package/bundle/app token?
...
0174. Represent known policy source?
...
0175. Represent install or update provenance?
...
0176. Represent known allowed/blocked/shielded state?
...
0177. Represent stronger app lifecycle action where the platform supports it?
...
0178. Represent running and foreground observation?
...
0179. Represent session duration?
...
0180. Represent category candidate?
...
0181. Represent ask-parent or warning events?
...
0182. Represent terminate where allowed?
...
0183. Represent report-only unknown or bypass state?
...
0185. Represent ask parent on first run?
...
0187. Represent count time under unknown-app budget?
...
0188. Represent block or terminate only when the parent selected that posture and the platform adapter can prove the action?
...
0189. Represent warn before time limit?
...
0190. Represent show time remaining?
...
0191. Represent show that parent approval is needed?
...
0192. Represent show whether an app is paused, shielded, blocked, or closed by parent policy?
...
0193. Represent offer ask-parent, request more time, or use allowed alternative where policy supports it?
...
0194. Represent hide parent diagnostics, evidence ids, adapter errors, hashes, and internal policy fields from the child surface?
...
0195. Represent installed/detectable app inventory?
...
0198. Represent recent sessions?
...
0199. Represent daily app/category rollups?
...
0200. Represent time budgets and remaining time?
...
0201. Represent unknown and permission-limited apps?
...
0202. Represent policy decisions?
...
0203. Represent enforcement actions and failures?
...
0204. Represent approval requests and parent responses?
...
0207. Represent capability state?
...
0209. Represent collection scope?
...
0210. Represent retention policy?
...
0211. Represent redaction status?
...
0212. Represent policy version and decision id when policy contributed?
...
0213. Represent adapter result id when enforcement contributed?
...
0214. Represent raw process/window evidence should be retained for the shortest useful local audit window?
...
0215. Represent daily rollups can be retained longer than raw observations if they are redacted?
...
0216. Represent exact executable paths and window titles may be sensitive and should have narrower retention and reveal controls?
...
0217. Represent ocentra-hosted storage is not the default child-activity store?
...
0218. Represent parent export and deletion must preserve audit integrity while respecting retention settings?
...
0219. Represent installed-app inventory from uninstall records, Start Menu shortcuts, Microsoft Store packages, known install paths, package query APIs, and executable metadata?
...
0220. Represent process enumeration and process metadata?
...
0221. Represent foreground-window observation?
...
0222. Represent running and foreground sessionization?
...
0223. Represent owned-process launch and termination?
...
0224. Represent narrow target process termination after typed policy decisions?
...
0225. Represent broad app control through AppLocker, WDAC/App Control for Business, managed installer policy, or equivalent only after explicit host proof?
...
0226. Represent package lifecycle actions through installer/package mechanisms where product setup owns the package?
...
0227. Represent appLocker/WDAC behavior depends on Windows edition, policy deployment, signing, administrator rights, audit/enforce mode, and reboot or refresh behavior?
...
0228. Represent microsoft Store package identity and Win32 executable identity are different evidence families?
...
0229. Represent blocking by path alone can be bypassed by copy/rename unless hash, signer, or managed installer proof is used?
...
0230. Represent the current roadmap distinguishes owned-process terminate and app time-limit proof from broad app blocking. Do not claim broad blocking until the adapter is proven?
...
0231. Represent application bundle inventory?
...
0232. Represent running process and window/frontmost app observation with the required permissions?
...
0233. Represent launch Services, bundle identifiers, code signing, and app metadata?
...
0234. Represent mDM managed app distribution and restrictions where enrolled?
...
0235. Represent system Extensions, Endpoint Security, or Network Extension paths where entitled and deployed?
...
0236. Represent managed browser/app controls through configuration profiles where supported?
...
0237. Represent accessibility, Screen Recording, Full Disk Access, Endpoint Security, Network Extension, and MDM posture change what is possible?
...
0238. Represent some controls require supervised or managed devices?
...
0239. Represent do not assume Windows process control maps directly to macOS?
...
0240. Represent desktop entries and menu categories?
...
0241. Represent package manager inventory?
...
0242. Represent flatpak, Snap, AppImage, or custom install metadata?
...
0243. Represent process observation through OS process tables?
...
0244. Represent foreground-window observation through X11, Wayland compositor protocols, or desktop-specific APIs where available?
...
0245. Represent process termination where permission permits?
...
0246. Represent policy controls through service, user session, package, desktop, firewall, or container mechanisms where proven?
...
0247. Represent wayland commonly restricts global window inspection compared with X11?
...
0248. Represent desktop entry category metadata is useful but not a complete app ontology?
...
0249. Represent package managers differ by distro?
...
0250. Represent broad app blocking should be treated as manual-required until a concrete adapter is proven on the target distro and desktop?
...
0251. Represent package inventory through PackageManager subject to package visibility rules?
...
0252. Represent usage events/statistics when the user grants Usage Access or the app has the required privileged context?
...
0253. Represent foreground visibility through UsageStatsManager or Accessibility where approved and enabled?
...
0254. Represent devicePolicyManager package hiding, suspension, permission policy, managed configuration, and package lifecycle control for device owner/profile owner contexts?
...
0255. Represent managed Play or MDM package installation/removal where deployed?
...
0256. Represent always-on VPN with lockdown for network mediation, separate from app foreground proof?
...
0257. Represent a normal Android app cannot generally control all other apps?
...
0258. Represent package visibility rules can hide installed apps from inventory queries?
...
0259. Represent usage access is permission-gated and can be revoked?
...
0260. Represent accessibility is sensitive and must not be used as a stealth content capture path?
...
0261. Represent device owner/profile owner changes the capability class and setup burden?
...
0262. Represent the roadmap currently treats Android package lifecycle proof as manual-required until real device artifacts exist?
...
0263. Configure screen time frameworks.
...
0264. Represent opaque selections for applications, categories, and web domains?
...
0265. Represent app/category shielding through Managed Settings?
...
0266. Represent threshold/event monitoring through Device Activity?
...
0267. Represent mDM managed app install/removal and restrictions for enrolled devices?
...
0268. Represent supervised-device restrictions where applicable?
...
0269. Represent third-party apps do not get a general raw list of every installed app for parental control?
...
0270. Represent screen Time APIs are privacy-preserving and token-based; tokens can be voided if authorization is revoked?
...
0271. Represent app shielding is not the same as process termination or raw app telemetry?
...
0272. Represent mDM and supervision determine app install/removal and restriction scope?
...
0273. Represent family Controls entitlements, App Store review, TestFlight, and runtime authorization are separate proof requirements?
...
0274. Represent the roadmap separates iOS signing/entitlements from TestFlight and runtime API entitlements; do not claim iOS control until each is proven?
...
0275. Represent detect installed or launchable apps where available?
...
0276. Represent show running and foreground state?
...
0277. Represent build running and foreground session durations?
...
0278. Represent show unknown, unsupported, stale, and permission-limited states?
...
0279. Represent produce reports without changing device behavior?
...
0281. Represent mobile device-owner/supervised setup?
...
0282. Represent broad application control?
...
0283. Represent guaranteed launch blocking?
...
0284. Represent install/uninstall control?
...
0285. Represent app-internal content knowledge?
...
0286. Represent matching app activity remains allowed temporarily?
...
0287. Represent the child sees a warning or ask-parent state?
...
0288. Represent the parent sees the app/session evidence and can approve, deny, or extend?
...
0289. Represent app/session evidence?
...
0290. Represent child-facing local UI or notification/shield path?
...
0291. Represent parent approval contract?
...
0292. Represent expiry and audit?
...
0293. Represent running or foreground time is counted against a parent budget?
...
0294. Represent warning and grace rules apply?
...
0295. Represent when the budget expires, the configured action runs if the adapter supports it?
...
0296. Represent sessionization from stored evidence?
...
0297. Represent timer recovery after restart?
...
0298. Represent policy decision and enforcement audit?
...
0299. Represent fallback when enforcement is unavailable?
...
0300. Represent parent rules prevent or interrupt app access?
...
0301. Represent on desktop this may mean launch block or process termination?
...
0302. Represent on mobile this may mean shield/hide/suspend through approved platform APIs?
...
0303. Represent platform-specific control proof?
...
0304. Represent parent exceptions?
...
0306. Represent audit for every strict action?
...
0308. Represent exact in-app activity knowledge?
...
0309. Represent parity across platforms?
...
0310. Represent install, update, uninstall, hide, or remove managed apps through a platform management boundary?
...
0311. Represent managed-device or package-management setup?
...
0312. Represent signing/store/MDM or installer proof?
...
0313. Represent custody model for personal vs managed apps?
...
0314. Represent explicit parent-visible state and audit?
...
0315. Represent personal device and family device expectations differ. The product should not imply corporate MDM behavior unless the device is actually managed that way?
...
0316. Represent app/game evidence contracts use stored local evidence and query/read models?
...
0317. Represent process/window evidence can support native app sessions, running time, and foreground time?
...
0318. Represent local AI can consume evidence references or structured digests; it does not scan the OS?
...
0319. Represent policy decisions must reference evidence and remain deterministic?
...
0320. Represent v0.8 enforcement work has typed contracts, capability status, timer/recovery, and audit scaffolding?
...
0321. Represent windows has proof direction for owned-process terminate and app time-limit behavior, but broad app blocking remains manual-required until a real adapter proves it?
...
0322. Represent android package lifecycle and iOS Screen Time/entitlement behavior are manual-required until real device/platform proof exists?
...
0323. Represent parent portal is an authoring and visibility surface. It does not run app inventory, timers, policy evaluation, or enforcement?
...
0324. Represent [`docs/architecture/app-game-evidence-sessions.md`](architecture/app-game-evidence-sessions.md)?
...
0325. Represent [`docs/expectations/app-game-evidence.md`](expectations/app-game-evidence.md)?
...
0326. Represent [`docs/expectations/policy.md`](expectations/policy.md)?
...
0327. Represent [`docs/expectations/enforcement.md`](expectations/enforcement.md)?
...
0328. Represent [`docs/product-roadmap.md`](product-roadmap.md)?
...
0329. Represent [`docs/managed-unmanaged-browser.md`](managed-unmanaged-browser.md)?
   ...
   - proofRequirement: none
   - sourceDocument: docs/app-control-capability-guide.md
   - sourceLine: 769; sourceText: [`docs/managed-unmanaged-browser.md`](managed-unmanaged-browser.md)
   - acceptedOptions: Represented | Not represented
   - helperText: app-control-capability-registry
...
0330. Represent show inventory, running, foreground, time-limit, install, uninstall, shield, suspend, block, and terminate as separate capability rows?
...
0331. Represent show the evidence source next to every app claim?
...
0332. Represent show exact package/process identity only when proof exists and retention allows it?
...
0333. Represent show unknown apps as unknown, not as risky by default?
...
0334. Represent show app category as a label with source/confidence, not as an automatic decision?
...
0335. Represent show app time limits only when duration evidence exists or when the rule is clearly marked as pending proof?
...
0336. Represent show strict actions as ready, unsupported, permission-required, manual-required, degraded, dry-run-only, adapter-error, or blocked-by-setup?
...
0337. Represent keep managed-device setup state close to mobile actions?
...
0338. Represent keep child-facing messages separate from parent diagnostics?
...
0339. Configure every strict action should have an audit path.
...
0341. Represent warn on app use;?
...
0342. Represent ask parent on app use;?
...
0343. Represent set app/category/unknown-app time budgets;?
...
0344. Represent close or terminate selected apps after a timer;?
...
0345. Represent shield, suspend, or block apps where the platform supports it;?
...
0346. Represent manage app installs/uninstalls only inside a platform-approved custody model?
