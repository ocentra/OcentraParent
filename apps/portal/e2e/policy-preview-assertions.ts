...
export async function assertPolicyPreviewBoundaryDetails(policyPreview: Locator): Promise<void> {
  ...
  await expect(detailValue(policyPreview, 'Adapter boundary')).toHaveText('local-adapter-unavailable');
  await expect(detailValue(policyPreview, 'Execution state')).toHaveText('disabled');
