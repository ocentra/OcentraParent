...
function usePortalBackgroundTunerControlState({
  config,
  onConfigChange,
  previewTheme,
  ...
  const updateConfig = (patch: Partial<PortalBackgroundConfig>): void => {
    onConfigChange({ ...config, ...patch });
  };
  const updateActivePalette = (patch: Partial<PortalBackgroundThemeColors>): void => {
  ...
  const updateBlobColor = (index: number, value: string): void => {
    const nextColors = [...config.blobColors] as [string, string, string, string, string, string];
    nextColors[index] = value;
    updateConfig({ blobColors: nextColors });
  };
  const updateBeamColor = (index: number, value: string): void => {
    const nextColors = [...config.beamColors] as [string, string, string];
    nextColors[index] = value;
    updateConfig({ beamColors: nextColors });
  };
  const selectJson = (): void => {
...
function PortalBackgroundTunerHeader({
  dirty,
  onOpenChange,
  open,
  status,
...
function PortalBackgroundTunerEditor({
  controls,
  dirty,
  onPreviewThemeChange,
  onResetAll,
