...
- Displays the tracking service read-model event, hosted child-safe check-in
  proof, hosted child-runtime UI disclosure/consent proof, and hosted
  unsupported/manual tracking platform state proof as narrow Policy Tracking
  route cards; this is not actual child-device delivery/runtime execution,
  physical-device proof, authority proof, or full parent/child tracking UI
  proof.
