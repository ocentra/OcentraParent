...
describe('parent portal app/game source freshness intent', () => {
  it('projects fresh and manual source rows into counts, evidence, and grouped sections', () => {
    ...
    expect(rowValues(dashboard, 'app-row-malicious-name')).toEqual({
      ...
      inventoryCount: 0, runningCount: 1, foregroundCount: 0, manualRequired: true, riskCandidate: true,
    });

    expect(dashboard.sourceStatusRows.map((row) => [row.parentLabel, row.sourceStatusKind, row.rowCount, row.evidenceCount]))
      .toContainEqual(['Study Timer', 'shortcut', 1, 1]);
    expect(dashboard.sourceStatusRows.map((row) => [row.parentLabel, row.sourceStatusLabel, row.lastObservedLabel]))
    ...
    expect(dashboard.sourcePanelSections.map((section) => section.subtitle)).toEqual([
      '3 fresh of 4 source rows; 1 manual-required', '1 fresh of 2 source rows; 1 manual-required',
    ]);
    ...
    log.logInfo('app-game source freshness intent projected service rows', getStackTrace(), {
      appGameRows: dashboard.rows.length, sourceRows: dashboard.sourceStatusRows.length, freshSources: 4,
    }, false);
  });
  ...
  it('fails closed when either service read model is absent or unavailable', () => {
    ...
    expect(metricPair(dashboard, 'Fresh sources')).toEqual(['Fresh sources', '0']);
    expect(metricPair(dashboard, 'Game budgets')).toEqual(['Game budgets', 'no game rows']);
    expect(metricPair(dashboard, 'Manual required')).toEqual(['Manual required', '0']);
  ...
  it('keeps unobserved and degraded source statuses reviewable instead of fresh', () => {
    const staleDashboard = createParentPortalAppGameDashboardIntent(
      sourceReadModel([sourceStatusRow('processSnapshot', 'ready', 1, null, 'ready', [])]), null
    );
    expect(sourceSectionValues(staleDashboard)).toEqual({
      title: 'App use sources', state: 'stale', tone: 'red', rowCount: 1, freshCount: 0, manualRequiredCount: 0, evidenceCount: 0,
    });
    expect(staleDashboard.sourcePanelSections[0]?.rows.map((row) => [
      row.sourceStatusLabel, row.freshnessLabel, row.lastObservedLabel,
    ])).toEqual([['Process Snapshot', 'Needs review', 'not observed']]);

    const degradedDashboard = createParentPortalAppGameDashboardIntent(
      sourceReadModel([sourceStatusRow('foregroundWindow', 'offline', 1, '2026-06-01T15:00:00Z', 'degraded', ['degraded-ref'])]), null
    );
    expect(sourceSectionValues(degradedDashboard)).toEqual({
      title: 'App use sources', state: 'manual-required', tone: 'gold', rowCount: 1, freshCount: 0, manualRequiredCount: 1, evidenceCount: 1,
    });
    expect(degradedDashboard.sourcePanelSections[0]?.rows.map((row) => [
      row.sourceStatusLabel, row.capabilityStatus, row.freshnessLabel,
    ])).toEqual([['Foreground Window', 'degraded', 'Needs review']]);
    log.logInfo('app-game source freshness intent retained review boundaries', getStackTrace(), {
      staleFreshSources: 0, degradedManualRequired: 1,
    }, false);
  });
});
...
function sourceSectionValues(dashboard: ParentPortalAppGameDashboardIntent): Record<string, unknown> {
  ...
  return {
    title: section.title, state: section.state, tone: section.tone, rowCount: section.rowCount,
    freshCount: section.freshCount, manualRequiredCount: section.manualRequiredCount, evidenceCount: section.evidenceCount,
  };
}
...
function appUseReadModel(): Record<string, unknown> {
  return { state: 'ready', rows: [
    appRow('app-row-study-timer', 'Study Timer', {
      inventoryState: 'installed', runtimeState: 'running', foregroundState: 'foreground', launchCount: 2,
      inventoryRowCount: 1, runningRowCount: 1, foregroundRowCount: 1, evidence: [evidence('app-ev-1'), evidence('app-ev-2')],
      sourceStatusRows: [sourceStatusRow('shortcut', 'ready', 1, '2026-06-01T15:00:00Z', 'available', ['app-source-1']), sourceStatusRow('processSnapshot', 'ready', 1, '2026-06-01T15:00:00Z', 'available', ['app-source-2']), sourceStatusRow('foregroundWindow', 'ready', 1, '2026-06-01T15:00:00Z', 'available', ['app-source-3'])],
    }),
...
function gamesReadModel(): Record<string, unknown> {
  return { state: 'ready', rows: [
    gameRow('game-row-launcher', 'Steam Launcher', {
      productKind: 'launcher', inventoryState: 'installed', capabilityStatus: 'manual-required', launcherRowCount: 1,
      runningRowCount: 1, evidence: [evidence('game-ev-1')], sourceStatusRows: [sourceStatusRow('launcherManifest', 'permission-required', 1, '2026-06-01T14:57:00Z', 'manual-required', ['game-source-1'])],
    }),
...
function sourceReadModel(sourceStatusRows: readonly Record<string, unknown>[]): Record<string, unknown> {
  return { state: 'ready', rows: [appRow('app-row-source-freshness', 'Source freshness sample', { sourceStatusRows })] };
}
...
function sourceStatusRow(sourceKind: string, state: string, rowCount: number, lastObservedAt: string | null, capabilityStatus: string, evidenceIds: readonly string[]): Record<string, unknown> {
