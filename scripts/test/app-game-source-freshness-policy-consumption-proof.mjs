...
const appGameProofDir = join(repoRoot, 'output', 'app-game-plan-proof', '48-source-freshness-policy-consumption');
const appProofDir = join(repoRoot, 'output', 'app-plan-proof', '48-source-freshness-policy-consumption');
const commands = [];
...
async function main() {
  ...
  const proof = {
    ...
    evidence: {
      ...
      appGameProofPack: 'output/app-game-plan-proof/48-source-freshness-policy-consumption',
      appProofPack: 'output/app-plan-proof/48-source-freshness-policy-consumption',
    },
    claimsProved: [
...
async function writeProofPack(proofDir, proof) {
  ...
  await writeText(
    ...
    [
      ...
      '- `docs/features/app-game-control.md` and app/app-game plan WP47 docs for remaining source freshness gaps.',
      '',
  ...
  await writeText(
    ...
    'Not applicable: WP48 is a parent-domain policy-readiness contract proof and does not add Rust protocol fields.\n'
  );
  await writeJson(join(proofDir, '03-runtime-evidence.json'), {
    schemaVersion: 1,
    runtimeEvidenceClaim: 'not-applicable',
    reason: 'WP48 consumes already-projected sourceStatusRows and does not add runtime capture or service polling.',
  });
  await writeJson(join(proofDir, '04-journal-sqlite-proof.json'), {
    schemaVersion: 1,
    journalSqliteClaim: 'not-applicable',
    reason: 'WP48 consumes backend read-model rows from WP47 and does not add journal or SQLite storage.',
  });
  ...
  await writeText(join(proofDir, '07-playwright-ui-proof.log'), 'Not applicable: no UI files changed in WP48.\n');
  await writeText(
  ...
  await writeText(
    ...
    '# Manual Platform Proof\n\nNot applicable: WP48 does not claim platform adapter support or hard-control behavior.\n'
  );
  await writeText(join(proofDir, '10-validation-commands.log'), `${proof.commands.join('\n')}\n`);
  await writeText(
    join(proofDir, '11-authority-tier-proof.md'),
    '# Authority Tier Proof\n\nNot applicable: WP48 does not change platform authority tiers.\n'
  );
  await writeText(
    join(proofDir, '12-rollback-proof.md'),
    '# Rollback Proof\n\nNot applicable: WP48 does not dispatch adapters or mutate child-device policy state.\n'
  );
}
