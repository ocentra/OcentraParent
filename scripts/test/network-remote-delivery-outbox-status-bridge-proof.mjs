...
const expectedStatus = {
  ...
  parserInvariants: [
    ...
    'Rust service serializes row10g outbox status into the protocol-owned payload field',
...
const proof = {
  ...
  provenBoundaries: [
    'agent-protocol status payload carries row10g outbox refs, handoff refs, replay/support refs, prepared candidate counts, duplicate rejection, and mismatch counters',
    'agent-service builds the remote-delivery status from row10g outbox handoff proof state without spawning a live transport dispatch',
    'agent-protocol-domain parser rejects stale outbox refs, nonzero dispatch/ack counters, and mismatched prepared candidate counts',
...
function assertSourceContracts() {
  ...
  const requiredSnippets = [
    ...
    [protocolTests, 'network_remote_delivery_status_serializes_row10h_outbox_bridge_without_product_claims'],
    [servicePayload, 'prove_network_runtime_remote_delivery_outbox_handoff'],
    [servicePayload, 'outbox_candidates_match_receipts'],
    [serviceTests, 'network_remote_delivery_status_payload_serializes_row10h_outbox_bridge'],
