...
const outputDir = join(repoRoot, 'test-results', 'app-install-purchase-external-runtime-device-delivery-proof');
...
async function main() {
  ...
  assert.deepEqual(
    parsedReadModel.externalRuntimeDeviceDeliveryRows.map(
  ...
  const proof = {
    ...
    evidence: {
      ...
      contractTest:
        'packages/schema-domain/tests/unit/app-install-purchase-external-runtime-device-delivery-proof.test.ts',
      featureDoc: 'docs/features/app-install-purchase-approval.md',
      expectationDoc: 'docs/expectations/app-install-purchase-approval.md',
      platformExpectationDoc: 'docs/expectations/platforms.md',
      packageExport: '@ocentra-parent/schema-domain/app-install-purchase-external-runtime-device-delivery-proof',
    ...
    externalRuntimeDeviceDeliverySummary: summary,
    externalRuntimeDeviceDeliveryRows: parsedReadModel.externalRuntimeDeviceDeliveryRows.map((row) => ({
      ...
      sourceChildDeviceDeliveryRuntimeWriterRowId: row.sourceChildDeviceDeliveryRuntimeWriterRowId,
      ...
      sourceRuntimeWriterReceiptClaim: row.sourceRuntimeWriterReceiptClaim,
      sourceChildDeliveryEnvelopeState: row.sourceChildDeliveryEnvelopeState,
      sourceChildDeliveryTargetRefs: row.sourceChildDeliveryTargetRefs,
      externalRuntimeEvidenceState: row.externalRuntimeEvidenceState,
  ...
  console.log(`app-install-purchase-external-runtime-device-delivery-proof-ok:${relative(repoRoot, proofPath)}`);
}

async function loadExternalRuntimeDeviceDeliveryProofModule() {
