...
const validationCommands = [
  [
    ...
    [
      '/c',
      'npm exec --workspace @ocentra-parent/activity-domain -- vitest run tests/screen-child-disclosure-ux.test.ts',
    ],
  ],
  [
    'cmd',
    [
      '/c',
      'npm exec --workspace @ocentra-parent/text-domain -- vitest run tests/screen-child-disclosure-ux-text.test.ts',
    ],
  ],
...
await writeFile(contractLogPath, validationResults.map((result) => formatCommandResult(result)).join('\n\n'), 'utf8');
await writeFile(
  validationLogPath,
...
try {
  ...
  const activeDisclosureCard = page
    .getByRole('heading', { name: 'Screen check is running' })
...
function formatCommandResult(result) {
  return [`$ ${result.command}`, `exit=${result.exitCode}`, result.stdout.trim(), result.stderr.trim()]
