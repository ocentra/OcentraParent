...
async function writeProofArtifacts({ checkedAt, commit, readModel }) {
  ...
  await writeFile(
    join(proofRoot, '03-runtime-location-evidence.json'),
    `${JSON.stringify(runtimeLocationEvidence(readModel), null, 2)}\n`
  );
  await writeFile(
    join(proofRoot, '09-policy-alert-proof.json'),
    `${JSON.stringify(policyAlertProof(readModel), null, 2)}\n`
  );
  ...
  await writeFile(
    ...
    `${JSON.stringify(retentionDeleteProof(readModel), null, 2)}\n`
  );
  await writeFile(
  ...
  await writeFile(
    join(testResultRoot, 'tracking-temporary-live-read-model.json'),
