...
async function main() {
  ...
  await runCommand(
    ...npmCommand([
      ...
      '--',
      'tests/app-install-purchase-child-device-delivery-readiness-proof.test.ts',
    ])
  );
  ...
  const proof = {
    ...
    evidence: {
      ...
      contractTest: 'packages/parent-domain/tests/app-install-purchase-child-device-delivery-readiness-proof.test.ts',
