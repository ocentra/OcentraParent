...
const generatedAppGameProofRoots = new Set(['28-e2e-manual-proof-rollout-pr-gate']);
const generatedAppProofRoots = new Set(['27-e2e-and-manual-proof-artifacts', '28-rollout-checklist-and-pr-gate']);

const expectedAppGameProofRoots = await proofRootsFromChecklist({
  checklistPath: 'docs/plans/app-game-plan/implementation-checklist.md',
  proofParent: 'output/app-game-plan-proof',
  generatedRoots: generatedAppGameProofRoots,
});

const expectedAppProofRoots = await proofRootsFromChecklist({
  checklistPath: 'docs/plans/app-plan/implementation-checklist.md',
  proofParent: 'output/app-plan-proof',
  generatedRoots: generatedAppProofRoots,
});
...
async function main() {
  ...
  const finalGate = {
    ...
    claimsProved: [
      'app-game and app-plan checklist proof roots exist before the final gate, excluding only proof packs regenerated by this gate',
      'app-game WP29-WP48 and app-plan WP29-WP48 continuation proof roots are included in the final rollout gate audit',
...
async function proofRootsFromChecklist({ checklistPath, proofParent, generatedRoots }) {
  const checklist = await readText(checklistPath);
  const proofPattern = new RegExp(`${escapeRegExp(proofParent)}/([a-z0-9-]+)`, 'g');
  const roots = Array.from(checklist.matchAll(proofPattern), (match) => match[1]);
  const uniqueRoots = Array.from(new Set(roots))
    .filter((root) => !generatedRoots.has(root))
    .sort(compareWorkpackRoots);

  if (uniqueRoots.length === 0) {
    throw new Error(`${checklistPath}: no proof roots found for ${proofParent}`);
  }

  return uniqueRoots;
}

function compareWorkpackRoots(left, right) {
  const leftStep = workpackStep(left);
  const rightStep = workpackStep(right);
...
function workpackStep(root) {
  const [prefix] = root.split('-');
  const step = Number.parseInt(prefix, 10);
  return Number.isNaN(step) ? Number.MAX_SAFE_INTEGER : step;
}
