...
const outputDir = join(repoRoot, 'test-results', 'app-install-purchase-product-claim-platform-preclaim-proof');
...
async function main() {
  ...
  const packageProofModule =
    await import('@ocentra-parent/parent-domain/app-install-purchase-product-claim-platform-preclaim-proof');
  assert.equal(
  ...
  const proof = {
    ...
    evidence: {
      ...
      packageExport: '@ocentra-parent/parent-domain/app-install-purchase-product-claim-platform-preclaim-proof',
      output: relative(repoRoot, proofPath),
    },
    platformPreclaimSummary: summary,
    platformPreclaimRows: parsedReadModel.platformPreclaimRows,
    nonClaims: parsedReadModel.nonClaims,
    knownGaps: parsedReadModel.knownGaps,
  };

  await writeFile(proofPath, `${JSON.stringify(proof, null, 2)}\n`);
  console.log(`app-install-purchase-product-claim-platform-preclaim-proof-ok:${relative(repoRoot, proofPath)}`);
}
