...
const proofSlug = '112-timer-service-response-consumer-parent-surface-status-read-model-parent-surface';
...
const timestamp = '2026-06-06T18:35:00Z';
const commands = [];
...
const sourceStatusReadModelHandoff =
  statusReadModelContract.AppGameTimerServiceResponseConsumerParentSurfaceStatusReadModelHandoffSchema.parse(
    await readJson(
      join(
        repoRoot,
        'test-results',
...
const parentSurfaceHandoff =
  contract.buildAppGameTimerServiceResponseConsumerParentSurfaceStatusReadModelParentSurfaceHandoff(
    parentSurfaceHandoffOptions(refs),
    sourceStatusReadModelHandoff
  );
const proof = {
  proofMode: 'app-game-timer-service-response-consumer-parent-surface-status-read-model-parent-surface-handoff',
  ...
  commands,
  stackedOn: {
    wp111Branch: 'codex/app-game-timer-service-response-consumer-parent-surface-status-read-model-handoff-wp111',
    reason:
      'WP112 consumes WP111 parent-domain service response-consumer parent-surface status read-model handoff rows and records the future parent-surface proof needed before parent-surface runtime, persistence, rendering, portal UI, adapters, child delivery, platform enforcement, or raw source rows are claimed.',
  },
  ...
  proofPaths: {
    ...
    test: 'packages/parent-domain/tests/app-game-timer-service-response-consumer-parent-surface-status-read-model-parent-surface-handoff.test.ts',
    harness:
...
await writeProofPack(appGameProofDir, proof, 'app-game WP112');
await writeProofPack(appProofDir, proof, 'app WP112');

console.log(
  'app-game-timer-service-response-consumer-parent-surface-status-read-model-parent-surface-handoff-proof-ok'
);
console.log(
  `evidence=${join(
    'test-results',
...
function summarize(parentSurfaceHandoff) {
  return {
    sourceServiceResponseConsumerParentSurfaceStatusReadModelHandoffId:
      parentSurfaceHandoff.sourceServiceResponseConsumerParentSurfaceStatusReadModelHandoffId,
...
function pickNonClaims(parentSurfaceHandoff) {
  return {
    ...
    parentSurfaceStatusRuntimeImplemented: parentSurfaceHandoff.parentSurfaceStatusRuntimeImplemented,
    parentSurfaceStatusPersisted: parentSurfaceHandoff.parentSurfaceStatusPersisted,
    parentSurfaceStatusReadModelRuntimeImplemented: parentSurfaceHandoff.parentSurfaceStatusReadModelRuntimeImplemented,
    parentSurfaceStatusReadModelPersisted: parentSurfaceHandoff.parentSurfaceStatusReadModelPersisted,
    parentSurfaceStatusReadModelParentSurfaceRuntimeImplemented:
      parentSurfaceHandoff.parentSurfaceStatusReadModelParentSurfaceRuntimeImplemented,
    parentSurfaceStatusReadModelParentSurfacePersisted:
      parentSurfaceHandoff.parentSurfaceStatusReadModelParentSurfacePersisted,
...
function assertProof(proof) {
  ...
  if (Object.values(proof.nonClaims).some((value) => value !== false)) {
    throw new Error(
      'Expected WP112 proof to avoid service runtime, parent-surface runtime, rendering, portal UI, evaluator, timer, scheduler storage, audit log, rollback execution, adapter, child, platform, and raw-source claims'
    );
  }
}
