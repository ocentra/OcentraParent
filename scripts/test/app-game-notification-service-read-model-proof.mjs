...
async function main() {
  ...
  const proof = {
    ...
    evidence: {
      ...
      servicePayloadTest: 'crates/agent-service/src/activity_api/app_game_notification_readiness_payload_tests.rs',
      serviceWebSocketTest: 'crates/agent-service/src/activity_api/app_game_notification_readiness_service_tests.rs',
...
async function writeProofPack(proofDir, proof, label) {
  ...
  await writeFile(
    ...
    [
      ...
      '- cmd /c npm run test --workspace @ocentra-parent/agent-protocol-domain -- app-game-notification-readiness contracts: PASS',
      '- Parser accepts only the dedicated notification readiness event and rejects providerDeliveryClaimed=true.',
      '',
    ].join('\n'),
    'utf8'
  );
