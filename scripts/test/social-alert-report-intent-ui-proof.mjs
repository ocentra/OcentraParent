...
const proofResultDir = path.join(repoRoot, 'test-results', 'social-alert-report-intent-ui-proof');
const proofPath = path.join(proofResultDir, 'proof.json');
const outputProofPath = path.join(proofRoot, '07-rendered-alert-report-ui-proof.json');
...
const playwrightLogPath = path.join(screenshotDir, 'social-alert-report-ui-playwright.log');
const desktopScreenshot = path.join(screenshotDir, 'social-alert-report-browser-route.png');
const mobileScreenshot = path.join(screenshotDir, 'social-alert-report-browser-route-mobile.png');
const accessibilitySummaryPath = path.join(proofResultDir, 'accessibility-summary.json');
const runRoot = await mkdtemp(path.join(tmpdir(), 'ocentra-parent-social-alert-report-ui-'));
...
try {
  ...
  await runNpmWorkspace('@ocentra-parent/agent-protocol-domain', [
    ...
    'test',
    '--',
    'social-alert-report-read-model.test.ts',
  ]);
  await runNpmWorkspace('@ocentra-parent/portal-domain', ['run', 'test', '--', 'social-alert-report-panel.test.ts']);
  await runNpmWorkspace('@ocentra-parent/portal', ['run', 'test', '--', 'social-alert-report-panel.test.ts']);
  await runNpmWorkspace('@ocentra-parent/portal', ['run', 'type-check']);
  await runCommand('cargo', ['test', '-p', 'ocentra-parent-agent-protocol', 'social_alert_report', '--quiet']);
  await runCommand('cargo', ['test', '-p', 'ocentra-parent-agent-service', 'social_alert_report', '--quiet']);
  ...
  console.log('social-alert-report-intent-ui-proof-ok=true');
...
async function writeProof(playwright) {
  ...
  const proof = {
    ...
    workpackIds: ['social-alert-report-intent-ui-proof', 'social-23-tests-fixtures-playwright-manual-proof'],
    proofMode: 'real-portal-browser-route-service-backed-social-alert-report-intents',
    route: '#/browser',
    currentStatus: 'service-backed-social-alert-report-intents-rendered',
    ...
    serviceBoundary: {
      ...
      command: 'agent.browser.social-alert-report.read-model.get',
      event: 'agent.browser.social-alert-report.read-model.reported',
      socialAlertReportSnapshot: 'reported',
      renderedState: 'two service-backed alert/report intent rows plus two provider status boundary rows',
    },
    assertions: [
      'Portal browser route renders the social alert/report region from the real app shell.',
      'The route starts with zero rows, then requests the service-backed alert/report read model over the real WebSocket path.',
      'The reported service snapshot renders one local-outbox high-risk alert intent, one manual-required proof row, and provider status boundary rows for manual-required provider handoff.',
      'Provider status rows keep provider receipts empty and provider delivery unobserved.',
      'The rendered copy explicitly keeps provider delivery, report delivery, notification UI delivery, final policy execution, and enforcement unclaimed.',
      'Desktop and mobile screenshots were captured from the real portal route.',
    ],
    nonClaims: [
      'This proof does not claim provider dispatch, provider receipt, parent notification UI delivery, report delivery, final policy decision, or enforcement.',
      'This proof does not claim native app control, connector authorization, or child-device execution.',
    ...
    remainingGapsBeforeProductReady: [
      'Provider/native runtime delivery remains pending.',
      'Parent report delivery and notification UI delivery remain pending.',
  ...
  await writeFile(
    ...
    [
      `checkedAt=${checkedAt}`,
      'asserted=no provider dispatched claim',
      'asserted=no provider receipt claim',
      'asserted=no parent notification UI delivered claim',
      'asserted=no report delivered claim',
