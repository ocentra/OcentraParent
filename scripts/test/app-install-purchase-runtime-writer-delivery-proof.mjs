...
async function main() {
  ...
  await runCommand(
    ...npmCommand([
      ...
      '--',
      'tests/app-install-purchase-runtime-writer-delivery-proof.test.ts',
    ])
  );

  const proofModule = await loadRuntimeWriterDeliveryProofModule();
  const packageModule =
    await import('@ocentra-parent/app-game-domain/app-install-purchase-runtime-writer-delivery-proof');
  ...
  assert.equal(
    packageModule.AppInstallPurchaseRuntimeWriterDeliveryProofReadModel.schemaVersion,
  ...
  const proof = {
    ...
    evidence: {
      runtimeWriterDeliveryContract: 'packages/parent-domain/src/app-install-purchase-runtime-writer-delivery-proof.ts',
      ...
      contractTest: 'packages/parent-domain/tests/app-install-purchase-runtime-writer-delivery-proof.test.ts',
