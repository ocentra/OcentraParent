...
const proofMode = 'app-game-android-child-runtime-local-receipt-channel-physical-proof';
...
async function main() {
  ...
  const proof = {
    ...
    evidence: {
      ...
      proof: 'test-results/app-game-android-child-runtime-local-receipt-channel-physical-proof/proof.json',
      outputProof:
    ...
    claimsProved: [
      ...
      'The activity-triggered in-package broadcast records the package-local receipt channel marker',
      'The same package-local execution path records receipt and receipt-ack markers readable through debug run-as',
    ],
    claimsNotProved: [
  ...
  await writeFile(join(appGameProofDir, '01-physical-android-channel-snapshot.md'), sourceSnapshot(proof));
  await writeFile(join(appGameProofDir, '11-physical-validation-commands.log'), `${commandResults.map((result) => result.command).join('\n')}\n`);

  console.log('app-game-android-child-runtime-local-receipt-channel-physical-proof-ok');
...
function sourceSnapshot(proof) {
  return [
    '# WP215 Android child runtime local receipt channel physical proof snapshot',
    '',
