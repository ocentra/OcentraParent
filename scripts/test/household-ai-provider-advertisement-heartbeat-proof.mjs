...
const proofModule = await import(
  pathToFileURL(
    resolve(repoRoot, 'packages', 'parent-domain', 'dist', 'household-ai-provider-advertisement-heartbeat-proof.js')
  )
);
...
const proofSummary = {
  ...
  sourceContracts: ['packages/parent-domain/src/household-ai-provider-advertisement-heartbeat-proof.ts'],
...
writeFileSync(
  ...
  [
    ...
    'cmd /c npm run test --workspace @ocentra-parent/parent-domain -- household-ai-provider-advertisement-heartbeat-proof',
  ].join('\n') + '\n'
);
