...
const rollupProof = await tsImport(
  pathToFileURL(join(repoRoot, 'packages', 'schema-domain', 'src', 'tracking-family-dashboard-rollup-proof.ts')).href,
  import.meta.url
);
...
const proof = {
  ...
  proofPaths: {
    source: 'packages/schema-domain/src/tracking-family-dashboard-rollup-proof.ts',
    test: 'packages/tracking-domain/tests/contract/tracking-family-dashboard-rollup-proof.test.ts',
...
async function writeCompanionPack(path, proof) {
  await writeFile(
    ...
    [
      ...
      '```',
      '',
      '- Scope: tracking-domain family dashboard rollup rows for existing tracking service read-model/product summary/report consumer proof refs.',
      '- Source inspected: feature list, location/geofence feature doc, location/geofence expectations, platform expectations, and WP32 checklist rows.',
      '',
    ].join('\n'),
    'utf8'
  );
  await writeFile(
    ...
    [
      ...
      '- cmd /c npm run test --workspace @ocentra-parent/tracking-domain -- tracking-family-dashboard-rollup-proof tracking-location-policy: PASS',
      '- Family active, child attention, and retention audit rollup rows parse through tracking-domain schemas.',
      '',
    ].join('\n'),
    'utf8'
  );
