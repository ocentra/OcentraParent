...
async function main() {
  ...
  const proof = {
    ...
    evidence: {
      ...
      analysisRuntimeTests: 'crates/agent-service/src/screen_ai_analysis_runtime_tests.rs',
    ...
    claimsProved: [
      ...
      'policy refs are produced in the Rust service event-record path before row-ready publication instead of by a duplicate JS read-model transform',
    ],
    claimsNotProved: [
...
async function assertSourceContracts() {
  ...
  const analysisTests = await readText('crates/agent-service/src/screen_ai_analysis_runtime_tests.rs');
  const subscriptionTests = await readText('crates/agent-service/src/screen_ai_service_event_subscription_tests.rs');
  ...
  assertIncludes(
    eventRecord,
    'policy_refs::service_policy_refs(&image.queue_job_id, policy_eligible)',
