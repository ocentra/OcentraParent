...
async function writeProof(playwright) {
  ...
  const proof = {
    ...
    nonClaims: [
      ...
      'This proof renders retention settings read-model rows and local service execution proof but does not claim writable product settings, durable production persistence, or platform runtime execution.',
      'This proof sends and renders a typed retention settings write result with local service mutation execution but does not claim product-ready service behavior, platform runtime execution, child-device delivery, provider delivery, physical-device proof, authority, or production behavior.',
  ...
  await writeFile(
    ...
    [
      ...
      'asserted=retention settings write result keeps productClaimReady=false while serviceMutationExecutedRows=1',
