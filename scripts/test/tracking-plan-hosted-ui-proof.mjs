...
async function writeProof(playwright) {
  ...
  const proof = {
    ...
    nonClaims: [
      ...
      'This proof renders retention settings read-model rows and local service execution proof with a local service state revision but does not claim writable product settings, durable production persistence, or platform runtime execution.',
  ...
  await writeFile(
    ...
    [
      ...
      'asserted=retention settings write result keeps productClaimReady=false while serviceMutationExecutedRows=1 and durableSettingsPersistedRows=0',
