...
const proofSlug = '93-source-gated-policy-preview-timer-service-readiness-read-api-response-handoff';
...
const timestamp = '2026-06-06T08:36:00Z';
const commands = [];
...
const readApiResponseHandoff =
  responseContract.buildAppGameSourceGatedPolicyPreviewTimerServiceReadinessReadApiResponseHandoff(
    readApiResponseHandoffOptions(refs),
    readApiHandoff
  );
const proof = {
  proofMode: 'app-game-source-gated-policy-preview-timer-service-readiness-read-api-response-handoff',
  ...
  nonClaims: {
    serviceCommandRegistered: readApiResponseHandoff.serviceCommandRegistered,
    serviceHandlerImplemented: readApiResponseHandoff.serviceHandlerImplemented,
    serviceReadApiImplemented: readApiResponseHandoff.serviceReadApiImplemented,
    serviceReadApiResponseImplemented: readApiResponseHandoff.serviceReadApiResponseImplemented,
    serviceEventEmitted: readApiResponseHandoff.serviceEventEmitted,
    agentProtocolImplemented: readApiResponseHandoff.agentProtocolImplemented,
    rustProtocolMirrored: readApiResponseHandoff.rustProtocolMirrored,
    portalUiRendered: readApiResponseHandoff.portalUiRendered,
    policyEvaluatorRuntimeClaimed: readApiResponseHandoff.policyEvaluatorRuntimeClaimed,
    timerRuntimeClaimed: readApiResponseHandoff.timerRuntimeClaimed,
    timerScheduled: readApiResponseHandoff.timerScheduled,
    schedulerPersistenceRuntimeClaimed: readApiResponseHandoff.schedulerPersistenceRuntimeClaimed,
    durableSchedulerStorageClaimed: readApiResponseHandoff.durableSchedulerStorageClaimed,
    auditRuntimeClaimed: readApiResponseHandoff.auditRuntimeClaimed,
    durableAuditLogClaimed: readApiResponseHandoff.durableAuditLogClaimed,
    rollbackRuntimeClaimed: readApiResponseHandoff.rollbackRuntimeClaimed,
    rollbackExecutionClaimed: readApiResponseHandoff.rollbackExecutionClaimed,
    adapterDispatchClaimed: readApiResponseHandoff.adapterDispatchClaimed,
    childDeliveryClaimed: readApiResponseHandoff.childDeliveryClaimed,
    platformEnforcementClaimed: readApiResponseHandoff.platformEnforcementClaimed,
...
assertProof(proof);
await writeJson(join(testOutputDir, 'timer-service-readiness-read-api-response-handoff.json'), readApiResponseHandoff);
await writeJson(join(testOutputDir, 'proof.json'), proof);
await writeProofPack(appGameProofDir, proof, 'app-game WP93');
await writeProofPack(appProofDir, proof, 'app WP93');

console.log('app-game-source-gated-policy-preview-timer-service-readiness-read-api-response-handoff-proof-ok');
console.log(
  `evidence=${join(
    'test-results',
    'app-game-source-gated-policy-preview-timer-service-readiness-read-api-response-handoff-proof',
...
function summarize(readApiResponseHandoff) {
  return {
    sourceReadApiHandoffId: readApiResponseHandoff.sourceReadApiHandoffId,
    nativeAppRowCount: readApiResponseHandoff.nativeAppRowCount,
    nativeGameRowCount: readApiResponseHandoff.nativeGameRowCount,
    readApiResponseProofRequiredCount: readApiResponseHandoff.readApiResponseProofRequiredCount,
    blockedBySourceFreshnessCount: readApiResponseHandoff.blockedBySourceFreshnessCount,
    blockedByCompilerDecisionCount: readApiResponseHandoff.blockedByCompilerDecisionCount,
  };
}

function assertProof(proof) {
  if (proof.summary.readApiResponseProofRequiredCount < 1) {
    throw new Error('Expected at least one read API response proof-required row');
  }
  ...
  if (Object.values(proof.nonClaims).some((value) => value !== false)) {
    throw new Error(
      'Expected WP93 proof to avoid service runtime, read API/response implementation, portal UI, evaluator, timer, scheduler storage, audit log, rollback execution, adapter, child, platform, and raw-source claims'
    );
  }
}
