...
const deletion = metadata.captured
  ? JSON.parse(readFileSync(join(outputDir, '04-deletion-proof.json'), 'utf8'))
  : null;
...
if (process.platform === 'win32' && !summary.realCaptureProof) {
  throw new Error(
    `Windows screen capture proof did not produce captured bytes; status=${metadata.status}`,
  );
}
