...
const proofRevision = 'network-manual-followup-owner-proof/v1';
const proofBranch = 'codex/network-manual-followup-owner-ledger-proof-restacked';
...
function trackedSourceFingerprint() {
  const trackedSource = runText('git', ['ls-files', '-s', '--', '.', ':(exclude)output', ':(exclude)test-results']);
  return createHash('sha256').update(trackedSource).digest('hex');
}
