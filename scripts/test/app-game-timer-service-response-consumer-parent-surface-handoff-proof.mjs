...
const proofSlug = '108-timer-service-response-consumer-parent-surface';
...
const timestamp = '2026-06-06T15:10:00Z';
const commands = [];
...
const contract = await importDist('app-game-timer-service-response-consumer-parent-surface-handoff.js');
const serviceResponseConsumerContract = await importDist(
  'app-game-timer-service-read-api-response-consumer-handoff.js'
);
const refs = await importDist('reference-primitives.js');
const sourceServiceResponseConsumerHandoff =
  serviceResponseConsumerContract.AppGameSourceGatedPolicyPreviewTimerServiceReadinessResponseConsumerParentSurfaceStatusReadModelParentSurfaceReadModelServiceReadApiResponseConsumerHandoffSchema.parse(
    await readJson(
      join(repoRoot, 'test-results', 'app-game-timer-service-read-api-response-consumer-handoff-proof', 'handoff.json')
    )
  );
const parentSurfaceHandoff = contract.buildAppGameTimerServiceResponseConsumerParentSurfaceHandoff(
  parentSurfaceHandoffOptions(refs),
  sourceServiceResponseConsumerHandoff
);
const proof = {
  proofMode: 'app-game-timer-service-response-consumer-parent-surface-handoff',
  ...
  stackedOn: {
    ...
    reason:
      'WP108 consumes WP107 parent-domain service read API response consumer handoff rows and records the future parent-surface proof needed before parent-surface rendering, portal response consumer rendering, adapters, child delivery, platform enforcement, or raw source rows are claimed.',
  },
  summary: summarize(parentSurfaceHandoff),
  nonClaims: pickNonClaims(parentSurfaceHandoff),
  proofPaths: {
    source: 'packages/parent-domain/src/app-game-timer-service-response-consumer-parent-surface-handoff.ts',
    rules: 'packages/parent-domain/src/app-game-timer-service-response-consumer-parent-surface-handoff-rules.ts',
    test: 'packages/parent-domain/tests/app-game-timer-service-response-consumer-parent-surface-handoff.test.ts',
    harness: 'scripts/test/app-game-timer-service-response-consumer-parent-surface-handoff-proof.mjs',
    evidence: 'test-results/app-game-timer-service-response-consumer-parent-surface-handoff-proof/proof.json',
...
console.log('app-game-timer-service-response-consumer-parent-surface-handoff-proof-ok');
console.log(
  `evidence=${join('test-results', 'app-game-timer-service-response-consumer-parent-surface-handoff-proof', 'proof.json')}`
);
...
function parentSurfaceHandoffOptions(refs) {
  return {
    ...
    serviceResponseConsumerParentSurfaceHandoffId:
      'app-game-timer-service-response-consumer-parent-surface-handoff-proof',
    generatedAt: timestamp,
    sourceContractRefs: [
      'app-game-timer-service-read-api-response-consumer-handoff',
...
function summarize(parentSurfaceHandoff) {
  return {
    sourceServiceReadApiResponseConsumerHandoffId: parentSurfaceHandoff.sourceServiceReadApiResponseConsumerHandoffId,
...
function assertProof(proof) {
  ...
  if (Object.values(proof.nonClaims).some((value) => value !== false)) {
    throw new Error(
      'Expected WP108 proof to avoid service runtime, response consumer implementation, parent-surface rendering, portal rendering, protocol, evaluator, timer, scheduler storage, audit log, rollback execution, adapter, child, platform, and raw-source claims'
    );
  }
}
