...
async function main() {
  ...
  await runCommand(
    ...npmCommand([
      ...
      '--',
      'tests/app-install-purchase-product-claim-platform-limitation-fallback-proof.test.ts',
    ])
  );
  ...
  const proof = {
    ...
    evidence: {
      ...
      contractTest:
        'packages/parent-domain/tests/app-install-purchase-product-claim-platform-limitation-fallback-proof.test.ts',
