...
const proofSlug = '109-timer-service-response-consumer-parent-surface-read-model';
...
const timestamp = '2026-06-06T15:30:00Z';
const commands = [];
...
run('cmd', [
  ...
  'app-game-timer-service-response-consumer-parent-surface-handoff',
]);

const contract = await importDist('app-game-timer-service-response-consumer-parent-surface-read-model-handoff.js');
const parentSurfaceContract = await importDist('app-game-timer-service-response-consumer-parent-surface-handoff.js');
const refs = await importDist('reference-primitives.js');
const sourceParentSurfaceHandoff =
  parentSurfaceContract.AppGameTimerServiceResponseConsumerParentSurfaceHandoffSchema.parse(
    await readJson(
      join(
        repoRoot,
        'test-results',
        'app-game-timer-service-response-consumer-parent-surface-handoff-proof',
        'handoff.json'
      )
    )
  );
const parentSurfaceReadModelHandoff = contract.buildAppGameTimerServiceResponseConsumerParentSurfaceReadModelHandoff(
  parentSurfaceReadModelHandoffOptions(refs),
  sourceParentSurfaceHandoff
);
const proof = {
  proofMode: 'app-game-timer-service-response-consumer-parent-surface-read-model-handoff',
  ...
  commands,
  stackedOn: {
    wp108Branch: 'codex/app-game-timer-service-response-consumer-parent-surface-handoff-wp108',
    reason:
      'WP109 consumes WP108 parent-domain service response-consumer parent-surface handoff rows and records the future parent-surface read-model proof needed before read-model runtime, rendering, portal response consumer rendering, adapters, child delivery, platform enforcement, or raw source rows are claimed.',
  },
  summary: summarize(parentSurfaceReadModelHandoff),
  nonClaims: pickNonClaims(parentSurfaceReadModelHandoff),
  proofPaths: {
    source: 'packages/parent-domain/src/app-game-timer-service-response-consumer-parent-surface-read-model-handoff.ts',
    rules:
      'packages/parent-domain/src/app-game-timer-service-response-consumer-parent-surface-read-model-handoff-rules.ts',
    test: 'packages/parent-domain/tests/app-game-timer-service-response-consumer-parent-surface-read-model-handoff.test.ts',
    harness: 'scripts/test/app-game-timer-service-response-consumer-parent-surface-read-model-handoff-proof.mjs',
    evidence:
...
await writeProofPack(appGameProofDir, proof, 'app-game WP109');
await writeProofPack(appProofDir, proof, 'app WP109');

console.log('app-game-timer-service-response-consumer-parent-surface-read-model-handoff-proof-ok');
console.log(
  `evidence=${join(
    'test-results',
    'app-game-timer-service-response-consumer-parent-surface-read-model-handoff-proof',
...
function summarize(parentSurfaceReadModelHandoff) {
  return {
    sourceServiceResponseConsumerParentSurfaceHandoffId:
      parentSurfaceReadModelHandoff.sourceServiceResponseConsumerParentSurfaceHandoffId,
...
function assertProof(proof) {
  ...
  if (Object.values(proof.nonClaims).some((value) => value !== false)) {
    throw new Error(
      'Expected WP109 proof to avoid service runtime, response consumer implementation, parent-surface read-model runtime, parent-surface rendering, portal rendering, protocol, evaluator, timer, scheduler storage, audit log, rollback execution, adapter, child, platform, and raw-source claims'
    );
  }
}
