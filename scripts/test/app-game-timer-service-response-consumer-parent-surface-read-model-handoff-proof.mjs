...
const parentSurfaceContract = await importDist(
  'app-game-timer-service-read-api-response-consumer-parent-surface-handoff.js'
);
const refs = await importDist('reference-primitives.js');
const sourceParentSurfaceHandoff =
  parentSurfaceContract.AppGameTimerServiceReadApiResponseConsumerParentSurfaceHandoffSchema.parse(
    await readJson(
      join(
        repoRoot,
        'test-results',
...
const proof = {
  ...
  commands,
  stackedOn: {
    wp108Branch: 'codex/app-game-timer-service-read-api-response-consumer-parent-surface-handoff-wp108',
    reason:
      'WP109 consumes WP108 parent-domain service read API response consumer parent-surface handoff rows and records the future parent-surface read-model proof needed before read-model runtime, rendering, portal response consumer rendering, adapters, child delivery, platform enforcement, or raw source rows are claimed.',
  },
