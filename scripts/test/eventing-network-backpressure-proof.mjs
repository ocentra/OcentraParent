...
function assertSourceContracts() {
  ...
  const queueTestsSource = readText('crates/agent-core/src/network_event_runtime_queue_tests.rs');
  const eventingQueueSource = readText('crates/ocentra-eventing/src/queue/state.rs');
  const eventingQueueTests = readText('crates/ocentra-eventing/src/tests/queue.rs');
...
function sourceStatusShort() {
  return runText('git', [
    ...
    '--',
    '.',
    ':(exclude)output/network-plan-proof/10-network-backpressure-depth',
    ':(exclude)test-results/eventing-network-backpressure-proof',
  ]);
}
