...
async function main() {
  ...
  const contractModule = await import(
    pathToFileURL(
      join(repoRoot, 'packages', 'schema-domain', 'dist', 'app-game-android-accessibility-enabled-sample-proof.js')
    ).href
  );
  ...
  const proof = {
    ...
    summary,
    evidence: {
      contract: 'packages/schema-domain/src/app-game-android-accessibility-enabled-sample-proof.ts',
    ...
    claimsProved: [
      ...
      'The centralized sample proof records only settings enablement, UI state, and event count without raw Accessibility event rows, service names, overlay content, delivery, dispatch, or enforcement claims',
    ],
    claimsNotProved: [
