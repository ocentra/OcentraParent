...
async function main() {
  ...
  runNpmCommand(run, [
    ...
    '--',
    'tests/contract/tracking-full-product-ui-runtime-artifact-gate-proof.test.ts',
  ]);

  const proofModule = await tsImport(
    pathToFileURL(
      path.join(repoRoot, 'packages', 'tracking-domain', 'src', 'tracking-full-product-ui-runtime-artifact-gate-proof.ts')
    ).href,
    import.meta.url
  );
  const inventory = {
