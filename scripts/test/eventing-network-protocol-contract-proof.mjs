...
async function main() {
  ...
  const proof = {
    ...
    proofLabels,
    linkedArtifacts: {
      protocolContracts: 'crates/agent-protocol/src/network_flow_events.rs',
      protocolContractExports: 'crates/agent-protocol/src/network_flow.rs',
      protocolConstants: 'crates/agent-protocol/src/constants/network_flow.rs',
      protocolTests: 'crates/agent-protocol/src/network_flow_tests.rs',
      protocolTestFixtures: 'crates/agent-protocol/src/network_flow_event_fixtures.rs',
...
async function assertSourceContracts() {
  ...
  const contracts = await readText('crates/agent-protocol/src/network_flow_events.rs');
  const constants = await readText('crates/agent-protocol/src/constants/network_flow.rs');
  const tests = await readText('crates/agent-protocol/src/network_flow_tests.rs');

  assertIncludes(contracts, 'pub trait NetworkRuntimeEventContract', 'event contract trait exists');
  assertIncludes(exports, 'pub use network_flow_events::*;', 'event contracts are exported');
