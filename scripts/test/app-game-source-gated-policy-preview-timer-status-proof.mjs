...
function assertProof(proof) {
  ...
  if (
    proof.summary.sourceFreshnessProofRequiredCount < 1 ||
    proof.summary.compilerDecisionProofRequiredCount < 1
  ) {
    throw new Error('Expected source-freshness and compiler-decision proof-required rows');
  }
  if (Object.values(proof.nonClaims).some((value) => value !== false)) {
    throw new Error('Expected WP79 proof to avoid runtime, UI, timer scheduling, adapter, child, platform, and raw-source claims');
  }
}
