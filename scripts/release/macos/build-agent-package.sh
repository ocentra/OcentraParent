...
package_name="ocentra-parent-agent-macos-v${version}.pkg"
package_path="$package_root/$package_name"
latest_name="ocentra-parent-agent-macos-latest.pkg"
latest_path="$package_root/$latest_name"

rm -rf "$payload_root" "$scripts_root"
mkdir -p "$payload_root/Library/Ocentra/Ocentra Parent Agent/bin"
...
install -m 0755 "$repo_root/target/release/ocentra-parent-agent-service" \
  "$payload_root/Library/Ocentra/Ocentra Parent Agent/bin/ocentra-parent-agent-service"
install -m 0644 "$repo_root/scripts/release/macos/ca.ocentra.parent.agent.plist" \
  "$payload_root/Library/LaunchDaemons/ca.ocentra.parent.agent.plist"

cat > "$scripts_root/postinstall" <<'POSTINSTALL'
#!/usr/bin/env bash
set -e
launchctl bootout system /Library/LaunchDaemons/ca.ocentra.parent.agent.plist >/dev/null 2>&1 || true
launchctl bootstrap system /Library/LaunchDaemons/ca.ocentra.parent.agent.plist >/dev/null 2>&1 || true
launchctl enable system/ca.ocentra.parent.agent >/dev/null 2>&1 || true
POSTINSTALL
...
pkgbuild \
  ...
  --identifier "ca.ocentra.parent.agent" \
  --version "$version" \
  --install-location "/" \
  "$package_path"
