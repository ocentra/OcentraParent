...
WP114 updates the app/game timer parent-surface service and portal intent so
existing app/game SQLite replay control action-result rows are visible to
parents as counts and result-reference ids.

## Implementation Evidence

- `crates/agent-service/src/activity_api/app_game_timer_parent_surface_payload.rs`
  derives action-result visibility from the existing service read model's
  `approval_action_result_rows`.
- `crates/agent-protocol/src/app_game_timer_parent_surface_read_model.rs`
  carries the action-result count and result-reference ids through the Rust
  protocol mirror.
- `packages/agent-protocol-domain/src/app-game-timer-parent-surface-read-model.ts`
  accepts the replay-visible count and references while continuing to reject
  adapter, child, platform, and raw private source overclaims.
- `packages/portal-domain/src/app-game-timer-parent-surface-panel.ts` and
  ...
  action-result visibility without claiming adapter execution.
- `crates/agent-service/src/activity_api/app_game_timer_parent_surface_service_tests.rs`
  keeps the real service path covered for timer parent-surface reporting.

## Claim Boundary

- Visible when replay rows report it: control action-result count and
  result-reference ids.
- Not claimed: live scheduling automation, adapter dispatch, child delivery,
  broad blocking, platform enforcement, raw private source rows.
- Central `docs/product-capability-checklist.md` wording is deferred because
  E-B owns that shared checklist lock; this proof updates the owning feature
  doc and app-game checklist/workpack.
