...
| Android | Physical device package/default-handler visibility proof present; owned-shell proof is emulator-scoped | `11-android-host-device-proof.json` records the physical Wi-Fi ADB target as a redacted Samsung Galaxy S9 SM-G965W/star2qltecs device ref, queries known browser package ids only, records Chrome/Edge/Samsung Internet visibility, captures a screenshot/UI-tree/logcat hashes, and records a redacted default VIEW handler ref. `15-android-owned-browser-shell-proof.json` records proof-created AVD Device Owner enrollment and browser-role routing policy mutation for an owned browser shell path. It does not claim physical-device owned-shell custody, exact URL proof, known-active tab proof, VPN/DNS browser proof, UsageStats/Accessibility route proof, Play signing, broad browser enforcement, content filtering, or final policy execution. |
