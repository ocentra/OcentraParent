...
| --- | --- | --- |
| Windows | Host browser inventory proof present; managed launch proof manual-required | `13-windows-host-browser-proof.json` records known Edge/Chrome/Firefox executable/App Paths visibility and queried HTTP/HTTPS URL-association-key boundary refs as redacted hashes; this host did not expose default-handler visibility. It does not claim managed launch, bridge custody, exact URL proof, known-active tab proof, managed profile repair, or enforcement. |
...
| Linux | WSL package/PATH boundary proof present; desktop adapter proof manual-required | `12-linux-host-package-proof.json` records WSL Ubuntu availability and queries only known browser commands, Debian package names, and desktop-entry globs. No Chrome/Chromium/Firefox/Edge command, package, or desktop entry is visible. It does not claim a Linux desktop session, managed profile, Snap/Flatpak proof, exact URL proof, known-active tab proof, Firefox BiDi, or enforcement. |
