...
export function ProfilePictureModal({ isOpen, onClose, user, onUpdatePhoto, getAvatars }: ProfilePictureModalProps) {
  ...
  const previewSrc =
    selectedAvatar || user?.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.email || 'default'}`;

  return (
    <div className={styles.profileModalOverlay} role="dialog" aria-modal="true" aria-labelledby="profile-modal-title">
      <button
        type="button"
