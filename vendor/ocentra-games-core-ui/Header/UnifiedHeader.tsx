...
const bundledProfileNames = Object.keys(BUNDLED_PROFILES)
...
function getBundledProfileConfig(name: string): UnifiedHeaderConfigInput | null {
  ...
  if (
    ...
    'config' in (bundledCandidate as Record<string, unknown>)
  ) {
...
function splitWords(text: string, style: TextStyleConfig) {
  ...
  return source.trim().length === 0
    ...
    : source
        ...
        .split(/\s+/)
        .filter(Boolean)
        .map((word) => splitWord(word, style.splitMode));
}
...
function estimateTextWidth(text: string, style: TextStyleConfig, fontSizeOverride?: number) {
  ...
  return words.reduce((total, word, index) => {
    ...
    const restWidth = Math.max(
      0,
      word.rest.length * (fontSize * normalizedStyle.restScale * 0.55 + normalizedStyle.letterSpacing)
    );
...
function AuthDoorIcon({ direction = 'out', ...svgProps }: { direction?: 'in' | 'out' } & SVGProps<SVGSVGElement>) {
  ...
  return (
    <svg viewBox="0 0 300 300" fill="none" aria-hidden="true" focusable="false" {...svgProps}>
      ...
      />
      <path d="M106 41H146" stroke={`url(#${doorHighlightId})`} strokeWidth="16" strokeLinecap="round" />
      <path d="M106 259H146" stroke={`url(#${doorHighlightId})`} strokeWidth="16" strokeLinecap="round" />
...
function estimateCenterBoxWidth(
  ...
  const leftIcons =
    config.modeB.leftIcons || (config.modeB.icons ? [config.modeB.icons[0], config.modeB.icons[1]] : []);
  const rightIcons =
    config.modeB.rightIcons || (config.modeB.icons ? [config.modeB.icons[2], config.modeB.icons[3]] : []);
  ...
  const leftIconsWidth =
    leftIconCount > 0
      ? leftIconCount * iconSize + Math.max(0, leftIconCount - 1) * Math.max(0, config.modeB.pairGap)
      : 0;
  const rightIconsWidth =
    rightIconCount > 0
      ? rightIconCount * iconSize + Math.max(0, rightIconCount - 1) * Math.max(0, config.modeB.pairGap)
      : 0;
...
function applyRuntimeOverlay(
  ...
  return {
    ...
    right: {
      ...
      ...(runtimeOverlay.right?.onUpgradeGuestClick !== undefined
        ? { onUpgradeGuestClick: runtimeOverlay.right.onUpgradeGuestClick }
        : {}),
      ...(runtimeOverlay.right?.onAdminDashboardClick !== undefined
        ? { onAdminDashboardClick: runtimeOverlay.right.onAdminDashboardClick }
        : {}),
      ...(runtimeOverlay.right?.onViewProfileClick !== undefined
        ? { onViewProfileClick: runtimeOverlay.right.onViewProfileClick }
        : {}),
      ...(runtimeOverlay.right?.onSettingsClick !== undefined
        ? { onSettingsClick: runtimeOverlay.right.onSettingsClick }
        : {}),
      ...(runtimeOverlay.right?.onSecurityClick !== undefined
        ? { onSecurityClick: runtimeOverlay.right.onSecurityClick }
        : {}),
      ...(runtimeOverlay.right?.onUpdatePhoto !== undefined
        ? { onUpdatePhoto: runtimeOverlay.right.onUpdatePhoto }
        : {}),
    ...
    center: {
      ...
      ...(runtimeOverlay.center?.customRenderer !== undefined
        ? { customRenderer: runtimeOverlay.center.customRenderer }
        : {}),
      modeA: {
        ...profileConfig.center.modeA,
        ...(runtimeOverlay.center?.modeA?.leftText !== undefined
          ? { leftText: runtimeOverlay.center.modeA.leftText }
          : {}),
        ...(runtimeOverlay.center?.modeA?.rightText !== undefined
          ? { rightText: runtimeOverlay.center.modeA.rightText }
          : {}),
        logo: {
          ...profileConfig.center.modeA.logo,
          ...(runtimeOverlay.center?.modeA?.logo?.renderer !== undefined
            ? { renderer: runtimeOverlay.center.modeA.logo.renderer }
            : {}),
        },
      },
      modeB: {
        ...
        ...(runtimeOverlay.center?.modeB?.tagline !== undefined
          ? { tagline: runtimeOverlay.center.modeB.tagline }
          : {}),
        ...
        ...(runtimeOverlay.center?.modeB?.leftIcons !== undefined
          ? { leftIcons: runtimeOverlay.center.modeB.leftIcons }
          : {}),
        ...(runtimeOverlay.center?.modeB?.rightIcons !== undefined
          ? { rightIcons: runtimeOverlay.center.modeB.rightIcons }
          : {}),
      },
    },
  };
...
export function UnifiedHeader({
  ...
  const runtimeConfig = useMemo(() => mergeHeaderConfig(derivedConfig, inputConfig), [derivedConfig, inputConfig]);
  ...
  const [profiles, setProfiles] = useState<string[]>(
    Array.from(new Set([DEFAULT_PROFILE_NAME, ...bundledProfileNames]))
  );
  const profileCacheRef = useRef<Record<string, UnifiedHeaderConfigInput | null>>({});

  const normalizeLoadedConfig = useCallback((loadedConfig?: UnifiedHeaderConfigInput) => {
  ...
  const resolveRouteProfile = useCallback(
      ...
      <div className={styles.debugPanel}>
        ...
        <div className={styles.debugScrollArea}>
          {activeTab === 'layout' && layoutSubtab === 'global' && (
            <>
              ...
              <Control
                label="Height"
                path="layout.height"
                hint="Overall header height."
                value={config.layout.height}
                min={36}
                max={120}
                step={1}
                onChange={(v) => updateLayout({ height: v })}
                onReset={() => updateLayout({ height: initialConfig.layout.height })}
              />
              <ToggleControl
                label="Limit width"
                path="layout.maxWidth"
                hint="Enable a maximum header width."
                value={Boolean(config.layout.maxWidth)}
                onChange={(v) => updateLayout({ maxWidth: v ? 1920 : undefined })}
              />
              <Control
                label="Max width"
                path="layout.maxWidth"
                value={config.layout.maxWidth || 1920}
                min={640}
                max={5120}
                step={10}
              ...
              <Control
                label="Outer margin"
                path="layout.outerMargin"
                value={config.layout.outerMargin}
                min={0}
                max={60}
                step={1}
                onChange={(v) => updateLayout({ outerMargin: v })}
              />
              <Control
                label="Box gap"
                path="layout.boxGap"
                hint="Gap between pills and connector bars."
                value={config.layout.boxGap}
                min={0}
                max={40}
                step={1}
                onChange={(v) => updateLayout({ boxGap: v })}
              />
              <Control
                label="Pill height"
                path="layout.boxHeight"
                value={config.layout.boxHeight}
                min={36}
                max={72}
                step={1}
                onChange={(v) => updateLayout({ boxHeight: v })}
              />
              <Control
                label="Center min width"
                path="layout.centerMinWidth"
                value={config.layout.centerMinWidth}
                min={60}
                max={320}
                step={1}
                onChange={(v) => updateLayout({ centerMinWidth: v })}
              />

              <div className={styles.debugDivider}>Global surface</div>
              <ColorControl
                label="Tint color"
                path="style.tintColor"
                value={config.style.tintColor}
                onChange={(v) => updateStyle({ tintColor: v })}
              />
              <ColorControl
                label="Edge color"
                path="style.edgeColor"
                value={config.style.edgeColor}
                onChange={(v) => updateStyle({ edgeColor: v })}
              />
              <ColorControl
                label="Circle fill"
                path="style.circleFillColor"
                value={config.style.circleFillColor}
                onChange={(v) => updateStyle({ circleFillColor: v })}
              />
              <Control
                label="Box opacity"
                path="style.boxOpacity"
                value={config.style.boxOpacity}
                min={0}
                max={1}
                step={0.01}
                onChange={(v) => updateStyle({ boxOpacity: v })}
              />
              <Control
                label="Pill glow"
                path="style.pillGlowBlur"
                value={config.style.pillGlowBlur}
                min={0}
                max={20}
                step={0.1}
                onChange={(v) => updateStyle({ pillGlowBlur: v })}
              />
              <Control
                label="Backdrop blur"
                path="style.backdropBlur"
                value={config.style.backdropBlur}
                min={0}
                max={40}
                step={1}
              ...
              <Control
                label="Icon glow"
                path="style.iconGlowBlur"
                hint="Global glow for icons, logos, and avatars only."
                value={config.style.iconGlowBlur}
                min={0}
                max={12}
                step={0.1}
                onChange={(v) => updateStyle({ iconGlowBlur: v })}
              />

              <div className={styles.debugDivider}>Hover</div>
              <ColorControl
                label="Hover tint"
                path="style.hoverTintColor"
                value={config.style.hoverTintColor}
                onChange={(v) => updateStyle({ hoverTintColor: v })}
              />
              <ColorControl
                label="Hover edge"
                path="style.hoverEdgeColor"
                value={config.style.hoverEdgeColor}
                onChange={(v) => updateStyle({ hoverEdgeColor: v })}
              />
              <Control
                label="Hover boost"
                path="style.hoverBoxOpacityBoost"
                value={config.style.hoverBoxOpacityBoost}
                min={0}
                max={0.4}
                step={0.01}
                onChange={(v) => updateStyle({ hoverBoxOpacityBoost: v })}
              />
            </>
          )}
          ...
          {activeTab === 'layout' && layoutSubtab === 'home' && (
            <>
              ...
              <TextInput
                ...
                onChange={(v) => updateLeft({ text: v })}
              />
              <Control
                label="Expanded width"
                path="layout.leftExpandedWidth"
                value={config.layout.leftExpandedWidth}
                min={72}
                max={240}
                step={1}
                onChange={(v) => updateLayout({ leftExpandedWidth: v })}
              />
              <Control
                label="Collapsed width"
                path="layout.leftCollapsedWidth"
                value={config.layout.leftCollapsedWidth}
                min={40}
                max={120}
                step={1}
                onChange={(v) => updateLayout({ leftCollapsedWidth: v })}
              />
              <Control
                label="Collapse at"
                path="layout.leftCollapseWidth"
                hint="Viewport width threshold for collapsing to icon-only."
                value={config.layout.leftCollapseWidth}
                min={320}
                max={1600}
                step={10}
                onChange={(v) => updateLayout({ leftCollapseWidth: v })}
              />
              <Control
                label="Icon size"
                path="left.iconSize"
                value={config.left.iconSize}
                min={8}
                max={44}
                step={1}
                onChange={(v) => updateLeft({ iconSize: v })}
              />
              <Control
                label="Icon offset X"
                path="left.iconOffsetX"
                value={config.left.iconOffsetX}
                min={-30}
                max={30}
                step={1}
                onChange={(v) => updateLeft({ iconOffsetX: v })}
              />
              <Control
                label="Icon offset Y"
                path="left.iconOffsetY"
                value={config.left.iconOffsetY}
                min={-20}
                max={20}
                step={1}
                onChange={(v) => updateLeft({ iconOffsetY: v })}
              />
              <ToggleControl
                label="Button"
                path="left.isButton"
                value={config.left.isButton ?? false}
                onChange={(v) => updateLeft({ isButton: v })}
              />
              <TextStyleControls
                title="Home text"
          ...
          {activeTab === 'layout' && layoutSubtab === 'wings' && (
            <>
              <div className={styles.debugSectionHeader}>
                <span>Connector Bars</span>
              </div>
              <Control
                label="Curve"
                path="layout.wingCurve"
                value={config.layout.wingCurve}
                min={0}
                max={60}
                step={1}
                onChange={(v) => updateLayout({ wingCurve: v })}
              />
              <Control
                label="Underlap"
                path="layout.wingUnderlap"
                value={config.layout.wingUnderlap}
                min={0}
                max={60}
                step={1}
                onChange={(v) => updateLayout({ wingUnderlap: v })}
              />
              <Control
                label="Thickness"
                path="layout.wingHeight"
                value={config.layout.wingHeight}
                min={0}
                max={36}
                step={1}
                onChange={(v) => updateLayout({ wingHeight: v })}
              />
              <Control
                label="Opacity"
                path="style.wingOpacity"
                value={config.style.wingOpacity}
                min={0}
                max={0.8}
                step={0.01}
                onChange={(v) => updateStyle({ wingOpacity: v })}
              />
            </>
          )}
            ...
            <>
              ...
              {centerPanelTab === 'general' && (
                <>
                  <label className={styles.debugSelectLabel}>
                    ...
                    <select
                      value={config.center.mode}
                      onChange={(e) => updateCenter({ mode: e.target.value as CenterMode })}
                    >
                  ...
                  <Control
                    ...
                    path="layout.centerWidth"
                    value={config.layout.centerWidth}
                    min={96}
                    max={520}
                    step={1}
                    onChange={(v) => updateLayout({ centerWidth: v })}
                  />
                  <Control
                    label="Min width"
                    path="layout.centerMinWidth"
                  ...
                  <Control
                    label="Side padding"
                    path="center.sidePadding"
                    value={config.center.sidePadding}
                    min={0}
                    max={40}
                    step={1}
                    onChange={(v) => updateCenter({ sidePadding: v })}
                  />
                  <Control
                    label="Content gap"
                    path="center.contentGap"
                    value={config.center.contentGap}
                    min={0}
                    max={24}
                    step={1}
                    onChange={(v) => updateCenter({ contentGap: v })}
                  />
                </>
              )}
              ...
              {centerPanelTab === 'modeA' && (
                <>
                  <TextInput
                    label="Left text"
                    path="center.modeA.leftText"
                    value={config.center.modeA.leftText}
                    onChange={(v) => updateCenterA({ leftText: v })}
                  />
                  <TextInput
                    label="Right text"
                    path="center.modeA.rightText"
                    value={config.center.modeA.rightText}
                    onChange={(v) => updateCenterA({ rightText: v })}
                  />
                  <SelectControl
                    label="Left align"
                    path="center.modeA.leftTextAlign"
                    hint="Anchor for the left title text inside its half."
                    value={config.center.modeA.leftTextAlign}
                    onChange={(next) => updateCenterA({ leftTextAlign: next as TextAnchorMode })}
                    options={[
                  ...
                  <SelectControl
                    label="Right align"
                    path="center.modeA.rightTextAlign"
                    hint="Anchor for the right title text inside its half."
                    value={config.center.modeA.rightTextAlign}
                    onChange={(next) => updateCenterA({ rightTextAlign: next as TextAnchorMode })}
                    options={[
                      { value: 'start', label: 'left' },
                      { value: 'middle', label: 'center' },
                      { value: 'end', label: 'right' },
                  ...
                  <Control
                    label="Logo size"
                    path="center.modeA.logo.size"
                    value={config.center.modeA.logo.size}
                    min={8}
                    max={64}
                    step={1}
                    onChange={(v) => updateCenterA({ logo: { ...config.center.modeA.logo, size: v } })}
                  />
                  <Control
                    label="Logo offset X"
                    path="center.modeA.logo.offsetX"
                    value={config.center.modeA.logo.offsetX}
                    min={-24}
                    max={24}
                    step={1}
                    onChange={(v) => updateCenterA({ logo: { ...config.center.modeA.logo, offsetX: v } })}
                  />
                  <Control
                    label="Logo offset Y"
                    path="center.modeA.logo.offsetY"
                    value={config.center.modeA.logo.offsetY}
                    min={-24}
                    max={24}
                    step={1}
                    onChange={(v) => updateCenterA({ logo: { ...config.center.modeA.logo, offsetY: v } })}
                  />
                  <Control
                    label="Logo stroke"
                    path="center.modeA.logo.strokeWidth"
                    value={config.center.modeA.logo.strokeWidth}
                    min={0}
                    max={4}
                    step={0.1}
                    onChange={(v) => updateCenterA({ logo: { ...config.center.modeA.logo, strokeWidth: v } })}
                  />
                  <Control
                    label="Logo fill opacity"
                    path="center.modeA.logo.innerOpacity"
                    value={config.center.modeA.logo.innerOpacity}
                    min={0}
                    max={1}
                    step={0.01}
                    onChange={(v) => updateCenterA({ logo: { ...config.center.modeA.logo, innerOpacity: v } })}
                  />
                  <TextStyleControls
                    title="Mode A text"
              ...
              {centerPanelTab === 'modeB' && (
                <>
                  <TextInput
                    label="Main text"
                    path="center.modeB.text"
                    value={config.center.modeB.text || ''}
                    onChange={(v) => updateCenterB({ text: v })}
                  />
                  <TextInput
                    label="Tagline"
                    path="center.modeB.tagline"
                    value={config.center.modeB.tagline || ''}
                    onChange={(v) => updateCenterB({ tagline: v })}
                  />
                  <SelectControl
                    label="Main font"
                    ...
                    onChange={(next) =>
                      updateCenterB({ textStyle: { ...config.center.modeB.textStyle, fontFamily: next } })
                    }
                  ...
                  <TextInput
                    ...
                    path="center.modeB.logo"
                    hint="Optional image URL for the center logo in mode B."
                    value={getModeBIconValue(config.center.modeB.logo)}
                    onChange={(v) => updateCenterB({ logo: v.trim() || undefined })}
                  />
                  <Control
                    label="Icon size"
                    path="center.modeB.iconSize"
                    value={config.center.modeB.iconSize}
                    min={8}
                    max={40}
                    step={1}
                    onChange={(v) => updateCenterB({ iconSize: v })}
                  />
                  <Control
                    label="Pair gap"
                    path="center.modeB.pairGap"
                    value={config.center.modeB.pairGap}
                    min={-12}
                    max={20}
                    step={1}
                    onChange={(v) => updateCenterB({ pairGap: v })}
                  />
                  <TextInput
                    label="Left icon 1"
                    path="center.modeB.leftIcons[0]"
                    value={getModeBIconValue(config.center.modeB.leftIcons?.[0])}
                    onChange={(v) => updateModeBIcon('leftIcons', 0, v)}
                  />
                  <TextInput
                    label="Left icon 2"
                    path="center.modeB.leftIcons[1]"
                    value={getModeBIconValue(config.center.modeB.leftIcons?.[1])}
                    onChange={(v) => updateModeBIcon('leftIcons', 1, v)}
                  />
                  <TextInput
                    label="Right icon 1"
                    path="center.modeB.rightIcons[0]"
                    value={getModeBIconValue(config.center.modeB.rightIcons?.[0])}
                    onChange={(v) => updateModeBIcon('rightIcons', 0, v)}
                  />
                  <TextInput
                    label="Right icon 2"
                    path="center.modeB.rightIcons[1]"
                    value={getModeBIconValue(config.center.modeB.rightIcons?.[1])}
                    onChange={(v) => updateModeBIcon('rightIcons', 1, v)}
                  />
                  <TextStyleControls
          ...
          {activeTab === 'layout' && layoutSubtab === 'login' && (
            <>
              <div className={styles.debugSectionHeader}>
                <span>Login / Profile Pill</span>
              </div>
              <TextInput
                label="Fallback label"
                path="right.text"
                value={config.right.text}
                onChange={(v) => updateRight({ text: v })}
              />
              <Control
                label="Expanded width"
                path="layout.rightWidth"
                value={config.layout.rightWidth}
                min={72}
                max={300}
                step={1}
                onChange={(v) => updateLayout({ rightWidth: v })}
              />
              <Control
                label="Collapsed width"
                path="layout.rightCollapsedWidth"
                value={config.layout.rightCollapsedWidth}
                min={40}
                max={160}
                step={1}
                onChange={(v) => updateLayout({ rightCollapsedWidth: v })}
              />
              <Control
                label="Collapse at"
                path="layout.rightCollapseWidth"
                value={config.layout.rightCollapseWidth}
                min={320}
                max={1600}
                step={10}
                onChange={(v) => updateLayout({ rightCollapseWidth: v })}
              />
              <ToggleControl
                label="Profile mode"
                path="right.isProfile"
                value={config.right.isProfile ?? false}
                onChange={(v) =>
                  updateRight({
                    isProfile: v,
                    user: v ? config.right.user || { name: 'Player One', isLoggedIn: true } : config.right.user,
                  })
                }
              />
              <ToggleControl
                label="Button"
                path="right.isButton"
                value={config.right.isButton ?? false}
                onChange={(v) => updateRight({ isButton: v })}
              />
              {config.right.isProfile && (
                <>
                  <TextInput
                    label="User name"
                    path="right.user.name"
                    value={config.right.user?.name || ''}
                    onChange={(v) => updateRight({ user: { ...config.right.user!, name: v } })}
                  />
                  <TextInput
                    label="Avatar URL"
                    path="right.user.avatarUrl"
                    value={config.right.user?.avatarUrl || ''}
                    onChange={(v) => updateRight({ user: { ...config.right.user!, avatarUrl: v } })}
                  />
                </>
              )}
              <TextStyleControls
              ...
              <ColorControl
                label="Dropdown tint"
                path="style.dropdownTint"
                value={config.style.dropdownTint}
                onChange={(v) => updateStyle({ dropdownTint: v })}
              />
              <TextInput
                label="Dropdown border"
                path="style.dropdownBorderColor"
                value={config.style.dropdownBorderColor}
                onChange={(v) => updateStyle({ dropdownBorderColor: v })}
              />
              <Control
                label="Section opacity"
                path="style.dropdownSectionOpacity"
                value={config.style.dropdownSectionOpacity}
                min={0}
                max={0.3}
                step={0.01}
                onChange={(v) => updateStyle({ dropdownSectionOpacity: v })}
              />
            </>
          )}
            ...
            <>
              ...
              {navPanelTab === 'layout' && (
                <>
                  <ToggleControl
                    label="Enabled"
                    path="navigation.enabled"
                    hint="Show the shared route nav under the header pills on pages that allow it."
                    value={config.navigation.enabled}
                    onChange={(v) => updateNavigation({ enabled: v })}
                  />
                  <Control
                    label="Height"
                    path="navigation.height"
                    hint="Overall nav strip height."
                    value={config.navigation.height}
                    min={32}
                    max={86}
                    step={1}
                    onChange={(v) => updateNavigation({ height: v })}
                  />
                  <Control
                    label="Gap below header"
                    path="navigation.gapBelowHeader"
                    hint="Measured offset below the visible bottom of the three header pills."
                    value={config.navigation.gapBelowHeader}
                    min={0}
                    max={24}
                    step={1}
                    onChange={(v) => updateNavigation({ gapBelowHeader: v })}
                  />
                  <Control
                    label="Outer margin"
                    path="navigation.outerMargin"
                    hint="Horizontal inset from the page edges."
                    value={config.navigation.outerMargin}
                    min={0}
                    max={80}
                    step={1}
                    onChange={(v) => updateNavigation({ outerMargin: v })}
                  />
                  <Control
                    label="Panel inset Y"
                    path="navigation.panelInsetY"
                    value={config.navigation.panelInsetY}
                    min={0}
                    max={16}
                    step={1}
                    onChange={(v) => updateNavigation({ panelInsetY: v })}
                  />
                  <Control
                    label="Shell inset"
                    path="navigation.shellInset"
                    value={config.navigation.shellInset}
                    min={0}
                    max={16}
                    step={1}
                    onChange={(v) => updateNavigation({ shellInset: v })}
                  />
                  <Control
                    label="Panel radius"
                    path="navigation.panelRadius"
                    value={config.navigation.panelRadius}
                    min={0}
                    max={44}
                    step={1}
                    onChange={(v) => updateNavigation({ panelRadius: v })}
                  />
                  <Control
                    label="Button radius"
                    path="navigation.buttonRadius"
                    value={config.navigation.buttonRadius}
                    min={0}
                    max={24}
                    step={1}
                    onChange={(v) => updateNavigation({ buttonRadius: v })}
                  />
                  <Control
                    label="Min button width"
                    path="navigation.minButtonWidth"
                    value={config.navigation.minButtonWidth}
                    min={50}
                    max={180}
                    step={1}
                    onChange={(v) => updateNavigation({ minButtonWidth: v })}
                  />
                  <Control
                    label="Max button width"
                    path="navigation.maxButtonWidth"
                    value={config.navigation.maxButtonWidth}
                    min={90}
                    max={280}
                    step={1}
                    onChange={(v) => updateNavigation({ maxButtonWidth: v })}
                  />
                  <Control
                    label="Button gap"
                    path="navigation.buttonGap"
                    value={config.navigation.buttonGap}
                    min={0}
                    max={18}
                    step={1}
                    onChange={(v) => updateNavigation({ buttonGap: v })}
                  />
                  <Control
                    label="Nav gap"
                    path="navigation.navGap"
                    hint="Gap between overflow arrow shells and the button run."
                    value={config.navigation.navGap}
                    min={0}
                    max={18}
                    step={1}
                    onChange={(v) => updateNavigation({ navGap: v })}
                  />
                  <Control
                    label="Accent inset"
                    path="navigation.accentInset"
                    value={config.navigation.accentInset}
                    min={2}
                    max={24}
                    step={1}
                    onChange={(v) => updateNavigation({ accentInset: v })}
                  />
                  <Control
                    label="Arrow inset"
                    path="navigation.sideArrowInset"
                    value={config.navigation.sideArrowInset}
                    min={4}
                    max={18}
                    step={1}
                    onChange={(v) => updateNavigation({ sideArrowInset: v })}
                  />
                  <Control
                    label="End curve padding"
                    path="navigation.endCurvePadding"
                    value={config.navigation.endCurvePadding}
                    min={0}
                    max={60}
                    step={1}
                    onChange={(v) => updateNavigation({ endCurvePadding: v })}
                  />
                </>
              )}
              ...
              {navPanelTab === 'surface' && (
                <>
                  <ColorControl
                    label="Tint color"
                    path="navigation.tintColor"
                    value={config.navigation.tintColor}
                    onChange={(v) => updateNavigation({ tintColor: v })}
                  />
                  <ColorControl
                    label="Edge color"
                    path="navigation.edgeColor"
                    value={config.navigation.edgeColor}
                    onChange={(v) => updateNavigation({ edgeColor: v })}
                  />
                  <Control
                    label="Box opacity"
                    path="navigation.boxOpacity"
                    value={config.navigation.boxOpacity}
                    min={0}
                    max={1}
                    step={0.01}
                    onChange={(v) => updateNavigation({ boxOpacity: v })}
                  />
                  <Control
                    label="Glow blur"
                    path="navigation.glowBlur"
                    value={config.navigation.glowBlur}
                    min={0}
                    max={20}
                    step={0.1}
                    onChange={(v) => updateNavigation({ glowBlur: v })}
                  />
                </>
              )}
              ...
              {navPanelTab === 'states' && (
                <>
                  <ColorControl
                    label="Hover tint"
                    path="navigation.hoverTintColor"
                    value={config.navigation.hoverTintColor}
                    onChange={(v) => updateNavigation({ hoverTintColor: v })}
                  />
                  <ColorControl
                    label="Hover edge"
                    path="navigation.hoverEdgeColor"
                    value={config.navigation.hoverEdgeColor}
                    onChange={(v) => updateNavigation({ hoverEdgeColor: v })}
                  />
                  <ColorControl
                    label="Active tint"
                    path="navigation.activeTintColor"
                    value={config.navigation.activeTintColor}
                    onChange={(v) => updateNavigation({ activeTintColor: v })}
                  />
                  <ColorControl
                    label="Active edge"
                    path="navigation.activeEdgeColor"
                    value={config.navigation.activeEdgeColor}
                    onChange={(v) => updateNavigation({ activeEdgeColor: v })}
                  />
                </>
              )}
            </>
            ...
            <>
              ...
              <div className={styles.debugControlRow} style={{ alignItems: 'center', gap: '0.5rem' }}>
                <ControlMeta
                  ...
                  hint="Load another profile into memory without saving yet."
                />
                <select
                ...
                <button className={styles.debugMiniButton} onClick={() => void handleLoadSelectedProfile()}>
                  Load
                </button>
                <input
                  type="text"
              ...
              <div className={styles.debugControlRow} style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
                <ControlMeta
                  label="Route match patterns"
                  path="metadata.matchPatterns"
                  hint="Routes that should auto-load this profile."
                />
                <textarea
                  style={{
                    width: '100%',
                ...
                />
                <button
                  className={styles.debugMiniButton}
  ...
  const geometry = useMemo(() => {
    ...
    const leftDesiredWidth = isLeftCollapsed
      ? leftCollapsedWidth
      : Math.min(
          layout.leftExpandedWidth,
          Math.max(leftCollapsedWidth, estimateLeftBoxWidth(left, boxHeight, substituteVariables))
        );
    ...
    const rightDesiredWidth = isRightCollapsed
      ...
      : Math.min(
          layout.rightWidth,
          Math.max(rightCollapsedWidth, estimateRightBoxWidth(right, boxHeight, substituteVariables))
        );
    ...
    const maxCenterWidthFromLeft = Math.max(
      layout.centerMinWidth,
      2 * (centerX - (outerMargin + finalLeftWidth + boxGap))
    );
    const maxCenterWidthFromRight = Math.max(
      layout.centerMinWidth,
      2 * (viewWidth - outerMargin - finalRightWidth - boxGap - centerX)
    );
    ...
    const rightBox: HeaderBoxRect = {
      x: viewWidth - outerMargin - finalRightWidth,
      y: boxY,
      w: finalRightWidth,
      h: boxHeight,
    };
    const centerBox: HeaderBoxRect = {
      x: centerX - responsiveCenterWidth / 2,
      y: boxY,
      w: responsiveCenterWidth,
      h: boxHeight,
    };
  ...
  const shouldKeepCollapsedLoginText =
    !right.isProfile && Boolean(right.onClick) && (right.text || '').trim().length > 0;

  if (debugControlsOnly) {
    return <div className={styles.debugControlsOnly}>{renderDebugPanel()}</div>;
  }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', display: 'flex', alignItems: 'center' }}>
      ...
      <div
        ref={wrapRef}
        className={[styles.unifiedHeaderWrapper, placement === 'contained' ? styles.unifiedHeaderWrapperContained : '']
          .filter(Boolean)
          .join(' ')}
        style={{
          flex: 1,
              ...
              {renderBackdropShape(
                {
                  ...
                  clipPath: buildWingClipPath(
                    Math.abs(geometry.leftWing.x2 - geometry.leftWing.x1) * scaleX,
                    geometry.leftWing.h * scaleY,
                    layout.wingCurve * scaleX
                  ),
                  backdropFilter: `blur(${style.backdropBlur}px)`,
                  WebkitBackdropFilter: `blur(${style.backdropBlur}px)`,
                },
              ...
              {renderBackdropShape(
                {
                  ...
                  clipPath: buildWingClipPath(
                    Math.abs(geometry.rightWing.x2 - geometry.rightWing.x1) * scaleX,
                    geometry.rightWing.h * scaleY,
                    layout.wingCurve * scaleX
                  ),
                  backdropFilter: `blur(${style.backdropBlur}px)`,
                  WebkitBackdropFilter: `blur(${style.backdropBlur}px)`,
          ...
          >
            ...
            <>
              ...
              <HeaderPill
                ...
                ariaLabel={left.ariaLabel ?? left.text}
              >
                {left.customRenderer ? (
          ...
          <div className={styles.dropdownHeader}>
            {isGuestUser ? (
              <div className={styles.dropdownAvatar}>
                <img src={authAnnonImageUrl} alt="Anonymous User" style={{ width: '100%', height: '100%' }} />
              </div>
            ) : (
              <button
            ...
            <div className={styles.dropdownInfo}>
              ...
              <div className={styles.dropdownStatus}>{isGuestUser ? 'Guest session' : right.user.email}</div>
            </div>
          </div>
          ...
          {isGuestUser ? (
            ...
            <>
              <div className={styles.dropdownStats}>
                <div>
                  <span>ELO</span>
                  <strong>{right.user.eloRating ?? 1200}</strong>
                </div>
                <div>
                  <span>Games</span>
                  <strong>{right.user.gamesPlayed ?? 0}</strong>
                </div>
                <div>
                  <span>Win</span>
                  <strong>{right.user.winRate?.toFixed(1) ?? '0'}%</strong>
                </div>
              </div>
              <div className={styles.dropdownDivider} />
              <div className={styles.dropdownMenuSections}>
        ...
        (() => {
          const debugControls = (
            ...
            >
              ...
              <button type="button" onClick={() => setShowControls((v) => !v)} className={styles.debugToggleButton}>
                {showControls ? 'Hide Header Controls' : 'Show Header Controls'}
              </button>
            </div>
          );

          return footerDebugDockHost ? createPortal(debugControls, footerDebugDockHost) : debugControls;
        })()}
    </div>
  );
}
...
function HeaderPill({
  ...
  const fillOpacity =
    interactive && hovered ? Math.min(1, style.boxOpacity + style.hoverBoxOpacityBoost) : style.boxOpacity;
...
function LeftHomeContent({
  ...
  return (
    <g>
      <g
        transform={`translate(${circleCx} ${circleCy}) scale(${aspectCorrection} 1) translate(${-circleCx} ${-circleCy})`}
      >
        <circle
          cx={circleCx}
          cy={circleCy}
...
function CenterModeB({
  ...
  const maxTextWidth = Math.max(
    1,
    box.w - config.sidePadding * 2 - iconSize * 4 - mode.pairGap * 2 - config.contentGap * 2
  );
  ...
  return (
    <g>
      ...
      {leftIcons[0] && (
        <CorrectedIcon
          cx={leftPairCenter - pairHalfSpread}
          cy={mainY}
          size={iconSize}
          color="#050505"
          style={style}
          aspectCorrection={aspectCorrection}
          renderer={leftIcons[0]}
        />
      )}
      {leftIcons[1] && (
        <CorrectedIcon
      ...
      {rightIcons[0] && (
        <CorrectedIcon
          cx={rightPairCenter - pairHalfSpread}
          cy={mainY}
          size={iconSize}
          color="#ff3b45"
          style={style}
          aspectCorrection={aspectCorrection}
          renderer={rightIcons[0]}
        />
      )}
      {rightIcons[1] && (
        <CorrectedIcon
...
function CorrectedIcon({
  ...
  return (
    <g
      filter={style.iconGlowBlur > 0 ? 'url(#iconGlow)' : undefined}
      transform={`translate(${cx} ${cy}) scale(${aspectCorrection} 1) translate(${-cx} ${-cy})`}
    >
  ...
  return (
    <g>
      ...
      >
        ...
        />
        <g filter={style.iconGlowBlur > 0 ? 'url(#iconGlow)' : undefined}>
          <AuthDoorIcon direction="in" x={iconX} y={iconY} width={iconSize} height={iconSize} />
        </g>
      </g>
...
function FitText({
  ...
  const adjustedFontSize =
    estimatedWidth > maxWidth ? Math.max(Math.max(8, safeFontSize * 0.62), safeFontSize * shrinkRatio) : safeFontSize;
  ...
  return (
    <g transform={`translate(${x} ${y}) scale(${aspectCorrection} 1) translate(${-x} ${-y})`}>
      <defs>
        <linearGradient id={ids.gradientgold} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#fff8c9" />
          <stop offset="39%" stopColor="#efc75c" />
          <stop offset="70%" stopColor="#a96f1d" />
          <stop offset="100%" stopColor="#ffe9a6" />
        </linearGradient>
        <linearGradient id={ids.gradientroyalGold} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#ffffff" />
          <stop offset="18%" stopColor="#ffeeb0" />
          <stop offset="48%" stopColor="#dfa832" />
          <stop offset="72%" stopColor="#7c4812" />
          <stop offset="100%" stopColor="#fff0a8" />
        </linearGradient>
        <linearGradient id={ids.gradientsilver} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#ffffff" />
          <stop offset="38%" stopColor="#d8e1e6" />
          <stop offset="72%" stopColor="#74818a" />
          <stop offset="100%" stopColor="#ffffff" />
        </linearGradient>
        <linearGradient id={ids.gradientemerald} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#e8fff0" />
          <stop offset="42%" stopColor="#72f0a9" />
          <stop offset="74%" stopColor="#137d49" />
          <stop offset="100%" stopColor="#cbffdf" />
        </linearGradient>
        <linearGradient id={ids.gradientice} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#ffffff" />
          <stop offset="45%" stopColor="#a8edff" />
          <stop offset="76%" stopColor="#2b86ae" />
          <stop offset="100%" stopColor="#eafcff" />
        </linearGradient>
        <linearGradient id={ids.gradientruby} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#ffe4e8" />
          <stop offset="42%" stopColor="#ff6575" />
          <stop offset="75%" stopColor="#8e1028" />
          <stop offset="100%" stopColor="#ffc2c8" />
        </linearGradient>
        <linearGradient id={ids.gradientfire} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#fff0a3" />
          <stop offset="34%" stopColor="#ff9d2e" />
          <stop offset="68%" stopColor="#d63616" />
          <stop offset="100%" stopColor="#ffe28a" />
        </linearGradient>
        <linearGradient id={ids.customGradient} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={safeStyle.customColorA} />
          <stop offset="48%" stopColor={safeStyle.customColorB} />
          <stop offset="100%" stopColor={safeStyle.customColorC} />
        </linearGradient>
        <filter id={ids.shadowsoft} x="-40%" y="-80%" width="180%" height="260%">
          <feDropShadow dx="0" dy="5" stdDeviation="5" floodColor="rgba(0,0,0,0.65)" />
        </filter>
        <filter id={ids.shadowdeep} x="-40%" y="-80%" width="180%" height="260%">
          <feDropShadow dx="0" dy="9" stdDeviation="7" floodColor="rgba(0,0,0,0.75)" />
        </filter>
        <filter id={ids.shadowglow} x="-40%" y="-80%" width="180%" height="260%">
          <feDropShadow dx="0" dy="0" stdDeviation="6" floodColor="rgba(120,230,255,0.6)" />
        </filter>
        <filter id={ids.shadowholy} x="-40%" y="-80%" width="180%" height="260%">
          <feDropShadow dx="0" dy="0" stdDeviation="7" floodColor="rgba(255,220,120,0.7)" />
        </filter>
        <filter id={ids.shadowneon} x="-40%" y="-80%" width="180%" height="260%">
          <feDropShadow dx="0" dy="0" stdDeviation="7" floodColor="rgba(80,255,160,0.7)" />
        </filter>
        <filter id={ids.shadowengraved} x="-40%" y="-80%" width="180%" height="260%">
          <feDropShadow dx="2" dy="3" stdDeviation="1" floodColor="rgba(0,0,0,0.75)" />
        </filter>
        <filter id={ids.shadowcustom} x="-60%" y="-100%" width="220%" height="300%">
          <feDropShadow
            dx={safeStyle.customShadowX}
            dy={safeStyle.customShadowY}
            stdDeviation={safeStyle.customShadowBlur}
            floodColor={safeStyle.customShadowColor}
          />
        </filter>
      </defs>
      <text
    ...
    <>
      ...
      {activeTextTab === 'basic' && (
        <>
          <SelectControl
            label="Preset"
            path={`${pathPrefix}.__preset`}
            hint="Apply a ready-made title preset."
            value={selectedPreset}
            onChange={applyPreset}
            options={[
              { value: 'custom', label: 'custom' },
              ...Object.keys(SVG_TITLE_PRESETS).map((preset) => ({ value: preset, label: preset })),
            ]}
          />
          <SelectControl
            label="fontFamily"
            path={`${pathPrefix}.fontFamily`}
            hint="Font stack for this text."
          ...
          <SelectControl
            label="splitMode"
            path={`${pathPrefix}.splitMode`}
            hint="How each word is split between first and rest glyphs."
            value={value.splitMode}
            onChange={(next) => onChange({ ...value, splitMode: next as SplitMode })}
            options={[
              { value: 'first-letter', label: 'first-letter' },
              { value: 'first-token', label: 'first-token' },
              { value: 'manual-pipe', label: 'manual-pipe' },
            ]}
          />
          <Control
            label="fontSize"
            path={`${pathPrefix}.fontSize`}
            hint="Base font size before split scaling."
            value={value.fontSize}
            min={7}
            max={96}
            step={1}
            onChange={(next) => onChange({ ...value, fontSize: next })}
          />
          <Control
            label="fontWeight"
            ...
            max={900}
            step={50}
            onChange={(next) => onChange({ ...value, fontWeight: next })}
          />
          <Control
            label="firstScale"
            path={`${pathPrefix}.firstScale`}
            hint="Scale applied to the leading chunk of each word."
            value={value.firstScale}
            min={0.5}
            max={1.8}
            step={0.01}
            onChange={(next) => onChange({ ...value, firstScale: next })}
          />
          <Control
            label="restScale"
            path={`${pathPrefix}.restScale`}
            hint="Scale applied to the remainder of each word."
            value={value.restScale}
            min={0.2}
            max={1.2}
            step={0.01}
            onChange={(next) => onChange({ ...value, restScale: next })}
          />
          <Control
            label="letterSpacing"
            path={`${pathPrefix}.letterSpacing`}
            hint="Tracking between rest letters."
          ...
          <Control
            label="wordGap"
            path={`${pathPrefix}.wordGap`}
            hint="Gap between words."
            value={value.wordGap}
            min={0}
            max={80}
            step={1}
            onChange={(next) => onChange({ ...value, wordGap: next })}
          />
          <Control
            label="restYOffset"
            path={`${pathPrefix}.restYOffset`}
            hint="Vertical offset applied to the smaller rest glyphs."
            value={value.restYOffset}
            min={-40}
            max={40}
            step={1}
            onChange={(next) => onChange({ ...value, restYOffset: next })}
          />
          <Control
            label="opacity"
          ...
          />
          <ToggleControl
            label="forceUppercase"
            path={`${pathPrefix}.forceUppercase`}
            hint="Uppercase text before rendering."
            value={value.forceUppercase}
            onChange={(next) => onChange({ ...value, forceUppercase: next })}
          />
        </>
      )}
      ...
      {activeTextTab === 'fill' && (
        <>
          <SelectControl
            ...
            hint="Gradient or flat fill style."
            value={value.fill}
            onChange={(next) => onChange({ ...value, fill: next as FillStyle })}
            options={[
              { value: 'gold', label: 'gold' },
              { value: 'royalGold', label: 'royalGold' },
          ...
          />
          <ColorControl
            label="customFlatFill"
            path={`${pathPrefix}.customFlatFill`}
            value={value.customFlatFill}
            onChange={(next) => onChange({ ...value, customFlatFill: next, fill: 'flat' })}
          />
          <ColorControl
            label="customColorA"
            path={`${pathPrefix}.customColorA`}
            value={value.customColorA}
            onChange={(next) => onChange({ ...value, customColorA: next, fill: 'custom' })}
          />
          <ColorControl
            label="customColorB"
            path={`${pathPrefix}.customColorB`}
            value={value.customColorB}
            onChange={(next) => onChange({ ...value, customColorB: next, fill: 'custom' })}
          />
          <ColorControl
            label="customColorC"
            path={`${pathPrefix}.customColorC`}
            value={value.customColorC}
            onChange={(next) => onChange({ ...value, customColorC: next, fill: 'custom' })}
          />
        </>
      )}
      ...
      {activeTextTab === 'edge' && (
        <>
          <SelectControl
            label="edge"
            path={`${pathPrefix}.edge`}
            hint="Text outline style."
            value={value.edge}
            onChange={(next) => onChange({ ...value, edge: next as EdgeStyle })}
            options={[
          ...
          />
          <Control
            label="edgeWidthFirst"
            path={`${pathPrefix}.edgeWidthFirst`}
            hint="Outline width for the leading chunk."
            value={value.edgeWidthFirst}
            min={0}
            max={8}
            step={0.1}
            onChange={(next) => onChange({ ...value, edgeWidthFirst: next })}
          />
          <Control
            label="edgeWidthRest"
            path={`${pathPrefix}.edgeWidthRest`}
            hint="Outline width for the rest chunk."
            value={value.edgeWidthRest}
            min={0}
            max={8}
            step={0.1}
            onChange={(next) => onChange({ ...value, edgeWidthRest: next })}
          />
          <ColorControl
            label="customEdgeColor"
            path={`${pathPrefix}.customEdgeColor`}
            value={value.customEdgeColor}
            onChange={(next) => onChange({ ...value, customEdgeColor: next, edge: 'custom' })}
          />
        </>
      )}
      ...
      {activeTextTab === 'shadow' && (
        <>
          <SelectControl
            label="shadow"
            path={`${pathPrefix}.shadow`}
            hint="Shadow or glow preset."
            value={value.shadow}
            onChange={(next) => onChange({ ...value, shadow: next as ShadowStyle })}
            options={[
              { value: 'none', label: 'none' },
          ...
          />
          <ColorControl
            label="customShadowColor"
            path={`${pathPrefix}.customShadowColor`}
            value={value.customShadowColor}
            onChange={(next) => onChange({ ...value, customShadowColor: next, shadow: 'custom' })}
          />
          <Control
            label="customShadowBlur"
            path={`${pathPrefix}.customShadowBlur`}
            value={value.customShadowBlur}
            min={0}
            max={30}
            step={1}
            onChange={(next) => onChange({ ...value, customShadowBlur: next, shadow: 'custom' })}
          />
          <Control
            label="customShadowX"
            path={`${pathPrefix}.customShadowX`}
            value={value.customShadowX}
            min={-40}
            max={40}
            step={1}
            onChange={(next) => onChange({ ...value, customShadowX: next, shadow: 'custom' })}
          />
          <Control
            label="customShadowY"
            path={`${pathPrefix}.customShadowY`}
            value={value.customShadowY}
            min={-40}
            max={40}
            step={1}
            onChange={(next) => onChange({ ...value, customShadowY: next, shadow: 'custom' })}
          />
        </>
      )}
      ...
      {activeTextTab === 'transform' && (
        <>
          <Control
            label="moveX"
            path={`${pathPrefix}.moveX`}
            value={value.moveX}
            min={-250}
            max={250}
            step={1}
            onChange={(next) => onChange({ ...value, moveX: next })}
          />
          <Control
            label="moveY"
            path={`${pathPrefix}.moveY`}
            value={value.moveY}
            min={-120}
            max={120}
            step={1}
            onChange={(next) => onChange({ ...value, moveY: next })}
          />
          <Control
            label="rotate"
            ...
            max={45}
            step={1}
            onChange={(next) => onChange({ ...value, rotate: next })}
          />
          <Control
            label="skewX"
            path={`${pathPrefix}.skewX`}
            value={value.skewX}
            min={-30}
            max={30}
            step={1}
            onChange={(next) => onChange({ ...value, skewX: next })}
          />
        </>
      )}
    </>
  ...
  return (
    <div className={styles.debugControlRow}>
      ...
      <div className={styles.debugControlInputs}>
        ...
        {onReset && (
          <button className={styles.debugResetLink} onClick={onReset} title="Reset">
            ↺
          </button>
        )}
      </div>
    ...
    <div className={styles.debugControlRow}>
      ...
      <div className={styles.debugControlInputs}>
        <input
          ...
          className={styles.debugColorInput}
        />
