...
function getButtonWidth(label: string, shellHeight: number, config: UnifiedHeaderNavigationConfig) {
  ...
  const labelWidth =
    estimateLabelWidth(getRenderedLabel(label, config), textSize) +
    config.textLetterSpacing * Math.max(0, label.length - 1);
...
function getLayout(items: SiteNavItem[], viewWidth: number, viewHeight: number, config: UnifiedHeaderNavigationConfig) {
  ...
  const navX =
    hasOverflow && leftShell
      ? leftShell.x + leftShell.w + config.navGap
      : panel.x + config.shellInset + endCurvePadding;
  const navRight =
    hasOverflow && rightShell ? rightShell.x - config.navGap : panel.x + panel.w - config.shellInset - endCurvePadding;
...
function SiteNavigationSvg({
  ...
  const maxStartIndex = layout.hasOverflow ? getMaxStartIndex(items, layout.navWidth, layout.shellHeight, config) : 0;
  ...
  const overflowNavLeft =
    layout.hasOverflow && layout.leftShell ? layout.leftShell.x + layout.leftShell.w + config.navGap : layout.panel.x;
  const overflowNavRight =
    layout.hasOverflow && layout.rightShell ? layout.rightShell.x - config.navGap : layout.panel.x + layout.panel.w;
    ...
    <svg viewBox={`0 0 ${viewWidth} ${viewHeight}`} preserveAspectRatio="none" className={styles.primaryNavSvg}>
      ...
      {visibleItems.map((item, index) => {
        const x =
          ...
          visibleItems.slice(0, index).reduce((total, previous) => total + previous.width + config.buttonGap, 0);
...
export function PrimarySiteNavigation({ includeAdmin = false, config, extraItems = [] }: PrimarySiteNavigationProps) {
  ...
  const items = useMemo<SiteNavItem[]>(
    () => [
      ...
      {
        label: 'Profile',
        path: PublicRoutePath[PublicRouteKey.PlayerHub],
        matchPrefixes: [PublicRoutePath[PublicRouteKey.PlayerHub], PublicRoutePath[PublicRouteKey.Settings]],
      },
