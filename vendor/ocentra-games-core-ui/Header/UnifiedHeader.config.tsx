...
const textStyleSchema = schema.object({
  fontFamily: schema.string(),
  fontSize: schema.number(),
  fontWeight: schema.number(),
  splitMode: schema.enum(splitModeValues),
  forceUppercase: schema.boolean(),
  firstScale: schema.number(),
  restScale: schema.number(),
  letterSpacing: schema.number(),
  wordGap: schema.number(),
  restYOffset: schema.number(),
  fill: schema.enum(fillStyleValues),
  edge: schema.enum(edgeStyleValues),
  shadow: schema.enum(shadowStyleValues),
  edgeWidthFirst: schema.number(),
  edgeWidthRest: schema.number(),
  opacity: schema.number(),
  moveX: schema.number(),
  moveY: schema.number(),
  rotate: schema.number(),
  skewX: schema.number(),
  customColorA: schema.string(),
  customColorB: schema.string(),
  customColorC: schema.string(),
  customFlatFill: schema.string(),
  customEdgeColor: schema.string(),
  customShadowColor: schema.string(),
  customShadowBlur: schema.number(),
  customShadowX: schema.number(),
  customShadowY: schema.number(),
});

export const serializedUnifiedHeaderConfigSchema = schema.object({
  layout: schema.object({
    height: schema.number(),
    maxWidth: schema.number().optional(),
    minViewWidth: schema.number(),
    outerMargin: schema.number(),
    leftExpandedWidth: schema.number(),
    leftCollapsedWidth: schema.number(),
    leftCollapseWidth: schema.number(),
    centerWidth: schema.number(),
    rightWidth: schema.number(),
    rightCollapsedWidth: schema.number(),
    rightCollapseWidth: schema.number(),
    centerMinWidth: schema.number(),
    boxGap: schema.number(),
    wingCurve: schema.number(),
    wingUnderlap: schema.number(),
    wingHeight: schema.number(),
    boxHeight: schema.number(),
  }),
  style: schema.object({
    tintColor: schema.string(),
    edgeColor: schema.string(),
    boxOpacity: schema.number(),
    wingOpacity: schema.number(),
    pillGlowBlur: schema.number(),
    iconGlowBlur: schema.number(),
    backdropBlur: schema.number(),
    circleFillColor: schema.string(),
    hoverEdgeColor: schema.string(),
    hoverTintColor: schema.string(),
    hoverBoxOpacityBoost: schema.number(),
    dropdownTint: schema.string(),
    dropdownBorderColor: schema.string(),
    dropdownSectionOpacity: schema.number(),
  }),
  left: schema.object({
    text: schema.string(),
    textStyle: textStyleSchema,
    iconSize: schema.number(),
    iconOffsetX: schema.number(),
    iconOffsetY: schema.number(),
    icon: schema.string().optional(),
    isButton: schema.boolean().optional(),
    ariaLabel: schema.string().optional(),
  }),
  right: schema.object({
    text: schema.string(),
    textStyle: textStyleSchema,
  ...
  center: schema.object({
    ...
    modeA: schema.object({
      ...
      leftTextAlign: schema.enum(textAnchorValues),
      rightTextAlign: schema.enum(textAnchorValues),
      textStyle: textStyleSchema,
      logo: schema.object({
        size: schema.number(),
    ...
    modeB: schema.object({
      ...
      iconSize: schema.number(),
      pairGap: schema.number(),
      leftIcons: schema.array(schema.string()).optional(),
      rightIcons: schema.array(schema.string()).optional(),
      logo: schema.string().optional(),
      icons: schema.array(schema.string()).optional(),
    }),
  }),
  navigation: schema.object({
    enabled: schema.boolean(),
    height: schema.number(),
    gapBelowHeader: schema.number(),
    outerMargin: schema.number(),
    panelInsetY: schema.number(),
    shellInset: schema.number(),
    panelRadius: schema.number(),
    buttonRadius: schema.number(),
    minButtonWidth: schema.number(),
    maxButtonWidth: schema.number(),
    textSidePadding: schema.number(),
    buttonGap: schema.number(),
    navGap: schema.number(),
    textScale: schema.number(),
    textFontFamily: schema.string().default(SAFE_SYSTEM_FONTS[14].value),
    textFontWeight: schema.number().default(850),
    textLetterSpacing: schema.number().default(0),
    textForceUppercase: schema.boolean().default(false),
    textColor: schema.string().default('#ffffff'),
    textStrokeColor: schema.string().default('rgba(0, 14, 32, 0.8)'),
    textStrokeWidth: schema.number().default(2.8),
    textOpacity: schema.number().default(1),
    textOffsetY: schema.number().default(0),
    accentInset: schema.number(),
    sideArrowInset: schema.number(),
    endCurvePadding: schema.number(),
    tintColor: schema.string(),
    edgeColor: schema.string(),
    boxOpacity: schema.number(),
    hoverEdgeColor: schema.string(),
    hoverTintColor: schema.string(),
    activeEdgeColor: schema.string(),
    activeTintColor: schema.string(),
    glowBlur: schema.number(),
  }),
  metadata: schema
    .object({
      displayName: schema.string().optional(),
      matchPatterns: schema.array(schema.string()).optional(),
    })
    .optional(),
});

export const unifiedHeaderProfileDocumentSchema = schema.object({
  profile: schema.string().min(1),
  config: serializedUnifiedHeaderConfigSchema,
});

export type SerializedUnifiedHeaderConfig = schema.infer<typeof serializedUnifiedHeaderConfigSchema>;
export type UnifiedHeaderProfileDocument = schema.infer<typeof unifiedHeaderProfileDocumentSchema>;
...
function defaultCircleLogo({ cx, cy, size, color, strokeWidth, innerOpacity, aspectCorrection }: CenterLogoRenderArgs) {
  ...
  return (
    <g transform={`translate(${cx} ${cy}) scale(${aspectCorrection} 1) translate(${-cx} ${-cy})`}>
      ...
      <circle cx={cx} cy={cy} r={innerRadius} fill={color} opacity={innerOpacity} />
...
function defaultSpadeIcon({ cx, cy, size, color }: HeaderIconRenderArgs) {
  ...
  return (
    <g fill={color} stroke="white" strokeWidth={0.45} strokeOpacity={0.45} vectorEffect="non-scaling-stroke">
      <path
        d={`M ${cx} ${cy - s * 0.55} C ${cx - s * 0.55} ${cy - s * 0.05} ${cx - s * 0.55} ${cy + s * 0.45} ${cx - s * 0.12} ${cy + s * 0.32} C ${cx - s * 0.05} ${cy + s * 0.3} ${cx - s * 0.02} ${cy + s * 0.22} ${cx} ${cy + s * 0.16} C ${cx + s * 0.02} ${cy + s * 0.22} ${cx + s * 0.05} ${cy + s * 0.3} ${cx + s * 0.12} ${cy + s * 0.32} C ${cx + s * 0.55} ${cy + s * 0.45} ${cx + s * 0.55} ${cy - s * 0.05} ${cx} ${cy - s * 0.55} Z`}
      />
      <path
        d={`M ${cx - s * 0.18} ${cy + s * 0.55} C ${cx - s * 0.04} ${cy + s * 0.36} ${cx - s * 0.03} ${cy + s * 0.25} ${cx} ${cy + s * 0.12} C ${cx + s * 0.03} ${cy + s * 0.25} ${cx + s * 0.04} ${cy + s * 0.36} ${cx + s * 0.18} ${cy + s * 0.55} Z`}
      />
    </g>
  );
}
...
function defaultDiamondIcon({ cx, cy, size, color }: HeaderIconRenderArgs) {
  ...
  return (
    <path
      d={`M ${cx} ${cy - s * 0.58} L ${cx + s * 0.42} ${cy} L ${cx} ${cy + s * 0.58} L ${cx - s * 0.42} ${cy} Z`}
      fill={color}
      stroke="white"
      strokeWidth={0.45}
      strokeOpacity={0.35}
...
function defaultClubIcon({ cx, cy, size, color }: HeaderIconRenderArgs) {
  ...
  return (
    <g fill={color} stroke="white" strokeWidth={0.45} strokeOpacity={0.45} vectorEffect="non-scaling-stroke">
      ...
      <circle cx={cx + s * 0.24} cy={cy + s * 0.06} r={s * 0.24} />
      <path
        d={`M ${cx - s * 0.16} ${cy + s * 0.55} C ${cx - s * 0.03} ${cy + s * 0.32} ${cx - s * 0.02} ${cy + s * 0.18} ${cx} ${cy + s * 0.08} C ${cx + s * 0.02} ${cy + s * 0.18} ${cx + s * 0.03} ${cy + s * 0.32} ${cx + s * 0.16} ${cy + s * 0.55} Z`}
      />
    </g>
  );
}
