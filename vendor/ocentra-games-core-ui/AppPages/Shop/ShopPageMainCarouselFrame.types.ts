...
export type FeaturedShowcaseControlTab = 'overall' | 'header' | 'body' | 'sideA' | 'sideB' | 'footer';
...
export type FeaturedShowcaseResponsiveVariants<T> = Partial<Record<FeaturedShowcaseControlVariant, T>>;
...
function serializePrimitiveControlGroup<T extends FeaturedShowcasePrimitiveGroup>(defaults: T, value: T): T {
  ...
  for (const key of Object.keys(defaults)) {
    ...
    if (typeof defaultValue === 'number' && typeof incoming === 'number' && Number.isFinite(incoming)) {
      next[key] = incoming;
    } else if (typeof defaultValue === 'boolean' && typeof incoming === 'boolean') {
      next[key] = incoming;
    } else if (typeof defaultValue === 'string' && typeof incoming === 'string') {
      next[key] = incoming;
    }
  }
...
function serializeFeaturedShowcaseControlGroups(
  ...
  return {
    overall: serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.overall, controls.overall),
    arrows: serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.arrows, controls.arrows),
    header: serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.header, controls.header),
    body: serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.body, controls.body),
    sideA: {
      ...serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.sideA, controls.sideA),
    ...
    sideB: {
      ...serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.sideB, controls.sideB),
    ...
    footer: serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.footer, controls.footer),
    colors: serializePrimitiveControlGroup(DEFAULT_FEATURED_SHOWCASE_CONTROLS.colors, controls.colors),
  };
}
...
function serializeFeaturedShowcaseVariants(
  ...
  return {
    wide: serializeFeaturedShowcaseControlGroups(mergeFeaturedShowcaseControlGroups(base, variants?.wide)),
    narrow: serializeFeaturedShowcaseControlGroups(mergeFeaturedShowcaseControlGroups(base, variants?.narrow)),
  };
}

export function serializeFeaturedShowcaseControls(controls: FeaturedShowcaseControls): FeaturedShowcaseControls {
...
export function resolveFeaturedShowcaseControlsForVariant(
  ...
  const base = serializeFeaturedShowcaseControlGroups(controls);
  return serializeFeaturedShowcaseControlGroups(mergeFeaturedShowcaseControlGroups(base, controls.variants?.[variant]));
}
