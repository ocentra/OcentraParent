...
function getMediaPreserveAspectRatio(fit: FeaturedShowcaseMediaFit, anchorX: number, anchorY: number): string {
...
function normalizeBadgeTone(value: unknown, fallback: BadgeTone): BadgeTone {
  return typeof value === 'string' && badgeTones.includes(value as BadgeTone) ? (value as BadgeTone) : fallback;
}
...
function getTopBadges(game: FeaturedGameItem): GameBadge[] {
  const authored = getAuthoredBadges(game.featuredTopBadges, [
    'bannerBlue',
    'bannerPurple',
    'bannerGreen',
    'bannerPink',
...
function SvgArrowButton({
  ...
  return (
    ...
    >
      <path
        d={outerPath}
        fill={isHover ? '#0d5930' : `url(#${ids('navIdle')})`}
        stroke={isHover ? hoverColor : '#69caff'}
        strokeWidth={isHover ? 2.3 : 1.5}
      />
  ...
  return (
    <g>
      {badges.map((badge, index) => {
        const priorWidth = widths.slice(0, index).reduce((sum, width) => sum + width + gap, 0);
  ...
  return (
    <g>
      ...
      <text
        ...
        fontWeight="800"
        letterSpacing="0"
        fill={`url(#${ids('logoStoneFill')})`}
        stroke="#d7d2c4"
        strokeWidth="0.42"
        strokeOpacity="0.42"
        paintOrder="stroke fill"
        filter={`url(#${ids('logoStoneShadow')})`}
      >
        {text}
      </text>
    </g>
  ...
  return (
    <g style={{ cursor: onClick ? 'pointer' : 'default' }} onClick={onClick}>
      ...
      <g filter={`url(#${ids('cyanGlow')})`}>
        <path
          d={innerPath}
          fill={`url(#${ids('learnMoreInnerBox')})`}
          stroke={strokeColor}
          strokeWidth="1.35"
          strokeOpacity="0.95"
        />
...
function ShowcaseDefs({ ids }: { ids: (name: string) => string }) {
  return (
    <defs>
      ...
      <linearGradient id={ids('badgeBlue')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#66c6ff" />
        <stop offset="1" stopColor="#287bc4" />
      </linearGradient>
      <linearGradient id={ids('badgeGreen')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#65d96f" />
        <stop offset="1" stopColor="#207d39" />
      </linearGradient>
      <linearGradient id={ids('badgePurple')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#a855f7" />
        <stop offset="1" stopColor="#5b21b6" />
      </linearGradient>
      <linearGradient id={ids('badgePink')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#f148a9" />
        <stop offset="1" stopColor="#a30d65" />
      </linearGradient>
      <linearGradient id={ids('badgeGold')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#ffd23f" />
        <stop offset="1" stopColor="#e79400" />
      </linearGradient>
      <linearGradient id={ids('bannerBadgeBlue')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#14384f" />
        <stop offset="1" stopColor="#0b1e30" />
      </linearGradient>
      <linearGradient id={ids('bannerBadgeGreen')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#143f32" />
        <stop offset="1" stopColor="#091f19" />
      </linearGradient>
      <linearGradient id={ids('bannerBadgePurple')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#302a58" />
        <stop offset="1" stopColor="#15142e" />
      </linearGradient>
      <linearGradient id={ids('bannerBadgePink')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#4a2144" />
        <stop offset="1" stopColor="#231124" />
      </linearGradient>
      <linearGradient id={ids('bannerBadgeGold')} x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stopColor="#4b3b12" />
        <stop offset="1" stopColor="#211806" />
      </linearGradient>
      <filter id={ids('cyanGlow')} x="-80%" y="-80%" width="260%" height="260%">
        ...
        <feColorMatrix
          in="blur"
          type="matrix"
          values="0 0 0 0 0.28 0 0 0 0 0.78 0 0 0 0 1 0 0 0 0.85 0"
          result="glow"
        />
        <feMerge>
          <feMergeNode in="glow" />
      ...
      <filter id={ids('goldGlow')} x="-60%" y="-80%" width="220%" height="260%">
        ...
        <feColorMatrix
          in="blur"
          type="matrix"
          values="1 0 0 0 0.95 0 1 0 0 0.62 0 0 1 0 0.12 0 0 0 0.85 0"
          result="glow"
        />
        <feMerge>
          <feMergeNode in="glow" />
...
function DefaultMediaSlot({
  ...
  const transition =
    slot.game.carouselTransitionType === 'cut' ? 'none' : `opacity ${transitionDurationMs}ms ease-in-out`;
  ...
  const titleFontSize = titleText ? clamp(slot.width / Math.max(5, titleText.length * 0.5), 24, slot.height * 0.34) : 0;
  ...
  if (!activeUrl) {
    return (
      <g>
        ...
        <circle
          cx={slot.x + slot.width * 0.38}
          cy={slot.y + slot.height * 0.42}
          r={slot.height * 0.42}
          fill="#9bd9ff"
          opacity="0.08"
        />
        <circle
          cx={slot.x + slot.width * 0.64}
          cy={slot.y + slot.height * 0.36}
          r={slot.height * 0.28}
          fill="#ffffff"
          opacity="0.055"
        />
        <FallbackLogoText
          x={slot.x + slot.width * 0.16}
          y={slot.y + slot.height * 0.28}
          width={slot.width * 0.68}
          height={slot.height * 0.38}
          text={getGameTitle(slot.game)}
          ids={ids}
          maxFontSize={slot.height * 0.24}
        />
      </g>
    );
  }
  ...
  return (
    <g>
      ...
      {imageEntries.map(({ hash, index }) => {
        ...
        if (!url) return null;
        const opacity = index === slot.activeImageIndex ? 1 : 0;
      ...
      {logoUrl ? (
        <image
          ...
          style={{
            transition: `opacity ${slot.game.bannerLogoDurationMs ?? 1200}ms ease-in-out, transform ${slot.game.bannerLogoDurationMs ?? 1200}ms ease-in-out`,
          }}
        />
      ) : titleText ? (
        <text
          ...
          style={{
            transition: `opacity ${slot.game.bannerTitleDurationMs ?? 1200}ms ease-in-out, transform ${slot.game.bannerTitleDurationMs ?? 1200}ms ease-in-out`,
          }}
        >
          {titleText}
        </text>
...
function getShopCardVisibleCount(width: number, total: number, gap: number, minCardW: number): number {
  if (total <= 0) return 1;
...
function ShopMainCarouselCard({
  ...
  const headerBodyBadgeW =
    badgeInHeader && card.badge
      ? Math.min(Math.max(44, card.badge.length * headerBodyBadgeFont * 0.62 + 14), width * 0.32)
      : 0;
  const headerBodyBadgeX =
    headerBodyBadgeW > 0 ? (card.price ? priceBadgeX : headerX + headerW) - headerBadgeGap - headerBodyBadgeW : 0;
  const titleRightEdge =
    headerBodyBadgeW > 0
  ...
  const bodyBadgeW =
    card.badge && !badgeInHeader
      ? Math.min(Math.max(48, card.badge.length * bodyBadgeFont * 0.66 + 16), width * 0.48)
      : 0;
  ...
  const rawSplitLines = (card.bodyLines && card.bodyLines.length > 0 ? card.bodyLines : [card.subtitle]).filter(
    Boolean
  );
  ...
  const imageCardPreserveAspectRatio =
    card.imageFit === 'fill' ? 'none' : card.imageFit === 'cover' ? `xMid${imageCardYAlign} slice` : 'xMidYMid meet';
  ...
  return (
    ...
    >
        ...
        cardImageUrl ? (
          ...
          <g opacity="0.72">
            <rect
              ...
              height={imageH}
              rx="0"
              fill="rgba(7,17,31,.50)"
              stroke="#2d5d7b"
              strokeDasharray="5 5"
            />
            <text
              x={imageX + imageW / 2}
              y={imageY + imageCardTopPad + imageH / 2 + 4}
              textAnchor="middle"
              fill={accent}
              fontFamily="Inter, Arial, sans-serif"
              fontSize={Math.min(22, imageH * 0.28)}
              fontWeight={900}
            >
        ...
        <g>
          {cardImageUrl ? (
            ...
            <g opacity="0.72">
              ...
              />
              <text
                x={splitImageX + splitImageW / 2}
                y={splitBodyY + splitBodyH / 2 + 4}
                textAnchor="middle"
                fill={accent}
                fontFamily="Inter, Arial, sans-serif"
                fontSize={Math.min(18, splitBodyH * 0.22)}
                fontWeight={900}
              >
                {card.icon.toUpperCase().slice(0, 2)}
              </text>
          ...
          <line
            ...
            y1={splitBodyY + 4}
            x2={splitDividerX}
            y2={splitBodyY + splitBodyH - 4}
            stroke={accent}
            strokeWidth="1"
            strokeOpacity="0.58"
          />
          <line
            x1={splitDividerX + 2.5}
            y1={splitBodyY + 10}
            x2={splitDividerX + 2.5}
            y2={splitBodyY + splitBodyH - 10}
            stroke={edgeAccent}
            strokeWidth="0.75"
            strokeOpacity="0.34"
          />
          ...
          {splitLines.map((line, index) => {
            ...
            return (
              <g key={`${card.key}-line-${index}`}>
                <circle
                  cx={splitTextX + 3}
                  cy={bulletY - splitTextSize * 0.34}
                  r={Math.max(1.4, 1.8 * scale)}
                  fill={edgeAccent}
                />
                <text
                  x={splitTextX + 9}
      ...
      {active ? (
        <>
          <path
            d={roundedRectPath(x - 2, y - 2, width + 4, height + 4, radius, 'top')}
            fill="none"
            stroke={edgeAccent}
            strokeWidth="2.2"
            opacity="0.3"
          ...
          />
          <path
            d={roundedRectPath(x + 3, y + 3, width - 6, height - 6, radius, 'top')}
            fill="none"
            stroke={edgeAccent}
            strokeWidth="1.7"
            opacity="0.74"
          ...
          />
          <line
            x1={x + hoverRailInset + hoverRailPairGap}
            y1={hoverRailY1 + 4}
            x2={x + hoverRailInset + hoverRailPairGap}
            y2={hoverRailY2 - 4}
            stroke={edgeAccent}
            strokeWidth="0.9"
            opacity="0.42"
          />
          <line
            x1={x + width - hoverRailInset}
            y1={hoverRailY1}
            x2={x + width - hoverRailInset}
            y2={hoverRailY2}
            stroke={edgeAccent}
            strokeWidth="1.25"
            opacity="0.82"
          />
          <line
            x1={x + width - hoverRailInset - hoverRailPairGap}
            y1={hoverRailY1 + 4}
            x2={x + width - hoverRailInset - hoverRailPairGap}
            y2={hoverRailY2 - 4}
            stroke={edgeAccent}
            strokeWidth="0.9"
            opacity="0.42"
...
export const ShopPageMainCarouselFrame: React.FC<FeaturedGameShowcaseProps> = ({
  ...
  const variantControls = useMemo(
    () => resolveFeaturedShowcaseControlsForVariant(mergedControls, previewLayoutMode === 'narrow' ? 'narrow' : 'wide'),
  ...
  const playbackImageCount = currentGame
    ...
    : 0;
  const safeActiveImageIndex = playbackImageCount > 0 ? Math.min(activeImageIndex, playbackImageCount - 1) : 0;
  ...
  useEffect(() => {
    ...
    const duration =
      frameDuration +
      (safeActiveImageIndex === maxIndex ? Math.max(0, currentGame.carouselSlideTransitionDelayMs ?? 0) : 0);
  ...
  const goToSlide = useCallback(
    (index: number, pageCount = activeGames.length) => {
  ...
  const isNarrow =
    previewLayoutMode === 'narrow' ? true : previewLayoutMode === 'wide' ? false : breakpointIsNarrow || autoIsNarrow;
  ...
  const tabTextSize = getFittingTabTextSize(
    tabs,
    contentW - 44,
    c.header.tabMinFont,
    c.header.tabMaxFont,
    c.header.tabFirstBoost,
    c.header.tabCountW
  );
  ...
  const cardVisibleCount =
    cards.length > 0 ? getShopCardVisibleCount(leftW, cards.length, c.sideA.cardGap, c.sideA.cardMinW) : 0;
  ...
  const hasExternalNavigation =
    typeof navigationPageCount === 'number' &&
    typeof navigationPageIndex === 'number' &&
    Boolean(onNavigationPageChange);
  const resolvedPageCount = hasExternalNavigation ? Math.max(1, Math.round(navigationPageCount ?? 1)) : activePageCount;
  const resolvedPageIndex = hasExternalNavigation
    ? clamp(Math.round(navigationPageIndex ?? 0), 0, resolvedPageCount - 1)
    : currentPageIndex;
  ...
  const indicatorWidthSum = visibleIndicatorIndexes.reduce(
    (sum, indicatorIndex) => sum + (indicatorIndex === resolvedPageIndex ? activeIndicatorW : inactiveIndicatorW),
    0
  );
  const indicatorGap =
    visibleIndicatorIndexes.length > 1
      ? Math.max(
          2,
          Math.min(indicatorMinGap, (indicatorTrackW - indicatorWidthSum) / (visibleIndicatorIndexes.length - 1))
        )
      : indicatorMinGap;
  ...
  const rightActionKind = rightActionLabel?.match(/^what is/i)
    ? 'arena'
    : rightActionLabel?.match(/^compare/i)
      ? 'compare'
      : 'default';
  const rightActionFullTextSize = Math.max(9.4, Math.min(11.8, tabTextSize * 0.64));
  const rightActionFullW = rightActionLabel
    ? Math.max(88, Math.min(178, rightActionLabel.length * rightActionFullTextSize * 0.58 + 28))
    : 0;
  const tabsTotalW = tabs.reduce(
    (sum, tab) => sum + getTabBlockWidth(tab.label, tabTextSize, c.header.tabFirstBoost, c.header.tabCountW),
    0
  );
  ...
  const rightActionDisplayLabel = rightActionCompact
    ? rightActionKind === 'arena'
      ? 'AC?'
      : ''
    : (rightActionLabel ?? '');
  const rightActionTextSize = rightActionCompact
    ? rightActionKind === 'arena'
      ? Math.max(10.2, Math.min(11.8, tabTextSize * 0.7))
      : Math.max(12.8, Math.min(15, tabTextSize * 0.86))
    : rightActionFullTextSize;
  const rightActionW = rightActionLabel
    ? rightActionCompact
      ? rightActionKind === 'arena'
        ? Math.max(36, rightActionH + 12)
        : Math.max(32, rightActionH + 6)
      : rightActionFullW
    : 0;
  ...
  const rightActionStroke =
    rightActionEnabled && rightActionHovered ? c.colors.tabHover : shopProductCardColors.buttonStroke;
  const rightActionFill =
    rightActionEnabled && rightActionHovered ? shopProductCardColors.buttonHoverFill : shopProductCardColors.buttonFill;
    ...
    >
      ...
      {rightActionHovered && rightActionEnabled ? (
        <rect
          x={rightActionX - 3}
          y={rightActionY - 3}
          width={rightActionW + 6}
          height={rightActionH + 6}
          rx={rightActionCompact ? rightActionH / 2 + 3 : 0}
          fill="none"
          stroke={rightActionStroke}
          strokeWidth="2.2"
          opacity="0.28"
          filter={`url(#${ids('cyanGlow')})`}
        />
      ) : null}
      <rect
        x={rightActionX}
        ...
        width={rightActionW}
        height={rightActionH}
        rx={rightActionCompact ? rightActionH / 2 : 0}
        fill={rightActionCompact ? 'rgba(84,226,255,.18)' : rightActionFill}
        stroke={rightActionStroke}
        strokeWidth={rightActionCompact ? 1.8 : 1.45}
        opacity={rightActionEnabled ? 1 : 0.62}
      />
      {rightActionCompact && rightActionKind === 'compare' ? (
        ...
        >
          <path
            d={compareActionIconPath}
            fill={`url(#${ids('compareActionIconFill')})`}
            stroke="#020813"
            strokeWidth="0.82"
            strokeLinejoin="round"
            paintOrder="stroke fill"
  ...
  return (
    <div ref={wrapperRef} className={className} style={wrapperStyle}>
      <div
        style={{
          paddingLeft: canvasInsetX,
          paddingRight: canvasInsetX,
          marginTop,
          marginBottom,
          outline: showBounds ? '2px dashed rgba(34, 211, 238, 0.55)' : undefined,
          outlineOffset: showBounds ? '-2px' : undefined,
        }}
      >
        <svg
          viewBox={`0 0 ${vw} ${vh}`}
        ...
        >
          ...
          {hasNavigation ? (
            <>
              <SvgArrowButton
                x={edgeInset}
                y={stageY + stageH / 2 - arrowH / 2}
                width={arrowW}
                height={arrowH}
                radius={c.arrows.radius}
                hoverColor={c.colors.arrowHover}
                direction="left"
                onClick={goPrev}
                ids={ids}
              />
              <SvgArrowButton
                x={vw - edgeInset - arrowW}
                y={stageY + stageH / 2 - arrowH / 2}
                width={arrowW}
                height={arrowH}
                radius={c.arrows.radius}
          ...
          <rect
            ...
            width={stageW}
            height={stageH}
            rx={c.overall.stageRadius}
            fill="#000000"
            opacity="0.28"
            filter={`url(#${ids('skeletonShadow')})`}
            pointerEvents="none"
          />
          <g>
            <defs>
            ...
            <rect
              ...
              height={stageH}
              rx={c.overall.stageRadius}
              fill={`url(#${ids('skeletonGlass')})`}
              stroke={c.colors.stageStroke}
              strokeWidth="2"
              strokeOpacity="0.9"
            />
            ...
            <g>
              {tabs.map((tab, tabIndex) => {
                const priorWidth = tabs
                  .slice(0, tabIndex)
                  .reduce(
                    (sum, prior) =>
                      sum + getTabBlockWidth(prior.label, tabTextSize, c.header.tabFirstBoost, c.header.tabCountW),
                    0
                  );
                ...
                return (
                  ...
                  >
                    <path
                      ...
                      fill={
                        isActive
                          ? `url(#${ids('tabSelectedFill')})`
                          : isHover
                            ? `url(#${ids('tabHoverFill')})`
                            : '#071827'
                      }
                      fillOpacity={isActive ? 0.9 : isHover ? 0.78 : 0.14}
                      stroke={isActive ? '#f5c84b' : isHover ? c.colors.tabHover : '#5fc9ff'}
                      strokeOpacity={isActive ? 0.82 : isHover ? 0.86 : 0.28}
                      strokeWidth="1.5"
                    />
                    <path
                      d={countPath}
                      fill={isHover ? `url(#${ids('tabHoverFill')})` : `url(#${ids('tabCountGoldFill')})`}
                      stroke={isHover ? c.colors.tabHover : '#f7c84a'}
                      strokeOpacity={isHover ? 0.86 : 0.92}
                      strokeWidth="1.35"
                      filter={!isHover ? `url(#${ids('goldGlow')})` : undefined}
                    />
                    <path
                      d={countPath}
                      fill={`url(#${ids('tabCountGoldShine')})`}
                      opacity={isHover ? 0.1 : 0.32}
                      pointerEvents="none"
                    />
                    <path
                      d={labelPath}
                      fill={isActive ? '#1b2f22' : isHover ? '#0d4029' : '#071827'}
                      fillOpacity={isActive ? 0.58 : isHover ? 0.66 : 0.18}
                      stroke={isActive ? '#f5c84b' : isHover ? c.colors.tabHover : '#5fc9ff'}
                      strokeOpacity={isActive ? 0.62 : isHover ? 0.78 : 0.22}
                      strokeWidth="1.1"
                    />
                    <text
                      x={mainX + countW / 2}
                      ...
                      strokeWidth={
                        isHover ? Math.max(1.2, countFontSize * 0.12) : Math.max(0.45, countFontSize * 0.035)
                      }
                      strokeLinejoin="round"
                      paintOrder="stroke fill"
                      filter={isHover ? `url(#${ids('tabGreenGlow')})` : undefined}
                    ...
                    <text
                      ...
                      fill={isActive ? '#8fd8ff' : isHover ? '#9dffc2' : '#d9edff'}
                      opacity={isActive || isHover ? 1 : 0.58}
                      filter={isActive ? `url(#${ids('cyanGlow')})` : undefined}
                    >
                      <tspan fontSize={tabFirstSize} letterSpacing="2">
                        {firstLetter}
                      </tspan>
                      <tspan fontSize={tabTextSize} letterSpacing="5" dx="4">
                        {restLetters}
                      </tspan>
                    </text>
                    {isActive ? (
                      <rect
                        ...
                        y={mainY + mainH - 5}
                        width={labelW}
                        height={c.header.activeLineH}
                        fill={`url(#${ids('selectedTabGradientLine')})`}
                      />
                    ) : null}
                  </g>
                );
            ...
            <g clipPath={`url(#${mediaClipPathId})`}>
              ...
              {showBadges ? (
                <>
                  <BadgeRow
                    x={bodyX + leftW - c.sideA.topBadgeInset}
                    y={topBadgeY}
                    badges={getTopBadges(currentGame)}
                    align="right"
                    height={c.sideA.topBadgeH}
                    minWidth={86}
                    fontSize={12}
                    fontScale={6.8}
                    ids={ids}
                  />
                  <BadgeRow
                    x={bodyX + c.sideA.bottomBadgeInset}
                    y={bottomBadgeY}
                    badges={getBottomBadges(currentGame)}
                    align="left"
                    height={c.sideA.bottomBadgeH}
                    minWidth={96}
                    fontSize={11.5}
                    fontScale={7.6}
                    ids={ids}
            ...
            {visibleCards.length > 0 ? null : (
              <rect
                ...
                fill="none"
                stroke={c.colors.bodyStroke}
                strokeWidth={c.body.outlineWidth}
              />
            )}
                ...
                {visibleIndicatorIndexes.map((indicatorIndex, visibleIndex) => {
                  ...
                  const x =
                    ...
                    visibleIndicatorIndexes
                      ...
                      .reduce(
                        (sum, priorIndex) =>
                          sum + (priorIndex === resolvedPageIndex ? activeIndicatorW : inactiveIndicatorW),
                        0
                      ) +
                    visibleIndex * indicatorGap;
                  return (
                    <g
                      key={indicatorIndex}
                      onClick={() => goToNavigationPage(indicatorIndex)}
                      style={{ cursor: 'pointer' }}
                      filter={isActive ? `url(#${ids('cyanGlow')})` : undefined}
                    >
                      <rect
                        x={x - indicatorGap * 0.5}
                        y={indicatorsY - 7}
                        width={width + indicatorGap}
                        height={indicatorPillH + 14}
                        rx={(indicatorPillH + 14) / 2}
                        fill="transparent"
                        opacity="0"
                        pointerEvents="all"
                      />
                      <rect
                        ...
                        y={indicatorsY}
                        width={width}
                        height={indicatorPillH}
                        rx={indicatorPillH / 2}
                        fill={isActive ? `url(#${ids('footerSelectedPill')})` : 'transparent'}
                        stroke={isActive ? '#ffe187' : '#64d8ff'}
                        strokeWidth={isActive ? 1.7 : 1.25}
                        strokeOpacity={isActive ? 0.95 : 0.58}
                      />
                    </g>
                  );
                })}
