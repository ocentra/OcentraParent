    ...
    >
      ...
      {arrow ? (
        <>
          ...
          <path
            d={`M ${x + w - arrowW + 9} ${y + h * 0.28} L ${x + w - 7} ${y + h * 0.5} L ${x + w - arrowW + 9} ${y + h * 0.72} Z`}
            fill={hovered ? cfg.colors.buttonArrowHoverFill : cfg.colors.buttonArrowFill}
          />
        </>
      ) : null}
    </g>
    ...
    return (
      <g>
        <rect
          ...
          width={w}
          height={h}
          fill={cfg.colors.productImageMissingFill}
          stroke={cfg.colors.productImageMissingStroke}
          strokeDasharray="5 5"
        />
        <line
          x1={x + 8}
        ...
        <Txt
          ...
          y={y + h / 2}
          anchor="middle"
          size={Math.min(10, Math.max(7, w / 18))}
          weight="900"
          fill={cfg.colors.missingText}
          cfg={cfg}
        >
          ASSET NEEDED
        </Txt>
  ...
  return (
    <g>
      ...
      <image
        ...
        opacity={cfg.primitives.imageOpacity}
      />
...
function IconShape({ shape }: { shape: IconShapeDefinition }) {
  ...
  return (
    <rect x={shape.x} y={shape.y} width={shape.width} height={shape.height} rx={shape.rx} transform={shape.transform} />
  );
}
...
export function MiniIcon({
  ...
  return (
    <g
      ...
      fill="none"
      stroke={color}
      strokeWidth={cfg.iconTokens.strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      {shapes.map((shape, index) => (
        <IconShape key={`${type}-${index}`} shape={shape} />
      ))}
    </g>
  );
}
  ...
  return (
    <g>
      ...
      <Txt
        x={x + (dotFill ? 28 : 14)}
        y={y + h / 2}
        fill={cfg.colors.tileSubtitleText}
        size="10.6"
        weight="650"
        cfg={cfg}
      >
      ...
      {value !== undefined ? (
        <Txt x={x + w - 12} y={y + h / 2} anchor="end" size="10.6" weight="950" cfg={cfg}>
          {value}
        </Txt>
      ) : null}
      {actionLabel ? (
        <SvgButton x={x + w - 64} y={y + 4} w={52} h={18} label={actionLabel} small onClick={onAction} cfg={cfg} />
      ) : null}
    </g>
  );
}
