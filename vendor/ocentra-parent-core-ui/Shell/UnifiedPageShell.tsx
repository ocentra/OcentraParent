...
function measureHeaderHeight(rootNode: HTMLDivElement | null, headerNode: HTMLDivElement | null): number {
  ...
  if (svgNode instanceof SVGGraphicsElement) {
    try {
      ...
      const svgBottom = Math.max(0, svgRect.top - rootRect.top + (box.y + box.height) * scaleY) / rootScaleY;
  ...
  return Math.max(Math.max(0, headerNode.getBoundingClientRect().bottom - rootRect.top) / rootScaleY, extensionBottom);
}

function measureFooterHeight(rootNode: HTMLDivElement | null, footerNode: HTMLDivElement | null): number {
  ...
  return (
    ...
    >
      ...
      <div className="oc-unified-shell__chrome">
        ...
        <div className={['oc-unified-shell__work', workClassName ?? ''].filter(Boolean).join(' ')}>{children}</div>
        {footer ? (
