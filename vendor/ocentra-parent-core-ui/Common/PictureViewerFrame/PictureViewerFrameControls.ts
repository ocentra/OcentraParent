...
export function getPictureViewerAnchoredFrame(
  ...
  const cameraSpace = getPictureViewerFrameSpaceForOrientation(cfg.viewBox, cfg.orientation);
  const originX = (cameraSpace.w - space.w) / 2;
  const originY = (cameraSpace.h - space.h) / 2;

  return {
    ...frame,
    x: originX + anchor.sideInset,
    y: originY + anchor.topInset,
...
export function getPictureViewerBulgeArrowCenters(
  ...
  const topLeft = p[2]!;
  const topRight = p[3]!;
  const bottomLeft = p[10]!;
  const bottomRight = p[11]!;

  return {
    top: {
      x: (topLeft.x + topRight.x) / 2,
      y: (topLeft.y + topRight.y) / 2 + arrows.topOffset,
    },
    bottom: {
      x: (bottomLeft.x + bottomRight.x) / 2,
      y: (bottomLeft.y + bottomRight.y) / 2 + arrows.bottomOffset,
    },
  };
}
...
function splitEdge(
  ...
  if (safeStart > 0)
    segments.push({
      id: `${id}Start` as PictureViewerFrameSegmentId,
      d: `M ${a.x} ${a.y} L ${p1.x} ${p1.y}`,
      kind: thickKind,
      a,
      b: p1,
    });
  segments.push({
    id: `${id}Mid` as PictureViewerFrameSegmentId,
  ...
  if (safeEnd > 0)
    segments.push({
      id: `${id}End` as PictureViewerFrameSegmentId,
      d: `M ${p2.x} ${p2.y} L ${b.x} ${b.y}`,
      kind: thickKind,
      a: p2,
      b,
    });
...
export function pictureViewerFrameSegments(frame: PictureViewerFrameControls): PictureViewerFrameSegment[] {
  const p = getPictureViewerFramePoints(frame);
  const point = (index: number): PictureViewerFramePoint => p[index]!;
  return [
    ...splitEdge(
      point(14),
      point(15),
    ...
    ...edge(point(15), point(0), 'leftTopCornerThickness', 'leftTopCorner'),
    ...splitEdge(
      point(0),
      point(1),
      'topLeftThickness',
    ...
    ...edge(point(1), point(2), 'topLeftConnectorThickness', 'topLeftConnector'),
    ...edge(point(2), point(3), 'topCenterThickness', 'topCenterRun'),
    ...edge(point(3), point(4), 'topRightConnectorThickness', 'topRightConnector'),
    ...splitEdge(
      point(4),
      point(5),
      'topRightThickness',
    ...
    ...edge(point(5), point(6), 'rightTopCornerThickness', 'rightTopCorner'),
    ...splitEdge(
      point(6),
      point(7),
      'rightSideThickness',
    ...
    ...edge(point(7), point(8), 'rightBottomCornerThickness', 'rightBottomCorner'),
    ...splitEdge(
      point(8),
      point(9),
    ...
    ...edge(point(9), point(10), 'bottomRightConnectorThickness', 'bottomRightConnector'),
    ...edge(point(10), point(11), 'bottomCenterThickness', 'bottomCenterRun'),
    ...edge(point(11), point(12), 'bottomLeftConnectorThickness', 'bottomLeftConnector'),
    ...splitEdge(
      point(12),
      point(13),
      'bottomLeftThickness',
    ...
    ...edge(point(13), point(14), 'leftBottomCornerThickness', 'leftBottomCorner'),
  ];
}
...
export function pictureViewerFrameSegmentThickness(
  ...
  const baseline = baselineFrame ?? frame;
  const fallback = frame[segment.kind];
  const base = frame.segmentThicknesses?.[segment.id] ?? (typeof fallback === 'number' ? fallback : 1);
  const kindDelta =
    frame.segmentThicknesses?.[segment.id] === undefined
      ? 0
      : numericDelta(frame[segment.kind], baseline[segment.kind]);
  const groupDelta = segmentGroupThickness(frame, segment.id) - segmentGroupThickness(baseline, segment.id);
  return Math.max(0, base + kindDelta + groupDelta);
}
...
function numericDelta(value: unknown, baseline: unknown): number {
  return typeof value === 'number' && typeof baseline === 'number' ? value - baseline : 0;
}

function segmentGroupThickness(frame: PictureViewerFrameControls, segmentId: PictureViewerFrameSegmentId): number {
  if (TOP_GROUP_SEGMENTS.has(segmentId)) {
    return frame.topGroupThickness;
  }
  if (BOTTOM_GROUP_SEGMENTS.has(segmentId)) {
    return frame.bottomGroupThickness;
  }
  if (CORNER_GROUP_SEGMENTS.has(segmentId)) {
    return frame.cornerGroupThickness;
  }
  if (THIN_LINE_GROUP_SEGMENTS.has(segmentId)) {
    return frame.thinLineGroupThickness;
  }
  return 0;
}

const TOP_GROUP_SEGMENTS = new Set<PictureViewerFrameSegmentId>([
  'topLeftRunEnd',
  'topLeftConnector',
  'topCenterRun',
  'topRightConnector',
...
const BOTTOM_GROUP_SEGMENTS = new Set<PictureViewerFrameSegmentId>([
  'bottomRightRunEnd',
  'bottomRightConnector',
  'bottomCenterRun',
  'bottomLeftConnector',
...
const CORNER_GROUP_SEGMENTS = new Set<PictureViewerFrameSegmentId>([
  'leftSideRunStart',
  'leftSideRunEnd',
  'leftTopCorner',
  'topLeftRunStart',
...
const THIN_LINE_GROUP_SEGMENTS = new Set<PictureViewerFrameSegmentId>([
  'topLeftRunMid',
  'topRightRunMid',
  'rightSideRunMid',
  'bottomRightRunMid',
