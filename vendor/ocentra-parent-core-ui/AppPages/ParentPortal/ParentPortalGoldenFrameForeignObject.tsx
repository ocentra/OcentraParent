...
function splitEdge(
  ...
  if (safeStart > 0)
    segments.push({ id: `${id}Start`, d: `M ${a.x} ${a.y} L ${p1.x} ${p1.y}`, kind: thickKind, a, b: p1 });
...
function frameSegments(frame: typeof OUTER_FRAME_DEFAULT) {
  ...
  return [
    ...splitEdge(
      ...
      p[15],
      'leftSideThickness',
      'leftSideSliceTransitionThickness',
      frame.leftSideStartGap,
      frame.leftSideEndGap,
      'leftSideRun'
    ),
    ...
    ...splitEdge(
      ...
      p[1],
      'topLeftThickness',
      'topLeftSliceTransitionThickness',
      frame.topLeftStartGap,
      frame.topLeftEndGap,
      'topLeftRun'
    ),
    ...
    ...splitEdge(
      ...
      p[5],
      'topRightThickness',
      'topRightSliceTransitionThickness',
      frame.topRightStartGap,
      frame.topRightEndGap,
      'topRightRun'
    ),
    ...
    ...splitEdge(
      ...
      p[7],
      'rightSideThickness',
      'rightSideSliceTransitionThickness',
      frame.rightSideStartGap,
      frame.rightSideEndGap,
      'rightSideRun'
    ),
    ...
    ...splitEdge(
      p[8],
      p[9],
      'bottomRightThickness',
      'bottomRightSliceTransitionThickness',
      frame.bottomRightStartGap,
      frame.bottomRightEndGap,
      'bottomRightRun'
    ),
    ...
    ...splitEdge(
      ...
      p[13],
      'bottomLeftThickness',
      'bottomLeftSliceTransitionThickness',
      frame.bottomLeftStartGap,
      frame.bottomLeftEndGap,
      'bottomLeftRun'
    ),
...
function copyRiseSegments(frame: typeof OUTER_FRAME_DEFAULT) {
  ...
  return [
    {
      id: 'bottomRiseCopyRight',
      d: `M ${p[9].x} ${bottomY} L ${p[10].x} ${tabY}`,
      kind: 'bottomRiseCopyThickness',
      a: { x: p[9].x, y: bottomY },
      b: { x: p[10].x, y: tabY },
    },
    {
      id: 'bottomRiseCopyCenter',
      d: `M ${p[10].x} ${tabY} L ${p[11].x} ${tabY}`,
      kind: 'bottomRiseCopyThickness',
      a: { x: p[10].x, y: tabY },
      b: { x: p[11].x, y: tabY },
    },
    {
      id: 'bottomRiseCopyLeft',
      d: `M ${p[11].x} ${tabY} L ${p[12].x} ${bottomY}`,
      kind: 'bottomRiseCopyThickness',
      a: { x: p[11].x, y: tabY },
      b: { x: p[12].x, y: bottomY },
    },
  ];
}
...
function CircleRingBoxes({ circle }) {
  const boxes = Array.from({ length: circle.boxCount }, (_, index) => {
    ...
    return (
      <g key={`center-box-${index}`} transform={`translate(${x} ${y}) rotate(${angle + 90})`}>
        <path d={d} fill={circle.boxGlow} opacity={circle.boxGlowOpacity} filter="url(#center-circle-box-glow)" />
        <path
          d={d}
          fill="url(#center-box-gold)"
          stroke={circle.boxStroke}
          strokeWidth={circle.boxStrokeWidth * scale}
        ...
        <path
          ...
          fill="none"
          stroke={circle.boxInnerEdge}
          strokeWidth={Math.max(1, 1.25 * scale)}
          strokeLinejoin="round"
          opacity="0.72"
          vectorEffect="non-scaling-stroke"
        />
...
function GemIcon({ x, y, size, stats }) {
  ...
  const d =
    [
      [x - w * 0.32, topY + h * 0.12],
      [x + w * 0.32, topY + h * 0.12],
      [x + w * 0.52, topY + h * 0.42],
      [x, topY + h],
      [x - w * 0.52, topY + h * 0.42],
    ]
  ...
  return (
    <g pointerEvents="none">
      <defs>
        <linearGradient id="right-stat-gem" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={stats.iconTop} />
          <stop offset="45%" stopColor={stats.iconMid} />
          <stop offset="100%" stopColor={stats.iconBottom} />
        </linearGradient>
        <filter id="right-stat-gem-glow" x="-80%" y="-80%" width="260%" height="260%">
          <feGaussianBlur stdDeviation={stats.iconGlowBlur} />
        </filter>
      </defs>
      <path d={d} fill={stats.iconGlow} opacity={stats.iconGlowOpacity} filter="url(#right-stat-gem-glow)" />
      <path
        d={d}
        fill="url(#right-stat-gem)"
        stroke={stats.iconStroke}
        strokeWidth="2"
        strokeLinejoin="round"
        vectorEffect="non-scaling-stroke"
      />
      <path
        ...
        vectorEffect="non-scaling-stroke"
      />
      <circle cx={x + w * 0.18} cy={topY + h * 0.24} r={w * 0.06} fill="#ffffff" opacity="0.85" />
    </g>
  );
}
...
function RightStats({ stats }) {
  ...
  return (
    <g pointerEvents="none">
      <defs>
        <filter id="right-stat-text-shadow" x="-25%" y="-45%" width="150%" height="190%">
          <feDropShadow dx="0" dy="3" stdDeviation="2.5" floodColor={stats.textShadow} floodOpacity="0.95" />
        </filter>
      </defs>
      <text
        x={stats.x}
        y={stats.y}
        textAnchor="middle"
        fontSize={stats.nameSize}
        fontWeight="800"
        fill={stats.textColor}
        stroke={stats.textShadow}
        strokeWidth="2.2"
        paintOrder="stroke fill"
        filter="url(#right-stat-text-shadow)"
        textLength={stats.boxWidth}
      ...
      <text
        ...
        y={valueY}
        textAnchor="middle"
        fontSize={stats.valueSize}
        fontWeight="900"
        fill={stats.valueColor}
        stroke={stats.valueStroke}
        strokeWidth="2"
        paintOrder="stroke fill"
        filter="url(#right-stat-text-shadow)"
        textLength={stats.boxWidth}
      ...
      <GemIcon x={iconX} y={iconY} size={stats.iconSize} stats={stats} />
    </g>
  );
}
...
function SimpleHexCrown({ crown }) {
  ...
  return (
    <g
      aria-hidden="true"
      data-crown-variant={rankOne ? 'rank-one' : 'runner-up'}
      pointerEvents="none"
      transform={`translate(${x} ${y}) scale(${scale})`}
      dangerouslySetInnerHTML={{ __html: rankOne ? RANK_ONE_CROWN_BODY : RUNNER_UP_CROWN_BODY }}
    />
  );
}
...
function NameBoxClamp({ x, y, rotation, box }) {
  ...
  return (
    <g transform={`translate(${x} ${y}) rotate(${rotation})`}>
      ...
      <path
        d={d}
        fill="url(#winner-name-ring)"
        stroke={box.ringShadowColor}
        strokeWidth="1.35"
        strokeLinejoin="round"
        vectorEffect="non-scaling-stroke"
      />
...
function WinnerNameBox({ box }) {
  ...
  return (
    <g pointerEvents="none">
      <defs>
        <linearGradient id="winner-name-fill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={box.fillTop} />
          <stop offset="58%" stopColor={box.fillMid} />
          <stop offset="100%" stopColor={box.fillBottom} />
        </linearGradient>
        <linearGradient id="winner-name-ring" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor={box.ringGoldBright} />
          <stop offset="18%" stopColor={box.ringGoldMid} />
          <stop offset="36%" stopColor={box.ringGoldDark} />
          <stop offset="58%" stopColor={box.ringGoldBright} />
          <stop offset="82%" stopColor={box.ringGoldMid} />
          <stop offset="100%" stopColor={box.ringGoldDark} />
        </linearGradient>
        <filter id="winner-name-glow" x="-25%" y="-60%" width="150%" height="220%">
          <feGaussianBlur stdDeviation={box.glowBlur} />
        </filter>
        <filter id="winner-name-edge" x="-25%" y="-60%" width="150%" height="220%">
          <feGaussianBlur stdDeviation={box.edgeBlur} />
        </filter>
        <filter id="winner-text-shadow" x="-20%" y="-60%" width="140%" height="220%">
          <feDropShadow dx="0" dy="1.6" stdDeviation="1.1" floodColor="#000000" floodOpacity="0.85" />
        </filter>
      </defs>
      <rect
        x={x}
        ...
        rx={box.radius}
        fill="none"
        stroke={box.glowColor}
        strokeWidth={box.edgeWidth + 3}
        opacity={box.glowOpacity}
        filter="url(#winner-name-glow)"
      />
      <rect
        x={x}
        y={y}
        ...
        fill="none"
        stroke={box.ringShadowColor}
        strokeWidth={box.edgeWidth + 1.5}
        opacity="0.34"
        filter="url(#winner-name-edge)"
      />
      <rect
        x={x}
        y={y}
        ...
        height={box.height}
        rx={box.radius}
        fill="url(#winner-name-fill)"
        stroke="url(#winner-name-ring)"
        strokeWidth={box.edgeWidth}
        opacity={box.edgeOpacity}
        vectorEffect="non-scaling-stroke"
      />
      <rect
        x={x + inset}
        ...
        rx={Math.max(4, box.radius - inset)}
        fill="none"
        stroke={box.highlightColor}
        strokeWidth="0.85"
        opacity="0.42"
      ...
      />
      <path
        d={`M ${x + box.radius} ${y + box.edgeWidth * 1.1} H ${x + box.width - box.radius}`}
        fill="none"
        stroke={box.highlightColor}
        strokeWidth="1"
        strokeLinecap="round"
      ...
      <text
        x={box.x}
        y={box.y + box.fontSize * 0.34}
        textAnchor="middle"
        fontSize={box.fontSize}
        fontWeight="900"
        fill={box.textColor}
        stroke={box.textStroke}
        strokeWidth={box.textStrokeWidth}
        paintOrder="stroke fill"
        filter="url(#winner-text-shadow)"
      >
        {box.text}
      </text>
      {clamps.map(([cx, cy, rot], i) => (
        <NameBoxClamp key={`winner-clamp-${i}`} x={cx} y={cy} rotation={rot} box={box} />
      ))}
    </g>
  );
}
...
function HexBadge({ badge }) {
  ...
  return (
    <>
      <defs>
        <linearGradient id="side-hex-ring" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor={badge.ringGoldBright} />
          <stop offset="18%" stopColor={badge.ringGoldMid} />
          <stop offset="36%" stopColor={badge.ringGoldDark} />
          <stop offset="58%" stopColor={badge.ringGoldBright} />
          <stop offset="82%" stopColor={badge.ringGoldMid} />
          <stop offset="100%" stopColor={badge.ringGoldDark} />
        </linearGradient>
        <filter id="side-hex-glow" x="-70%" y="-70%" width="240%" height="240%">
          <feGaussianBlur stdDeviation={badge.glowBlur} />
        </filter>
        <filter id="side-hex-edge" x="-70%" y="-70%" width="240%" height="240%">
          <feGaussianBlur stdDeviation={badge.edgeBlur} />
        </filter>
      </defs>
      {badge.wingEnabled && (
        <g pointerEvents="none">
          {feathers.map((f) => (
            <path
              ...
              strokeWidth={badge.wingStrokeWidth + 5}
              strokeLinejoin="round"
              opacity={badge.wingGlowOpacity}
              filter="url(#side-hex-glow)"
            />
          ))}
          {feathers.map((f) => (
            <path
              key={f.id}
              d={f.d}
              fill="url(#side-hex-ring)"
              stroke={badge.ringShadowColor}
              strokeWidth={badge.wingStrokeWidth}
              strokeLinejoin="round"
              vectorEffect="non-scaling-stroke"
            />
          ))}
          {feathers.map((f) => (
            <path
              ...
              d={f.shine}
              fill="none"
              stroke={badge.highlightColor}
              strokeWidth={badge.highlightWidth * 0.75}
              strokeLinecap="round"
              opacity="0.64"
              vectorEffect="non-scaling-stroke"
            />
      ...
      <path
        ...
        fill="none"
        stroke={badge.glowColor}
        strokeWidth={badge.glowWidth}
        opacity={badge.glowOpacity}
        filter="url(#side-hex-glow)"
        pointerEvents="none"
      />
      <path
        ...
        fill="none"
        stroke={badge.ringShadowColor}
        strokeWidth={badge.edgeWidth + 5}
        opacity="0.58"
        filter="url(#side-hex-edge)"
        pointerEvents="none"
      />
      <path
        d={outerPath}
        fill="none"
        stroke="url(#side-hex-ring)"
        strokeWidth={badge.edgeWidth}
        strokeLinejoin="round"
        opacity={badge.edgeOpacity}
        vectorEffect="non-scaling-stroke"
      />
      <path
        d={innerPath}
        fill="none"
        stroke={badge.highlightColor}
        strokeWidth={badge.highlightWidth}
        strokeLinejoin="round"
        opacity={badge.innerEdgeOpacity}
        vectorEffect="non-scaling-stroke"
      />
      <text
        x={badge.cx}
        y={badge.cy + badge.fontSize * 0.34}
        textAnchor="middle"
        fontSize={badge.fontSize}
        fontWeight="900"
        fill={badge.textColor}
        stroke={badge.textStroke}
        strokeWidth={badge.textStrokeWidth}
        paintOrder="stroke fill"
      >
        {badge.label}
      </text>
...
function CenterCircle({ circle }) {
  ...
  return (
    <>
      <defs>
        <clipPath id="center-circle-image-clip">
          <circle cx={circle.cx} cy={circle.cy} r={imageRadius} />
        </clipPath>
        <radialGradient id="center-circle-fill" cx="50%" cy="42%" r="66%">
          <stop offset="0%" stopColor={circle.fillTopColor} />
          <stop offset="58%" stopColor={circle.fillMidColor} />
          <stop offset="100%" stopColor={circle.fillBottomColor} />
        </radialGradient>
        <linearGradient id="center-circle-ring" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor={circle.ringGoldBright} />
          <stop offset="18%" stopColor={circle.ringGoldMid} />
          <stop offset="36%" stopColor={circle.ringGoldDark} />
          <stop offset="58%" stopColor={circle.ringGoldBright} />
          <stop offset="82%" stopColor={circle.ringGoldMid} />
          <stop offset="100%" stopColor={circle.ringGoldDark} />
        </linearGradient>
        <linearGradient id="center-circle-highlight" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#ffffff" stopOpacity="0.95" />
          <stop offset="60%" stopColor={circle.bevelColor} stopOpacity="0.35" />
          <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
        </linearGradient>
        <filter id="center-circle-glow" x="-45%" y="-45%" width="190%" height="190%">
          <feGaussianBlur stdDeviation={circle.glowBlur} />
        </filter>
        <filter id="center-circle-edge" x="-45%" y="-45%" width="190%" height="190%">
          <feGaussianBlur stdDeviation={circle.edgeBlur} />
        </filter>
        <filter id="center-circle-box-glow" x="-80%" y="-80%" width="260%" height="260%">
          <feGaussianBlur stdDeviation={circle.boxGlowBlur} />
        </filter>
        <linearGradient id="center-box-gold" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor={circle.boxFillTop} />
          <stop offset="28%" stopColor={circle.boxFillMid} />
          <stop offset="48%" stopColor={circle.boxFillMid} />
          <stop offset="68%" stopColor={circle.boxFillTop} />
          <stop offset="100%" stopColor={circle.boxFillBottom} />
        </linearGradient>
      </defs>
      {circle.glowOpacity > 0 && (
        <circle
          cx={circle.cx}
          cy={circle.cy}
          r={circle.radius}
          fill="none"
          stroke={circle.glowColor}
          strokeWidth={circle.glowWidth}
          opacity={circle.glowOpacity}
          filter="url(#center-circle-glow)"
          pointerEvents="none"
        />
      )}
      <circle
        ...
        r={circle.radius}
        fill="none"
        stroke={circle.ringShadowColor}
        strokeWidth={circle.edgeWidth + 3}
        opacity="0.58"
        filter="url(#center-circle-edge)"
        pointerEvents="none"
      />
      <circle
        cx={circle.cx}
        ...
        r={circle.radius}
        fill="none"
        stroke="url(#center-circle-ring)"
        strokeWidth={circle.edgeWidth}
      ...
      <circle
        cx={circle.cx}
        cy={circle.cy}
        r={circle.radius - circle.edgeWidth * 0.5}
        fill="none"
        stroke={circle.ringGoldBright}
        strokeWidth="1"
        opacity="0.62"
        vectorEffect="non-scaling-stroke"
      />
      ...
      {profileImageUrl && (
        <image
          ...
          opacity={circle.imageOpacity}
          clipPath="url(#center-circle-image-clip)"
        />
      )}
      <circle
        cx={circle.cx}
        ...
        r={imageRadius}
        fill="none"
        stroke={circle.innerShadowColor}
        strokeWidth={circle.innerShadowWidth}
        opacity={circle.innerShadowOpacity}
        vectorEffect="non-scaling-stroke"
      />
      <circle
        cx={circle.cx}
        cy={circle.cy}
        r={imageRadius + circle.innerRingWidth}
        fill="none"
        stroke="url(#center-circle-ring)"
        strokeWidth={circle.innerRingWidth}
        opacity="0.92"
        vectorEffect="non-scaling-stroke"
      />
      <path
        d={`M ${circle.cx - circle.radius * 0.62} ${circle.cy - circle.radius * 0.36} C ${circle.cx - circle.radius * 0.28} ${circle.cy - circle.radius * 0.72}, ${circle.cx + circle.radius * 0.32} ${circle.cy - circle.radius * 0.68}, ${circle.cx + circle.radius * 0.62} ${circle.cy - circle.radius * 0.28}`}
        fill="none"
        stroke="url(#center-circle-highlight)"
        strokeWidth={circle.highlightArcWidth}
        strokeLinecap="round"
        opacity={circle.bevelOpacity}
        vectorEffect="non-scaling-stroke"
      />
      <circle
        cx={circle.cx - circle.bevelWidth * 0.75}
        cy={circle.cy - circle.bevelWidth * 0.75}
        r={circle.radius - circle.bevelWidth}
        fill="none"
        stroke={circle.bevelColor}
        strokeWidth={circle.bevelWidth}
        opacity={circle.bevelOpacity * 0.36}
        vectorEffect="non-scaling-stroke"
      />
      <CircleRingBoxes circle={circle} />
    </>
  );
}
...
function DrawFrame({
  ...
  frame: typeof OUTER_FRAME_DEFAULT;
  segments: Segment[];
  showNumbers: boolean;
}) {
  return (
    <>
      {segments.map((segment) => (
        <path
          ...
          fill="none"
          stroke={frame.color}
          strokeWidth={thicknessForSegment(frame, segment)}
          strokeLinejoin="round"
      ...
      {showNumbers &&
        segments.map((segment, index) => {
          ...
          return (
            <g key={`num-${segment.id}`} pointerEvents="none">
              <circle cx={x} cy={y} r="13" fill="black" stroke="white" strokeWidth="1.5" opacity="0.9" />
              <text x={x} y={y + 4} textAnchor="middle" fontSize="12" fontWeight="700" fill="white">
                {index + 1}
              </text>
            </g>
          );
...
function SvgDefs({
  ...
  outerFrame: typeof OUTER_FRAME_DEFAULT;
  innerFrame: typeof OUTER_FRAME_DEFAULT;
}) {
  return (
    <defs>
      <filter id="outer-frame-glow" x="-12%" y="-12%" width="124%" height="124%">
        <feGaussianBlur stdDeviation={outerFrame.glowBlur ?? 0} />
      </filter>
      <filter id="outer-frame-glow-edge" x="-12%" y="-12%" width="124%" height="124%">
        <feGaussianBlur stdDeviation={outerFrame.edgeBlur ?? 0} />
      </filter>
      <filter id="inner-frame-glow" x="-12%" y="-12%" width="124%" height="124%">
        <feGaussianBlur stdDeviation={innerFrame.glowBlur ?? 0} />
      </filter>
      <filter id="inner-frame-glow-edge" x="-12%" y="-12%" width="124%" height="124%">
        <feGaussianBlur stdDeviation={innerFrame.edgeBlur ?? 0} />
      </filter>
    </defs>
  );
}
...
function Guides({
  ...
  viewBox: typeof DEFAULT_CFG.viewBox;
  guides: typeof DEFAULT_CFG.viewportGuide;
}) {
  ...
  return (
    <rect
      x={guides.inset}
      y={guides.inset}
      width={viewBox.w - guides.inset * 2}
      height={viewBox.h - guides.inset * 2}
      fill="none"
      stroke={guides.color}
...
function FrameLayer({
  ...
  const pathFor = (segment: Segment, props: Record<string, unknown>, prefix: string) => (
    <path
  ...
  return (
    <>
      ...
      {(frame.glowOpacity ?? 0) > 0 && (frame.glowWidth ?? 0) > 0 && (
        <g opacity={frame.glowOpacity} filter={`url(#${filterId})`} pointerEvents="none">
          {segments.map((segment) =>
            pathFor(
              segment,
              { stroke: frame.glowColor, strokeWidth: thicknessForSegment(frame, segment) + (frame.glowWidth ?? 0) },
              'glow'
            )
          )}
        </g>
      )}
      {(frame.edgeOpacity ?? 0) > 0 && (frame.edgeWidth ?? 0) > 0 && (
        <g opacity={frame.edgeOpacity} filter={`url(#${filterId}-edge)`} pointerEvents="none">
          {segments.map((segment) =>
            pathFor(
              segment,
              { stroke: frame.edgeColor, strokeWidth: thicknessForSegment(frame, segment) + (frame.edgeWidth ?? 0) },
              'edge'
            )
          )}
        </g>
      ...
      <g opacity={frame.opacity ?? 1}>
        ...
        {(frame.overlayGradientOpacity ?? 0) > 0 &&
          segments.map((segment) =>
            pathFor(
              segment,
              {
                stroke: `url(#${overlayId})`,
                opacity: frame.overlayGradientOpacity,
                style: { mixBlendMode: frame.overlayBlend },
              },
              'overlay'
            )
          )}
...
function Controls({
  ...
  cfg: typeof DEFAULT_CFG;
  setCfg: React.Dispatch<React.SetStateAction<typeof DEFAULT_CFG>>;
}) {
  ...
  const updateAnchor = (key: string, value: unknown) =>
    setCfg((old) => ({ ...old, [anchorKey]: { ...old[anchorKey], [key]: Number(value) } }));
  const updateViewportGuide = (key: string, value: unknown) =>
    setCfg((old) => ({
      ...old,
      viewportGuide: {
        ...old.viewportGuide,
        [key]:
          key === 'enabled' || key === 'showCenterLines' || key === 'showSegmentNumbers' || key === 'gangedSlices'
            ? Boolean(value)
            : key === 'color' || key === 'centerLineColor'
              ? value
              : Number(value),
      },
    }));
  const updateArt = (group: string, key: string, value: unknown) =>
    setCfg((old) => ({
      ...old,
      [group]: {
        ...old[group],
        [key]:
          typeof value === 'string' && key !== 'text' && key !== 'name' && key !== 'value' && key !== 'profileImageUrl'
            ? Number(value)
            : value,
      },
    }));

  const updateSegmentThickness = (segment: Segment, value: unknown) =>
    setCfg((old) => ({
      ...old,
      [frameKey]: {
        ...old[frameKey],
        segmentThicknesses: { ...(old[frameKey].segmentThicknesses ?? {}), [segment.id]: Number(value) },
      },
    }));
  ...
  const sliderRow = (
    ...
    <label className="grid grid-cols-[170px_1fr_64px_28px] items-center gap-2 text-xs text-amber-100">
      <span>{label}</span>
      <input
        className="w-full"
        type="range"
      ...
      />
      <input
        className="w-16 rounded bg-black/60 px-1 py-0.5 text-right text-amber-100 outline outline-1 outline-amber-400/20"
        type="number"
        value={value}
        onChange={(e) => onChange(e.target.value)}
      />
  ...
  const anchorSlider = (key: string, max = 300, min = 0, step = 1) =>
    sliderRow(
      key,
      activeAnchor[key as keyof typeof activeAnchor] as number,
      (v) => updateAnchor(key, v),
      () => updateAnchor(key, defaultAnchor[key as keyof typeof defaultAnchor]),
      max,
      min,
      step
    );
  const viewportSlider = (key: keyof typeof DEFAULT_CFG.viewportGuide, max = 20, min = 0, step = 1) =>
    sliderRow(
      key,
      cfg.viewportGuide[key] as number,
      (v) => updateViewportGuide(key, v),
      () => updateViewportGuide(key, DEFAULT_CFG.viewportGuide[key]),
      max,
      min,
      step
    );
  ...
  return (
    <div className="mt-1 rounded-2xl border border-amber-400/20 bg-black/50 p-3 backdrop-blur">
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        {['outer', 'inner', 'art', 'viewportOnly'].map((id) => (
          <button
            key={id}
            className={`rounded-lg px-3 py-1 text-xs ${mainTab === id ? 'bg-amber-400 text-black' : 'bg-white/5 text-amber-100'}`}
      ...
      {mainTab !== 'viewportOnly' && (
        <div className="mb-3 flex flex-wrap gap-2">
          {['segments', 'anchor', 'top', 'bottom', 'corners', 'line', 'slices'].map((id) => (
            <button
              key={id}
              className={`rounded-lg px-3 py-1 text-xs ${tab === id ? 'bg-amber-400 text-black' : 'bg-white/5 text-amber-100'}`}
              onClick={() => setTab(id)}
              type="button"
            >
              {id}
      ...
      <div className="grid gap-2 md:grid-cols-2">
        {mainTab !== 'viewportOnly' &&
          ...
          liveSegments.map((segment, index) =>
            sliderRow(
              `${index + 1}. ${segment.id}`,
              thicknessForSegment(liveFrame, segment),
              (v) => updateSegmentThickness(segment, v),
              () =>
                setCfg((old) => ({
                  ...old,
                  [frameKey]: { ...old[frameKey], segmentThicknesses: { ...defaultFrame.segmentThicknesses } },
                })),
              120
            )
          )}
        {mainTab !== 'viewportOnly' && tab === 'anchor' && (
          <>
            {anchorSlider('sideInset', 400)}
            {anchorSlider('topInset', 300)}
            {anchorSlider('bottomInset', 300)}
            <div className="col-span-full rounded-xl border border-amber-400/20 bg-black/40 p-2 text-xs text-amber-100">
              Live: x {liveFrame.x}, y {liveFrame.y}, w {liveFrame.w}, h {liveFrame.h}
            </div>
          </>
        )}
        {mainTab === 'art' && (
          <>
            ...
            {sliderRow(
              'centerCircle.cx',
              cfg.centerCircle.cx,
              (v) => updateArt('centerCircle', 'cx', v),
              () => updateArt('centerCircle', 'cx', DEFAULT_CFG.centerCircle.cx),
              1400,
              0
            )}
            {sliderRow(
              'centerCircle.cy',
              cfg.centerCircle.cy,
              (v) => updateArt('centerCircle', 'cy', v),
              () => updateArt('centerCircle', 'cy', DEFAULT_CFG.centerCircle.cy),
              1000,
              -500
            )}
            {sliderRow(
              'centerCircle.radius',
              cfg.centerCircle.radius,
              (v) => updateArt('centerCircle', 'radius', v),
              () => updateArt('centerCircle', 'radius', DEFAULT_CFG.centerCircle.radius),
              400,
              40
            )}
            <label className="col-span-full grid grid-cols-[170px_1fr] items-center gap-2 text-xs text-amber-100">
              <span>centerCircle.profileImageUrl</span>
              <input
                className="rounded bg-black/60 px-2 py-1 text-amber-100 outline outline-1 outline-amber-400/20"
                value={cfg.centerCircle.profileImageUrl}
                onChange={(e) => updateArt('centerCircle', 'profileImageUrl', e.target.value)}
              />
            </label>
            {sliderRow(
              'centerCircle.imageScale',
              cfg.centerCircle.imageScale,
              (v) => updateArt('centerCircle', 'imageScale', v),
              () => updateArt('centerCircle', 'imageScale', DEFAULT_CFG.centerCircle.imageScale),
              3,
              0.2,
              0.01
            )}
            {sliderRow(
              'centerCircle.imageYOffset',
              cfg.centerCircle.imageYOffset,
              (v) => updateArt('centerCircle', 'imageYOffset', v),
              () => updateArt('centerCircle', 'imageYOffset', DEFAULT_CFG.centerCircle.imageYOffset),
              100,
              -100
            )}
            ...
            {sliderRow(
              'centerCrown.cy',
              cfg.centerCrown.cy,
              (v) => updateArt('centerCrown', 'cy', v),
              () => updateArt('centerCrown', 'cy', DEFAULT_CFG.centerCrown.cy),
              200,
              -220
            )}
            {sliderRow(
              'sideHexBadge.cx',
              cfg.sideHexBadge.cx,
              (v) => updateArt('sideHexBadge', 'cx', v),
              () => updateArt('sideHexBadge', 'cx', DEFAULT_CFG.sideHexBadge.cx),
              800,
              0
            )}
            {sliderRow(
              'sideHexBadge.cy',
              cfg.sideHexBadge.cy,
              (v) => updateArt('sideHexBadge', 'cy', v),
              () => updateArt('sideHexBadge', 'cy', DEFAULT_CFG.sideHexBadge.cy),
              1000,
              -500
            )}
            {sliderRow(
              'sideHexBadge.radius',
              cfg.sideHexBadge.radius,
              (v) => updateArt('sideHexBadge', 'radius', v),
              () => updateArt('sideHexBadge', 'radius', DEFAULT_CFG.sideHexBadge.radius),
              220,
              30
            )}
            <label className="col-span-full grid grid-cols-[170px_1fr] items-center gap-2 text-xs text-amber-100">
              <span>sideHexBadge.label</span>
              <input
                className="rounded bg-black/60 px-2 py-1 text-amber-100 outline outline-1 outline-amber-400/20"
                value={cfg.sideHexBadge.label}
                onChange={(e) => updateArt('sideHexBadge', 'label', e.target.value)}
              />
            </label>
            {sliderRow(
              'winnerName.x',
              cfg.winnerName.x,
              (v) => updateArt('winnerName', 'x', v),
              () => updateArt('winnerName', 'x', DEFAULT_CFG.winnerName.x),
              1400,
              0
            )}
            {sliderRow(
              'winnerName.y',
              cfg.winnerName.y,
              (v) => updateArt('winnerName', 'y', v),
              () => updateArt('winnerName', 'y', DEFAULT_CFG.winnerName.y),
              1000,
              -500
            )}
            <label className="col-span-full grid grid-cols-[170px_1fr] items-center gap-2 text-xs text-amber-100">
              <span>winnerName.text</span>
              <input
                className="rounded bg-black/60 px-2 py-1 text-amber-100 outline outline-1 outline-amber-400/20"
                value={cfg.winnerName.text}
                onChange={(e) => updateArt('winnerName', 'text', e.target.value)}
              />
            </label>
            {sliderRow(
              'rightStats.x',
              cfg.rightStats.x,
              (v) => updateArt('rightStats', 'x', v),
              () => updateArt('rightStats', 'x', DEFAULT_CFG.rightStats.x),
              1400,
              0
            )}
            {sliderRow(
              'rightStats.y',
              cfg.rightStats.y,
              (v) => updateArt('rightStats', 'y', v),
              () => updateArt('rightStats', 'y', DEFAULT_CFG.rightStats.y),
              1000,
              -500
            )}
            <label className="col-span-full grid grid-cols-[170px_1fr] items-center gap-2 text-xs text-amber-100">
              <span>rightStats.name</span>
              <input
                className="rounded bg-black/60 px-2 py-1 text-amber-100 outline outline-1 outline-amber-400/20"
                value={cfg.rightStats.name}
                onChange={(e) => updateArt('rightStats', 'name', e.target.value)}
              />
            </label>
            <label className="col-span-full grid grid-cols-[170px_1fr] items-center gap-2 text-xs text-amber-100">
              <span>rightStats.value</span>
              <input
                className="rounded bg-black/60 px-2 py-1 text-amber-100 outline outline-1 outline-amber-400/20"
                value={cfg.rightStats.value}
                onChange={(e) => updateArt('rightStats', 'value', e.target.value)}
              />
            </label>
          </>
        )}
        {mainTab === 'viewportOnly' && (
          <>
            {viewportSlider('inset', 40)}
            {viewportSlider('width', 10)}
            {viewportSlider('opacity', 1, 0, 0.01)}
          </>
        )}
      </div>
    </div>
...
export function GoldenFrameForeignObjectArtSvg({
  ...
  return (
    <GoldenFrameForeignObjectArtSvgFrame
      cfg={cfg}
      className={className}
...
function GoldenFrameForeignObjectArtSvgFrame({
  ...
  return (
    <svg
      ...
      width={width}
      height={height}
      viewBox={`0 ${cfg.viewBox.y ?? 0} ${cfg.viewBox.w} ${cfg.viewBox.h}`}
      preserveAspectRatio={preserveAspectRatio}
      role="img"
    >
      {backgroundFill !== 'none' && (
        <rect x="0" y={cfg.viewBox.y ?? 0} width={cfg.viewBox.w} height={cfg.viewBox.h} fill={backgroundFill} />
      )}
      ...
      <FrameLayer
        frame={outerLiveFrame}
        segments={outerSegments}
        filterId="outer-frame-glow"
        showNumbers={cfg.viewportGuide.showSegmentNumbers}
      />
...
export function createGoldenFrameFrameOnlySvgDataUri(cfg: typeof DEFAULT_CFG) {
  const svg = renderToStaticMarkup(
    <GoldenFrameForeignObjectArtSvgFrame
      cfg={cfg}
      className="gold-card-embed-svg"
...
function FinalArtStage({ cfg, fullBleed = false }) {
  return (
    <div className="preview-stage" style={fullBleed ? { height: '100vh', margin: 0 } : { height: '58vh' }}>
...
export function GoldenFrameForeignObject() {
  ...
  return (
    <div className="parent-portal-page svg-route">
...
export function GoldenFrameForeignObjectSvgRoute() {
  ...
  return (
    <div className="parent-portal-page svg-route">
      <FinalArtStage cfg={cfg} fullBleed />
    </div>
  );
}
...
export function GoldenFrameForeignObjectControlsRoute() {
  const [cfg, setCfg] = useSharedForeignConfig();
  return (
    <div className="parent-portal-page controls-route">
      <div className="min-h-0 flex-1 overflow-y-auto pr-2">
        <Controls cfg={cfg} setCfg={setCfg} />
      </div>
    </div>
