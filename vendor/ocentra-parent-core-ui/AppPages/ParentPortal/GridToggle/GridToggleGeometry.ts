export function clampGridToggleInt(value: number, min: number, max: number): number {
...
export function estimateGridToggleTextWidth(text: string, fontSize: number): number {
  return Math.ceil(text.length * fontSize * 0.62);
}

export function roundedGridToggleRectPath(x: number, y: number, width: number, height: number, radius: number): string {
  return roundedGridToggleRectPathByCorner(x, y, width, height, radius, radius, radius, radius);
}

export function roundedGridToggleRectPathByCorner(
  x: number,
  y: number,
  width: number,
  height: number,
