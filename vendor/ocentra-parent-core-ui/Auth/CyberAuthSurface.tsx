...
export const AUTH_PAGE_SVG_BOOLEAN_FIELDS: Record<
  Exclude<AuthPageSvgControlGroup, 'colors'>,
  AuthPageSvgBooleanField[]
> = {
  fit: [
  ...
  dock: [{ group: 'dock', key: 'showBottomDock', label: 'Show bottom dock' }],
  close: [
...
export const AUTH_PAGE_SVG_TEXT_FIELDS: Record<Exclude<AuthPageSvgControlGroup, 'colors'>, AuthPageSvgTextField[]> = {
  ...
  auth: [],
  brand: [],
  fields: [{ group: 'fields', key: 'avatarText', label: 'Avatar Text' }],
  cta: [],
  social: [{ group: 'social', key: 'continueText', label: 'Divider Text' }],
  decor: [],
  dock: [],
  close: [],
};

export const AUTH_PAGE_SVG_NUMBER_FIELDS: Record<
  Exclude<AuthPageSvgControlGroup, 'colors'>,
  AuthPageSvgNumberField[]
> = {
  fit: [
...
export function normalizeAuthPageSvgControls(value?: Partial<AuthPageSvgControls> | null): AuthPageSvgControls {
  ...
  (Object.keys(DEFAULT_AUTH_PAGE_SVG_CONTROLS) as Array<keyof AuthPageSvgControls>).forEach((key) => {
    ...
    if (typeof fallback === 'number') {
      normalized[key] = (typeof nextValue === 'number' && Number.isFinite(nextValue) ? nextValue : fallback) as never;
      return;
    }
...
export function serializeAuthPageSvgControls(value?: Partial<AuthPageSvgControls> | null): AuthPageSvgControls {
  return normalizeAuthPageSvgControls(value);
}

const AuthPageSvgControlsContext = React.createContext<AuthPageSvgControls>(DEFAULT_AUTH_PAGE_SVG_CONTROLS);
...
function makeFramePath(config: CyberAuthConfig, inset = 0, useInsetControls = false, usePanelControls = false) {
  ...
  const sd = usePanelControls
    ? config.insetPanelSideNotchD
    : useInsetControls
      ? config.insetSideNotchD
      : config.sideNotchD;
  ...
  const y2 = useInsetControls
    ? config.lockInsetSideNotches
      ? H - y1 - sh
      : config.insetLowerSideNotchY
    : config.lockSideNotches
      ? H - y1 - sh
      : config.lowerSideNotchY;
  const cx = W / 2;
...
function SideNotchRail({
  ...
  return (
    <g transform={`translate(${x} 0) scale(${s} 1)`}>
      <path
        d={fullRail}
        fill={c.show3D ? 'url(#rail3dFill)' : c.sideRailMainColor}
        stroke={c.decorDark}
        strokeWidth={c.sideRailOutlineW}
      ...
      {(['left', 'right'] as const).map((side) => (
        <MirroredSide key={side} side={side}>
          <g filter="url(#decorGlow)" transform={`translate(${c.cornerClusterX} ${c.cornerClusterY})`}>
            ...
            <path
              d={`M75 70 L${shortEndX} ${shortEndTopY}`}
              stroke={c.decorCyan}
              strokeWidth={cornerStrokeW}
              strokeLinecap="round"
            />
            <path
              d={`M52 100 L${midEndX} ${midEndTopY}`}
              stroke={c.decorCyan}
              strokeWidth={cornerStrokeW}
              strokeLinecap="round"
            />
            <path
              d={`M37 136 V${longEndTopY}`}
              stroke={c.decorCyan}
              strokeWidth={cornerStrokeW}
              strokeLinecap="round"
            />
          ...
          <g filter="url(#decorGlow)" transform={`translate(${c.cornerClusterX} ${c.cornerClusterBottomY})`}>
            ...
            <path
              d={`M75 ${c.frameH - 70} L${shortEndX} ${shortEndBottomY}`}
              stroke={c.decorCyan}
              strokeWidth={cornerStrokeW}
              strokeLinecap="round"
            />
            <path
              d={`M52 ${c.frameH - 100} L${midEndX} ${midEndBottomY}`}
              stroke={c.decorCyan}
              strokeWidth={cornerStrokeW}
              strokeLinecap="round"
            />
            <path
              d={`M37 ${c.frameH - 136} V${longEndBottomY}`}
              stroke={c.decorCyan}
              strokeWidth={cornerStrokeW}
              strokeLinecap="round"
            />
          ...
          <SidePanelWithScrews
            panelY={c.sidePanelBottomY}
            panelH={c.sidePanelBottomH}
            screwYs={[c.screwY3, c.screwY4]}
          />
    ...
    <g filter={`url(#${glowFilterId})`}>
      ...
      {config.ctaShowInnerGlow ? (
        <path
          d={edgePath}
          fill="none"
          stroke={config.ctaInnerGlowColor}
          strokeWidth={config.ctaInnerGlowW}
          opacity={config.ctaInnerGlowOpacity}
          strokeLinejoin="round"
          filter={`url(#${innerGlowFilterId})`}
        />
      ) : null}
      {config.ctaShowInnerEdge ? (
        <path
          d={edgePath}
          fill="none"
          stroke={config.ctaInnerEdgeColor}
          strokeWidth={config.ctaInnerEdgeW}
          opacity={config.ctaInnerEdgeOpacity}
          strokeLinejoin="round"
        />
      ) : null}
      {config.ctaShineOpacity > 0 ? (
        <path
          d={`M ${faceX + 18} ${faceY + 7} L ${faceX + faceW - 18} ${faceY + 7}`}
          stroke="#ffffff"
          strokeWidth="2"
          opacity={config.ctaShineOpacity}
          strokeLinecap="round"
        />
      ) : null}
      ...
      {showTicks && config.ctaShowSideTicks ? (
        <g opacity="0.85">
          {Array.from({ length: config.ctaSideTickCount }).map((_, i) => (
            <rect
              key={`l${i}`}
              x={x + config.ctaSideTickX}
              y={y + config.ctaSideTickY + i * config.ctaSideTickGap}
              width={config.ctaSideTickW}
              height={config.ctaSideTickH}
              rx={config.ctaSideTickRadius}
              fill={i % 2 ? config.authGreen : config.authCyan}
            />
          ))}
          {Array.from({ length: config.ctaSideTickCount }).map((_, i) => (
            <rect
              key={`r${i}`}
              x={x + w - config.ctaSideTickX - config.ctaSideTickW}
              y={y + config.ctaSideTickY + i * config.ctaSideTickGap}
              width={config.ctaSideTickW}
              height={config.ctaSideTickH}
              rx={config.ctaSideTickRadius}
              fill={i % 2 ? config.authBlue : config.authCyan}
            />
          ))}
        </g>
      ) : null}
...
function FieldIcon({ kind, x, y, size }: { kind: FieldKind; x: number; y: number; size: number }) {
  ...
  if (kind === 'email') {
    return (
      <g
        fill="none"
        stroke={c.fieldIconColor}
        strokeWidth={c.fieldIconStrokeW}
        strokeLinecap="round"
        strokeLinejoin="round"
      >
        <rect x={x - size * 0.48} y={y - size * 0.32} width={size * 0.96} height={size * 0.64} rx="4" />
        <path
          d={`M ${x - size * 0.45} ${y - size * 0.25} L ${x} ${y + size * 0.05} L ${x + size * 0.45} ${y - size * 0.25}`}
        />
      </g>
    );
  }
  if (kind === 'password') {
    return (
      ...
      >
        <path
          d={`M ${x - size * 0.36} ${y - size * 0.02} V ${y - size * 0.22} C ${x - size * 0.36} ${y - size * 0.58} ${x + size * 0.36} ${y - size * 0.58} ${x + size * 0.36} ${y - size * 0.22} V ${y - size * 0.02}`}
        />
  ...
  return (
    ...
    >
      ...
      <path
        d={`M ${x - size * 0.42} ${y + size * 0.46} C ${x - size * 0.3} ${y + size * 0.08} ${x + size * 0.3} ${y + size * 0.08} ${x + size * 0.42} ${y + size * 0.46}`}
      />
    </g>
  );
}
...
function AuthField({
  ...
  return (
    <CyberButtonFrame
      ...
      h={c.fieldH}
      config={config}
      faceFill={c.fieldFaceFill}
      glowFilterId="fieldGlow"
      innerGlowFilterId="fieldInnerGlow"
    >
      {({ faceX, faceY, faceW, faceH }) => (
        <>
          <path
            d={chamferRectPath(
              faceX + c.fieldIconBoxInset,
              faceY + c.fieldIconBoxInset,
              c.fieldIconW - c.fieldIconBoxInset * 2,
              faceH - c.fieldIconBoxInset * 2,
              12
            )}
            fill={c.fieldIconFill}
            stroke={c.fieldIconStroke}
            strokeWidth={c.fieldIconStrokeW}
            opacity={c.fieldIconOpacity}
          />
          <FieldIcon kind={kind} x={faceX + c.fieldIconW / 2} y={faceY + faceH / 2} size={c.fieldIconSize} />
          <line
            x1={faceX + c.fieldIconW + c.fieldDividerX}
            y1={faceY + 10}
            x2={faceX + c.fieldIconW + c.fieldDividerX}
            y2={faceY + faceH - 10}
            stroke={hasError ? '#ff7aa8' : c.fieldStroke}
            strokeWidth={c.fieldDividerW}
            opacity="0.6"
          />
          <foreignObject
            x={faceX + c.fieldIconW + c.fieldTextX - 6}
            y={faceY + 2}
...
function LockIcon({ cx, cy }: { cx: number; cy: number }) {
  ...
  return (
    <g fill="none" stroke={c.authText} strokeWidth={c.ctaLockStrokeW} strokeLinecap="round" strokeLinejoin="round">
      <path
        d={`M ${shackleX} ${bodyY + 4} V ${shackleY + shackleH * 0.55} C ${shackleX} ${shackleY} ${shackleX + shackleW} ${shackleY} ${shackleX + shackleW} ${shackleY + shackleH * 0.55} V ${bodyY + 4}`}
        opacity="0.92"
      />
...
function BrandPlate({
  ...
  return (
    <CyberButtonFrame
      ...
      h={h}
      config={brandConfig}
      faceFill={c.brandFaceFill}
      showTicks={false}
      glowFilterId="brandGlow"
      innerGlowFilterId="brandInnerGlow"
    >
      {() => (
        <>
          <circle
            ...
            cy={orbCy}
            r={c.brandOrbR}
            fill={c.brandOrbOuterFill}
            stroke={c.brandDarkStroke}
            strokeWidth={c.brandOrbStrokeW}
          />
          <circle
            cx={orbCx}
            cy={orbCy}
            r={Math.max(0, c.brandOrbR - 8)}
            fill={c.brandOrbRingFill}
            stroke={c.brandOuterStroke}
            strokeWidth={c.brandOuterStrokeW}
          />
          <circle
            cx={orbCx}
            cy={orbCy}
            r={c.brandOrbInnerR}
            fill="url(#authOrbGrad)"
            stroke={c.authCyan}
            strokeWidth={c.brandOrbInnerStrokeW}
            filter="url(#authGlow)"
          />
          <image
            href={parentPortalLogoImageUrl}
            x={orbCx - logoSize / 2 + c.brandLogoX}
            y={orbCy - logoSize / 2 + c.brandLogoY}
            width={logoSize}
            height={logoSize}
            preserveAspectRatio="xMidYMid meet"
          />
          <text
            x={orbCx - c.brandOrbR - c.brandGap + c.brandLeftTextX}
            y={y + h / 2 + c.brandTextY}
            textAnchor="end"
            fill={c.authText}
            fontSize={c.brandTextSize}
            fontWeight="900"
          ...
          <text
            x={orbCx + c.brandOrbR + c.brandGap + c.brandRightTextX}
            y={y + h / 2 + c.brandTextY}
            fill={c.authText}
            fontSize={c.brandTextSize}
            fontWeight="900"
  ...
  return (
    <g transform={`translate(${x} ${y}) scale(${dir} 1)`} opacity={c.continueArtOpacity}>
      ...
      <line
        x1="10"
        y1="0"
        x2={c.continueArtW - 34}
        y2="0"
        stroke={c.continueArtColor}
        strokeWidth={c.continueLineStrokeW}
      />
      <rect
        x={c.continueArtW - 30}
        y={-c.continueArtMidTickH / 2}
        width={c.continueArtMidTickW}
        height={c.continueArtMidTickH}
        rx={c.continueArtMidTickH / 2}
        fill={c.continueArtColor}
      />
  ...
  return (
    <g>
      {c.continueShowSideArt ? (
        ...
        <>
          <circle
            cx={textLeftX - c.continueLineW}
            cy={lineY}
            r={c.continueEndDotR}
            fill={c.continueLineColor}
            opacity={c.continueLineOpacity}
          />
          <line
            x1={textLeftX - c.continueLineW + 10}
            y1={lineY}
            x2={textLeftX}
            y2={lineY}
            stroke={c.continueLineColor}
            strokeWidth={c.continueLineStrokeW}
            opacity={c.continueLineOpacity}
          />
          <circle
          ...
          <line
            x1={textRightX}
            y1={lineY}
            x2={textRightX + c.continueLineW - 10}
            y2={lineY}
            stroke={c.continueLineColor}
            strokeWidth={c.continueLineStrokeW}
            opacity={c.continueLineOpacity}
      ...
      <text
        x={cx + c.continueX}
        y={lineY + c.continueTextSize / 3}
        textAnchor="middle"
        fill={c.continueColor}
        fontSize={c.continueTextSize}
        fontWeight="900"
        letterSpacing={c.continueLetterSpacing}
      >
        {c.continueText}
      </text>
    </g>
...
function SocialButton({ cx, cy, option }: { cx: number; cy: number; option: CyberSocialOption }) {
  ...
  return (
    <g filter="url(#socialGlow)" opacity={option.disabled ? 0.5 : 1}>
      {c.socialShowOuterRing ? (
        <circle
          cx={cx}
          cy={cy}
          r={c.socialRingR}
          fill={c.socialOuterRingFill}
          stroke={c.socialOuterRingStroke}
          strokeWidth={c.socialOuterRingW}
        />
      ) : null}
      {c.socialShowMidRing ? (
        <circle
          cx={cx}
          cy={cy}
          r={Math.max(0, c.socialRingR - 9)}
          fill={c.socialMidRingFill}
          stroke={c.socialMidRingStroke}
          strokeWidth={c.socialMidRingW}
        />
      ) : null}
      {c.socialShowInnerFace ? (
        <circle
          cx={cx}
          cy={cy}
          r={c.socialR}
          fill={c.socialFaceFill}
          stroke={c.authStroke}
          strokeWidth={c.socialInnerRingW}
        />
      ) : null}
      {c.socialShowBottomGlow ? (
        <path
          d={`M ${cx - c.socialBottomGlowW / 2} ${cy + c.socialBottomGlowY} L ${cx + c.socialBottomGlowW / 2} ${cy + c.socialBottomGlowY}`}
          stroke={c.authBlue}
          strokeWidth="6"
          strokeLinecap="round"
          opacity={c.socialBottomGlowOpacity}
        />
      ) : null}
      <foreignObject x={cx - c.socialR} y={cy - c.socialR} width={buttonSize} height={buttonSize}>
        <button
          className="login-cyber-social-button"
          type="button"
          onClick={option.onClick}
          disabled={option.disabled}
          aria-label={option.alt}
          style={{ padding: `${c.socialIconPadding}px` }}
        >
...
function SocialLoginPanel({ cx, y, options }: { cx: number; y: number; options: CyberSocialOption[] }) {
  ...
  const gap = Math.min(
    c.socialGap,
    Math.max(0, c.socialPanelW / 2 - Math.max(c.socialR, c.socialRingR) - c.socialChildInset)
  );
  ...
  return (
    <g>
      ...
      <path
        d={panelPath}
        fill={c.socialPanelFill}
        stroke={c.socialPanelStroke}
        strokeWidth={c.socialPanelStrokeW}
        filter="url(#authGlow)"
        opacity="0.96"
      />
      ...
      <g clipPath={c.socialClipToPanel ? `url(#${clipId})` : undefined}>
        {options.map((option, index) => (
          <SocialButton
            key={option.key}
            cx={cx + c.socialPanelX + c.socialX + (index - offset) * gap}
            cy={buttonY}
            option={option}
          />
        ))}
...
function BottomDock() {
  ...
  return (
    <g filter="url(#authGlow)">
      <path
        d={outer}
        fill={c.bottomDockFill}
        stroke="#08111c"
        strokeWidth={c.bottomDockStrokeW + 3}
        strokeLinejoin="round"
      />
      <path
        d={outer}
        fill={c.bottomDockFill}
        stroke={c.bottomDockStroke}
        strokeWidth={c.bottomDockStrokeW}
        strokeLinejoin="round"
        opacity="0.96"
      />
      ...
      <g filter="url(#decorGlow)" opacity={c.bottomDockVentOpacity}>
        {Array.from({ length: ventCount }).map((_, i) => (
          <rect
            key={`vent-${i}`}
            x={ventX + i * (c.bottomDockVentW + c.bottomDockVentGap)}
            y={y + h / 2 - c.bottomDockVentH / 2 + c.bottomDockVentY}
            width={c.bottomDockVentW}
            height={c.bottomDockVentH}
            rx={c.bottomDockVentW / 2}
            fill={c.bottomDockVentColor}
          />
        ))}
      </g>
    </g>
...
function CloseButton({ onClose, label }: { onClose?: (() => void) | undefined; label: string }) {
  ...
  if (c.closeUseCornerTab) {
    ...
    return (
      <g
        ...
        transform={
          hover
            ? `translate(${centerX} ${centerY}) scale(${c.closeHoverScale}) translate(${-centerX} ${-centerY})`
            : undefined
        }
      ...
      >
        ...
        <path
          ...
          stroke="#08111c"
          strokeWidth={Math.max(2, c.closeStrokeW + 2)}
          strokeLinejoin="round"
        />
        <path
          d={inner}
          fill={c.closeInnerFill}
          stroke={c.closeStroke}
          strokeWidth={c.closeStrokeW}
          strokeLinejoin="round"
        />
        {c.closeShowCircle ? (
          <circle
            ...
            cy={cy}
            r={c.closeR * 0.58}
            fill={c.buttonBaseFill}
            stroke={c.closeStroke}
            strokeWidth={Math.max(1, c.closeStrokeW * 0.7)}
          />
        ) : null}
        <text
          x={cx + c.closeTextX}
          y={cy + c.closeTextY + c.closeTextSize / 3.6}
          textAnchor="middle"
          fill={c.closeTextColor}
          fontSize={c.closeTextSize * 0.72}
          fontWeight="900"
        >
          ×
        </text>
  ...
  return (
    ...
    >
      ...
      <text
        x={c.closeTextX}
        y={c.closeTextY + c.closeTextSize / 3}
        textAnchor="middle"
        fill={c.closeTextColor}
        fontSize={c.closeTextSize}
        fontWeight="900"
      >
        ×
      </text>
...
}: Pick<CyberAuthSurfaceProps, 'avatar' | 'showAvatarSelector' | 'onToggleAvatarSelector'> & {
  cx: number;
  y: number;
}) {
  const c = useAuthPageSvgControls();
  ...
  return (
    <g filter="url(#authGlow)">
      <circle
        ...
        fill={c.avatarFill}
        stroke={c.authStroke}
        strokeWidth={c.avatarOuterStrokeW}
      />
      <circle
        cx={avatarCx}
        cy={avatarCy}
        r={c.avatarR - c.avatarRingW}
        fill={c.avatarInnerFill}
        stroke={c.authCyan}
        strokeWidth={c.avatarInnerStrokeW}
      />
      {avatar ? (
        <image
          href={avatar}
          x={avatarCx - avatarImageSize / 2}
          y={avatarCy - avatarImageSize / 2}
      ...
      {!avatar ? (
        <text
          x={avatarCx}
          y={avatarCy + c.avatarTextSize / 3}
          textAnchor="middle"
          fill={c.authText}
          fontSize={c.avatarTextSize}
          fontWeight="900"
        >
          {c.avatarText}
        </text>
      ...
      <foreignObject x={avatarCx - c.avatarR} y={avatarCy - c.avatarR} width={c.avatarR * 2} height={c.avatarR * 2}>
        <button
          className="login-cyber-avatar-button"
          type="button"
...
> & { cx: number; y: number }) {
  ...
  return (
    <foreignObject x={panelX} y={panelY} width={c.fieldW} height={panelH}>
      <div
        className="login-cyber-avatar-selector-panel"
        ref={avatarSelectorRef}
        style={{ padding: `${c.avatarSelectorPadRem}rem` }}
      >
        <div
          className="login-cyber-avatar-grid"
          style={{
            gridTemplateColumns: `repeat(${avatarColumns}, minmax(0, 1fr))`,
            gap: `${c.avatarSelectorGapRem}rem`,
          }}
        >
          {avatarOptions.map((option) => (
            <button
              key={option.id}
              type="button"
              className={`login-cyber-avatar-option ${avatar === option.url ? 'is-selected' : ''}`}
              onClick={() => onAvatarSelect(option.url)}
          ...
          <button
            type="button"
            className="login-cyber-avatar-option login-cyber-avatar-upload"
        ...
        <button
          type="button"
          className="login-cyber-back-button login-cyber-avatar-back-button"
          onClick={onToggleAvatarSelector}
        >
          BACK
        </button>
...
function lineClampStyle(
  maxLines: number
): Pick<React.CSSProperties, 'display' | 'overflow' | 'WebkitLineClamp' | 'WebkitBoxOrient'> {
...
function AuthCopyBlock({
  ...
  const height =
    eyebrowLine * clampLines(c.subtitleMaxLines) +
    titleMargin +
...
function AuthLayer(props: CyberAuthSurfaceProps) {
    ...
    : isSignin || !props.signUpEnabled
      ...
      : [
          ...
          },
          {
            ...
            error: props.validationErrors.confirmPassword,
          },
        ];
  ...
  const isAvatarSelectorOpen =
    !isSignin && props.signUpEnabled && !props.showForgotPassword && props.showAvatarSelector;
    ...
    <g>
      ...
      <g>
        {props.signUpEnabled && !props.showForgotPassword ? (
          <>
            <AuthButton
              x={cx + c.modeX - c.modeW / 2}
              y={y + c.modeY}
              w={c.modeW / 2 + c.modeOverlap}
              h={c.modeH}
              label="SIGN IN"
              active={isSignin}
          ...
          <AuthButton
            ...
            w={c.modeW}
            h={c.modeH}
            label={props.showForgotPassword ? 'PASSWORD RESET' : 'SIGN IN'}
            active
            onClick={props.showForgotPassword ? props.onBackToSignIn : () => props.onModeChange('signin')}
            disabled={props.isLoading}
          />
        )}
      </g>
      {!isSignin && props.signUpEnabled && !props.showForgotPassword ? (
        <AvatarControl
          cx={cx}
          y={y}
          avatar={props.avatar}
          showAvatarSelector={props.showAvatarSelector}
          onToggleAvatarSelector={props.onToggleAvatarSelector}
        />
      ) : null}
      {isAvatarSelectorOpen ? (
        <AvatarSelectorPanel
          ...
          onAvatarUploadClick={props.onAvatarUploadClick}
          onFileChange={props.onFileChange}
          avatarSelectorRef={props.avatarSelectorRef}
        ...
        visibleFields.map((field, i) => (
          <AuthField
            key={field.key}
            x={cx + c.fieldX - c.fieldW / 2}
            y={y + c.fieldY + i * (c.fieldH + c.fieldGap) + (fieldOffsets[i] ?? 0)}
            label={field.label}
            kind={field.kind}
            name={field.key}
            type={field.type}
            ...
            disabled={props.disableCredentials && !props.showForgotPassword}
            hasError={Boolean(field.error)}
          />
        ))
      )}
      {!isAvatarSelectorOpen && isSignin && !props.showForgotPassword && props.canSendPasswordReset ? (
        <ForgotPasswordLink cx={cx} y={y} onClick={props.onForgotPassword} />
      ) : null}
      {!isAvatarSelectorOpen ? (
        <CtaButton
          x={cx - c.ctaW / 2}
          y={y + ctaY}
          label={props.isLoading ? 'LOADING' : ctaLabel}
          disabled={props.isLoading || (props.disableCredentials && !props.showForgotPassword)}
        />
      ) : null}
      {!isAvatarSelectorOpen && props.showForgotPassword ? (
        <foreignObject x={cx - 210} y={y + ctaY + 132} width="420" height="54">
          <button type="button" className="login-cyber-back-button" onClick={props.onBackToSignIn}>
            Back to Sign In
          </button>
        </foreignObject>
      ) : null}
      {!isAvatarSelectorOpen && socialOptions.length > 0 ? <ContinueDivider cx={cx} y={y} /> : null}
      {!isAvatarSelectorOpen && socialOptions.length > 0 ? (
        <SocialLoginPanel cx={cx} y={y} options={socialOptions} />
      ) : null}
...
export function CyberAuthSurface(props: CyberAuthSurfaceProps) {
  const controls = React.useMemo(() => normalizeAuthPageSvgControls(props.layoutControls), [props.layoutControls]);
  const c = controls;
  ...
  return (
    <AuthPageSvgControlsContext.Provider value={controls}>
      <svg
        width={c.canvasW}
        height={c.canvasH}
        viewBox={`0 0 ${c.canvasW} ${c.canvasH}`}
        className="login-cyber-svg"
        preserveAspectRatio="xMidYMid meet"
        role="presentation"
        aria-hidden={false}
      >
        <defs>
          <clipPath id="avatarClip">
            <circle
              cx={c.frameW / 2 + c.authX + c.avatarX}
              cy={c.authY + c.avatarY}
              r={c.avatarR - c.avatarRingW - 4}
            />
          </clipPath>
          <filter id="sideRailGlow" x="-150%" y="-150%" width="400%" height="400%">
            <feGaussianBlur stdDeviation={c.sideRailGlow} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="decorGlow" x="-150%" y="-150%" width="400%" height="400%">
            <feGaussianBlur stdDeviation={c.decorGlow} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          ...
          <filter id="outerGlow" x="-80%" y="-80%" width="260%" height="260%">
            <feGaussianBlur stdDeviation={c.outerGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="drop3d" x="-80%" y="-80%" width="260%" height="260%">
            <feDropShadow
              dx={c.dropShadowDx}
              dy={c.dropShadowDy}
              stdDeviation={c.dropShadowBlur}
              floodColor={c.bevelShadow}
              floodOpacity={c.dropShadowOpacity}
            />
          </filter>
          <linearGradient id="outer3dFill" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor="#1d2b3a" />
            <stop offset="0.42" stopColor={c.outlineFill} />
            <stop offset="1" stopColor="#050b14" />
          </linearGradient>
          <linearGradient id="outer3dStroke" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor={c.bevelLight} />
            <stop offset="0.42" stopColor={c.outlineStroke} />
            <stop offset="1" stopColor={c.bevelDark} />
          </linearGradient>
          <linearGradient id="rim3dFill" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor="#7a8796" />
            <stop offset="0.45" stopColor={c.insetRimFill} />
            <stop offset="1" stopColor="#202a36" />
          </linearGradient>
          <linearGradient id="panel3dFill" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor="#243244" />
            <stop offset="0.52" stopColor={c.insetPanelFill} />
            <stop offset="1" stopColor="#08101b" />
          </linearGradient>
          <linearGradient id="rail3dFill" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0" stopColor="#788592" />
            <stop offset="0.5" stopColor={c.sideRailMainColor} />
            <stop offset="1" stopColor="#252f3a" />
          </linearGradient>
          <linearGradient id="authActionGrad" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0" stopColor={c.authGreen} />
            <stop offset="1" stopColor={c.authBlue} />
          </linearGradient>
          <radialGradient id="authOrbGrad" cx="50%" cy="50%" r="60%">
            <stop offset="0" stopColor="#83ffce" />
            <stop offset="0.45" stopColor={c.authCyan} />
            <stop offset="1" stopColor="#1150e8" />
          </radialGradient>
          <filter id="authGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.authGlow} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="closeHoverGlow" x="-150%" y="-150%" width="400%" height="400%">
            <feGaussianBlur stdDeviation={c.closeHoverGlowBlur} result="blur" />
            <feFlood floodColor={c.closeHoverGlowColor} floodOpacity={c.closeHoverGlowOpacity} result="glowColor" />
          ...
          <filter id="brandGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.brandGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="brandInnerGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.brandInnerGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="modeGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.modeGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="modeActiveGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.modeActiveGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="modeInnerGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.modeInnerGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          ...
          <filter id="fieldGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.fieldGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="fieldInnerGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.fieldInnerGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          ...
          <filter id="ctaGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.ctaGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <filter id="ctaInnerGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.ctaInnerGlowBlur} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
          <linearGradient id="ctaFaceGrad" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0" stopColor="#29e68d" />
            <stop offset="0.48" stopColor="#1fb8d2" />
            <stop offset="1" stopColor="#168cff" />
          </linearGradient>
          <filter id="socialGlow" x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur stdDeviation={c.socialGlow} result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
        ...
        <g transform={`translate(${tx} ${ty}) scale(${finalScale})`}>
          ...
          {c.showOuterGlow ? (
            <path
              d={outerPath}
              fill="none"
              stroke={c.outerGlowColor}
              strokeWidth={c.outerGlowStrokeW}
              strokeLinejoin="round"
              opacity={c.outerGlowOpacity}
          ...
          {c.show3D ? (
            <path
              d={outerPath}
              fill="none"
              stroke={c.bevelShadow}
              strokeWidth={c.outlineStrokeW + c.bevelShadowW}
              strokeLinejoin="round"
              opacity="0.55"
              filter="url(#drop3d)"
            />
          ...
          <path
            d={outerPath}
            fill={c.show3D ? 'url(#outer3dFill)' : c.outlineFill}
            stroke={c.show3D ? 'url(#outer3dStroke)' : c.outlineStroke}
            strokeWidth={c.outlineStrokeW}
            strokeLinejoin="round"
          />
          {c.show3D ? (
            <path
          ...
          {c.showInsetFrame ? (
            <>
              <path
                d={insetRimPath}
                fill={c.show3D ? 'url(#rim3dFill)' : c.insetRimFill}
                stroke={c.insetRimStroke}
                strokeWidth={c.insetStrokeW}
                strokeLinejoin="round"
              />
              {c.show3D ? (
                <path
              ...
              <path
                d={insetPanelPath}
                fill={c.show3D ? 'url(#panel3dFill)' : c.insetPanelFill}
                stroke={c.insetPanelStroke}
                strokeWidth={c.insetStrokeW}
                strokeLinejoin="round"
              />
              {c.show3D ? (
          ...
          {c.showSideNotchRails
            ? sideNotchYs.map((notchY, railIndex) => (
